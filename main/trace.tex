...
\todo{how about infinite trace}
\begin{defn}(An Event Belongs to A Trace)
  An event $\event \in \eventset$ belongs to a trace $\trace \in \ftdom$, i.e., $\event \in \trace$ are defined as follows:
%
\begin{equation*}
  \event \in \trace  
  \triangleq \left\{
...
\end{array}
\]
% {When the input trace is infinite, $\counter(\trace, l)$ returns $\infty$ for any program label $l$.}
\end{defn}
...
\end{array}
\]
% {When the input trace is infinite, $\lcounter(\trace, L)$ returns $\infty$ for any list $L$ as well.}
\end{defn}
%
