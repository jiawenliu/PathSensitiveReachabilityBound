...
\\
\highlight{
A trace can be finite ($\trace \in \ftdom$) or infinite $\trace \in \tdom^{\infty}$ where is the set of all finite or infinite traces.
If a program doesn't terminate when executing under some initial trace,
it produces the infinite trace 
from $ \tdom^{\infty}$, which records a non-terminating computation.
So $\inftdom = \ftdom \cup \tdom^{\infty}$.
...
\\
Trace concatenation $\tracecat$ is defined as follows.
\highlight{
\begin{defn}[Trace Concatenation, $\tracecat: \inftdom \to \inftdom\to \inftdom $]
Given two traces $\trace_1 \in \inftdom, \trace_2 \in \inftdom$, the trace concatenation operator 
$\tracecat$ is defined as:
\[
\trace_1 \tracecat \trace_2 \triangleq
\left\{
\begin{array}{ll} 
  \trace_1 & \trace_2 = [] \lor \trace_1 \in \tdom^{\infty} \\
  \trace_2 & \trace_1 = [] \lor \trace_2 \in \tdom^{\infty} \\
  (\trace_1  \tracecat \trace_2'):: \event & \trace_1, \trace_2 \in \ftdom \land \trace_2 = \trace_2' :: \event
\end{array}
\right.
\]
\end{defn}
...
\[
\begin{array}{llll}
\counter(\trace :: (x, l, v), l ) \triangleq \counter(\trace, l) + 1 & \text{if}~ l = l
&
\counter(\trace  ::(b, l, v), l) \triangleq \counter(\trace, l) + 1 & \text{if}~ l = l
\\
\counter(\trace  :: (x, l', v), l) \triangleq \counter(\trace, l)   & \text{if}~ l' \neq l
&
\counter(\trace  :: (b, l', v), l) \triangleq \counter(\trace, l)   & \text{if}~ l' \neq l
\\
\counter(\trace, l) \triangleq \infty & \text{if}~ \trace \in \tdom^{\infty}
&
\counter({[]}, l) \triangleq 0
\end{array}
\]
\highlight{When the input trace is infinite, $\counter$ returns $\infty$.}
...
%   \left\{
%   \begin{array}{ll} 
%   p(\trace' \tracecate (x, l, v)) & = p(\trace') \tracecate (x, l, \counter(\trace') l + 1, v) \\
%   p(\trace' \tracecate (b, l, v)) & = p(\trace') \tracecate (b, l, \counter(\trace') l + 1, v) \\
%   p(\trace' \tracecate (x, l, \qval, v)) & = p(\trace') \tracecate (x, l, \counter(\trace') l + 1, \qval, v) \\
%   p([]) & = []
%   \end{array}
%   \right.
% \]
...
%   \left\{
%   \begin{array}{ll} 
%   p(\trace' \tracecat [(x, l, v)]) & = p(\trace') \tracecat [(x, l, \counter(\trace') l + 1, v)] \\
%   p(\trace' \tracecat [(b, l, v)]) & = p(\trace') \tracecat [(b, l, \counter(\trace') l + 1, v)] \\
%   p(\trace' \tracecat [(x, l, \qval, v)]) & = p(\trace') \tracecat [(x, l, \counter(\trace') l + 1, \qval, v)] \\
%   p({[]}) & = []
%   \end{array}
%   \right.
% \]
