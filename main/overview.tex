...
and $1$ for locations $0, 1$ and $\lex$. Notice here the bounds for locations on first branch, $4$ and $5$ and second branch, $6$ are different.
% \\
...
We call this the \emph{loop reachability} of the loops $L_3$ and $L_1$ w.r.t the loop $L_6$.
Then by multiplying the loop bounds of the $L_6$ with the \emph{loop reachability} of $L_3$ and $L_1$ w.r.t the $L_6$, we can compute the precise
...
$\tpath_6$,
where the $\tpath_0, \ldots$ are shown in Figure~\ref{fig:threeWhile-overview}(b) in the first step.
...
we compute \emph{loop reachability-bound} for each $\tpath_i$ w.r.t. each of its nested loop.
For example, for $\tpath_3$ we compute
$\lpchB(1: \rprog_1, \tpath_3) = 1$ and
$\lpchB(3: \rprog_3, \tpath_3) = 1$.
Both are tight because loop $L_6$ will only be entered once among all iterations of $L_1$ and $L_3$.
Among all the other iterations, the loop $L_6$ isn't executed at all.
...
Then we sum up the bound of the path where each location shows up as its \emph{reachability-bound} still as before,
