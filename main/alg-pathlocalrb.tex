...
\]
\end{defn}
\begin{defn}[Loop Bound Computation - 2]
  \label{def:loopbound}
    \[
      \begin{array}{rcl}
        BD(\rprepeat(\tpath), c) & \triangleq &  \max\limits_{x \in \left\{ \locbound(\absevent, c) | \tpath \in \rprog' \land \absevent \in \tpath \right\}}
        ...
        \frac{\rfinit(\rprog_1) - \varGD(\rprog_1) - \rffinal(\rprog_2)}{ \min\{\varGD(\rprog_1), \varGD(\rprog_2)\} } 
        \\
