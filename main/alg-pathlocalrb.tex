...
\todo{Notice here we can already compute the path reachability-bound simply use the Loop Bound $BD$ recursively. By simply remove the restriction, $l: \rprog_l = \kw{enclosed}(\rprog, \tpath)$ in Definition~\ref{def:pathlocalrb} and use $\rprog$
as the input. Then Definition~\ref{def:pathlocalrb} can compute $\outinB(\rprog, \tpath)$ over the entire program.
Then we have already been able to compute the \emph{Reachability-bound} for every program point  $l \in \lvar(c)$ by replacing the equation in Definition~\ref{def:point_psrb} with the following one,
\[ 
  \psRB(c, l) = 
  \sum
...
\]
But this computation is loose, we compute the \emph{loop reachability-bound} in the next for a tighter program point \emph{Reachability-bound}.
}
\paragraph{Example.}
