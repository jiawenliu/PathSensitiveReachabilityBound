...
\begin{algorithm}
  ...
  \begin{algorithm}
    ...
    $R_l = \{\}$ initial set for each loop with header at program point $l$, (i.e., set of the transition sequences from interleaving-refinement) .
    % \STATE \todo{Define algorithm $\kw{dfs(w, I_{l}, FS)}$}
    \STATE \textbf{Define} \highlight{$\kw{dfs(wking-path, inv-list, done-set)}$:}
    \STATE \quad {$\kw{unfinished-set = \{\}}$:}
    \STATE \quad {$\kw{inv0 = inv-list[-1]}$:}
    \STATE \quad \textbf{For} $(\tpath, \tpath_i) \in cxlE(c)$ and $\tpath \neq \tpath'$:
    \\
    \STATE \quad \quad \textbf{If} \highlight{$\kw{visited[\tpath_i]}$} skip the loop
    \STATE \quad \quad $\kw{visited[\tpath_i]} = \etrue$
    \STATE \quad \quad $\kw{w_i = wking-path.append(\tpath')}$
    \\
    \quad \quad \highlight{Compute Invariant $\kw{inv_i = merge(w_i, inv-list[-1])}$}
    \STATE \quad \quad \textbf{If} \highlight{$\kw{inv_i = \bot}$} $\eskip$
    \STATE \quad \quad \textbf{Elif} \highlight{$\kw{inv_i = inv-list[0]}$} $\kw{done-set.add(\rprepeat(w_i))}$.
    \STATE \quad \quad \textbf{Elif} \highlight{$\kw{inv_i \notin inv-list}$} 
