...
%   \end{algorithm}
  ...
  \begin{algorithm}
    ...
    % \\
    % \STATE \todo{Define algorithm $\kw{dfs(w, I_{l}, FS)}$}
    \STATE \textbf{Define} \highlight{$\kw{dfs(wking\_path, inv\_list, done\_set)}$:}
    ...
    \\
    \quad \quad \highlight{Compute Invariant $\kw{inv_i = merge(w_i, inv\_list[-1])}$}
    \STATE \quad \quad \textbf{If} \highlight{$\kw{inv_i = \bot}$} $\eskip$
    \STATE \quad \quad \textbf{Elif} \highlight{$\kw{inv_i = inv\_list[0]}$} $\kw{done\_set.add(\rprepeat(w_i))}$.
