...
\begin{itemize}
\item Operator: $\absexpr : \mathcal{A} \cup \booldom \to DC(\vardom  \cup \constdom)\cup \booldom \cup \{\top\}$
%
...
\\
The \emph{Symbolic Constant} universe, $\constdom \subseteq \mathbb{N} \cup \vardom \cup \{\infty \}$
...
\begin{defn}[Symbolic Expression ($\scexpr(c)$)]
...
\begin{defn}[Constraint Computation]
  ...
  \[
    \scvar(c) = \mathbb{N} \cup \inpvar(c) \cup \{\infty \} \subseteq \constdom
  \]
  \item Initialize 
  ...
  \[
    \begin{array}{ll} 
      ...
      & x \in \grdvar(c)\land y \notin \grdvar(c) \land v \in \constdom  \\
