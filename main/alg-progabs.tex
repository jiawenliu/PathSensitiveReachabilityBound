...
form $x' \leq y + v$ or $x' \leq v$ where $x \in \mathcal{VAR} $, 
...
\begin{defn}[Constraint Computation]
  ...
  \end{defn}
%
where $\grdvar$ is the set of the variable used in the expression of every while or if guard in the program $c$ and
$FV: \expr \to \mathcal{P}(\mathcal{VAR})$, computes the set of free variables in an expression. 
...
i.e., $\scexpr \subseteq \mathcal{A}_{\lin}$.
...
\highlight{Computation Steps:}
\begin{itemize}
  \item The \emph{initial execution points}, $\absinit(c) \in \mathcal{P}(\ldom)$
  for a command $c$.
...
%
%
\item The \emph{continuation execution points}, $\absfinal(c)$ of program $c$, 
