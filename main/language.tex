...
\paragraph{Free Variables and Input Variables}
  ...
  We use the same notation as \redd{PART \romannum{1}}, where $\mathcal{V}_{\lin}$ denotes the universe of all input variables and $\kw{V_{\lin}}(c)$ the set of input variables in a given program $c$.
...
Similar to \redd{PART \romannum{1}}, we don't use separate data structure as the environment to record each variable's value or state. Instead, we use the operator $\env (\trace) x$ to fetch the latest value assigned to $x$ in the trace $\trace$. 
\[
\begin{array}{lll}
