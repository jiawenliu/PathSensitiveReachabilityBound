...
\begin{defn}[Non-terminating and Infinite Trace]
  ...
  $\config{c, \trace_0} \rightarrow^{\infty} \config{c', \trace'} \in \lim(\rightarrow)$ for some $c' \in \cdom, \trace' \in \inftdom$
  where the limit is 
  \[
    \begin{array}{l}
      \lim(\rightarrow) 
    ...
    \\ \quad
    \Big\{
      (\config{c, \trace}, \config{c', \trace'}) ~\vert~ 
      c, c' \in \cdom, \trace \in \ftdom_0(c),
      \trace' \in \inftdom 
      \land \exists \trace_0 \in \tdom, c_0 \in \cdom \st 
