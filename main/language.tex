...
\highlight{
  Given an initial trace $\trace_0 \in \tdom_0(c)$ of the program $c$,
we use $\to^*$ for the reflexive and transitive closure of $\to$. 
If $\config{c, \trace_0} \to^{*} \config{\eskip, \trace_0 \tracecat \trace}$,
then the program's execution terminates and produces a finite execution trace $\trace \in \ftdom$.
\\
For non-terminating program, we pass to the limit of
$\config{c, \trace_0} \to^{*}$ 
by defining $\config{c, \trace_0} \to^{\infty} 
% \config{\cdot, \trace_0 \tracecat []^{\infty} } 
\triangleq \lim(\config{c, \trace_0} \to^{*})$.
...
For any program $c \in \cdom$ and traces $\trace_0 \in \tdom_0(c), \trace \in \inftdom$ and $c' \in \cdom $, if 
$\config{c, \trace_0} \rightarrow^{*} \config{c', \trace} $
% or 
% $\config{c, \trace_0} \rightarrow^{\infty} \config{\cdot, \trace}$,
then there exists a trace $\trace' \in \inftdom$ such that $\trace_0 \tracecat \trace' = \trace$
%
$$
\forall \trace_0 \in \tdom_0(c), \trace \in \inftdom, c, c' \in \cdom \st
  \config{c, \trace_0} \rightarrow^{*} \config{c', \trace} 
\implies \exists \trace' \in \inftdom \st \trace_0 \tracecat \trace' = \trace
$$
\end{lem}
}
\todo{Question: this expression $\config{c, \trace_0} \rightarrow^{*} \config{c', \trace} $ should include its
limitation case, i.e.,
if I wrote $\forall \trace_0 \in \tdom_0(c), \trace \in \inftdom, c, c' \in \cdom \st \config{c, \trace_0} \rightarrow^{*} \config{c', \trace} $
then 
the non-terminating case 
where
$\config{c, \trace_0} \rightarrow^{\infty} \land  \counter(\infty, l) = \infty \land f(c, l) (\trace_0) \geq \infty$ is included. }
