...
$\config{c, \trace_0} \to^{*}$ 
by defining $\config{c, \trace_0} \to^{\infty} \config{\cdot, \trace_0 \tracecat []^{\infty} } 
\triangleq \lim(\config{c, \trace_0} \to^{*})$ for arbitrary $c'$.
This follows the maximal trace semantics in \cite{cousot2019abstract} Section 2.5.
...
% represent the non-terminated execution w.r.t. the infinite trace $\trace_0 \in \inftdom$.
\\
