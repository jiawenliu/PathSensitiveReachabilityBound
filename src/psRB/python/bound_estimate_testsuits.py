...
class TestUnits:
    ...
    def seq(self):
        query = [1, 1, 1, 1]
        edges = [(0, 1), (1, 2), (2, 3)]
        weights = [SymbolicExpression(SymbolicConst("INF"))]*len(query)
        ...
        BoundEstimate.bound_estimate(DCFGraph(edges, weights, query), TransitionGraph(ctl_edges, transitions))
    ...
    def seq_multivar(self):
        ...
        # adapt_search = self.ALG(DCFGraph(edges, weights, query))
    ...
    def if_ctldep(self):
        ...
        # bound_infer = self.ALG(DCFGraph(edges, weights, query), TransitionGraph(ctl_edges, transitions))
    ...
    def nested_while_valdep(self):
        # weights = [SymbolicExpression(1), SymbolicExpression(1), SymbolicExpression("k"), SymbolicExpression("k"), SymbolicExpression("k"), SymbolicExpression("k * k"), SymbolicExpression("k * k")]
        query = [0, 1, 0, 0, 1, 0, 1]
    ...
    # i.e., [SymbolicExpression(1), SymbolicExpression(1), SymbolicExpression("k"), SymbolicExpression("k"), SymbolicExpression("k"), SymbolicExpression(1)]
    def two_round(self):
        query = [0, 0, 0, 1, 0, 1]
...
test_bound_infer = TestUnits(BoundEstimate)
