...
class TransitionBound:
        ...
        def dfs(v, visited, currrent_chain):
            for (transition_index, next_var, dc_const) in self.reset_transitions[v]:
                if next_var and not visited[next_var]:
                    ...
                    visited[next_var] = True
                    self.var_reset_chains[v].append(dfs(next_var, visited, currrent_chain + [(transition_index, next_var, dc_const)]))
                else:
                    self.var_reset_chains[v].append(currrent_chain + [(transition_index, next_var, dc_const)])
                    # renaming_chains.append(currrent_chain + [(transition_index, next_var, dc_const)])
            return currrent_chain
        for v in self.all_vars:
            dfs(v, {u: True if u == v else False for u in self.all_vars}, [])
    ...
    def compute_var_invariant(self, v, visited):
        ...
        for (t, dc_const) in self.inc_transitions[v]:
            ...
            if self.transition_bounds[t] == "∞":
                ...
                var_inc.append(self.transition_bounds[t] + " * " + dc_const)
        self.inc_transitions_bound[v] = "+".join(var_inc)
        ...
        for (_, reset_var, reset_const) in self.reset_transitions[v]:
            if reset_var and (not self.var_invariant[reset_var]) and (not visited[reset_var]):
                ...
                return
            var_reset.append(self.var_invariant[reset_var] + reset_const)
        
        self.var_invariant[v] = self.inc_transitions_bound[v] + "+" + "max{{{}}}".format(",".join(var_reset))
    ...
    def compute_transition_bound_closure(self, t_index, visited):
        ...
        if v == "-1":
            ...
            '''
            if not (self.var_invariant[v] and visited[v]):
                visited[v] = True
            ...
            for reset_chain in self.var_reset_chains[v]:
                ...
                for (reset_t, reset_var, reset_const) in reset_chain:
                    ...
                    '''
                    #     return "∞"
                    min_transition = "∞" if self.transition_bounds[reset_t] == "∞" else self.transition_bounds[reset_t] if not min_transition else  "min({},{})".format(self.transition_bounds[reset_t], min_transition)
                    ...
                    '''
                    if reset_var: 
                        if not (self.var_invariant[reset_var] and visited[reset_var]):
                            visited[reset_var] = True
                            self.compute_var_invariant(reset_var, visited)
    ...
    def compute_transition_bounds(self):
        ...
        for transition_index in (range(len(self.transition_graph.transitions))):
            ...
            print("VISITING STATE AFTER COMPUTE BD OF TRANSITION:", transition_index, visited)
