...
class PathSensitiveReachabilityBound():
    ...
    def __init__(self, transition_graph=TransitionGraph()) -> None:
        ...
        self.tpath_loc_rb =  defaultdict(str)
...
############################################################################################################################################################################
    ...
    def get_ranks(self, prog):
        ...
        for t_id in prog.get_transitions():
            r.add(self.transition_bound.transition_local_bounds[t_id])
    ...
    def prog_initial(self, prog):
        def rank_inital(rank):
            ...
            for (transition_index, reset_var, reset_const) in self.transition_bound.reset_transitions[x]:
                if self.transition_graph.transitions[transition_index][0]  <= prog.start_point:
                    rank_initial_set.add("({}) + ({})".format(self.transition_bound.var_invariant[reset_var], reset_const))
    ...
    def prog_next(self, prog):
        ...
        ### THE VALUE THAT THE RANK IS INCREASED IN ONE EXCUTION OF THE PROGAM
            rank_inc_set = set(["0"])
            for (transition_index, inc_const) in self.transition_bound.inc_transitions[x]:
                (ls, dc, le, _) = self.transition_graph.transitions[transition_index][0]
        ...
        ### THE VALUE THAT THE RANK IS DECREASE IN ONE EXCUTION OF THE PROGAM
            rank_dec_set = set(["0"])
            for (transition_index, reset_var, dec_const) in self.transition_bound.dec_transitions[x]:
                if self.transition_graph.transitions[transition_index][0]  <= prog.start_point:
        ...
        ### THE VALUE THAT THE RANK IS RESET IN ONE EXCUTION OF THE PROGAM
            ...
            for (transition_index, reset_var, reset_const) in self.transition_bound.reset_transitions[x]:
                if self.transition_graph.transitions[transition_index][0]  >= last_reset_point:
                    rank_reset_value, last_reset_point = ("({})+({})".format(self.transition_bound.var_invariant[reset_var], reset_const)), self.transition_graph.transitions[transition_index][0]
    ...
    def path_local_reachability_bound(self, prog):
        def local_nested_loops_dfs(prog, rp_bound, L=None):
            ...
            if prog.type == RefinedProg.RType.CHOICE:
                ...
                self.tpath_loc_rb[prog.get_id()] = (L, rp_bound)
            else:
                return
    ...
    def loop_reachability_bound(self, tp_prog, loop_chain):
        ...
        def compute_loop_chain_bound(tp_prog, loop_chain):
            ...
            (_, tp_rpchain_bound) = self.tpath_loc_rb[tp_prog.get_id()]
    ...
    def path_reachability_bound(self, prog):
        ...
        def compute_transition_path_ps_bound(prog):
            if prog.type == RefinedProg.RType.TP:
                self.transition_path_ps_bound[prog.get_id()] = "max(" + ",".join(self.loop_reachability_bound(prog, loop_chain) for loop_chain in self.loop_chains[prog.get_id()]) + ")"
            if prog.type == RefinedProg.RType.CHOICE:
                for choice_prog in prog.get_choices():
                    (compute_transition_path_ps_bound(choice_prog))
            elif prog.type == RefinedProg.RType.REPEAT:
                compute_transition_path_ps_bound(prog.get_repeat())
            elif prog.type == RefinedProg.RType.SEQ:
                for seq_prog in prog.get_seqs():
                    (compute_transition_path_ps_bound(seq_prog))
            else:
                return
        
        return compute_transition_path_ps_bound(prog)
