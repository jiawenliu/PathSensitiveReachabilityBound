...
static int screen_lines = 24, screen_columns = 80;
...
static void getScreenSize(void)
{
    ...
    if (envLines != ((void *) 0) && (rowTemp = atol(envLines)) > 0)
	screen_lines = (int) rowTemp;
    envColumns = getenv("COLUMNS");
    if (envColumns != ((void *) 0) && (colTemp = atol(envColumns)) > 0)
	screen_columns = (int) colTemp;
    if ((!screen_lines || !screen_columns) && ioctl(fileno(stderr), 0x5413, &windowInfo) != -1) {
...
int more_file(char *fileName, boolean eyes_only)
{
	...
	if (ch == '\n') {
	    ...
	    chars = 0;
	    ++lineno;
	    if ((ch = _IO_getc(inFile)) != '\n' && ch != (-1))
		ungetc(ch, inFile);
	} else if (((unsigned char) ch >= ' ' && ch != (-1)) || ch == 0x09) {
	    putchar(ch);
	    chars += (ch == 0x09) ? 8 : 1;
	}
	if (chars == screen_columns - 1) {
	    chars = 0;
	    while ((ch = _IO_getc(inFile)) != '\r' && ch != '\n' && ch != (-1));
	    if (ch != (-1))
		ungetc(ch, inFile);
	}
	while (ch == (-1) || lines == screen_lines - 1) {;
	    if (ch == (-1))
		printf(LANG("\nDone...hit any key\r"));
	    else
		printf(LANG("More -- %d%% -- Space: next screen, Enter: next line, 'B': back, 'Q': quit --\r"), (100 * ftell(inFile)) / fileLen);;
	    fflush(stdout);
	    c = getch();
	    c = toupper(c);
	    for (i = 0; i < 79; i++)
...
static char *mfile = ((void *) 0);
static boolean piping = 0;
...
int open_more(void)
{
	...
	if ((pgpout = popen(pager, "w")) != ((void *) 0)) {
	    piping = (!0);
	    return 0;
	}
	perror("popen");
    ...
    if ((mfile = tempfile(4 | 1)) == ((void *) 0))
	return -1;
    if ((pgpout = fopen(mfile, "w")) == ((void *) 0)) {
	pgpout = savepgpout;
	rmtemp(mfile);
	return -1;
    }
