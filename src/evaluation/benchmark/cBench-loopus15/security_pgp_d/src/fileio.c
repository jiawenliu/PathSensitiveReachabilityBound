...
byte textbuf[4096];
static unsigned textbuf2[2 * 4096 / sizeof(unsigned)];
boolean file_exists(char *filename)
{
...
{
    struct stat st;
    return stat(filename, &st) != -1 && ((((st.st_mode)) & 0170000) == (0100000));
}
...
static void wipeout(FILE * f)
{
    ...
    do {
	for (i = 0; i < 32; i++)
	    t = (t & 0x80000000) ? t << 1 ^ 0x04c11db7 : t << 1;
	*--p1 = (unsigned) t;
...
{
    char *p = strrchr(path, '.');
    return p != ((void *) 0) && p[1] == '$' && ((*__ctype_b_loc())[(int) ((p[2]))] & (unsigned short int) _ISdigit) && ((*__ctype_b_loc())[(int) ((p[3]))] & (unsigned short int) _ISdigit) && p[4] == '\0';
} boolean no_extension(char *filename)
{
    if (has_extension(filename, ASC_EXTENSION) || has_extension(filename, PGP_EXTENSION) || has_extension(filename, SIG_EXTENSION) || is_tempfile(filename))
	return 0;
    else
	return (!0);
}
...
char *buildsysfilename(char *result, char *fname)
{
    strcpy(result, "/usr/local/lib/pgp/");
    strcat(result, fname);
...
int copyfile(FILE * f, FILE * g, word32 longcount)
{
	...
	if (count > 0) {
		...
		int i;
		for (i = 0; i < count; i++)
		    textbuf[i] = (CONVERSION == 2) ? EXT_C(textbuf[i]) : INT_C(textbuf[i]);
	    }
...
int copyfile_to_canon(FILE * f, FILE * g, word32 longcount)
{
    ...
    memset((void *) &(textbuf), 0, sizeof(textbuf));
    memset((void *) &(textbuf2), 0, sizeof(textbuf2));
    return status;
}
...
int make_canonical(char *srcFile, char *destFile)
{
    ...
    int status = 0;
    long fileLength;
    if (((f = fopen(srcFile, "r")) == ((void *) 0)) || ((g = fopen(destFile, "w")) == ((void *) 0)))
	return -1;
    fseek(f, 0L, 2);
    fileLength = ftell(f);
    rewind(f);
...
int readPhantomInput(char *filename)
{
    ...
    if ((outFilePtr = fopen(filename, "w")) == ((void *) 0))
	return -1;
...
static char tmpbasename[64] = "pgptemp";
void settmpdir(char *path)
{
    ...
    if (*p != '/' && *p != '\\' && *p != ']' && *p != ':') {
	if ((p = strchr(tmpdir, '/')) == ((void *) 0) && (p = strchr(tmpdir, '\\')) == ((void *) 0))
	    p = "/";
	strncat(tmpdir, p, 1);
    }
...
char *tempfile(int flags)
{
  ...
  again:num = 0;
	...
	if (j < 8)
	    continue;
	sprintf(tmpf[i].path, "%s%s.%c%02d", ((flags & 4) && *tmpdir ? tmpdir : outdir), tmpbasename, '$', num);
    ...
    if ((fd = open(tmpf[i].path, 0200 | 02 | 0100, 0600)) != -1)
	close(fd);
    if (fd == -1) {
	if (!(flags & 4)) {
	    flags |= 4;
...
void rmtemp(char *name)
{
	...
	if (tmpf[i].flags && strcmp(tmpf[i].path, name) == 0)
	    break;
    if (i < 8) {
	if (strlen(name) > 3 && name[strlen(name) - 3] == '$') {
	    if (verbose)
...
char *savetemp(char *name, char *newname)
{
    ...
    newname = ck_dup_output(newname, 0, (!0));
    if (newname == ((void *) 0))
	return (((void *) 0));
    if (verbose)
...
int savetempbak(char *tmpname, char *destname)
{
    ...
    if (savetemp(tmpname, destname) == ((void *) 0))
	return -1;
    if (mode != -1)
	chmod(destname, mode);
    return 0;
...
{
    static char const *const extensions[] = { ".doc", ".txt", ".man", ".tex", "", 0 };
    char const *const *p;
...
{
    static char const *const names[] = { "pgpdoc1", "pgpdoc2", 0 };
    char const *const *p;
    unsigned bit, mask = 3;
    int len = strlen(prefix);
    if (prefix[0] && !file_exists(prefix))
	return mask;
...
char const *const manual_dirs[] = { "$PGPPATH", "", "pgp", "pgp26", "pgp263", "/usr/local/lib/pgp/", "$HOME/.pgp", "$HOME", "$HOME/pgp", "$HOME/pgp26", "..", 0 };
