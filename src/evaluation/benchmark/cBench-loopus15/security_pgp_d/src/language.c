...
static int line = 0;
static char *readstr(FILE * f, char *buf, int nlabort)
{
    int c, d;
    char *p = buf;
    int state = 0;
    int i = 0;
    while ((c = _IO_getc(f)) != (-1)) {
	if (c == '\r')
	    continue;
	if (line && c == '\n')
	    ++line;
	...
	case 0:
	    ...
	    default:
		if (i == 0 && ((*__ctype_b_loc())[(int) ((c))] & (unsigned short int) _ISalnum)) {
		    state = 4;
		    lang[i++] = c;
		    break;
		}
	...
	case 4:
	    ...
	    lang[i++] = c;
	    if (i == 15 || !((*__ctype_b_loc())[(int) ((c))] & (unsigned short int) _ISalnum) && !((*__ctype_b_loc())[(int) ((c))] & (unsigned short int) _ISspace)) {
		lang[i] = '\0';
...
static word32 message_crc(char *s)
{
...
static void init_lang()
{
    ...
    buildfilename(subtitles_file, "language.txt");
    langf = fopen(subtitles_file, "r");
    if (langf == ((void *) 0)) {
	strcpy(subtitles_file, "/usr/local/lib/pgp/");
	strcat(subtitles_file, "language.txt");
	langf = fopen(subtitles_file, "r");
	...
	strcpy(indexfile, "/usr/local/lib/pgp/");
	strcat(indexfile, "language.idx");
    } else
	buildfilename(indexfile, "language.idx");
    indexf = fopen(indexfile, "r");
    if (indexf != ((void *) 0)) {
	if (fread(&indx_hdr, 1, sizeof(indx_hdr), indexf) == sizeof(indx_hdr) && indx_hdr.lang_fsize == fsize(langf) && strcmp(indx_hdr.lang, language) == 0) {
	    nmsg = indx_hdr.nmsg;
    ...
    if (indx_tbl == ((void *) 0) && make_indexfile(indexfile) < 0) {
	fclose(langf);
	subtitles_available = -1;
    } else {
	subtitles_available = 1;
...
static int make_indexfile(char *indexfile)
{
	...
	if (nmsg == max_msgs) {
	    if (max_msgs) {
		max_msgs *= 2;
		indx_tbl = (struct indx_ent *) realloc(indx_tbl, max_msgs * sizeof(struct indx_ent));
	    } else {
		max_msgs = 400;
		indx_tbl = (struct indx_ent *) malloc(max_msgs * sizeof(struct indx_ent));
	    } if (indx_tbl == ((void *) 0)) {
		fprintf(stderr, "Not enough memory for foreign subtitles\n");
		return -1;
	    }
    ...
    if ((indexf = fopen(indexfile, "w")) == ((void *) 0)) {
