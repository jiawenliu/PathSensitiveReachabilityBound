...
static word32 randPool[((3072 + 127 & ~127) >> 5)];
static unsigned randPoolGetPos = sizeof(randPool);
...
void randPoolStir(void)
{
    ...
    p = (byte *) randPool;
    for (i = 0; i < ((3072 + 127 & ~127) >> 5); i++) {
	t = (word32) ((unsigned) p[3] << 8 | p[2]) << 16 | (unsigned) p[1] << 8 | p[0];
	randPool[i] = t;
	p += 4;
    } memcpy(iv, randPool + ((3072 + 127 & ~127) >> 5) - 4, sizeof(iv));
    for (i = 0; i < ((3072 + 127 & ~127) >> 5); i += 4) {
