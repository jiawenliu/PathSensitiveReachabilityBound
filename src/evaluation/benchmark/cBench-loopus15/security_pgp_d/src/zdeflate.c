...
static config configuration_table[10] = { {0, 0, 0, 0}, {4, 4, 16, 4}, {6, 8, 16, 0}, {8, 16, 32, 0}, {8, 32, 64, 0}, {8, 64, 128, 0}, {8, 128, 256, 0}, {8, 128, 512, 0}, {32, 258, 1024, 0}, {32, 258, 4096, 2} };
...
ush *flags;
{
    ...
    *flags |= configuration_table[pack_level].flag;
    strstart = 0;
    block_start = 0L;
    lookahead = read_buf((char *) slide, 2 * 8192);
    ...
    ins_h = 0;
    for (j = 0; j < 3 - 1; j++)
	(ins_h = (((ins_h) << ((15 + 3 - 1) / 3)) ^ (slide[j])) & ((unsigned) (1 << 15) - 1));
} void lm_free()
{
...
IPos cur_match;
{
    ...
    register uch *scan = slide + strstart;
    register uch *match = scan;
    register int len;
    int best_len = prev_length;
    IPos limit = strstart > (IPos) (8192 - (258 + 3 + 1)) ? strstart - (IPos) (8192 - (258 + 3 + 1)) : 0;
	...
	len = 3 - 4;
	do {
	} while ((len += 4) < 258 - 3 && *(ulg *) (scan + len) == *(ulg *) (match + len));
	if (*(ush *) (scan + len) == *(ush *) (match + len))
	    len += 2;
	if (scan[len] == match[len])
	    len++;
	if (len > best_len) {
    ...
    } while (--chain_length != 0 && (cur_match = prev[cur_match & (8192 - 1)]) > limit);
    return best_len;
}
...
static void fill_window()
{
    ...
    unsigned more = (unsigned) ((ulg) 2 * 8192 - (ulg) lookahead - (ulg) strstart);
    if (more == (unsigned) (-1)) {
	more--;
    } else if (more <= 1) {
	memcpy((char *) slide, (char *) slide + 8192, (unsigned) 8192);
	match_start -= 8192;
	strstart -= 8192;;
	block_start -= (long) 8192;
	for (n = 0; n < (unsigned) (1 << 15); n++) {
	    m = head[n];
	    head[n] = (Pos) (m >= 8192 ? m - 8192 : 0);
	} for (n = 0; n < 8192; n++) {
	    m = prev[n];
	    prev[n] = (Pos) (m >= 8192 ? m - 8192 : 0);
	}
	more += 8192;
    }
...
ulg deflate()
{
    ...
    while (lookahead != 0) {
	((ins_h = (((ins_h) << ((15 + 3 - 1) / 3)) ^ (slide[(strstart) + 3 - 1])) & ((unsigned) (1 << 15) - 1)), prev[(strstart) & (8192 - 1)] = hash_head = head[ins_h], head[ins_h] = (strstart));
	prev_length = match_length, prev_match = match_start;
	match_length = 3 - 1;
	if (hash_head != 0 && prev_length < max_lazy_match && strstart - hash_head <= (8192 - (258 + 3 + 1))) {
	...
	if (prev_length >= 3 && match_length <= prev_length) {;
	    ...
	    strstart++;
	    if (flush)
		flush_block(block_start >= 0L ? (char *) &slide[block_start] : (char *) ((void *) 0), (long) strstart - block_start, (0)), block_start = strstart;
    ...
    if (match_available)
	ct_tally(0, slide[strstart - 1]);
    return flush_block(block_start >= 0L ? (char *) &slide[block_start] : (char *) ((void *) 0), (long) strstart - block_start, (1));
}
