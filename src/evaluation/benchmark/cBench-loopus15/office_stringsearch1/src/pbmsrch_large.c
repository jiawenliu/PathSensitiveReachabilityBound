...
int main(int argc, char *argv[])
{
    ...
    if ((fmisc1 = fopen(argv[1], "rt")) == ((void *) 0)) {
	fprintf(stderr, "\nError: Can't find text!\n");
	exit(1);
    }
    if ((fmisc2 = fopen(argv[2], "rt")) == ((void *) 0)) {
	fprintf(stderr, "\nError: Can't find search strings!\n");
	exit(1);
    }
    i1 = 0;
    while ((fgets(str_misc, 1024, fmisc1) != ((void *) 0)) && (feof(fmisc1) == 0)) {
	i1++;
	size1 += strlen(str_misc) + 1;
    } i2 = 0;
    while ((fgets(str_misc, 1024, fmisc2) != ((void *) 0)) && (feof(fmisc2) == 0)) {
	i2++;
    ...
    fclose(fmisc1);
    fclose(fmisc2);
    if ((fmisc3 = fopen(argv[3], "w")) == ((void *) 0)) {
	...
	fprintf(fmisc3, "\"%s\" is%s in \"%s\"", find_strings[i], here ? "" : " not", search_strings[i]);
	if (here)
