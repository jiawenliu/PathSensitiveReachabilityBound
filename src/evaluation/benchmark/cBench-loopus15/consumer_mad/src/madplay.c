...
static struct option const options[] = { {"amplify", 1, 0, 'a'}, {"attenuate", 1, 0, 'a'}, {"author", 0, 0, -'a'}, {"bit-depth", 1, 0, 'b'}, {"downsample", 0, 0, -'d'}, {"fade-in", 2, 0, -'i'}, {"help", 0, 0, 'h'}, {"ignore-crc", 0, 0, 'i'}, {"left", 0, 0, '1'}, {"license", 0, 0, -'l'}, {"mono", 0, 0, 'm'}, {"no-dither", 0, 0, 'd'}, {"output", 1, 0, 'o'}, {"quiet", 0, 0, 'q'}, {"repeat", 2, 0, 'r'}, {"right", 0, 0, '2'}, {"shuffle", 0, 0, 'z'}, {"start", 1, 0, 's'}, {"stereo", 0, 0, 'S'}, {"time", 1, 0, 't'}, {"verbose", 0, 0, 'v'}, {"version", 0, 0, 'V'}, {"very-quiet", 0, 0, 'Q'}, {"tty-control", 0, 0, -'c'}, {"no-tty-control", 0, 0, -'C'}, {0} };
...
static void show_usage(int verbose)
{
    ...
    fputs(("Decode and play MPEG audio FILE(s).\n"), stderr);
    fputs(("\nVerbosity:\n"), stderr);
    fputs(("  -v, --verbose              show status while decoding\n"), stderr);
    fputs(("  -q, --quiet                be quiet but show warnings\n"), stderr);
    fputs(("  -Q, --very-quiet           be quiet and do not show warnings\n"), stderr);
    fputs(("\nDecoding:\n"), stderr);
    fputs(("      --downsample           reduce output sample rate 2:1\n"), stderr);
    fputs(("  -i, --ignore-crc           ignore CRC errors\n"), stderr);
    fputs(("\nAudio output:\n"), stderr);
    fputs(("  -o, --output=[TYPE:]PATH   send output to PATH with format TYPE" " (see below)\n"), stderr);
    fputs(("  -b, --bit-depth=DEPTH      request DEPTH bits per sample\n"), stderr);
    fputs(("  -d, --no-dither            do not dither output PCM samples\n"), stderr);
    fprintf(stderr, "      --fade-in[=DURATION]   fade-in songs over DURATION" " (default %s)\n", "0:05");
    fputs(("  -a, --attenuate=DECIBELS   attenuate signal by DECIBELS (-)\n"), stderr);
    fputs(("  -a, --amplify=DECIBELS     amplify signal by DECIBELS (+)\n"), stderr);
    fputs(("\nChannel selection:\n"), stderr);
    fputs(("  -1, --left                 output first (left) channel only\n"), stderr);
    fputs(("  -2, --right                output second (right) channel" " only\n"), stderr);
    fputs(("  -m, --mono                 mix left and right channels" " for monaural output\n"), stderr);
    fputs(("  -S, --stereo               force stereo output\n"), stderr);
    fputs(("\nPlayback:\n"), stderr);
    fputs(("  -s, --start=TIME           skip to begin at TIME" " (HH:MM:SS.DDD)\n"), stderr);
    fputs(("  -t, --time=DURATION        play only for DURATION" " (HH:MM:SS.DDD)\n"), stderr);
    fputs(("  -z, --shuffle              randomize file list\n"), stderr);
    fputs(("  -r, --repeat[=MAX]         play files MAX times," " or indefinitely\n"), stderr);
    fputs(("      --tty-control          enable keyboard controls\n"), stderr);
    fputs(("      --no-tty-control       disable keyboard controls\n"), stderr);
    fputs(("\nMiscellaneous:\n"), stderr);
    fputs(("  -V, --version              display version number and exit\n"), stderr);
    fputs(("      --license              show copyright/license message" " and exit\n"), stderr);
    fputs(("  -h, --help                 display this help and exit\n"), stderr);
    fputs(("\nSupported output formats:\n"), stderr);
    fputs(("  cdda    CD audio, 16-bit 44100 Hz stereo PCM (*.cdr, *.cda)\n"), stderr);
    fputs(("  aiff    Audio IFF, [16-bit] PCM (*.aif, *.aiff)\n"), stderr);
    fputs(("  wave    Microsoft RIFF/WAVE, [16-bit] PCM (*.wav)\n"), stderr);
    fputs(("  snd     Sun/NeXT audio, 8-bit ISDN mu-law (*.au, *.snd)\n"), stderr);
    fputs(("  raw     binary [16-bit] host-endian linear PCM\n"), stderr);
    fputs(("  null    no output (decode only)\n"), stderr);
}
...
static mad_timer_t get_time(char const *str, int positive, char const *name)
{
	...
	die("invalid %s specification \"%s\"", name, str);
    if (positive && mad_timer_compare((time), mad_timer_zero) <= 0)
...
{
    double db;
    enum { DB_MIN = -175, DB_MAX = +18 };
    if (parse_decibels(&db, str) == -1)
	die("invalid decibel specification \"%s\"", str);
    if (db < DB_MIN || db > DB_MAX)
	die("decibel value must be in the range %+d to %+d", DB_MIN, DB_MAX);
    return ((mad_fixed_t) ((pow(10, db / 20)) * (double) (1L << 28) + 0.5));
} static void get_options(int argc, char *argv[], struct player *player)
{
    int opt, index;
    int ttyset = 0;
    while ((opt = getopt_long(argc, argv, "vqQ" "i" "o:b:da:" "12mS" "s:t:zr::" "Vh", options, &index)) != -1) {
	switch (opt) {
	case 0:
	    break;
	case '1':
	...
	case 'b':
	    opt = atoi(optarg);
	    if (opt <= 0)
		die("invalid bit depth \"%s\"", optarg);
	...
	case 'i':
	    ...
	    break;
	case -'i':
	    player->fade_in = get_time(optarg ? optarg : "0:05", 1, "fade-in time");
	...
	case 'r':
		...
		if (player->repeat <= 0)
		    die("invalid repeat count \"%s\"", optarg);
	    }
	    break;
	case 's':
	    player->global_start = get_time(optarg, 0, "start time");
	...
	case 'S':
	    ...
	    break;
	case 't':
	    player->global_stop = get_time(optarg, 1, "playing time");
	...
	case '?':
	    ...
	    exit(1);
	default:
	    ((!"option handler") ? (void) (0) : __assert_fail("!\"option handler\"", "madplay.c", 575, __PRETTY_FUNCTION__));
    }} if (optind == argc) {
	show_usage(0);
	exit(2);
    }
...
int main(int argc, char *argv[])
{
	...
	if (!(player.options & PLAYER_OPTION_GAP))
	    warn("cross-fade ignored without gap");
	else if (mad_timer_compare((player.gap), mad_timer_zero) >= 0)
	    warn("cross-fade ignored without negative gap");
    }
