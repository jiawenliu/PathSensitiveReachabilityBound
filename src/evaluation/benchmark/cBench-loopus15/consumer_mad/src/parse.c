...
signed long id3_parse_int(id3_byte_t const **ptr, unsigned int bytes)
{
    ...
    ((bytes >= 1 && bytes <= 4) ? (void) (0) : __assert_fail("bytes >= 1 && bytes <= 4", "parse.c", 45, __PRETTY_FUNCTION__));
    if (**ptr & 0x80)
	value = ~0;
    switch (bytes) {
    case 4:
...
{
    unsigned long value = 0;
    ((bytes >= 1 && bytes <= 4) ? (void) (0) : __assert_fail("bytes >= 1 && bytes <= 4", "parse.c", 68, __PRETTY_FUNCTION__));
    switch (bytes) {
    case 4:
	value = (value << 8) | *(*ptr)++;
    case 3:
...
{
    unsigned long value = 0;
    ((bytes == 4 || bytes == 5) ? (void) (0) : __assert_fail("bytes == 4 || bytes == 5", "parse.c", 88, __PRETTY_FUNCTION__));
    switch (bytes) {
    case 5:
...
{
    ((value) ? (void) (0) : __assert_fail("value", "parse.c", 106, __PRETTY_FUNCTION__));
    ((bytes == 8 || bytes == 4 || bytes == 3) ? (void) (0) : __assert_fail("bytes == 8 || bytes == 4 || bytes == 3", "parse.c", 107, __PRETTY_FUNCTION__));
    switch (bytes) {
    case 8:
	*value++ = *(*ptr)++;
	*value++ = *(*ptr)++;
...
id3_ucs4_t *id3_parse_string(id3_byte_t const **ptr, id3_length_t length, enum id3_field_textencoding encoding, int full)
{
    id3_ucs4_t *ucs4 = 0;
