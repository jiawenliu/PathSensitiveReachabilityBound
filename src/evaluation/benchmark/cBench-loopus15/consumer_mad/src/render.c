...
enum id3_utf16_byteorder { ID3_UTF16_BYTEORDER_ANY, ID3_UTF16_BYTEORDER_BE, ID3_UTF16_BYTEORDER_LE };
...
id3_length_t id3_utf16_put(id3_byte_t **, id3_utf16_t, enum id3_utf16_byteorder);
id3_utf16_t id3_utf16_get(id3_byte_t const **, enum id3_utf16_byteorder);
id3_length_t id3_utf16_serialize(id3_byte_t **, id3_ucs4_t const *, enum id3_utf16_byteorder, int);
id3_ucs4_t *id3_utf16_deserialize(id3_byte_t const **, id3_length_t, enum id3_utf16_byteorder);
...
{
    ((value) ? (void) (0) : __assert_fail("value", "render.c", 45, __PRETTY_FUNCTION__));
    ((bytes == 8 || bytes == 4 || bytes == 3) ? (void) (0) : __assert_fail("bytes == 8 || bytes == 4 || bytes == 3", "render.c", 46, __PRETTY_FUNCTION__));
    if (ptr) {
	switch (bytes) {
	case 8:
	    *(*ptr)++ = *value++;
...
{
    ((bytes == 4 || bytes == 5) ? (void) (0) : __assert_fail("bytes == 4 || bytes == 5", "render.c", 70, __PRETTY_FUNCTION__));
    if (ptr) {
	switch (bytes) {
	case 5:
	    *(*ptr)++ = (num >> 28) & 0x0f;
...
id3_length_t id3_render_int(id3_byte_t ** ptr, signed long num, unsigned int bytes)
{
    ((bytes >= 1 && bytes <= 4) ? (void) (0) : __assert_fail("bytes >= 1 && bytes <= 4", "render.c", 90, __PRETTY_FUNCTION__));
    if (ptr) {
	switch (bytes) {
	case 4:
	    *(*ptr)++ = num >> 24;
...
id3_length_t id3_render_binary(id3_byte_t ** ptr, id3_byte_t const *data, id3_length_t size)
{
    if (data == 0)
	return 0;
    if (ptr) {
...
id3_length_t id3_render_string(id3_byte_t ** ptr, id3_ucs4_t const *ucs4, enum id3_field_textencoding encoding, int terminate)
{
...
id3_length_t id3_render_padding(id3_byte_t ** ptr, id3_byte_t value, id3_length_t size)
{
    if (ptr) {
...
id3_length_t id3_render_paddedstring(id3_byte_t ** ptr, id3_ucs4_t const *ucs4, id3_length_t length)
{
    ...
    ((length <= 30) ? (void) (0) : __assert_fail("length <= 30", "render.c", 189, __PRETTY_FUNCTION__));
    data = padded;
    if (ucs4) {
	while (*ucs4 && length--) {
	    *data++ = *ucs4++;
