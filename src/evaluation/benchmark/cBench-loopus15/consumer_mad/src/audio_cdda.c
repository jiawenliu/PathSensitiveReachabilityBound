...
static int output(unsigned char const *data, unsigned int nsamples)
{
    unsigned int count;
    int result = 0;
    ...
    samplecount = (samplecount + count) % (44100 / 75);
    return result;
}
...
static int play(struct audio_play *play)
{
    ...
    ((play->samples[1]) ? (void) (0) : __assert_fail("play->samples[1]", "audio_cdda.c", 96, __PRETTY_FUNCTION__));
    audio_pcm_s16be(data, play->nsamples, play->samples[0], play->samples[1], play->mode, play->stats);
...
static int finish(struct audio_finish *finish)
{
    int result = 0;
    if (samplecount) {
	unsigned char padding[(44100 / 75) * 2 * 2];
	unsigned int padsz;
	((samplecount < (44100 / 75)) ? (void) (0) : __assert_fail("samplecount < (44100 / 75)", "audio_cdda.c", 121, __PRETTY_FUNCTION__));
	padsz = (44100 / 75) - samplecount;
