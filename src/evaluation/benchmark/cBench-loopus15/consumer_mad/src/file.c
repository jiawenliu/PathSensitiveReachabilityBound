...
enum { ID3_FILE_OPTION_ID3V1 = 0x0001 };
...
enum { ID3_FILE_FLAG_ID3V1 = 0x0001 };
static signed long query_tag(FILE * iofile)
{
...
static int add_tag(struct id3_file *file, id3_length_t length)
{
    long location;
    unsigned int i;
...
static int search_tags(struct id3_file *file)
{
	...
	if (add_tag(file, size) == -1)
	    goto fail;
	while ((frame = id3_tag_findframe(file->tags[file->ntags - 1].tag, "SEEK", 0))) {
	    long seek;
	    seek = id3_field_getint(&frame->fields[0]);
	    if (seek < 0 || fseek(file->iofile, seek, 1) == -1)
		break;
	    size = query_tag(file->iofile);
	    if (size <= 0)
		break;
    ...
    if (fseek(file->iofile, ((file->flags & ID3_FILE_FLAG_ID3V1) ? -128 : 0) + -10, 2) == 0) {
	size = query_tag(file->iofile);
	if (size < 0 && fseek(file->iofile, size, 1) == 0) {
	    size = query_tag(file->iofile);
...
static struct id3_file *new_file(FILE * iofile, enum id3_file_mode mode)
{
...
int id3_file_update(struct id3_file *file)
{
	...
	if (size) {
	    ((size == sizeof(id3v1_data)) ? (void) (0) : __assert_fail("size == sizeof(id3v1_data)", "file.c", 429, __PRETTY_FUNCTION__));
	    size = id3_tag_render(file->primary, id3v1_data);
	    if (size) {
		((size == sizeof(id3v1_data)) ? (void) (0) : __assert_fail("size == sizeof(id3v1_data)", "file.c", 433, __PRETTY_FUNCTION__));
		id3v1 = id3v1_data;
	    }
	}
    }
