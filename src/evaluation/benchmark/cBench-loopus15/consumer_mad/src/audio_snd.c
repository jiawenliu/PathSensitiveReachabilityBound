...
static int config(struct audio_config *config)
{
    ...
    int32(&header[0], (('.' << 24) | ('s' << 16) | ('n' << 8) | 'd'));
    int32(&header[4], sizeof(header));
    int32(&header[8], (~0));
    int32(&header[12], 1);
    int32(&header[16], config->speed);
    int32(&header[20], config->channels);
...
{
    unsigned char data[(1152 * 3) * 2];
    unsigned int len;
    len = audio_pcm_mulaw(data, play->nsamples, play->samples[0], play->samples[1], play->mode, play->stats);
    if (fwrite(data, play->samples[1] ? 2 : 1, play->nsamples, outfile) != play->nsamples) {
	audio_error = ":fwrite";
	return -1;
    }
    data_len += len;
