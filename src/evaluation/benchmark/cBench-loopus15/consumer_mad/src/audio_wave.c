...
static int init(struct audio_init *init)
{
	...
	outfile = stdout;
    if (fwrite("RIFF" "\xff\xff\xff\xff" "WAVE", 4 + 4 + 4, 1, outfile) != 1) {
	audio_error = ":fwrite";
	return -1;
    }
    riff_len = 4;
...
} static int config(struct audio_config *config)
{
    ...
    int32(&chunk[4], 16);
    int16(&chunk[8], 0x0001);
    int16(&chunk[10], config->channels);
    int32(&chunk[12], config->speed);
    int32(&chunk[16], bytes_ps);
    int16(&chunk[20], block_al);
    int16(&chunk[22], bitdepth);
    ...
    if (fwrite("data" "\xff\xff\xff\xff", 8 + 0, 1, outfile) != 1) {
	audio_error = ":fwrite";
	return -1;
    }
    riff_len += 8 + 0;
...
static int finish(struct audio_finish *finish)
{
    ...
    if (result == 0 && data_chunk != -1 && patch_int32(data_chunk + 4, data_len) == -1)
	result = -1;
    if (result == 0)
