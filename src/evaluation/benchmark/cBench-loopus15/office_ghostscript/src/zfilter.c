...
static int zAXE(os_ptr op)
{
    return filter_write_simple(op, &s_AXE_template);
}

static int zAXD(os_ptr op)
{
    return filter_read_simple(op, &s_AXD_template);
}

static int zNullE(os_ptr op)
{
    return filter_write_simple(op, &s_Null_template);
}

static int zPFBD(os_ptr op)
{
	...
	++npop, --sop;
    if (!((((const byte *) &((&*sop)->tas.type_attrs))[1]) == (t_boolean)))
	return (check_type_failed(&*sop));
    state.binary_to_hex = sop->value.boolval;
    return filter_read(op, npop, &s_PFBD_template, (stream_state *) & state, 0);
}

static int zPSSE(os_ptr op)
{
    return filter_write_simple(op, &s_PSSE_template);
}

static int rl_setup(os_ptr op, bool * eod)
{
    if (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary))) {
	int code;
	if (!(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)))
	    return ((-7));
	if ((code = dict_bool_param(op, "EndOfData", ((bool) 1), eod)) < 0)
	    return code;
	return 1;
    } else {
	*eod = ((bool) 1);
...
static int zRLE(register os_ptr op)
{
    stream_RLE_state state;
    int code = rl_setup(op, &state.EndOfData);
    if (code < 0)
	return code;
    if (!((((const byte *) &((&op[-code])->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&op[-code]));
    if ((ulong) (op[-code]).value.intval > (((unsigned int) 0xffffffff + (unsigned int) 0)))
	return ((-15));
    state.record_size = op->value.intval;
    return filter_write(op, 1 + code, &s_RLE_template, (stream_state *) & state, 0);
}

static int zRLD(os_ptr op)
{
...
static int zSFD(os_ptr op)
{
    ...
    if (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary)))
	++npop, --sop;
    if (!((((const byte *) &((&sop[-1])->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&sop[-1]));
    if (!(((&*sop)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))))
	return ((!((((const byte *) &((&*sop)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*sop) : (-7)));
    if (sop[-1].value.intval < 0)
	return ((-15));
    state.count = sop[-1].value.intval;
    state.eod.data = sop->value.const_bytes;
    state.eod.size = ((sop)->tas.rsize);
    return filter_read(op, npop, &s_SFD_template, (stream_state *) & state, (avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)));
}
...
static int BSE_close(stream *);
static int zBSE(os_ptr op)
{
    stream *s;
    byte *data;
    long len;
    if (!((((const byte *) &((&op[-0])->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&op[-0]));
    len = op[-0].value.intval;
    if (len < 0)
	return ((-15));
    data = (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_string) (((gs_memory_t *) (gs_imemory.current)), len, "BigStringEncode(string)");
    if (!data)
	return ((-25));
    s = file_alloc_stream(((gs_memory_t *) (gs_imemory.current)), "BigStringEncode(stream)");
    if (!s) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_string) (((gs_memory_t *) (gs_imemory.current)), data, len, "BigStringEncode(string)");
	return ((-25));
    }
    ...
    ((op)->value.pfile = (s), ((op)->tas.type_attrs = ((t_file) << 8) + (((0x10 | 0x40) | (((&gs_imemory))->current_space)))), ((op)->tas.rsize = (s->write_id)));
    return 0;
}

static int BSE_close(stream * s)
{
    return 0;
}

static int filter_ensure_buf(stream **, uint, bool);
const stream_procs s_new_read_procs = { s_std_noavailable, s_std_noseek, s_std_read_reset, s_std_read_flush, s_filter_close };

int filter_read(os_ptr op, int npop, const stream_template * template, stream_state * st, uint space)
{
    uint min_size = template->min_out_size + 1;
    ...
    case t_string:
	if (!(((&*sop)->tas.type_attrs) & (0x20)))
	    return ((-7));
	ialloc_set_space((&gs_imemory), (((space) > ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))) ? (space) : ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))));
	sstrm = file_alloc_stream(((gs_memory_t *) (gs_imemory.current)), "filter_read(string stream)");
	if (sstrm == 0) {
	    code = ((-25));
	    goto out;
	}
    ...
    case t_file:
	do {
	    sstrm = (sop)->value.pfile;
	    if (sstrm->read_id != ((sop)->tas.rsize)) {
		if (sstrm->read_id == 0 && sstrm->write_id == ((sop)->tas.rsize)) {
		    int fcode = file_switch_to_read(sop);
		    if (fcode < 0)
			return (fcode);
		} else
		    do {
			sstrm = invalid_file_entry;
		    } while (0);
	    }
	} while (0);
	ialloc_set_space((&gs_imemory), (((space) > ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))) ? (space) : ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))));
	goto ens;
    default:
	if (!(((&*sop)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	    return (check_proc_failed(&*sop));;
	ialloc_set_space((&gs_imemory), (((space) > ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))) ? (space) : ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))));
	code = sread_proc(sop, &sstrm);
	if (code < 0)
	    goto out;
	sstrm->is_temp = 2;
      ens:code = filter_ensure_buf(&sstrm, template->min_in_size + sstrm->state->template->min_out_size, ((bool) 0));
	if (code < 0)
	    goto out;
	break;
    }
    ...
    code = filter_open("r", min_size, (ref *) sop, &s_new_read_procs, template, st);
    if (code < 0)
	goto out;
    s = (sop)->value.pfile;
    s->strm = sstrm;
    ((o_stack.p) -= (npop));
  out:ialloc_set_space((&gs_imemory), save_space);
    return code;
}

int filter_read_simple(os_ptr op, const stream_template * template)
{
    return filter_read(op, (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary)) ? 1 : 0), template, ((void *) 0), 0);
} const stream_procs s_new_write_procs = { s_std_noavailable, s_std_noseek, s_std_write_reset, s_std_write_flush, s_filter_close };

int filter_write(os_ptr op, int npop, const stream_template * template, stream_state * st, uint space)
{
    uint min_size = template->min_in_size + 1;
    uint save_space = (((&gs_imemory))->current_space);
    register os_ptr sop = op - npop;
    stream *s;
    stream *sstrm;
    ...
    switch ((((const byte *) &((sop)->tas.type_attrs))[1])) {
    case t_string:
	if (!(((&*sop)->tas.type_attrs) & (0x10)))
	    return ((-7));
	ialloc_set_space((&gs_imemory), (((space) > ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))) ? (space) : ((avm_space) (((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))));
	sstrm = file_alloc_stream(((gs_memory_t *) (gs_imemory.current)), "filter_write(string)");
	if (sstrm == 0) {
	    code = ((-25));
	    goto out;
	}
	...
	do {
	    sstrm = (sop)->value.pfile;
	    if (sstrm->write_id != ((sop)->tas.rsize)) {
		int fcode = file_switch_to_write(sop);
		if (fcode < 0)
		    return (fcode);
	    }
	} while (0);
	...
	if (code < 0)
	    goto out;
	sstrm->is_temp = 2;
      ens:code = filter_ensure_buf(&sstrm, template->min_out_size + sstrm->state->template->min_in_size, ((bool) 1));
	if (code < 0)
	    goto out;
	break;
    }
    if (min_size < 128)
	min_size = file_default_buffer_size;
    code = filter_open("w", min_size, (ref *) sop, &s_new_write_procs, template, st);
    if (code < 0)
	goto out;
    s = (sop)->value.pfile;
    s->strm = sstrm;
...
int filter_write_simple(os_ptr op, const stream_template * template)
{
    return filter_write(op, (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary)) ? 1 : 0), template, ((void *) 0), 0);
} static int s_Null1D_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * pw, bool last)
{
    if (pr->ptr >= pr->limit)
	return 0;
    if (pw->ptr >= pw->limit)
...
static const stream_template s_Null1D_template = { &st_stream_state, ((void *) 0), s_Null1D_process, 1, 1 };

static int filter_ensure_buf(stream ** ps, uint min_buf_size, bool writing)
{
    stream *s = *ps;
	...
	byte *buf = (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_bytes) (((gs_memory_t *) (gs_imemory.current)), len, "filter_ensure_buf");
	if (buf == 0)
	    return ((-25));
	s->cbuf = buf;
	...
	if (writing)
	    code = filter_open("w", min_size, &bsop, &s_new_write_procs, &s_Null_template, ((void *) 0));
	else
	    code = filter_open("r", min_size, &bsop, &s_new_read_procs, &s_Null1D_template, ((void *) 0));
	if (code < 0)
	    return code;
	bs = (&bsop)->value.pfile;
	bs->strm = s;
...
} const op_def *zfilter_op_defs(void)
{
    static const op_def op_defs_[] = { {"1.psstringencode", zPSSE}, {"3.subfiledecode", zSFD}, {"filterdict", 0}, {"1ASCIIHexEncode", zAXE}, {"1ASCIIHexDecode", zAXD}, {"1NullEncode", zNullE}, {"2PFBDecode", zPFBD}, {"1PSStringEncode", zPSSE}, {"2RunLengthEncode", zRLE}, {"1RunLengthDecode", zRLD}, {"3SubFileDecode", zSFD}, {"1BigStringEncode", zBSE}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
