...
static void init_block(ref_stack *, ref *, uint);
int ref_stack_push_block(ref_stack *, uint, uint);
static void ref_stack_clear_marks(void *vptr, uint size)
{
    ((&((ref_stack *) vptr)->current)->tas.type_attrs &= ~(1));
} static gs_ptr_type_t ref_stack_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	*pep = &((ref_stack *) vptr)->current;
	return (&ptr_ref_procs);
    }
}

static void ref_stack_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	ref_packed *bot = (ref_packed *) ((ref_stack *) vptr)->current.value.refs;
	long reloc;
	gs_reloc_refs((ref_packed *) & ((ref_stack *) vptr)->current, (ref_packed *) (&((ref_stack *) vptr)->current + 1), gcst);
	((&((ref_stack *) vptr)->current)->tas.type_attrs &= ~(1));
	reloc = bot - (ref_packed *) ((ref_stack *) vptr)->current.value.refs;
	((ref_stack *) vptr)->p = (ref *) ((ref_packed *) ((ref_stack *) vptr)->p - reloc);;
	((ref_stack *) vptr)->bot = (ref *) ((ref_packed *) ((ref_stack *) vptr)->bot - reloc);;
	((ref_stack *) vptr)->top = (ref *) ((ref_packed *) ((ref_stack *) vptr)->top - reloc);;
}} const gs_memory_struct_type_t st_ref_stack = { sizeof(ref_stack), "ref_stack", 0, ref_stack_clear_marks, ref_stack_enum_ptrs, ref_stack_reloc_ptrs, 0 };

void ref_stack_init(register ref_stack * pstack, ref * psb, uint bot_guard, uint top_guard, ref * pguard, gs_ref_memory_t * mem)
{
    uint size = ((psb)->tas.rsize);
    ...
    pstack->block_size = size - 0;
    pstack->body_size = avail;
    if (pguard != 0)
	pstack->guard_value = *pguard;
    else
	((&pstack->guard_value)->value.intval = (0), ((&pstack->guard_value)->tas.type_attrs = ((t__invalid) << 8) + (0)));
    pstack->underflow_error = -1;
    pstack->overflow_error = -1;
    pstack->allow_expansion = ((bool) 1);
    pstack->memory = mem;
...
} int ref_stack_set_max_count(ref_stack * pstack, long nmax)
{
    uint nmin = pstack->extension_size + (pstack->top - pstack->bot + 1);
    if (nmax < nmin)
	nmax = nmin;
    if (nmax > ((unsigned int) 0xffffffff + (unsigned int) 0) / sizeof(ref))
	nmax = ((unsigned int) 0xffffffff + (unsigned int) 0) / sizeof(ref);
...
uint ref_stack_count(const ref_stack * pstack)
{
    return pstack->extension_used + (pstack->p - pstack->bot + 1);
}

ref *ref_stack_index(const ref_stack * pstack, long idx)
{
...
uint ref_stack_counttomark(const ref_stack * pstack)
{
    uint scanned = 0; {
...
int ref_stack_store(const ref_stack * pstack, ref * parray, uint count, uint skip, int age, bool check, client_name_t cname)
{
    uint left, pass;
    ref *to;
	...
	left = count, pass = skip; {
	    ref_stack_block *pblock_ = (ref_stack_block *) (pstack)->current.value.refs;
	    ref *ptr = (pstack)->bot;
	    uint size = (pstack)->p + 1 - (pstack)->bot;
	    for (;;) {
		if (size <= pass)
		    pass -= size;
		else {
	...
	for (;;) {
		...
		case 0:
		    while (size--) {
			from--, to--;
			(((void) ((((((to)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), parray, (ref_packed *) (to), cname) : 0))), ((void) (((*(to) = *(from)), ((to)->tas.type_attrs |= ((&gs_imemory)->new_mask))))));
		    } break;
		case 1:
		    while (size--) {
			from--, to--;
			((void) (((*(to) = *(from)), ((to)->tas.type_attrs |= ((&gs_imemory)->new_mask)))));
		    } break;
		}
		if (left == 0)
		    break;
...
void ref_stack_pop(register ref_stack * pstack, uint count)
{
    uint used;
...
int ref_stack_pop_block(register ref_stack * pstack)
{
    s_ptr bot = pstack->bot;
    uint count = pstack->p + 1 - bot;
    ref_stack_block *pcur = (ref_stack_block *) pstack->current.value.refs;
    register ref_stack_block *pnext = (ref_stack_block *) pcur->next.value.refs;
    uint used;
    ref *body;
    ref next;
    if (pnext == 0)
	return (pstack->underflow_error);
	...
	gs_free_ref_array(pstack->memory, &pstack->current, "ref_stack_pop_block");
	pstack->current = next;
...
int ref_stack_push(register ref_stack * pstack, uint count)
{
    uint needed = count;
    uint added;
	...
	int code;
	pstack->p = pstack->top;
	code = ref_stack_push_block(pstack, (pstack->top - pstack->bot + 1) / 3, count);
	if (code < 0) {
...
int ref_stack_push_block(register ref_stack * pstack, uint keep, uint add)
{
    uint count = pstack->p - pstack->bot + 1;
    ...
    if (keep > count)
	return ((-100));
    if (pstack->memory == 0 || pstack->extension_used + (pstack->top - pstack->bot) + add >= pstack->max_stack.value.intval || !pstack->allow_expansion)
	return (pstack->overflow_error);
    code = gs_alloc_ref_array(pstack->memory, &next, 0, pstack->block_size, "ref_stack_push_block");
    if (code < 0)
	return code;
...
void ref_stack_cleanup(ref_stack * pstack)
{
    ref_stack_block *pblock = (ref_stack_block *) pstack->current.value.refs;
...
} static void init_block(ref_stack * pstack, ref * psb, uint used)
{
    ref *brefs = psb->value.refs;
    register uint i;
    register ref *p;
    for (i = pstack->bot_guard, p = brefs + (sizeof(ref_stack_block) / sizeof(ref)); i != 0; i--, p++)
	(*(p) = *(&pstack->guard_value));
    if (pstack->top_guard) {
	ref *top = brefs + ((psb)->tas.rsize);
	int top_guard = pstack->top_guard;
	refset_null(top - top_guard, top_guard);
    }
    ((ref_stack_block *) brefs)->used = *psb;
    ((ref_stack_block *) brefs)->used.value.refs = brefs + (sizeof(ref_stack_block) / sizeof(ref)) + pstack->bot_guard;
    ((&((ref_stack_block *) brefs)->used)->tas.rsize = (0));
}
