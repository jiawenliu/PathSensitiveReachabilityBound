...
static int screen_cleanup(os_ptr);
static int zcurrenthalftone(register os_ptr op)
{
    gs_halftone ht;
	...
	op[-1] = ((int_gstate *) gs_state_client_data(igs))->screen_procs.colored.gray;
	((op)->value.intval = (1), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
	break;
    case ht_type_colorscreen:
	do {
	    if ((op += (13)) > (o_stack.top)) {
		o_stack.requested = (13);
		return ((-16));
	    } else
		(o_stack.p) = op;
	} while (0); {
		...
		gs_screen_halftone *pht = &ht.params.colorscreen.screens.indexed[i];
		...
		opc[2] = ((int_gstate *) gs_state_client_data(igs))->screen_procs.indexed[i];
	    }
	}
	...
	op[-1] = ((int_gstate *) gs_state_client_data(igs))->halftone;
...
static int zcurrentscreenlevels(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.intval = (gs_currentscreenlevels(igs)), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
    return 0;
}

int zscreen_enum_init(os_ptr, const gx_ht_order *, gs_screen_halftone *, ref *, int, int (*)(os_ptr), gs_memory_t *);
static int setscreen_finish(os_ptr);
static int zsetscreen(register os_ptr op)
{
    gs_screen_halftone screen;
    gx_ht_order order;
    ...
    if (code < 0)
	return code;
    mem = (gs_memory_t *) (&gs_imemory)->spaces.indexed[((int) (avm_space) (((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)) >> 2)];
    code = gs_screen_order_init_memory(&order, igs, &screen, gs_currentaccuratescreens(), mem);
    if (code < 0)
	return code;
    return zscreen_enum_init(op, &order, &screen, op, 3, setscreen_finish, mem);
}

int zscreen_enum_init(os_ptr op, const gx_ht_order * porder, gs_screen_halftone * psp, ref * pproc, int npop, int (*finish_proc) (os_ptr), gs_memory_t * mem)
{
    gs_screen_enum *penum;
    int code;
    if ((e_stack.p) > (e_stack.top) - (4 + 1)) {
	int es_code_ = ref_stack_extend(&e_stack, 4 + 1);
	if (es_code_ < 0)
	    return es_code_;
    };
    penum = gs_screen_enum_alloc(((gs_memory_t *) (gs_imemory.current)), "setscreen");
    if (penum == 0)
	return ((-25));
    (((e_stack.p) + 4)->value.pstruct = ((obj_header_t *) (penum)), (((e_stack.p) + 4)->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&gs_imemory))->current_space))));
    code = gs_screen_enum_init_memory(penum, porder, igs, psp, mem);
    if (code < 0) {
	screen_cleanup(op);
	return code;
    }
    (((e_stack.p) + 1)->value.opproc = (screen_cleanup), (((e_stack.p) + 1)->tas.type_attrs = ((t_null) << 8) + (0x80)), (((e_stack.p) + 1)->tas.rsize = (0)));
    (e_stack.p) += 4;
    (((e_stack.p) - 2)->value.opproc = (finish_proc), (((e_stack.p) - 2)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) - 2)->tas.rsize = (0)));
    (e_stack.p)[-1] = *pproc;
    (++(e_stack.p), (((e_stack.p))->value.opproc = (screen_sample), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    ((o_stack.p) -= (npop));
    return 5;
}

static int screen_sample(register os_ptr op)
{
    gs_screen_enum *penum = ((gs_screen_enum *) (((e_stack.p))->value.pstruct));
    gs_point pt;
	...
	return code;
    case 1:
	if ((((e_stack.p) - 2)->value.opproc) != 0)
	    code = (*(((e_stack.p) - 2)->value.opproc)) (op);
	(e_stack.p) -= 4;
	screen_cleanup(op);
    ...
    proc = (e_stack.p)[-1];
    (++(e_stack.p), (((e_stack.p))->value.opproc = (set_screen_continue), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    *++(e_stack.p) = proc;
    return 5;
}

static int set_screen_continue(register os_ptr op)
{
    float value;
    int code = num_params(op, 1, &value);
    if (code < 0)
	return code;
    code = gs_screen_next(((gs_screen_enum *) (((e_stack.p))->value.pstruct)), value);
    if (code < 0)
	return code;
    ((o_stack.p) -= (1));
    op--;
...
static int setscreen_finish(os_ptr op)
{
    gs_screen_install(((gs_screen_enum *) (((e_stack.p))->value.pstruct)));
    ((int_gstate *) gs_state_client_data(igs))->screen_procs.colored.red = (e_stack.p)[-1];
    ((int_gstate *) gs_state_client_data(igs))->screen_procs.colored.green = (e_stack.p)[-1];
    ((int_gstate *) gs_state_client_data(igs))->screen_procs.colored.blue = (e_stack.p)[-1];
    ((int_gstate *) gs_state_client_data(igs))->screen_procs.colored.gray = (e_stack.p)[-1];
    (((&((int_gstate *) gs_state_client_data(igs))->halftone)->tas.type_attrs = ((t_null) << 8) + (0)));
    return 0;
}

static int screen_cleanup(os_ptr op)
{
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), (e_stack.p)[4].value.pstruct, "screen_cleanup");
    return 0;
}
...
const op_def *zht_op_defs(void)
{
    static const op_def op_defs_[] = { {"0.currenthalftone", zcurrenthalftone}, {"0.currentscreenlevels", zcurrentscreenlevels}, {"3setscreen", zsetscreen}, {"0%screen_sample", screen_sample}, {"1%set_screen_continue", set_screen_continue}, {"0%setscreen_finish", setscreen_finish}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
