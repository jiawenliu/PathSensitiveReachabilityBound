...
int clist_fopen(char *fname, const char *fmode, clist_file_ptr * pcf, gs_memory_t * mem, bool ok_to_compress)
{
	...
	if (fmode[0] == 'r')
	    return ((-9));
	*pcf = (clist_file_ptr) gp_open_scratch_file(gp_scratch_file_name_prefix, fname, fmode);
    } else
	*pcf = gp_fopen(fname, fmode);
    if (*pcf == ((void *) 0)) {
	(fprintf(stderr, "Could not open the scratch file %s.\n", fname));
	return ((-9));
    }
    return 0;
}
...
int clist_fclose(clist_file_ptr cf, const char *fname, bool delete)
{
    return (fclose((FILE *) cf) != 0 ? ((-12)) : delete ? clist_unlink(fname) : 0);
}
...
int clist_fseek(clist_file_ptr cf, long offset, int mode, const char *ignore_fname)
{
