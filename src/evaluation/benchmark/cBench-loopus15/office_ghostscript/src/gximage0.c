...
int gx_default_image_data(gx_device * dev, void *info, const byte ** planes, int data_x, uint raster, int height)
{
    ...
    uint bcount = (width_spp / penum->num_planes * penum->bps + 7) >> 3;
	...
	int px;
	int sourcex = data_x;
	const byte *buffer = (*penum->unpack) (penum->buffer, &sourcex, planes[0] + offset, data_x, bcount, &penum->map[0].table, penum->spread);
	for (px = 1; px < nplanes; ++px)
	    (*penum->unpack) (penum->buffer + (px << penum->log2_xbytes), &ignore_data_x, planes[px] + offset, data_x, bcount, &penum->map[px].table, penum->spread);
	penum->cur.x = ((penum->dda.row.x).state).Q;
	((penum->dda.row.x).state).Q += (((penum->dda.row.x).state).R >= ((penum->dda.row.x).step).dR ? (((penum->dda.row.x).state).R -= ((penum->dda.row.x).step).dR, ((penum->dda.row.x).step).dQ) : (((penum->dda.row.x).state).R += ((penum->dda.row.x).step).NdR, ((penum->dda.row.x).step).dQ + 1));
	penum->cur.y = ((penum->dda.row.y).state).Q;
	((penum->dda.row.y).state).Q += (((penum->dda.row.y).state).R >= ((penum->dda.row.y).step).dR ? (((penum->dda.row.y).state).R -= ((penum->dda.row.y).step).dR, ((penum->dda.row.y).step).dQ) : (((penum->dda.row.y).state).R += ((penum->dda.row.y).step).NdR, ((penum->dda.row.y).step).dQ + 1));
	if (!penum->interpolate)
	    switch (penum->posture) {
	    case image_portrait:{
		    fixed yc = penum->cur.y, yn = ((penum->dda.row.y).state).Q;
		    if (yn < yc) {
			fixed temp = yn;
			yn = yc;
			yc = temp;
			...
			goto mt;
		    penum->yci = ((int) ((((yc) + (((1 << 12) >> 1) - 1L))) >> (12)));
		    penum->hci = ((int) ((((yn) + (((1 << 12) >> 1) - 1L))) >> (12))) - penum->yci;
		    if (penum->hci == 0)
			goto mt;
		}
		break;
	    case image_landscape:{
		    fixed xc = penum->cur.x, xn = ((penum->dda.row.x).state).Q;
		    if (xn < xc) {
			fixed temp = xn;
			xn = xc;
			xc = temp;
			...
			goto mt;
		    penum->xci = ((int) ((((xc) + (((1 << 12) >> 1) - 1L))) >> (12)));
		    penum->wci = ((int) ((((xn) + (((1 << 12) >> 1) - 1L))) >> (12))) - penum->xci;
		    if (penum->wci == 0)
			goto mt;
		}
		break;
	...
	(((penum->dda.pixel0.x).state).Q += (penum->cur.x - penum->prev.x));
	(((penum->dda.pixel0.y).state).Q += (penum->cur.y - penum->prev.y));
	penum->prev = penum->cur;
	code = (*penum->render) (penum, buffer, sourcex, width_spp, 1, dev);
	if (code < 0)
	    goto err;
      mt:;
    }
    ...
    switch (penum->posture) {
    case image_portrait:{
	    fixed yc = ((penum->dda.row.y).state).Q;
	    penum->yci = ((int) (((yc - adjust) + ((1 << 12) >> 1)) >> (12)));
	    penum->hci = ((int) (((yc + adjust) + ((1 << 12) >> 1)) >> (12))) - penum->yci;
	} break;
    case image_landscape:{
	    fixed xc = ((penum->dda.row.x).state).Q;
	    penum->xci = ((int) (((xc - adjust) + ((1 << 12) >> 1)) >> (12)));
	    penum->wci = ((int) (((xc + adjust) + ((1 << 12) >> 1)) >> (12))) - penum->xci;
	} break;
    case image_skewed:;
    }
    (((penum->dda.pixel0.x).state).Q += (penum->cur.x - penum->prev.x));
    (((penum->dda.pixel0.y).state).Q += (penum->cur.y - penum->prev.y));
    code = (*penum->render) (penum, ((void *) 0), 0, width_spp, 0, dev);
    if (code < 0) {
	penum->y--;
	goto err;
    }
    ...
    goto out;
  err:while (penum->y > y) {
	((penum->dda.row.x).state).Q -= (((penum->dda.row.x).state).R < ((penum->dda.row.x).step).NdR ? (((penum->dda.row.x).state).R += ((penum->dda.row.x).step).dR, ((penum->dda.row.x).step).dQ) : (((penum->dda.row.x).state).R -= ((penum->dda.row.x).step).NdR, ((penum->dda.row.x).step).dQ + 1));
	((penum->dda.row.y).state).Q -= (((penum->dda.row.y).state).R < ((penum->dda.row.y).step).NdR ? (((penum->dda.row.y).state).R += ((penum->dda.row.y).step).dR, ((penum->dda.row.y).step).dQ) : (((penum->dda.row.y).state).R -= ((penum->dda.row.y).step).NdR, ((penum->dda.row.y).step).dQ + 1));
	--(penum->y);
    }
  out:return code;
}
...
int gx_default_end_image(gx_device * dev, void *info, bool draw_last)
{
    ...
    (*(mem)->procs.free_object) (mem, penum, "gx_default_end_image");
    return 0;
}
