...
static gs_ptr_type_t lzwd_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (((stream_LZW_state *) vptr)->table.decode);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void lzwd_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((stream_LZW_state *) vptr)->table.decode = gs_reloc_struct_ptr((const void *) ((const stream_LZW_state *) vptr)->table.decode, gcst);
} const gs_memory_struct_type_t st_LZW_state = { sizeof(stream_LZW_state), "LZWDecode state", 0, 0, lzwd_enum_ptrs, lzwd_reloc_ptrs, 0 };

void s_LZW_set_defaults(stream_state * st)
{
    ((((stream_LZW_state *) st))->InitialCodeLength = 8, (((stream_LZW_state *) st))->FirstBitLowOrder = ((bool) 0), (((stream_LZW_state *) st))->BlockData = ((bool) 0), (((stream_LZW_state *) st))->EarlyChange = 1);
} void s_LZW_release(stream_state * st)
{
    (*(st->memory)->procs.free_object) (st->memory, ((stream_LZW_state *) st)->table.decode, "LZW(close)");
}
