...
static int zcurrentpacking(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    (*(op) = *(&ref_array_packing));
    return 0;
}

int zpackedarray(register os_ptr op)
{
    int code;
    ref parr;
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
    if (op->value.intval < 0 || (op->value.intval > op - (o_stack.bot) && op->value.intval >= ref_stack_count(&o_stack)))
	return ((-15));
    (o_stack.p)--;
    code = make_packed_array(&parr, &o_stack, (uint) op->value.intval, "packedarray");
    (o_stack.p)++;
    if (code >= 0)
	*(o_stack.p) = parr;
    return code;
...
static int zsetpacking(register os_ptr op)
{
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_boolean)))
	return (check_type_failed(&*op));
    (((void) ((((((&ref_array_packing)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), ((void *) 0), (ref_packed *) (&ref_array_packing), "setpacking") : 0))), ((void) (((*(&ref_array_packing) = *(op)), ((&ref_array_packing)->tas.type_attrs |= ((&gs_imemory)->new_mask))))));
    ((o_stack.p) -= (1));
    return 0;
}

int make_packed_array(ref * parr, ref_stack * pstack, uint size, client_name_t cname)
{
    uint i;
    const ref *pref;
	...
	case t_name:
	    if (name_index(pref) >= ((1 << 12) - 1))
		break;
	    idest++;
	    continue;
	case t_integer:
	    if (pref->value.intval < (-(1 << (12 - 1))) || pref->value.intval > ((1 << (12 - 1)) - 1))
		break;
	    idest++;
	    continue;
	case t_oparray:
	    if ((avm_space) (((pref)->tas.type_attrs) & (((1 << 2) - 1) << 2)) > (space))
		return ((-7));
	case t_operator:{
		uint oidx;
	...
	{
	    int i = (idest - ishort) & (((((4 - 1) | (4 - 1) | (4 - 1)) + 1) / 2) - 1);
	    if (ishort == 0)
		idest += skip = -i & (((((4 - 1) | (4 - 1) | (4 - 1)) + 1) / 2) - 1);
	    else
		idest += ((sizeof(ref) / sizeof(ref_packed)) - 1) * i;
	}
	ishort = idest += (sizeof(ref) / sizeof(ref_packed));
    }
    pad = -idest & ((sizeof(ref) / sizeof(ref_packed)) - 1);
    code = gs_alloc_ref_array((gs_imemory.current), &rtemp, 0, (idest + pad) / (sizeof(ref) / sizeof(ref_packed)), cname);
    if (code < 0)
	return code;
	...
	*pbody++ = ((ref_packed) (pt_integer) << 13);
	...
	case t_name:{
		...
		if (nidx >= ((1 << 12) - 1))
		    break;
		*pdest++ = nidx + ((((pref)->tas.type_attrs) & (0x80)) ? ((ref_packed) (pt_executable_name) << 13) : ((ref_packed) (pt_literal_name) << 13));
	    }
	    continue;
	case t_integer:
	    if (pref->value.intval < (-(1 << (12 - 1))) || pref->value.intval > ((1 << (12 - 1)) - 1))
		break;
	    *pdest++ = ((ref_packed) (pt_integer) << 13) + ((short) pref->value.intval - (-(1 << (12 - 1))));
	    continue;
	case t_oparray:
	case t_operator:{
		uint oidx;
	...
	{
	    int i = (pdest - pshort) & (((((4 - 1) | (4 - 1) | (4 - 1)) + 1) / 2) - 1);
	    const ref_packed *psrc = pdest;
	    ref *pmove = (ref *) (pdest += ((sizeof(ref) / sizeof(ref_packed)) - 1) * i);
	    ((void) (((*(pmove) = *(pref)), ((pmove)->tas.type_attrs |= ((&gs_imemory)->new_mask)))));
	    while (--i >= 0) {
		--psrc;
		--pmove;
    ...
    {
	int atype = (pdest == pbody + size ? t_shortarray : t_mixedarray);
	for (; pad; pad--)
	    *pdest++ = ((ref_packed) (pt_integer) << 13);
	ref_stack_pop(pstack, size);
	((parr)->value.packed = (pbody + skip), ((parr)->tas.type_attrs = ((atype) << 8) + (((0x20 + 0x40) | space) | ((&gs_imemory)->new_mask))), ((parr)->tas.rsize = (size)));
    }
    return 0;
}

const op_def *zpacked_op_defs(void)
{
    static const op_def op_defs_[] = { {"0currentpacking", zcurrentpacking}, {"1packedarray", zpackedarray}, {"1setpacking", zsetpacking}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
