...
int gs_lib_device_list(const gx_device *** plist, gs_memory_struct_type_t ** pst);
void gx_device_finalize(void *vptr)
{
    ((void) (gs_closedevice((gx_device *) vptr)));
} static gs_ptr_type_t device_forward_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (gx_device_enum_ptr(((gx_device_forward *) vptr)->target));
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void device_forward_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	((gx_device_forward *) vptr)->target = gx_device_reloc_ptr(((gx_device_forward *) vptr)->target, gcst);
}} const gs_memory_struct_type_t st_device = { sizeof(gx_device), "gx_device", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, gx_device_finalize };
const gs_memory_struct_type_t st_device_forward = { sizeof(gx_device_forward), "gx_device_forward", 0, 0, device_forward_enum_ptrs, device_forward_reloc_ptrs, gx_device_finalize };
const gs_memory_struct_type_t st_device_null = { sizeof(gx_device_null), "gx_device_null", 0, 0, device_forward_enum_ptrs, device_forward_reloc_ptrs, gx_device_finalize };
static const gs_memory_struct_type_t st_device_unknown = { sizeof(byte), "gx_device(unknown)", 0, 0, 0, 0, gx_device_finalize };
...
int gdev_initialize(gx_device * dev)
{
    *dev = *(gx_device *) & gs_null_device;
    return 0;
}

int gs_flushpage(gs_state * pgs)
{
    gx_device *dev = gs_currentdevice(pgs);
    return (*((dev)->std_procs.sync_output)) (dev);
}

int gs_copypage(gs_state * pgs)
{
    return gs_output_page(pgs, 1, 0);
}

int gs_output_page(gs_state * pgs, int num_copies, int flush)
{
    gx_device *dev = gs_currentdevice(pgs);
    int code;
    if (dev->IgnoreNumCopies)
	num_copies = 1;
    code = (*((dev)->std_procs.output_page)) (dev, num_copies, flush);
    if (code >= 0)
...
int gs_copyscanlines(gx_device * dev, int start_y, byte * data, uint size, int *plines_copied, uint * pbytes_copied)
{
	...
	int code = (*((dev)->std_procs.get_bits)) (dev, start_y + i, dest, ((void *) 0));
	if (code < 0) {
...
gx_device *gs_currentdevice(const gs_state * pgs)
{
    return pgs->device;
}
...
void gs_deviceinitialmatrix(gx_device * dev, gs_matrix * pmat)
{
    ...
    (*((dev)->std_procs.get_initial_matrix)) (dev, pmat);
}
...
int gs_copydevice(gx_device ** pnew_dev, const gx_device * dev, gs_memory_t * mem)
{
	...
	for (i = 0; list[i] != dev; ++i)
		...
		new_dev = (gx_device *) (*(mem)->procs.alloc_struct_array_immovable) (mem, dev->params_size, st, "gs_copydevice(unknown)");
		goto out;
	    }
	st += i;
	if (procs == 0)
	    procs = &dev->std_procs;
	if (procs->fill_rectangle == 0 || procs->get_xfont_procs == gx_forward_get_xfont_procs)
	    forward = ((bool) 1);
	if (forward)
    ...
    new_dev = (gx_device *) (*(mem)->procs.alloc_struct_immovable) (mem, std, "gs_copydevice");
  out:if (new_dev == 0)
	return ((-25));
...
int gs_setdevice(gs_state * pgs, gx_device * dev)
{
...
int gs_setdevice_no_erase(gs_state * pgs, gx_device * dev)
{
	...
	code = (*((dev)->std_procs.open_device)) (dev);
	if (code < 0)
	    return (code);
	dev->is_open = ((bool) 1);
    }
...
void gs_make_null_device(gx_device_null * dev, gs_memory_t * mem)
{
    *dev = gs_null_device;
    dev->memory = mem;
} void gs_nulldevice(gs_state * pgs)
{
...
} int gs_closedevice(gx_device * dev)
{
    int code = 0;
    if (dev->is_open) {
	code = (*((dev)->std_procs.close_device)) (dev);
	if (code < 0)
	    return (code);
	dev->is_open = ((bool) 0);
    }
...
} void gx_set_device_only(gs_state * pgs, gx_device * dev)
{
    pgs->device = dev;
} uint gx_device_raster(const gx_device * dev, bool pad)
{
    ulong bits = (ulong) dev->width * dev->color_info.depth;
    return (pad ? ((uint) (((bits + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2)) : (uint) ((bits + 7) >> 3));
}
...
int gx_device_adjust_resolution(gx_device * dev, int actual_width, int actual_height, int fit)
{
    ...
    double ratio = (fit ? (((width_ratio) < (height_ratio)) ? (width_ratio) : (height_ratio)) : (((width_ratio) > (height_ratio)) ? (width_ratio) : (height_ratio)));
