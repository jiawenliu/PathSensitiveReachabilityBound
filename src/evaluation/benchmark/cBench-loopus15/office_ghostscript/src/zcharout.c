...
int zsetcachedevice(os_ptr);
int zsetcachedevice2(os_ptr);
int zchar_exec_char_proc(os_ptr op)
{
    es_ptr ep;
...
int zchar_get_metrics(const gs_font_base * pbfont, const ref * pcnref, float psbw[4])
{
    const ref *pfdict = &((font_data *) ((pbfont)->client_data))->dict;
    ref *pmdict;
    if (dict_find_string(pfdict, "Metrics", &pmdict) > 0) {
	ref *pmvalue;
	if (!((((const byte *) &((&*pmdict)->tas.type_attrs))[1]) == (t_dictionary)))
	    return ((-20));
	if (!(((&*(&(&*pmdict)->value.pdict->values))->tas.type_attrs) & (0x20)))
	    return ((-7));
	if (dict_find(pmdict, pcnref, &pmvalue) > 0) {
		...
		int code;
		if (!(((&*pmvalue)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_array) << 8) + (0x20))))
		    return ((!((((const byte *) &((&*pmvalue)->tas.type_attrs))[1]) == (t_array)) ? (-20) : (-7)));
		switch (((pmvalue)->tas.rsize)) {
		case 2:
		    code = num_params(pmvalue->value.refs + 1, 2, psbw);
		    psbw[2] = psbw[1];
		    psbw[1] = psbw[3] = 0;
		    break;
		case 4:
		    code = num_params(pmvalue->value.refs + 3, 4, psbw);
		    break;
		default:
		    return ((-15));
		}
...
int zchar_set_cache(os_ptr op, const gs_font_base * pbfont, const ref * pcnref, const float psb[2], const float pwidth[2], const gs_rect * pbbox, int (*cont_fill) (os_ptr), int (*cont_stroke) (os_ptr))
{
	...
	double expand = (((1.415) > (gs_currentmiterlimit(igs))) ? (1.415) : (gs_currentmiterlimit(igs))) * gs_currentlinewidth(igs) / 2;
	...
	int nparams;
	if (have_cdevproc) {
	    if (!(((&*pcdevproc)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
		return (check_proc_failed(&*pcdevproc));;
	...
	if (psb != 0) {
	    do {
		if ((op += (nparams + 3)) > (o_stack.top)) {
		    o_stack.requested = (nparams + 3);
		    return ((-16));
		} else
		    (o_stack.p) = op;
	    } while (0);
	    ...
	    do {
		if ((op += (nparams + 1)) > (o_stack.top)) {
		    o_stack.requested = (nparams + 1);
		    return ((-16));
		} else
		    (o_stack.p) = op;
	    } while (0);
	...
	(++(e_stack.p), (((e_stack.p))->value.opproc = (zsetc), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
	++(e_stack.p);
	(*((e_stack.p)) = *(pcdevproc));
	return 5;
    }
    {
	int code = (metrics2 ? gs_setcachedevice2(penum, igs, w2) : gs_setcachedevice(penum, igs, w2));
	if (code < 0)
	    return code;
    }
    if (psb != 0) {
