...
float spot_dummy(floatp x, floatp y)
{
    return (x + y) / 2;
}

static int setcolorscreen_finish(os_ptr);
static int setcolorscreen_cleanup(os_ptr);
static int zsetcolorscreen(register os_ptr op)
{
    ...
    } mem = (gs_memory_t *) (&gs_imemory)->spaces.indexed[space];
    ...
    do {
	if (((pht) = (gs_halftone *) (*(mem)->procs.alloc_struct) (mem, &st_halftone, "setcolorscreen(halftone)")) == 0) {
	    pht = 0;
	} else {
	    (pht)->rc.ref_count = 0;
	    (pht)->rc.memory = mem;
    ...
    do {
	if (((pdht) = (gx_device_halftone *) (*(mem)->procs.alloc_struct) (mem, &st_device_halftone, "setcolorscreen(device halftone)")) == 0) {
	    pdht = 0;
	} else {
	    (pdht)->rc.ref_count = 0;
	    (pdht)->rc.memory = mem;
    ...
    if (code >= 0) {
	es_ptr esp0 = (e_stack.p);
	(e_stack.p) += 8;
	(((e_stack.p) - 7)->value.opproc = (setcolorscreen_cleanup), (((e_stack.p) - 7)->tas.type_attrs = ((t_null) << 8) + (0x80)), (((e_stack.p) - 7)->tas.rsize = (0)));
	memcpy((e_stack.p) - 6, sprocs, sizeof(ref) * 4);
	...
	(((e_stack.p))->value.opproc = (setcolorscreen_finish), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0)));
	for (i = 0; i < 4; i++) {
	    code = zscreen_enum_init(op, &pdht->components[(i + 1) & 3].corder, &pht->params.colorscreen.screens.indexed[i], &sprocs[i], 0, 0, mem);
	    if (code < 0) {
		(e_stack.p) = esp0;
		break;
	    }
...
static int setcolorscreen_finish(os_ptr op)
{
    ...
    if (code < 0)
	return code;
    memcpy(((int_gstate *) gs_state_client_data(igs))->screen_procs.indexed, (e_stack.p) - 5, sizeof(ref) * 4);
...
static int setcolorscreen_cleanup(os_ptr op)
{
    gs_halftone *pht = ((gs_halftone *) (((e_stack.p) + 6)->value.pstruct));
    gx_device_halftone *pdht = ((gx_device_halftone *) (((e_stack.p) + 7)->value.pstruct));
...
const op_def *zht1_op_defs(void)
{
    static const op_def op_defs_[] = { {"<setcolorscreen", zsetcolorscreen}, {"0%setcolorscreen_finish", setcolorscreen_finish}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
