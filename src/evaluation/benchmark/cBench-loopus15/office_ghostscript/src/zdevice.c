...
static int zcopydevice(register os_ptr op)
{
    gx_device *new_dev;
    int code;
    if (!(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_device) << 8) + (0x20))))
	return ((!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&*op) : (-7)));
    code = gs_copydevice(&new_dev, op->value.pdevice, ((gs_memory_t *) (gs_imemory.current)));
    if (code < 0)
	return code;
    new_dev->memory = ((gs_memory_t *) (gs_imemory.current));
    ((op)->value.pdevice = (new_dev), ((op)->tas.type_attrs = ((t_device) << 8) + ((((&gs_imemory))->current_space) | (0x10 + 0x20 + 0x40))));
    return 0;
}

static int zcopyscanlines(register os_ptr op)
{
    os_ptr op1 = op - 1;
    os_ptr op2 = op - 2;
    gx_device *dev;
    int code;
    uint bytes_copied;
    if (!(((&*op2)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_device) << 8) + (0x20))))
	return ((!((((const byte *) &((&*op2)->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&*op2) : (-7)));
    ...
    code = gs_copyscanlines(dev, (int) op1->value.intval, op->value.bytes, ((op)->tas.rsize), ((void *) 0), &bytes_copied);
    if (code < 0)
	return ((-15));
    *op2 = *op;
    ((op2)->tas.rsize = (bytes_copied));
    ((o_stack.p) -= (2));
    return 0;
}

int zcurrentdevice(register os_ptr op)
{
...
int zdevicename(register os_ptr op)
{
    const char *dname;
    if (!(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_device) << 8) + (0x20))))
	return ((!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&*op) : (-7)));
...
int zflushpage(register os_ptr op)
{
    return gs_flushpage(igs);
}

static int zgetdevice(register os_ptr op)
{
    const gx_device *dev;
...
static int zgetdeviceparams(os_ptr op)
{
    ref rkeys;
    gx_device *dev;
    ...
    int code;
    ref *pmark;
    if (!(((&op[-1])->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_device) << 8) + (0x20))))
	return ((!((((const byte *) &((&op[-1])->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&op[-1]) : (-7)));
    rkeys = *op;
...
static int zmakewordimagedevice(register os_ptr op)
{
    os_ptr op1 = op - 1;
    gs_matrix imat;
    gx_device *new_dev;
    ...
    if ((ulong) (op[-3]).value.intval > (((unsigned int) 0xffffffff + (unsigned int) 0) >> 1))
	return ((-15));
    if (!((((const byte *) &((&op[-2])->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&op[-2]));
    if ((ulong) (op[-2]).value.intval > (((unsigned int) 0xffffffff + (unsigned int) 0) >> 1))
	return ((-15));
	...
	if (!((((const byte *) &((&*op1)->tas.type_attrs))[1]) == (t_string)))
	    return (check_type_failed(&*op1));
	...
	return code;
    code = gs_makewordimagedevice(&new_dev, &imat, (int) op[-3].value.intval, (int) op[-2].value.intval, colors, colors_size, op->value.boolval, ((bool) 1), ((gs_memory_t *) (gs_imemory.current)));
    if (code == 0) {
	new_dev->memory = ((gs_memory_t *) (gs_imemory.current));
	((op - 4)->value.pdevice = (new_dev), ((op - 4)->tas.type_attrs = ((t_device) << 8) + (imemory_space((gs_imemory.current)) | (0x10 + 0x20 + 0x40))));
	((o_stack.p) -= (4));
    }
    return code;
}

static int znulldevice(register os_ptr op)
{
    gs_nulldevice(igs);
    ((((int_gstate *) gs_state_client_data(igs)))->pagedevice = i_null_pagedevice);
    return 0;
}

static int zoutputpage(register os_ptr op)
{
    int code;
...
static int zputdeviceparams(os_ptr op)
{
    ...
    if (pdev == 0)
	return ((-17));
    if (!((((const byte *) &((&*prequire_all)->tas.type_attrs))[1]) == (t_boolean)))
	return ((-20));
    if (!(((&*pdev)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_device) << 8) + (0x10))))
	return ((!((((const byte *) &((&*pdev)->tas.type_attrs))[1]) == (t_device)) ? (-20) : (-7)));
    dev = pdev->value.pdevice;
    code = stack_param_list_read(&list, &o_stack, 0, ppolicy, prequire_all->value.boolval);
    if (code < 0)
	return code;
    old_width = dev->width;
	...
	if (list.results[i] < 0) {
	    *ref_stack_index(&o_stack, dest) = *ref_stack_index(&o_stack, count - (i << 1) - 2);
	    gs_errorname(list.results[i], ref_stack_index(&o_stack, dest - 1));
	    dest -= 2;
	}
    ...
    (((o_stack.p))->value.boolval = (code), (((o_stack.p))->tas.type_attrs = ((t_boolean) << 8) + (0)));
    ((((int_gstate *) gs_state_client_data(igs)))->pagedevice = i_null_pagedevice);
    return 0;
}

int zsetdevice(register os_ptr op)
{
    int code;
    if (!(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_device) << 8) + (0x10))))
	return ((!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&*op) : (-7)));
...
const op_def *zdevice_op_defs(void)
{
    static const op_def op_defs_[] = { {"1copydevice", zcopydevice}, {"3copyscanlines", zcopyscanlines}, {"0currentdevice", zcurrentdevice}, {"1.devicename", zdevicename}, {"0flushpage", zflushpage}, {"1.getdevice", zgetdevice}, {"2.getdeviceparams", zgetdeviceparams}, {"5makewordimagedevice", zmakewordimagedevice}, {"0nulldevice", znulldevice}, {"2.outputpage", zoutputpage}, {"3.putdeviceparams", zputdeviceparams}, {"1.setdevice", zsetdevice}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
