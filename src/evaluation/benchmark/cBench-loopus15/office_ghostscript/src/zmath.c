...
long zrand_state;
void zrand_state_init(long *pstate)
{
    *pstate = 1;
} static void zmath_init(void)
{
    zrand_state_init(&zrand_state);
} static int zsqrt(register os_ptr op)
{
    float num;
...
static int zarccos(register os_ptr op)
{
    float num, result;
    int code = num_params(op, 1, &num);
    if (code < 0)
...
static int zarcsin(register os_ptr op)
{
    float num, result;
    int code = num_params(op, 1, &num);
    if (code < 0)
...
static int zatan(register os_ptr op)
{
    float args[2];
    float result;
	...
	result = atan2(args[0], args[1]) * (180.0 / 3.14159265358979323846);
	if (result < 0)
	    result += 360;
    }
...
static int zcos(register os_ptr op)
{
    float angle;
...
static int zsin(register os_ptr op)
{
    float angle;
    int code = num_params(op, 1, &angle);
    if (code < 0)
...
static int zexp(register os_ptr op)
{
    float args[2];
    float result;
    double ipart;
...
static int zln(register os_ptr op)
{
    float num;
    int code = num_params(op, 1, &num);
    if (code < 0)
...
static int zlog(register os_ptr op)
{
    float num;
    int code = num_params(op, 1, &num);
    if (code < 0)
...
static int zrand(register os_ptr op)
{
    zrand_state = 16807 * (zrand_state % 127773) - 2836 * (zrand_state / 127773);
    if (zrand_state <= 0)
	zrand_state += 0x7fffffff;
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
	    return ((-16));
    ...
    ((op)->value.intval = (zrand_state), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
    return 0;
}

static int zsrand(register os_ptr op)
{
    long state;
...
static int zrrand(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
...
const op_def *zmath_op_defs(void)
{
    static const op_def op_defs_[] = { {"1arccos", zarccos}, {"1arcsin", zarcsin}, {"2atan", zatan}, {"1cos", zcos}, {"2exp", zexp}, {"1ln", zln}, {"1log", zlog}, {"0rand", zrand}, {"0rrand", zrrand}, {"1sin", zsin}, {"1sqrt", zsqrt}, {"1srand", zsrand}, {(char *) 0, (op_proc_p) zmath_init} };
    return op_defs_;
}
