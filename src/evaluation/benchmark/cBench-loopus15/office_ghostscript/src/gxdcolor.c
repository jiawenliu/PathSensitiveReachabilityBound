...
const gx_device_color_procs gx_dc_procs_none = { gx_dc_no_load, gx_dc_no_fill_rectangle, gx_dc_no_fill_masked, 0, 0 }, gx_dc_procs_null = {
gx_dc_no_load, gx_dc_no_fill_rectangle, gx_dc_no_fill_masked, 0, 0}, gx_dc_procs_pure = {
gx_dc_pure_load, gx_dc_pure_fill_rectangle, gx_dc_pure_fill_masked, 0, 0};

const gx_device_color_procs *gx_dc_type_none = &gx_dc_procs_none;
const gx_device_color_procs *gx_dc_type_null = &gx_dc_procs_null;
const gx_device_color_procs *gx_dc_type_pure = &gx_dc_procs_pure;
const gx_rop_source_t gx_rop_no_source_0 = { ((void *) 0), 0, 0, 0L, {0, 0}, ((bool) 1) };

static int gx_dc_no_load(gx_device_color * pdevc, const gs_imager_state * ignore_pis, gx_device * ignore_dev, gs_color_select_t ignore_select)
{
    return 0;
}
...
static int gx_dc_pure_load(gx_device_color * pdevc, const gs_imager_state * ignore_pis, gx_device * ignore_dev, gs_color_select_t ignore_select)
{
    return 0;
}
...
static int gx_dc_pure_fill_rectangle(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source)
{
	...
	return (*((dev)->std_procs.fill_rectangle)) (dev, x, y, w, h, pdevc->colors.pure); {
	...
	return (*((dev)->std_procs.strip_copy_rop)) (dev, source->sdata, source->sourcex, source->sraster, source->id, (source->use_scolors ? source->scolors : ((void *) 0)), ((void *) 0), colors, x, y, w, h, 0, 0, lop);
}} static int gx_dc_pure_fill_masked(const gx_device_color * pdevc, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, bool invert)
{
    if ((((lop) & (0x200 | (rop3_1 - rop3_S))) == (rop3_T & ~rop3_S))) {
	gx_color_index color0, color1;
	if (invert)
	    color0 = pdevc->colors.pure, color1 = ((unsigned long) (-1));
	else
	    color1 = pdevc->colors.pure, color0 = ((unsigned long) (-1));
	return (*((dev)->std_procs.copy_mono)) (dev, data, data_x, raster, id, x, y, w, h, color0, color1);
    }
    {
	gx_color_index scolors[2];
	gx_color_index tcolors[2];
	scolors[0] = (*((dev)->std_procs.map_rgb_color)) (dev, (gx_color_value) 0, (gx_color_value) 0, (gx_color_value) 0);
	scolors[1] = (*((dev)->std_procs.map_rgb_color)) (dev, ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)), ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)), ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)));
	tcolors[0] = tcolors[1] = pdevc->colors.pure;
	return (*((dev)->std_procs.strip_copy_rop)) (dev, data, data_x, raster, id, scolors, ((void *) 0), tcolors, x, y, w, h, 0, 0, (invert ? ((((lop) & rop3_S) >> 2) | (((lop) & (rop3_1 - rop3_S)) << 2) | ((lop) & ~rop3_1)) : lop) | 0x100);
}} int gx_dc_default_fill_masked(const gx_device_color * pdevc, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, bool invert)
{
    int lbit = data_x & 7;
