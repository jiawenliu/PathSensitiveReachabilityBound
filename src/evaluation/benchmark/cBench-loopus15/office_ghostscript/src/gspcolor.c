...
void gs_setrasterop(gs_state *, gs_rop3_t);
gs_rop3_t gs_currentrasterop(const gs_state *);
void gs_setsourcetransparent(gs_state *, bool);
bool gs_currentsourcetransparent(const gs_state *);
void gs_settexturetransparent(gs_state *, bool);
bool gs_currenttexturetransparent(const gs_state *);
int gs_setrenderalgorithm(gs_state *, int);
int gs_currentrenderalgorithm(const gs_state *);
gs_logical_operation_t gs_current_logical_op(const gs_state *);
void gs_set_logical_op(gs_state *, gs_logical_operation_t);
...
typedef struct gx_device_tile_clip_s {
    ...
    gs_int_point phase;
    union _b {
	byte bytes[((300 / (1 << 2)) * (1 << 2))];
	ulong longs[((300 / (1 << 2)) * (1 << 2)) / (1 << 2)];
    } buffer;
} gx_device_tile_clip;
int tile_clip_initialize(gx_device_tile_clip *, const gx_strip_bitmap *, gx_device *, int, int);
void tile_clip_set_phase(gx_device_tile_clip *, int, int);
...
static gs_ptr_type_t client_pattern_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (((gs_client_pattern *) vptr)->client_data);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void client_pattern_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((gs_client_pattern *) vptr)->client_data = gs_reloc_struct_ptr((const void *) ((const gs_client_pattern *) vptr)->client_data, gcst);
} static const gs_memory_struct_type_t st_client_pattern = { sizeof(gs_client_pattern), "client pattern", 0, 0, client_pattern_enum_ptrs, client_pattern_reloc_ptrs, 0 };

static gs_ptr_type_t pattern_instance_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return (*st_client_pattern.enum_ptrs) (&((gs_pattern_instance *) vptr)->template, sizeof(((gs_pattern_instance *) vptr)->template), index - (1), pep);
    case 0:
	do {
	    *pep = (const void *) (((gs_pattern_instance *) vptr)->saved);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void pattern_instance_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{
    (*st_client_pattern.reloc_ptrs) (&((gs_pattern_instance *) vptr)->template, sizeof(((gs_pattern_instance *) vptr)->template), gcst);
    ((gs_pattern_instance *) vptr)->saved = gs_reloc_struct_ptr((const void *) ((const gs_pattern_instance *) vptr)->saved, gcst);
} const gs_memory_struct_type_t st_pattern_instance = { sizeof(gs_pattern_instance), "pattern instance", 0, 0, pattern_instance_enum_ptrs, pattern_instance_reloc_ptrs, 0 };

int gx_pattern_load(gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
extern int gx_remap_Pattern(const gs_client_color *, const gs_color_space *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static int gx_install_Pattern(gs_color_space *, gs_state *);
static void gx_adjust_cspace_Pattern(const gs_color_space *, gs_memory_t *, int);
static void gx_init_Pattern(gs_client_color *, const gs_color_space *);
static void gx_adjust_color_Pattern(const gs_client_color *, const gs_color_space *, gs_memory_t *, int);
static gs_ptr_type_t gx_enum_ptrs_Pattern(void *ptr, uint size, uint index, const void **pep);
static void gx_reloc_ptrs_Pattern(void *ptr, uint size, gc_state_t * gcst);
const gs_color_space_type gs_color_space_type_Pattern = { gs_color_space_index_Pattern, -1, ((bool) 0), gx_init_Pattern, gx_no_concrete_space, gx_no_concretize_color, ((void *) 0), gx_remap_Pattern, gx_install_Pattern, gx_adjust_cspace_Pattern, gx_adjust_color_Pattern, gx_enum_ptrs_Pattern, gx_reloc_ptrs_Pattern };

static int compute_inst_matrix(gs_pattern_instance * pinst, const gs_state * saved, gs_rect * pbbox);
int gs_makepattern(gs_client_color * pcc, const gs_client_pattern * pcp, const gs_matrix * pmat, gs_state * pgs, gs_memory_t * mem)
{
	...
	mem = pgs->memory;
    do {
	if (((pinst) = (gs_pattern_instance *) (*(mem)->procs.alloc_struct) (mem, &st_pattern_instance, "gs_makepattern")) == 0) {
	    return ((-25));
	} else {
	    (pinst)->rc.ref_count = 1;
	    (pinst)->rc.memory = mem;
	    (pinst)->rc.free = rc_free_struct_only;
	}
    } while (0);
    inst.rc = pinst->rc;
    ...
    gs_newpath(saved);
    gs_set_logical_op(saved, (rop3_default | (((bool) 0) ? 0x100 : 0) | (((bool) 0) ? 0x200 : 0) | (0 << 10)));
    inst.template = *pcp;
    inst.saved = saved;
	...
	if (inst.step_matrix.xy == 0 && inst.step_matrix.yx == 0 && fabs(fabs(inst.step_matrix.xx) - bbw) < 0.5 && fabs(fabs(inst.step_matrix.yy) - bbh) < 0.5) {
	    gs_scale(saved, fabs(inst.size.x / inst.step_matrix.xx), fabs(inst.size.y / inst.step_matrix.yy));
	    code = compute_inst_matrix(&inst, saved, &bbox);
	    if (code < 0)
		goto fsaved;
	    do {
    ...
    inst.is_simple = (fabs(inst.step_matrix.xx) == inst.size.x && inst.step_matrix.xy == 0 && inst.step_matrix.yx == 0 && fabs(inst.step_matrix.yy) == inst.size.y);
    do {
    } while (0);
    gx_translate_to_fixed(saved, ((fixed) ((inst.step_matrix.tx - bbox.p.x) * (float) (1 << 12))), ((fixed) ((inst.step_matrix.ty - bbox.p.y) * (float) (1 << 12))));
    inst.step_matrix.tx = bbox.p.x;
    inst.step_matrix.ty = bbox.p.y;
    cbox.p.x = 0L;
    cbox.p.y = 0L;
    cbox.q.x = ((fixed) (inst.size.x) << 12);
    cbox.q.y = ((fixed) (inst.size.y) << 12);
  ...
  finst:(*(mem)->procs.free_object) (mem, pinst, "gs_makepattern");
    return code;
}
...
static int compute_inst_matrix(gs_pattern_instance * pinst, const gs_state * saved, gs_rect * pbbox)
{
    ...
    return gs_bbox_transform(&pinst->template.BBox, &(*(const gs_matrix *) &(saved)->ctm), pbbox);
} int gs_setpattern(gs_state * pgs, const gs_client_color * pcc)
{
...
int gs_setpatternspace(gs_state * pgs)
{
    int code = 0;
    if (pgs->color_space->type->index != gs_color_space_index_Pattern) {
	gs_color_space cs;
	cs.params.pattern.base_space = *(gs_paint_color_space *) pgs->color_space;
	cs.params.pattern.has_base_space = ((bool) 1);
...
const gs_client_pattern *gs_getpattern(const gs_client_color * pcc)
{
    return &pcc->pattern->template;
}

static int bitmap_PaintProc(const gs_client_color *, gs_state *);
int gs_makebitmappattern(gs_client_color * pcc, const gx_tile_bitmap * tile, bool mask, gs_state * pgs, gs_memory_t * mem)
{
    gs_client_pattern pat;
    gs_matrix mat, smat;
    if (tile->raster != ((uint) (((tile->size.x + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2)))
	return ((-15));
    ((&pat.uid)->id = gs_next_ids(1), (&pat.uid)->xvalues = 0);
...
static int bitmap_PaintProc(const gs_client_color * pcolor, gs_state * pgs)
{
    gs_image_enum *pen = gs_image_enum_alloc(gs_state_memory(pgs), "bitmap_PaintProc");
    ...
    raster = ((uint) (((image.Width + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2));
    nbytes = (image.Width + 7) >> 3;
    gs_image_init(pen, &image, ((bool) 0), pgs);
    for (n = image.Height; n > 0; dp += raster, --n)
	gs_image_next(pen, dp, nbytes, &used);
    gs_image_cleanup(pen);
    (*(gs_state_memory(pgs))->procs.free_object) (gs_state_memory(pgs), pen, "bitmap_PaintProc");
    return 0;
}

static int gx_dc_pattern_load(gx_device_color * pdevc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select);
static int gx_dc_pattern_fill_rectangle(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source);
static gs_ptr_type_t dc_pattern_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void dc_pattern_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
static int gx_dc_pure_masked_load(gx_device_color * pdevc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select);
static int gx_dc_pure_masked_fill_rect(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source);
static gs_ptr_type_t dc_masked_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void dc_masked_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
...
static int gx_dc_binary_masked_fill_rect(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source);
static gs_ptr_type_t dc_binary_masked_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void dc_binary_masked_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
...
static int gx_dc_colored_masked_fill_rect(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source);
const gx_device_color_procs gx_dc_pattern = { gx_dc_pattern_load, gx_dc_pattern_fill_rectangle, gx_dc_default_fill_masked, dc_pattern_enum_ptrs, dc_pattern_reloc_ptrs }, gx_dc_pure_masked = {
gx_dc_pure_masked_load, gx_dc_pure_masked_fill_rect, gx_dc_default_fill_masked, dc_masked_enum_ptrs, dc_masked_reloc_ptrs}, gx_dc_binary_masked = {
gx_dc_binary_masked_load, gx_dc_binary_masked_fill_rect, gx_dc_default_fill_masked, dc_binary_masked_enum_ptrs, dc_binary_masked_reloc_ptrs}, gx_dc_colored_masked = {
gx_dc_colored_masked_load, gx_dc_colored_masked_fill_rect, gx_dc_default_fill_masked, dc_masked_enum_ptrs, dc_masked_reloc_ptrs};

const gx_device_color_procs *gx_dc_type_pattern = &gx_dc_pattern;
static gs_ptr_type_t dc_pattern_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
    ...
    case 0:{
	    gx_color_tile *tile = ((gx_device_color *) vptr)->colors.pattern.p_tile;
	    do {
		*pep = (const void *) ((tile == 0 ? tile : tile - tile->index));
		return (&ptr_struct_procs);
	    } while (0);
	}
    }
...
static void dc_pattern_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	gx_color_tile *tile = ((gx_device_color *) vptr)->colors.pattern.p_tile;
	if (tile != 0) {
	    uint index = tile->index;
	    (((gx_device_color *) vptr)->colors.pattern.p_tile = (void *) gs_reloc_struct_ptr(((gx_device_color *) vptr)->colors.pattern.p_tile - (index), gcst), ((gx_device_color *) vptr)->colors.pattern.p_tile += (index));
	}
	dc_masked_reloc_ptrs(vptr, size, gcst);
}} static gs_ptr_type_t dc_masked_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return (*st_client_color.enum_ptrs) (&((gx_device_color *) vptr)->mask.ccolor, sizeof(((gx_device_color *) vptr)->mask.ccolor), index - (1), pep);
    case 0:{
	    gx_color_tile *mask = ((gx_device_color *) vptr)->mask.m_tile;
	    do {
		*pep = (const void *) ((mask == 0 ? mask : mask - mask->index));
		return (&ptr_struct_procs);
	    } while (0);
	}
    }
...
static void dc_masked_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	gx_color_tile *mask = ((gx_device_color *) vptr)->mask.m_tile;
	(*st_client_color.reloc_ptrs) (&((gx_device_color *) vptr)->mask.ccolor, sizeof(((gx_device_color *) vptr)->mask.ccolor), gcst);
	if (mask != 0) {
	    uint index = mask->index;
	    (((gx_device_color *) vptr)->mask.m_tile = (void *) gs_reloc_struct_ptr(((gx_device_color *) vptr)->mask.m_tile - (index), gcst), ((gx_device_color *) vptr)->mask.m_tile += (index));
	}
}} static gs_ptr_type_t dc_binary_masked_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
...
static void dc_binary_masked_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	dc_masked_reloc_ptrs(vptr, size, gcst);
...
bool gx_pattern_cache_lookup(gx_device_color * pdevc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
	...
	if (ctile->id == id && (pdevc->type != &gx_dc_pattern || ctile->depth == dev->color_info.depth)) {
	    if (pdevc->type == &gx_dc_pattern) {
		pdevc->colors.pattern.p_tile = ctile;
		((pdevc)->phase.x = (imod(-(pis->screen_phase[select].x), ctile->tbits.rep_width)), (pdevc)->phase.y = (imod(-(pis->screen_phase[select].y), ctile->tbits.rep_height)));
	    }
	    pdevc->mask.m_tile = (ctile->tmask.data == 0 ? (gx_color_tile *) 0 : ctile);
	    return ((bool) 1);
	}
    }
    return ((bool) 0);
...
static int gx_dc_pattern_fill_rectangle(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source)
{
	...
	if (ptile->is_simple) {
		...
		code = (*((pcdev)->std_procs.strip_tile_rectangle)) (pcdev, bits, x, y, w, h, ((unsigned long) (-1)), ((unsigned long) (-1)), px, py);
	    else
		code = (*((dev)->std_procs.strip_copy_rop)) (dev, rop_source->sdata, rop_source->sourcex, rop_source->sraster, rop_source->id, (rop_source->use_scolors ? rop_source->scolors : ((void *) 0)), bits, ((void *) 0), x, y, w, h, px, py, lop);
	} else { {
		gs_rect bbox, ibbox;
		gs_point offset;
		...
		for (i = i0; i < i1; i++)
		    for (j = j0; j < j1; j++) {
			int xoff, yoff;
			x = (int) (ptile->step_matrix.xx * i + ptile->step_matrix.yx * j + offset.x);
			y = (int) (ptile->step_matrix.xy * i + ptile->step_matrix.yy * j + offset.y);
			do {
			} while (0);
			w = ptile->tbits.size.x;
			h = ptile->tbits.size.y;
			if (x < x0)
			...
			if (w > 0 && h > 0) {
			    if ((pcdev == (gx_device *) & cdev))
				tile_clip_set_phase(&cdev, imod(xoff - x, tmask->rep_width), imod(yoff - y, tmask->rep_height)); {
				const byte *data = bits->data;
				bool full_transfer = (w == w0 && h == h0);
				gx_bitmap_id source_id = (full_transfer ? rop_source->id : 0L);
				if (source == ((void *) 0) && (((lop) & (0x200 | (rop3_1 - rop3_S))) == (rop3_T & ~rop3_S)))
				    code = (*((pcdev)->std_procs.copy_color)) (pcdev, data + bits->raster * yoff, xoff, bits->raster, (full_transfer ? bits->id : 0L), x, y, w, h);
				else {
				    ...
				    code = (*((dev)->std_procs.strip_copy_rop)) (dev, rop_source->sdata + (y - y0) * rop_source->sraster, rop_source->sourcex + (x - x0), rop_source->sraster, source_id, (rop_source->use_scolors ? rop_source->scolors : ((void *) 0)), &data_tile, ((void *) 0), x, y, w, h, imod(xoff - x, data_tile.rep_width), imod(yoff - y, data_tile.rep_height), lop);
				} if (code < 0)
				    return code;
				}
			}
...
static int gx_dc_pure_masked_fill_rect(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source)
{
	...
	if (ptile == 0 || ptile->is_simple)
	    code = (*gx_dc_procs_pure.fill_rectangle) (pdevc, x, y, w, h, pcdev, lop, source);
	else { {
		gs_rect bbox, ibbox;
		gs_point offset;
		int x0 = x, x1 = x + w, y0 = y, y1 = y + h;
		...
		for (i = i0; i < i1; i++)
			...
			w = ptile->tmask.size.x;
			h = ptile->tmask.size.y;
			if (x < x0)
			    xoff = x0 - x, x = x0, w -= xoff;
			else
			    xoff = 0;
...
static int gx_dc_binary_masked_fill_rect(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source)
{
	...
	if (ptile == 0 || ptile->is_simple)
	    code = (*gx_dc_procs_ht_binary.fill_rectangle) (pdevc, x, y, w, h, pcdev, lop, source);
	else { {
		gs_rect bbox, ibbox;
		gs_point offset;
		int x0 = x, x1 = x + w, y0 = y, y1 = y + h;
		...
		for (i = i0; i < i1; i++)
			...
			if (w > 0 && h > 0) {
				...
				(step_source).scolors[1] = source->scolors[1];
				(step_source).use_scolors = source->use_scolors;
				code = (*gx_dc_procs_ht_binary.fill_rectangle) (pdevc, x, y, w, h, pcdev, lop, &step_source);
			    }
			    if (code < 0)
				return code;
			}
...
static int gx_dc_colored_masked_fill_rect(const gx_device_color * pdevc, int x, int y, int w, int h, gx_device * dev, gs_logical_operation_t lop, const gx_rop_source_t * source)
{
	...
	if (ptile == 0 || ptile->is_simple)
	    code = (*gx_dc_procs_ht_colored.fill_rectangle) (pdevc, x, y, w, h, pcdev, lop, source);
	else { {
		gs_rect bbox, ibbox;
		gs_point offset;
		int x0 = x, x1 = x + w, y0 = y, y1 = y + h;
		...
		for (i = i0; i < i1; i++)
			...
			if (w > 0 && h > 0) {
				...
				(step_source).scolors[1] = source->scolors[1];
				(step_source).use_scolors = source->use_scolors;
				code = (*gx_dc_procs_ht_colored.fill_rectangle) (pdevc, x, y, w, h, pcdev, lop, &step_source);
			    }
			    if (code < 0)
				return code;
			}
...
static void gx_init_Pattern(gs_client_color * pcc, const gs_color_space * pcs)
{
    if (pcs->params.pattern.has_base_space) {
	const gs_color_space *pbcs = (const gs_color_space *) &pcs->params.pattern.base_space;
	(*(pbcs)->type->init_color) (pcc, pbcs);
    }
} static int gx_install_Pattern(gs_color_space * pcs, gs_state * pgs)
{
    if (!pcs->params.pattern.has_base_space)
	return 0;
    return (*pcs->params.pattern.base_space.type->install_cspace) ((gs_color_space *) & pcs->params.pattern.base_space, pgs);
}

static void gx_adjust_cspace_Pattern(const gs_color_space * pcs, gs_memory_t * mem, int delta)
{
    if (pcs->params.pattern.has_base_space)
	(*pcs->params.pattern.base_space.type->adjust_cspace_count) ((const gs_color_space *) &pcs->params.pattern.base_space, mem, delta);
} static void gx_adjust_color_Pattern(const gs_client_color * pcc, const gs_color_space * pcs, gs_memory_t * mem, int delta)
{
    gs_pattern_instance *pinst = pcc->pattern;
    if (pinst != 0 && (pinst->rc.ref_count += delta) == 0) {
	gs_state_free(pinst->saved);
	(*(mem)->procs.free_object) (mem, pinst, "gx_adjust_color_Pattern");
    }
    if (pcs->params.pattern.has_base_space)
	(*pcs->params.pattern.base_space.type->adjust_color_count) (pcc, (const gs_color_space *) &pcs->params.pattern.base_space, mem, delta);
} static gs_ptr_type_t gx_enum_ptrs_Pattern(void *vptr, uint size, uint index, const void **pep)
{
    if (!((gs_color_space *) vptr)->params.pattern.has_base_space)
	return 0;
    return (*((gs_color_space *) vptr)->params.pattern.base_space.type->enum_ptrs) (&((gs_color_space *) vptr)->params.pattern.base_space, sizeof(((gs_color_space *) vptr)->params.pattern.base_space), index, pep);
}

static void gx_reloc_ptrs_Pattern(void *vptr, uint size, gc_state_t * gcst)
{ {
	if (!((gs_color_space *) vptr)->params.pattern.has_base_space)
	    return;
	(*((gs_color_space *) vptr)->params.pattern.base_space.type->reloc_ptrs) (&((gs_color_space *) vptr)->params.pattern.base_space, sizeof(gs_paint_color_space), gcst);
}
}
