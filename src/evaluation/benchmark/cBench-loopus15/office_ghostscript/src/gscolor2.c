...
static gs_ptr_type_t indexed_map_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (((gs_indexed_map *) vptr)->values);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void indexed_map_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((gs_indexed_map *) vptr)->values = gs_reloc_struct_ptr((const void *) ((const gs_indexed_map *) vptr)->values, gcst);
} const gs_memory_struct_type_t st_indexed_map = { sizeof(gs_indexed_map), "gs_indexed_map", 0, 0, indexed_map_enum_ptrs, indexed_map_reloc_ptrs, 0 };

static int gx_concretize_Indexed(const gs_client_color *, const gs_color_space *, frac *, const gs_imager_state *);
static int gx_install_Indexed(gs_color_space *, gs_state *);
static void gx_adjust_cspace_Indexed(const gs_color_space *, gs_memory_t *, int);
static gs_ptr_type_t gx_enum_ptrs_Indexed(void *ptr, uint size, uint index, const void **pep);
static void gx_reloc_ptrs_Indexed(void *ptr, uint size, gc_state_t * gcst);
static const gs_color_space *gx_concrete_space_Indexed(const gs_color_space *, const gs_imager_state *);
const gs_color_space_type gs_color_space_type_Indexed = { gs_color_space_index_Indexed, 1, ((bool) 0), gx_init_paint_1, gx_concrete_space_Indexed, gx_concretize_Indexed, ((void *) 0), gx_default_remap_color, gx_install_Indexed, gx_adjust_cspace_Indexed, gx_no_adjust_color_count, gx_enum_ptrs_Indexed, gx_reloc_ptrs_Indexed };

int gs_setcolorspace(gs_state * pgs, gs_color_space * pcs)
{
    ...
    *pgs->color_space = *pcs;
	...
	goto rcs;
    ((pgs->ccolor)->pattern = 0, (*(pcs)->type->init_color) (pgs->ccolor, pcs));
...
const gs_color_space *gs_currentcolorspace(const gs_state * pgs)
{
...
const gs_client_color *gs_currentcolor(const gs_state * pgs)
{
    return pgs->ccolor;
}

static const gs_color_space *gx_concrete_space_Indexed(const gs_color_space * pcs, const gs_imager_state * pis)
{
    const gs_color_space *pbcs = (const gs_color_space *) &pcs->params.indexed.base_space;
    return (*(pbcs)->type->concrete_space) (pbcs, pis);
}

static int gx_concretize_Indexed(const gs_client_color * pc, const gs_color_space * pcs, frac * pconc, const gs_imager_state * pis)
{
    float value = pc->paint.values[0];
    int index = (((value) < 0.0) ? 0 : value >= pcs->params.indexed.hival ? pcs->params.indexed.hival : (int) value);
    gs_client_color cc;
    const gs_color_space *pbcs = (const gs_color_space *) &pcs->params.indexed.base_space;
	...
	const byte *pcomp = pcs->params.indexed.lookup.table.data + m * index;
	switch (m) {
	default:
	    return ((-15));
	case 4:
...
static int gx_install_Indexed(gs_color_space * pcs, gs_state * pgs)
{
    return (*pcs->params.indexed.base_space.type->install_cspace) ((gs_color_space *) & pcs->params.indexed.base_space, pgs);
}

static void gx_adjust_cspace_Indexed(const gs_color_space * pcs, gs_memory_t * mem, int delta)
{
    if (pcs->params.indexed.use_proc) {
	do {
	    if ((pcs->params.indexed.lookup.map) != 0 && !((pcs->params.indexed.lookup.map)->rc.ref_count += delta)) {
		(*(pcs->params.indexed.lookup.map)->rc.free) ((pcs->params.indexed.lookup.map)->rc.memory, (void *) (pcs->params.indexed.lookup.map), "gx_adjust_Indexed");
		do {
		} while (0);
	    }
	} while (0);
    }
    (*pcs->params.indexed.base_space.type->adjust_cspace_count) ((const gs_color_space *) &pcs->params.indexed.base_space, mem, delta);
} static gs_ptr_type_t gx_enum_ptrs_Indexed(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
	    return (*((gs_color_space *) vptr)->params.indexed.base_space.type->enum_ptrs) (&((gs_color_space *) vptr)->params.indexed.base_space, sizeof(((gs_color_space *) vptr)->params.indexed.base_space), index - 1, pep);
	}
    case 0:
	if (((gs_color_space *) vptr)->params.indexed.use_proc)
	    do {
		*pep = (const void *) ((void *) ((gs_color_space *) vptr)->params.indexed.lookup.map);
		return (&ptr_struct_procs);
	    } while (0);
	else {
	    ((gs_color_space *) vptr)->params.indexed.lookup.table.size = (((gs_color_space *) vptr)->params.indexed.hival + 1) * ((gs_color_space *) vptr)->params.indexed.base_space.type->num_components;
	    *pep = &((gs_color_space *) vptr)->params.indexed.lookup.table;
	    return (&ptr_const_string_procs);
	}
    }
}

static void gx_reloc_ptrs_Indexed(void *vptr, uint size, gc_state_t * gcst)
{ {
	(*((gs_color_space *) vptr)->params.indexed.base_space.type->reloc_ptrs) (&((gs_color_space *) vptr)->params.indexed.base_space, sizeof(gs_base_color_space), gcst);
	if (((gs_color_space *) vptr)->params.indexed.use_proc)
	    ((gs_color_space *) vptr)->params.indexed.lookup.map = gs_reloc_struct_ptr((const void *) ((const gs_color_space *) vptr)->params.indexed.lookup.map, gcst);
	else
	    gs_reloc_const_string(&((gs_color_space *) vptr)->params.indexed.lookup.table, gcst);
}
}
