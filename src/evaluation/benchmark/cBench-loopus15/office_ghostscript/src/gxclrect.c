...
static int cmd_size_rect(register const gx_cmd_rect * prect)
{
    return ((!((prect->x) & ~0x7f)) ? 1 : (!((prect->x) & ~0x3fff)) ? 2 : cmd_size_w((uint) (prect->x))) + ((!((prect->y) & ~0x7f)) ? 1 : (!((prect->y) & ~0x3fff)) ? 2 : cmd_size_w((uint) (prect->y))) + ((!((prect->width) & ~0x7f)) ? 1 : (!((prect->width) & ~0x3fff)) ? 2 : cmd_size_w((uint) (prect->width))) + ((!((prect->height) & ~0x7f)) ? 1 : (!((prect->height) & ~0x3fff)) ? 2 : cmd_size_w((uint) (prect->height)));
}

static byte *cmd_put_rect(register const gx_cmd_rect * prect, register byte * dp)
{
...
int cmd_write_rect_cmd(gx_device_clist_writer * cldev, gx_clist_state * pcls, int op, int x, int y, int width, int height)
{
    ...
    if (dheight == 0 && ((unsigned) (dwidth - (-4)) <= (3 - (-4))) && ((unsigned) (dx - (-8)) <= (7 - (-8)) && (unsigned) (dy - (-8)) <= (7 - (-8)))) {
	...
	dp[1] = dx - (-128);
	dp[2] = dwidth - (-128);
    } else if (dy >= -2 && dy <= 1 && dheight >= -2 && dheight <= 1 && (dy + dheight) != -4) {
	byte *dp;
	int rcsize = 1 + ((!((x) & ~0x7f)) ? 1 : (!((x) & ~0x3fff)) ? 2 : cmd_size_w((uint) (x))) + ((!((width) & ~0x7f)) ? 1 : (!((width) & ~0x3fff)) ? 2 : cmd_size_w((uint) (width)));
	do {
	    if ((dp = cmd_put_list_op(cldev, &(pcls)->list, rcsize)) == 0)
		return (cldev)->error_code;
	    *dp = (op + ((dy + 2) << 2) + dheight + 2);
	} while (0);
	++dp;
...
int clist_fill_rectangle(gx_device * dev, int x, int y, int width, int height, gx_color_index color)
{
	...
	do {
		...
		if (color != pcls->colors[1]) {
		    int code = cmd_put_color((&((gx_device_clist *) dev)->writer), pcls, &clist_select_color1, color, &pcls->colors[1]);
		    if (code < 0)
			return code;
		}
		{
		    int code = cmd_write_rect_cmd((&((gx_device_clist *) dev)->writer), pcls, cmd_op_fill_rect, x, y, width, height);
		    if (code < 0)
			return code;
		}
	    }
...
int clist_strip_tile_rectangle(gx_device * dev, const gx_strip_bitmap * tile, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1, int px, int py)
{
    int depth = (color1 == ((unsigned long) (-1)) && color0 == ((unsigned long) (-1)) ? dev->color_info.depth : 1);
    if ((x | y) < 0) {
	if (x < 0)
	    width += x, x = 0;
	if (y < 0)
	...
	do {
		...
		if ((pcls)->lop_enabled == ((0) ^ 1) && cmd_put_enable_lop((&((gx_device_clist *) dev)->writer), pcls, 0) < 0)
		    return ((&((gx_device_clist *) dev)->writer))->error_code;
		if (!((pcls)->tile_id == (tile->id) && (offset_temp = (&((gx_device_clist *) dev)->writer)->tile_table[(pcls)->tile_index].offset) != 0 && ((tile_slot *) ((&((gx_device_clist *) dev)->writer)->data + offset_temp))->id == (tile->id))) {
		    if (tile->id == 0L || clist_change_tile((&((gx_device_clist *) dev)->writer), pcls, tile, depth) < 0) {
			int code = gx_default_strip_tile_rectangle(dev, tile, x, y, width, height, color0, color1, px, py);
			if (code < 0)
			    return code;
			goto endr;
		    }
		...
		if (color0 != pcls->tile_colors[0] || color1 != pcls->tile_colors[1]) {
		    int code = cmd_set_tile_colors((&((gx_device_clist *) dev)->writer), pcls, color0, color1);
		    if (code < 0)
			return code;
		}
		if (px != pcls->tile_phase.x || py != pcls->tile_phase.y) {
		    int code = cmd_set_tile_phase((&((gx_device_clist *) dev)->writer), pcls, px, py);
		    if (code < 0)
			return code;
		}
		{
		    int code = cmd_write_rect_cmd((&((gx_device_clist *) dev)->writer), pcls, cmd_op_tile_rect, x, y, width, height);
		    if (code < 0)
			return code;
		}
	      endr:;
...
int clist_copy_mono(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1)
{
	...
	do {
		...
		if ((pcls)->lop_enabled == ((0) ^ 1) && cmd_put_enable_lop((&((gx_device_clist *) dev)->writer), pcls, 0) < 0)
		    return ((&((gx_device_clist *) dev)->writer))->error_code;
		if ((pcls)->clip_enabled == ((0) ^ 1) && cmd_put_enable_clip((&((gx_device_clist *) dev)->writer), pcls, 0) < 0)
		    return ((&((gx_device_clist *) dev)->writer))->error_code;
		if (color0 != pcls->colors[0]) {
		    code = cmd_put_color((&((gx_device_clist *) dev)->writer), pcls, &clist_select_color0, color0, &(pcls)->colors[0]);
		    if (code < 0)
			return code;
		}
		if (color1 != pcls->colors[1]) {
		    code = cmd_put_color((&((gx_device_clist *) dev)->writer), pcls, &clist_select_color1, color1, &(pcls)->colors[1]);
		    if (code < 0)
			return code;
		}
	      copy:{
		    ...
		    code = cmd_put_bits((&((gx_device_clist *) dev)->writer), pcls, row, w1, height, raster, rsize, (orig_id == 0L ? 1 << 1 : ((1 << 1) | (1 << 2))), &dp, &csize);
		    if (code < 0) {
			if (code != (-13))
			    return code;
			if (height > 1) {
			...
			{
			    int w2 = w1 >> 1;
			    code = clist_copy_mono(dev, row, dx + w2, raster, 0L, x + w2, y, w1 - w2, 1, color0, color1);
			    if (code < 0)
				return code;
			    w1 = w2;
			    goto copy;
		    ...
		    *dp++ = (op);
		    ((!(((x) | (y)) & ~0x7f)) ? (dp[0] = (x), dp[1] = (y), dp += 2) : (dp = cmd_put_w((uint) (y), cmd_put_w((uint) (x), dp))));
		    ((!(((w1) | (height)) & ~0x7f)) ? (dp[0] = (w1), dp[1] = (height), dp += 2) : (dp = cmd_put_w((uint) (height), cmd_put_w((uint) (w1), dp))));
		    pcls->rect = rect;
		}
	    }
	} while ((y += height) < yend);
	...
	do {
		...
		if (pcls->color_is_alpha) {
	      ...
	      copy:{
		    ...
		    rect.width = w1, rect.height = height;
		    rsize = (dx ? 3 : 1) + cmd_size_rect(&rect);
		    code = cmd_put_bits((&((gx_device_clist *) dev)->writer), pcls, row, w1 * depth, height, raster, rsize, 1 << 1, &dp, &csize);
		    if (code < 0) {
			if (code != (-13))
			    return code;
			if (height > 1) {
			...
			{
			    int w2 = w1 >> 1;
			    code = clist_copy_color(dev, row, dx + w2, raster, 0L, x + w2, y, w1 - w2, 1);
			    if (code < 0)
				return code;
			    w1 = w2;
			    goto copy;
	...
	do {
		...
		if (color != pcls->colors[1]) {
	      ...
	      copy:{
		    ...
		    code = cmd_put_bits((&((gx_device_clist *) dev)->writer), pcls, row, w1 << log2_depth, height, raster, rsize, 1 << 1, &dp, &csize);
		    if (code < 0) {
			if (code != (-13))
			    return code;
			if (height > 1) {
			...
			{
			    int w2 = w1 >> 1;
			    code = clist_copy_alpha(dev, row, dx + w2, raster, 0L, x + w2, y, w1 - w2, 1, color, depth);
			    if (code < 0)
				return code;
			    w1 = w2;
			    goto copy;
...
int clist_strip_copy_rop(gx_device * dev, const byte * sdata, int sourcex, uint sraster, gx_bitmap_id id, const gx_color_index * scolors, const gx_strip_bitmap * textures, const gx_color_index * tcolors, int x, int y, int width, int height, int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	do {
		...
		int code;
		if ((((((rop) << 4) ^ (rop)) & rop3_T) != 0)) {
		    if (tcolors == 0 || tcolors[0] != tcolors[1]) {
			ulong offset_temp;
			if (!((pcls)->tile_id == (tiles->id) && (offset_temp = (&((gx_device_clist *) dev)->writer)->tile_table[(pcls)->tile_index].offset) != 0 && ((tile_slot *) ((&((gx_device_clist *) dev)->writer)->data + offset_temp))->id == (tiles->id))) {
			    if (tiles->id == 0L) {
				tile_with_id = *tiles;
				tile_with_id.id = gs_next_ids(1);
				tiles = &tile_with_id;
			    }
			    code = clist_change_tile((&((gx_device_clist *) dev)->writer), pcls, tiles, (tcolors != 0 ? 1 : dev->color_info.depth));
			    if (code < 0) {
				uint rep_height = tiles->rep_height;
				gs_id ids;
				gx_strip_bitmap line_tile;
				int iy;
				if (code != (-13) || rep_height == 1 || tiles->rep_shift != 0)
				    return code;
				ids = gs_next_ids((((height) < (rep_height)) ? (height) : (rep_height)));
				line_tile = *tiles;
				line_tile.size.y = 1;
				line_tile.rep_height = 1;
				for (iy = 0; iy < height; ++iy) {
				    line_tile.data = tiles->data + line_tile.raster * ((y + iy + phase_y) % rep_height);
				    line_tile.id = ids + (iy % rep_height);
				    code = clist_strip_copy_rop(dev, (sdata == 0 ? 0 : row + iy * sraster), sourcex, sraster, 0L, scolors, &line_tile, tcolors, x, y + iy, width, 1, phase_x, pcls->tile_phase.y, lop);
				    if (code < 0)
					return code;
				}
				continue;
			    }
			    if (phase_x != pcls->tile_phase.x || phase_y != pcls->tile_phase.y) {
				code = cmd_set_tile_phase((&((gx_device_clist *) dev)->writer), pcls, phase_x, phase_y);
				if (code < 0)
				    return code;
			    }
			}
		    }
		    code = (tcolors != 0 ? cmd_set_tile_colors((&((gx_device_clist *) dev)->writer), pcls, tcolors[0], tcolors[1]) : cmd_set_tile_colors((&((gx_device_clist *) dev)->writer), pcls, ((unsigned long) (-1)), ((unsigned long) (-1))));
		    if (code < 0)
			return code;
		}
		if (lop != pcls->lop) {
		...
		if (scolors != 0) {
			...
			code = clist_fill_rectangle(dev, x, y, width, height, scolors[1]);
		    else
			code = clist_copy_mono(dev, row, sourcex, sraster, id, x, y, width, height, scolors[0], scolors[1]);
		} else
		    code = clist_copy_color(dev, row, sourcex, sraster, id, x, y, width, height);
		pcls->lop_enabled = 1;
		if (code < 0)
		    return code;
	    }
