...
static int store_color_params(os_ptr, const gs_paint_color *, const gs_color_space *);
static int load_color_params(os_ptr, gs_paint_color *, const gs_color_space *);
static int zcurrentcolor(register os_ptr op)
{
    const gs_client_color *pc = gs_currentcolor(igs);
    const gs_color_space *pcs = gs_currentcolorspace(igs);
    int n;
    if ((o_stack.top) - (o_stack.p) < (5)) {
	o_stack.requested = (5);
	return ((-16));
    };
    if (pcs->type->index == gs_color_space_index_Pattern) {
	n = 1;
	if (pc->pattern != 0 && pc->pattern->template.PaintType == 2)
	    n += store_color_params(op, &pc->paint, (const gs_color_space *) &pcs->params.pattern.base_space);
	op[n] = ((int_gstate *) gs_state_client_data(igs))->pattern;
    } else
...
static int zcurrentcolorspace(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    if (((((const byte *) &((&((int_gstate *) gs_state_client_data(igs))->colorspace.array)->tas.type_attrs))[1]) == (t_null))) {
	((op)->value.intval = ((int) (gs_currentcolorspace(igs)->type->index)), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
    } else
	*op = ((int_gstate *) gs_state_client_data(igs))->colorspace.array;
    return 0;
}

static int zsetcolor(register os_ptr op)
{
    gs_client_color c;
    const gs_color_space *pcs = gs_currentcolorspace(igs);
    int n, code;
	...
	if (!(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)))
	    return ((-7));
	if (dict_find_string(op, "Implementation", &pImpl) <= 0 || !((((pImpl)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *) (gs_imemory.current)))->procs.object_type) (((gs_memory_t *) (gs_imemory.current)), (pImpl)->value.pstruct) == &*zcolor2_st_pattern_instance_p))
	    return ((-15));
	pinst = ((gs_pattern_instance *) ((pImpl)->value.pstruct));
	c.pattern = pinst;
	if (pinst->template.PaintType == 2) {
	    if (!pcs->params.pattern.has_base_space)
		return ((-15));
	    n = load_color_params(op - 1, &c.paint, (const gs_color_space *) &pcs->params.pattern.base_space);
	    if (n < 0)
		return n;
	    n++;
	} else
	...
	return code;
    if (pinst != 0)
	((int_gstate *) gs_state_client_data(igs))->pattern = *op;
    ((o_stack.p) -= (n));
    return code;
}

static int zsetcolorspace(register os_ptr op)
{
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_array)))
	return (check_type_failed(&*op));
    ((int_gstate *) gs_state_client_data(igs))->colorspace.array = *op;
    ((o_stack.p) -= (1));
    return 0;
}

const op_def *zcolor2_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"0currentcolor", zcurrentcolor}, {"0.currentcolorspace", zcurrentcolorspace}, {"1setcolor", zsetcolor}, {"1.setcolorspace", zsetcolorspace}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}

static int store_color_params(os_ptr op, const gs_paint_color * pc, const gs_color_space * pcs)
{
    int n = pcs->type->num_components;
    if (pcs->type->index == gs_color_space_index_Indexed)
	((op + 1)->value.intval = ((int) pc->values[0]), ((op + 1)->tas.type_attrs = ((t_integer) << 8) + (0)));
    else
	make_reals(op + 1, pc->values, n);
    return n;
}

static int load_color_params(os_ptr op, gs_paint_color * pc, const gs_color_space * pcs)
{
    int n = pcs->type->num_components;
