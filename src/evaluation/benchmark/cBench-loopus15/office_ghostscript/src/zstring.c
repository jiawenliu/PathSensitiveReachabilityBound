...
int zstring(register os_ptr op)
{
    byte *sbody;
    uint size;
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
    if ((ulong) (*op).value.intval > (((unsigned short) 0xffff + (unsigned short) 0)))
	return ((-15));
    size = op->value.intval;
    sbody = (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_string) (((gs_memory_t *) (gs_imemory.current)), size, "string");
    if (sbody == 0)
	return ((-25));
    ((op)->value.bytes = (sbody), ((op)->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 + 0x40) | (((&gs_imemory))->current_space))), ((op)->tas.rsize = (size)));
    memset(sbody, 0, size);
    return 0;
}

static int znamestring(register os_ptr op)
{
...
static int zanchorsearch(register os_ptr op)
{
    os_ptr op1 = op - 1;
    uint size = ((op)->tas.rsize);
    if (!(((&*op1)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))))
	return ((!((((const byte *) &((&*op1)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op1) : (-7)));
...
static int zsearch(register os_ptr op)
{
    os_ptr op1 = op - 1;
    uint size = ((op)->tas.rsize);
    uint count;
...
static int zstringmatch(register os_ptr op)
{
    os_ptr op1 = op - 1;
    bool result;
    if (!(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))))
	...
	goto cmp;
    case t_name:
	name_string_ref(op1, op1);
      cmp:result = string_match(op1->value.const_bytes, ((op1)->tas.rsize), op->value.const_bytes, ((op)->tas.rsize), ((void *) 0));
	break;
    default:
...
const op_def *zstring_op_defs(void)
{
    static const op_def op_defs_[] = { {"2anchorsearch", zanchorsearch}, {"1.namestring", znamestring}, {"2search", zsearch}, {"1string", zstring}, {"2.stringmatch", zstringmatch}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
