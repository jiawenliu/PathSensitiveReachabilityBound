...
static void adjust_point_to_tangent(segment *, const segment *, const gs_fixed_point *);
static int monotonize_internal(gx_path *, const curve_segment *);
int gx_path_copy_reducing(const gx_path * ppath_old, gx_path * ppath, fixed fixed_flatness, gx_path_copy_options options)
{
    gx_path old;
    const segment *pseg;
    int code;
	...
	case s_curve:{
		...
		if (fixed_flatness == (~(-1L << ((1 << 2) * 8 - 1)))) {
		    if (options & pco_monotonize)
			code = monotonize_internal(ppath, pc);
		    else
			code = gx_path_add_curve_notes(ppath, pc->p1.x, pc->p1.y, pc->p2.x, pc->p2.y, pc->pt.x, pc->pt.y, pseg->notes);
		} else {
		    fixed x0 = ppath->position.x;
		    fixed y0 = ppath->position.y;
		    int k = gx_curve_log2_samples(x0, y0, pc, fixed_flatness);
		    segment_notes notes = pseg->notes;
		    segment *start;
		    curve_segment cseg;
		    if (options & pco_accurate) {
			code = gx_path_add_line_notes(ppath, x0, y0, notes);
			if (code < 0)
			    break;
			...
			if (code < 0 || (code = gx_path_add_line_notes(ppath, ppath->position.x, ppath->position.y, pseg->notes | sn_not_first)) < 0)
			    break;
	...
	case s_line:
	    code = gx_path_add_line_notes(ppath, pseg->pt.x, pseg->pt.y, pseg->notes);
	    break;
	case s_line_close:
    ...
    if ((((&old)->state_flags & ~psf_outside_range) == psf_last_moveto))
	gx_path_add_point(ppath, old.position.x, old.position.y);
    return 0;
}

static void adjust_point_to_tangent(segment * pseg, const segment * next, const gs_fixed_point * p1)
{
    fixed x0 = pseg->pt.x, y0 = pseg->pt.y;
    double C = p1->x - x0, D = p1->y - y0;
    if (!(C == 0 && D == 0)) {
	double T = (C * (next->pt.x - x0) + D * (next->pt.y - y0)) * 0.3 / (C * C + D * D);
	do {
	} while (0);
	if (T >= 0) {
...
void gx_curve_cursor_init(curve_cursor * prc, fixed x0, fixed y0, const curve_segment * pc, int k)
{
	...
	(v01 = (w1) - (w0), prc->c = (v01 << 1) + v01, v12 = (w2) - (w1), prc->b = (v12 << 1) + v12 - prc->c, prc->a = (w3) - prc->b - prc->c - (w0));
    }
    prc->double_set = ((bool) 0);
    prc->fixed_limit = ((k3 <= sizeof(fixed) * 8 - 3 && (((prc->a) < 0 ? -(prc->a) : (prc->a)) <= (~(-1L << ((1 << 2) * 8 - 1))) >> (k3 + 2)) && (((prc->b) < 0 ? -(prc->b) : (prc->b)) <= (~(-1L << ((1 << 2) * 8 - 1))) >> (k2 + 2)) && (((prc->c) < 0 ? -(prc->c) : (prc->c)) <= (~(-1L << ((1 << 2) * 8 - 1))) >> (k + 1))) ? (1 << k) - 1 : -1);
...
fixed gx_curve_x_at_y(curve_cursor * prc, fixed y)
{
	...
	fixed cy0 = prc->p0.y, cy1, cy2, cy3 = pc->pt.y;
	fixed cx0;
	int t = 0;
	int k, i;
	if (cy0 > cy3)
	    cx0 = pc->pt.x, cy0 = pc->pt.y, cy1 = pc->p2.y, cy2 = pc->p1.y, cy3 = prc->p0.y;
	else
	    cx0 = prc->p0.x, cy1 = pc->p1.y, cy2 = pc->p2.y;
	for (i = k = prc->k; i > 0; --i) {
	    fixed ym = (((cy1) + (cy2) + 1) >> 1);
	    fixed yn = ym + ((cy0 - cy1 - cy2 + cy3 + 4) >> (3));
	    t <<= 1;
	    if (y < yn)
		cy1 = (((cy0) + (cy1) + 1) >> 1), cy2 = (((cy1) + (ym) + 1) >> 1), cy3 = yn;
	    else
		t++, cy2 = (((cy2) + (cy3) + 1) >> 1), cy1 = (((ym) + (cy2) + 1) >> 1), cy0 = yn;
	}
	{
	    fixed a = prc->a, b = prc->b, c = prc->c;
	    static const double k_denom[11] = { (1.0 / (1L << (0))), (1.0 / (1L << (1))), (1.0 / (1L << (2))), (1.0 / (1L << (3))), (1.0 / (1L << (4))), (1.0 / (1L << (5))), (1.0 / (1L << (6))), (1.0 / (1L << (7))), (1.0 / (1L << (8))), (1.0 / (1L << (9))), (1.0 / (1L << (10))) };
	    static const double k2_denom[11] = { (1.0 / (1L << (0))), (1.0 / (1L << (2))), (1.0 / (1L << (4))), (1.0 / (1L << (6))), (1.0 / (1L << (8))), (1.0 / (1L << (10))), (1.0 / (1L << (12))), (1.0 / (1L << (14))), (1.0 / (1L << (16))), (1.0 / (1L << (18))), (1.0 / (1L << (20))) };
	    static const double k3_denom[11] = { (1.0 / (1L << (0))), (1.0 / (1L << (3))), (1.0 / (1L << (6))), (1.0 / (1L << (9))), (1.0 / (1L << (12))), (1.0 / (1L << (15))), (1.0 / (1L << (18))), (1.0 / (1L << (21))), (1.0 / (1L << (24))), (1.0 / (1L << (27))), (1.0 / (1L << (30))) };
	    double den1, den2;
	    if (t <= prc->fixed_limit) {
		int t2 = t * t, t3 = t2 * t;
		int t3d = (t2 + t) * 3 + 1, t2d = t + t + 1;
		xl = ((((((a * t3) >> (k)) + b * t2) >> (k)) + c * t + ((1 << k) >> 1)) >> (k)) + cx0;
		xd = ((((((a * t3d) >> (k)) + b * t2d) >> (k)) + c) >> (k));
	    } else {
		if (!prc->double_set) {
		    (k >= (sizeof(k_denom) / sizeof((k_denom)[0])) ? (den1 = ldexp(1.0, -k), den2 = den1 * den1, prc->da = (den2 * den1) * a, prc->db = den2 * b, prc->dc = den1 * c) : (prc->da = k3_denom[k] * a, prc->db = k2_denom[k] * b, prc->dc = k_denom[k] * c));
		    prc->double_set = ((bool) 1);
		}
		if (t < 1L << ((sizeof(long) * 8 - 1) / 3)) {
		    long t2 = (long) t * t, t3 = t2 * t;
		    long t3d = (t2 + t) * 3 + 1, t2d = t + t + 1;
		    xl = ((fixed) (prc->da * t3 + prc->db * t2 + prc->dc * t + 0.5)) + cx0;
		    xd = ((fixed) (prc->da * t3d + prc->db * t2d + prc->dc));
		} else {
	...
	return xl;
    if (yrel < ((fixed) 1 << (sizeof(fixed) * 4))) {
	if (xd >= 0) {
	    if (xd < ((fixed) 1 << (sizeof(fixed) * 4)))
    ...
    return ((fixed) floor((double) (xd) * (yrel) / (yd))) + xl;
} bool gx_path_is_monotonic(const gx_path * ppath)
{
	...
	case s_curve:{
		const curve_segment *pc = (const curve_segment *) pseg;
		double t[2];
		int nz = gx_curve_monotonic_points(pt0.y, pc->p1.y, pc->p2.y, pc->pt.y, t);
		if (nz != 0)
		    return ((bool) 0);
		nz = gx_curve_monotonic_points(pt0.x, pc->p1.x, pc->p2.x, pc->pt.x, t);
		if (nz != 0)
		    return ((bool) 0);
	    }
	    break;
...
static int monotonize_internal(gx_path * ppath, const curve_segment * pc)
{
	...
	*pcd = *pc;
	...
	if (nz == 2)
	    gx_curve_split(pcd->pt.x, pcd->pt.y, pcd + 1, (t[1] - t[0]) / (1 - t[0]), pcd + 1, pcd + 2);
    }
    for (pcs = pcd, pcd = cs, j = nseg; j < 9; ++pcs, ++j) {
	nz = gx_curve_monotonic_points(x0, pcs->p1.x, pcs->p2.x, pcs->pt.x, t);
	if (nz == 0)
	    *pcd = *pcs;
	else {
	...
	int code = gx_path_add_curve_notes(ppath, pcs->p1.x, pcs->p1.y, pcs->p2.x, pcs->p2.y, pcs->pt.x, pcs->pt.y, notes | (i > 0 ? sn_not_first : sn_none));
	if (code < 0)
	    return code;
    }
    return 0;
...
int gx_curve_monotonic_points(fixed v0, fixed v1, fixed v2, fixed v3, double pst[2])
{
	...
	if ((a ^ b) < 0 && (b2abs = ((b2) < 0 ? -(b2) : (b2))) > (a3abs = ((a3) < 0 ? -(a3) : (a3))) && b2abs < a3abs << 1) {
	    *pst = (double) (-b2 - a3) / a3;
	    return 1;
	} else
	    return 0;
...
void gx_curve_split(fixed x0, fixed y0, const curve_segment * pc, double t, curve_segment * pc1, curve_segment * pc2)
{
    ...
    (v01 = (pc->p1.x) - (x0), c = (v01 << 1) + v01, v12 = (pc->p2.x) - (pc->p1.x), b = (v12 << 1) + v12 - c, a = (pc->pt.x) - b - c - (x0));
    na = (fixed) (a * t3), nb = (fixed) (b * t2), nc = (fixed) (c * t);
    (pc1->p1.x = (x0) + ((nc) / 3), pc1->p2.x = pc1->p1.x + (((nb) + (nc)) / 3), pc1->pt.x = (na) + (nb) + (nc) + (x0));
    na = (fixed) (a * omt3);
    nb = (fixed) ((a * t * 3 + b) * omt2);
    nc = (fixed) ((a * t2 * 3 + b * 2 * t + c) * omt);
    (pc2->p1.x = (pc1->pt.x) + ((nc) / 3), pc2->p2.x = pc2->p1.x + (((nb) + (nc)) / 3), pc2->pt.x = (na) + (nb) + (nc) + (pc1->pt.x));
    (v01 = (pc->p1.y) - (y0), c = (v01 << 1) + v01, v12 = (pc->p2.y) - (pc->p1.y), b = (v12 << 1) + v12 - c, a = (pc->pt.y) - b - c - (y0));
    na = (fixed) (a * t3), nb = (fixed) (b * t2), nc = (fixed) (c * t);
    (pc1->p1.y = (y0) + ((nc) / 3), pc1->p2.y = pc1->p1.y + (((nb) + (nc)) / 3), pc1->pt.y = (na) + (nb) + (nc) + (y0));
    na = (fixed) (a * omt3);
    nb = (fixed) ((a * t * 3 + b) * omt2);
    nc = (fixed) ((a * t2 * 3 + b * 2 * t + c) * omt);
    (pc2->p1.y = (pc1->pt.y) + ((nc) / 3), pc2->p2.y = pc2->p1.y + (((nb) + (nc)) / 3), pc2->pt.y = (na) + (nb) + (nc) + (pc1->pt.y));
}
