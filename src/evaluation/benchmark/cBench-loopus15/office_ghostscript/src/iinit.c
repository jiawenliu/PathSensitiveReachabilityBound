...
const char *gs_error_names[] = { "unknownerror", "dictfull", "dictstackoverflow", "dictstackunderflow", "execstackoverflow", "interrupt", "invalidaccess", "invalidexit", "invalidfileaccess", "invalidfont", "invalidrestore", "ioerror", "limitcheck", "nocurrentpoint", "rangecheck", "stackoverflow", "stackunderflow", "syntaxerror", "timeout", "typecheck", "undefined", "undefinedfilename", "undefinedresult", "unmatchedmark", "VMerror", "configurationerror", "invalidcontext", "undefinedresource", "unregistered", "invalidid", 0 };
...
extern op_def_ptr(*(op_defs_all[])) (void);
const op_def **op_def_table;
uint op_def_count;
op_array_table op_array_table_global, op_array_table_local;
uint op_array_count;
static gs_gc_root_t op_def_root, op_array_root_global, op_array_root_local, op_array_nx_root_global, op_array_nx_root_local;
void initial_enter_name_in(const char *nstr, const ref * pref, ref * pdict)
{
    int code = dict_put_string(pdict, nstr, pref);
    if (code < 0) {
	(lprintf_file_and_line(gs_stderr, "iinit.c", 96), fprintf(gs_stderr, "initial_enter failed (%d), entering /%s in -dict:%u/%u-\n", code, nstr, dict_length(pdict), dict_maxlength(pdict)));
	gs_exit(1);
    }
}

void initial_enter_name(const char *nstr, const ref * pref)
{
    initial_enter_name_in(nstr, pref, (&ref_systemdict));
} void initial_remove_name(const char *nstr)
{
    ref nref;
    if (name_ref((const byte *) nstr, strlen(nstr), &nref, -1) >= 0)
	dict_undef((&ref_systemdict), &nref);
} static void name_enter(const char *str, ref * pref)
{
    if (name_enter_string(str, pref) != 0) {
	(lprintf_file_and_line(gs_stderr, "iinit.c", 117), fprintf(gs_stderr, "name_enter failed - %s\n", str));
	gs_exit(1);
    }
}
...
"level2dict", 191, ((bool) 0)}, {
"globaldict", 0, ((bool) 0)}, {
"userdict", 0, ((bool) 1)}, {
"filterdict", 43, ((bool) 0)}, {
"internaldict", 3, ((bool) 1)}};
const char *initial_dstack[] = { "userdict" };

bool gs_have_level2(void)
{
    op_def_ptr(**tptr) (void);
    for (tptr = op_defs_all; *tptr != 0; tptr++) {
	const op_def *def;
	for (def = (*tptr) (); def->oname != 0; def++)
	    if (((def)->proc == 0) && !strcmp(def->oname, "level2dict"))
		return ((bool) 1);
    }
    return ((bool) 0);
}

static ref *make_initial_dict(const char *iname, ref idicts[])
{
    int i;
	...
	if (!strcmp(iname, dname)) {
		...
		ialloc_set_space((&gs_imemory), (initial_dictionaries[i].local ? avm_local : avm_global));
...
void obj_init(void)
{
    ...
    ((&ref_language_level)->value.intval = (1), ((&ref_language_level)->tas.type_attrs = ((t_integer) << 8) + (0)));
    gs_interp_init(); {
	ref idicts[(sizeof(initial_dictionaries) / sizeof((initial_dictionaries)[0]))];
	int i;
	op_def_ptr(**tptr) (void);
	min_dstack_size = ((sizeof(initial_dstack) / sizeof((initial_dstack)[0])) + 1);
	refset_null(idicts, (sizeof(initial_dictionaries) / sizeof((initial_dictionaries)[0])));
	ialloc_set_space((&gs_imemory), avm_global);
	if (level2) {
	    (d_stack.p) += 2;
	    dict_create(809, (d_stack.p));
	    (d_stack.p)[-1] = *(d_stack.p);
	    min_dstack_size++;
	} else {
	    ++(d_stack.p);
	    dict_create(563, (d_stack.p));
	}
	ref_systemdict = *(d_stack.p);
	ialloc_set_space((&gs_imemory), space);
	for (tptr = op_defs_all; *tptr != 0; tptr++) {
	    const op_def *def;
	    for (def = (*tptr) (); def->oname != 0; def++)
	...
	for (i = 0; i < (sizeof(initial_dstack) / sizeof((initial_dstack)[0])); i++) {
	    ...
	    ++(d_stack.p);
	    (*((d_stack.p)) = *(make_initial_dict(dname, idicts)));
	} initial_enter_name("systemdict", &ref_systemdict);
	for (i = 0; i < (sizeof(initial_dictionaries) / sizeof((initial_dictionaries)[0])); i++) {
	    ref *idict = &idicts[i];
	    if (!((((const byte *) &((idict)->tas.type_attrs))[1]) == (t_null))) {
		uint save_space = (avm_space) ((((&ref_systemdict))->tas.type_attrs) & (((1 << 2) - 1) << 2));
		(((&ref_systemdict))->tas.type_attrs = (((&ref_systemdict))->tas.type_attrs & ~((((1 << 2) - 1) << 2))) | ((uint) avm_local));
		initial_enter_name(initial_dictionaries[i].name, idict);
		(((&ref_systemdict))->tas.type_attrs = (((&ref_systemdict))->tas.type_attrs & ~((((1 << 2) - 1) << 2))) | ((uint) save_space));
	    }
    }} gs_interp_reset(); {
	ref vtemp;
	(((&vtemp)->tas.type_attrs = ((t_null) << 8) + (0)));
	initial_enter_name("null", &vtemp);
	((&vtemp)->value.boolval = (1), ((&vtemp)->tas.type_attrs = ((t_boolean) << 8) + (0)));
	initial_enter_name("true", &vtemp);
	((&vtemp)->value.boolval = (0), ((&vtemp)->tas.type_attrs = ((t_boolean) << 8) + (0)));
	initial_enter_name("false", &vtemp);
    } {
	int n = (sizeof(gs_error_names) / sizeof((gs_error_names)[0])) - 1;
	int i;
	ref era;
	gs_alloc_ref_array((gs_imemory.current), &era, (0x20 + 0x40), n, "ErrorNames");
	for (i = 0; i < n; i++)
	    name_enter((const char *) gs_error_names[i], era.value.refs + i);
...
}} void zop_init(void)
{
	...
	ref vtemp;
	((&vtemp)->value.const_bytes = ((const byte *) gs_copyright), ((&vtemp)->tas.type_attrs = ((t_string) << 8) + ((0x20 + 0x40) | avm_foreign)), ((&vtemp)->tas.rsize = (strlen(gs_copyright))));
	initial_enter_name("copyright", &vtemp);
	((&vtemp)->value.const_bytes = ((const byte *) gs_product), ((&vtemp)->tas.type_attrs = ((t_string) << 8) + ((0x20 + 0x40) | avm_foreign)), ((&vtemp)->tas.rsize = (strlen(gs_product))));
	initial_enter_name("product", &vtemp);
	((&vtemp)->value.intval = (gs_revision), ((&vtemp)->tas.type_attrs = ((t_integer) << 8) + (0)));
	initial_enter_name("revision", &vtemp);
	((&vtemp)->value.intval = (gs_revisiondate), ((&vtemp)->tas.type_attrs = ((t_integer) << 8) + (0)));
	initial_enter_name("revisiondate", &vtemp);
}} static int alloc_op_array_table(uint size, uint space, op_array_table * opt)
{
    uint save_space = (((&gs_imemory))->current_space);
    int code;
    ialloc_set_space((&gs_imemory), space);
    code = gs_alloc_ref_array((gs_imemory.current), &opt->table, (0x20 + 0x40), size, "op_array table");
    ialloc_set_space((&gs_imemory), save_space);
    if (code < 0)
	return code;
    refset_null(opt->table.value.refs, size);
    opt->nx_table = (ushort *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_byte_array) (((gs_memory_t *) (gs_imemory.current)), size, sizeof(ushort), "op_array nx_table");
    if (opt->nx_table == 0)
	return ((-25));
    opt->count = 0;
...
void op_init(void)
{
    ...
    op_def_table = (const op_def **) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_byte_array) (((gs_memory_t *) (gs_imemory.current)), count, sizeof(op_def_ptr), "op_init(op_def_table)");
    op_def_count = count;
    for (count = 0; count <= gs_interp_num_special_ops; count++)
	op_def_table[count] = 0;
    count = gs_interp_num_special_ops + 1;
    for (tptr = op_defs_all; *tptr != 0; tptr++) {
	ref *pdict = (&ref_systemdict);
	for (def = (*tptr) (); (nstr = def->oname) != 0; def++)
	    if (((def)->proc == 0)) {
		ref nref;
		int code = name_ref((const byte *) nstr, strlen(nstr), &nref, -1);
		if (code != 0)
		    gs_exit(1);
		if (!dict_find((&ref_systemdict), &nref, &pdict))
		    gs_exit(1);
	...
	if (op_def_table[count] == 0)
	    gs_exit(1);
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &op_def_root, (&ptr_struct_procs), (void **) &op_def_table, "op_def_table");
    if (alloc_op_array_table(120, avm_global, &op_array_table_global) < 0)
	gs_exit(1);
    op_array_table_global.base_index = op_def_count;
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &op_array_root_global, (&ptr_ref_procs), (void **) &op_array_table_global.root_p, "op_array_table(global)");
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &op_array_nx_root_global, (&ptr_struct_procs), (void **) &op_array_table_global.nx_table, "op_array nx_table(global)");
    if (alloc_op_array_table((120 / 2), avm_local, &op_array_table_local) < 0)
	gs_exit(1);
    op_array_table_local.base_index = op_array_table_global.base_index + ((&op_array_table_global.table)->tas.rsize);
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &op_array_root_local, (&ptr_ref_procs), (void **) &op_array_table_local.root_p, "op_array_table(local)");
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &op_array_nx_root_local, (&ptr_struct_procs), (void **) &op_array_table_local.nx_table, "op_array nx_table(local)");
}
