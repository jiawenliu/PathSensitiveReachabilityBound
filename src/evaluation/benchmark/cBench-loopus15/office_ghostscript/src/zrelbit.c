...
static int zeq(register os_ptr op)
{
    ...
    ((op1)->value.boolval = ((obj_eq(op1, op) ? 1 : 0)), ((op1)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    ((o_stack.p) -= (1));
    return 0;
}

static int zne(register os_ptr op)
{
    int code = zeq(op);
    if (!code)
...
static int zge(register os_ptr op)
{
...
static int zgt(register os_ptr op)
{
...
static int zle(register os_ptr op)
{
    int code = obj_le(op - 1, op);
    if (code < 0)
	return code;
...
static int zlt(register os_ptr op)
{
    int code = obj_le(op, op - 1);
    if (code < 0)
	return code;
...
static int zmax(register os_ptr op)
{
    int code = obj_le(op - 1, op);
    if (code < 0)
	return code;
...
static int zmin(register os_ptr op)
{
    int code = obj_le(op - 1, op);
    if (code < 0)
	return code;
...
static int zand(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    case t_boolean:
...
static int znot(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    case t_boolean:
...
static int zor(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    case t_boolean:
	if (!((((const byte *) &((&op[-1])->tas.type_attrs))[1]) == (t_boolean)))
...
static int zxor(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    case t_boolean:
	if (!((((const byte *) &((&op[-1])->tas.type_attrs))[1]) == (t_boolean)))
...
static int zbitshift(register os_ptr op)
{
    int shift;
    ...
    if (!((((const byte *) &((&op[-1])->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&op[-1]));
    if (op->value.intval < -((1 << 2) * 8 - 1) || op->value.intval > ((1 << 2) * 8 - 1))
	op[-1].value.intval = 0;
...
static int zidenteq(register os_ptr op)
{
    register os_ptr op1 = op - 1;
    switch ((((const byte *) &((op1)->tas.type_attrs))[1])) {
    case t_string:
    ...
    ((op1)->value.boolval = ((obj_ident_eq(op1, op) ? 1 : 0)), ((op1)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    ((o_stack.p) -= (1));
    return 0;
}

static int zidentne(register os_ptr op)
{
...
const op_def *zrelbit_op_defs(void)
{
    static const op_def op_defs_[] = { {"2and", zand}, {"2bitshift", zbitshift}, {"2eq", zeq}, {"2ge", zge}, {"2gt", zgt}, {"2le", zle}, {"2lt", zlt}, {"2.max", zmax}, {"2.min", zmin}, {"2ne", zne}, {"1not", znot}, {"2or", zor}, {"2xor", zxor}, {"2.identeq", zidenteq}, {"2.identne", zidentne}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}

static int obj_le(register os_ptr op1, register os_ptr op)
{
    switch ((((const byte *) &((op1)->tas.type_attrs))[1])) {
    case t_integer:
	switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
	case t_integer:
	    return ((op1->value.intval <= op->value.intval) ? 1 : 0);
	case t_real:
	    return (((double) op1->value.intval <= op->value.realval) ? 1 : 0);
	default:
	    return (check_type_failed(op));
	}
    case t_real:
	switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
	case t_real:
	    return ((op1->value.realval <= op->value.realval) ? 1 : 0);
	case t_integer:
	    return ((op1->value.realval <= (double) op->value.intval) ? 1 : 0);
	default:
	    return (check_type_failed(op));
	}
    case t_string:
	...
	return ((bytes_compare(op1->value.bytes, ((op1)->tas.rsize), op->value.bytes, ((op)->tas.rsize)) <= 0) ? 1 : 0);
    default:
