...
static int type42_fill(os_ptr);
static int type42_stroke(os_ptr);
static int ztype42execchar(register os_ptr op)
{
    gs_font *pfont;
	...
	code = gs_type42_get_metrics(((gs_font_type42 *) pfont), (uint) op->value.intval, sbw);
	if (code < 0)
	    return code;
    }
    return zchar_set_cache(op, ((gs_font_base *) pfont), op - 1, (present == metricsSideBearingAndWidth ? sbw : ((void *) 0)), sbw + 2, &((gs_font_base *) pfont)->FontBBox, type42_fill, type42_stroke);
} static int type42_finish(os_ptr op, int (*cont) (gs_state *));

static int type42_fill(os_ptr op)
{
    return type42_finish(op, gs_fill);
}

static int type42_stroke(os_ptr op)
{
...
static int type42_finish(os_ptr op, int (*cont) (gs_state *))
{
    ...
    gs_point *psbpt = 0;
    os_ptr opc = op;
    if (!((((const byte *) &((op - 3)->tas.type_attrs))[1]) == (t_dictionary))) {
	if (op < (o_stack.bot) + ((6) - 1))
	    return ((-17));
    ...
    if (!((((const byte *) &((&*opc)->tas.type_attrs))[1]) == (t_integer)))
    ...
    if (penum == 0 || (pfont->FontType != ft_TrueType && pfont->FontType != ft_CID_TrueType))
	return ((-21));
    code = gs_type42_append((uint) opc->value.intval, (gs_imager_state *) penum->pgs, penum->pgs->path, &penum->log2_current_scale, gs_show_in_charpath(penum) != cpm_show, pfont->PaintType, ((gs_font_type42 *) pfont));
    if (code < 0)
	return code;
    ((o_stack.p) -= ((psbpt == 0 ? 4 : 6)));
    return (*cont) (penum->pgs);
}

const op_def *zchar42_op_defs(void)
{
    static const op_def op_defs_[] = { {"4.type42execchar", ztype42execchar}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
