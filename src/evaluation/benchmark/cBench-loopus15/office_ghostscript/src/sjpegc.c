...
{
    jpeg_stream_data *jcomdp = (jpeg_stream_data *) ((char *) cinfo - ((int) ((char *) &((jpeg_compress_data *) 0)->cinfo - (char *) ((jpeg_compress_data *) 0))));
    longjmp(jcomdp->exit_jmpbuf, 1);
} static void gs_jpeg_emit_message(j_common_ptr cinfo, int msg_level)
{
    if (msg_level < 0) {
	jpeg_stream_data *jcomdp = (jpeg_stream_data *) ((char *) cinfo - ((int) ((char *) &((jpeg_compress_data *) 0)->cinfo - (char *) ((jpeg_compress_data *) 0))));
	if (jcomdp->Picky)
...
static void gs_jpeg_format_message(j_common_ptr cinfo, char *buffer)
{
    ...
    } else if (err->addon_message_table != ((void *) 0) && msg_code >= err->first_addon_message && msg_code <= err->last_addon_message) {
...
static void gs_jpeg_reset_error_mgr(j_common_ptr cinfo)
{
...
JQUANT_TBL *gs_jpeg_alloc_quant_table(stream_DCT_state * st)
{
    if (_setjmp(st->data.common->exit_jmpbuf)) {
	gs_jpeg_log_error(st);
	return ((void *) 0);
    }
    return jpeg_alloc_quant_table((j_common_ptr) & st->data.compress->cinfo);
}
...
JHUFF_TBL *gs_jpeg_alloc_huff_table(stream_DCT_state * st)
{
    ...
    return jpeg_alloc_huff_table((j_common_ptr) & st->data.compress->cinfo);
}
...
{
    return (void *) (*(&gs_memory_default)->procs.alloc_byte_array) (&gs_memory_default, 1, sizeofobject, "JPEG small internal data allocation");
} void jpeg_free_small(j_common_ptr cinfo, void *object, size_t sizeofobject)
{
    (*(&gs_memory_default)->procs.free_object) (&gs_memory_default, object, "Freeing JPEG small internal data");
} void *jpeg_get_large(j_common_ptr cinfo, size_t sizeofobject)
{
    return (void *) (*(&gs_memory_default)->procs.alloc_byte_array) (&gs_memory_default, 1, sizeofobject, "JPEG large internal data allocation");
} void jpeg_free_large(j_common_ptr cinfo, void *object, size_t sizeofobject)
{
    (*(&gs_memory_default)->procs.free_object) (&gs_memory_default, object, "Freeing JPEG large internal data");
} long jpeg_mem_available(j_common_ptr cinfo, long min_bytes_needed, long max_bytes_needed, long already_allocated)
{
    return max_bytes_needed;
}

void jpeg_open_backing_store(j_common_ptr cinfo, void *info, long total_bytes_needed)
{
