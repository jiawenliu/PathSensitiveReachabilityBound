...
static int type1crypt(os_ptr, int (*)(byte *, const byte *, uint, ushort *));
static int ztype1encrypt(os_ptr op)
{
    return type1crypt(op, gs_type1_encrypt);
}

static int ztype1decrypt(os_ptr op)
{
    return type1crypt(op, gs_type1_decrypt);
}

static int type1crypt(register os_ptr op, int (*proc) (byte *, const byte *, uint, ushort *))
{
    crypt_state state;
    uint ssize;
...
static int eexec_param(os_ptr op, ushort * pcstate)
{
    int npop = 1;
...
static int zexE(register os_ptr op)
{
...
static int zexD(register os_ptr op)
{
	...
	if (!(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)))
	    return ((-7));
	if ((code = dict_uint_param(op, "seed", 0, 0xffff, 0x10000, &cstate)) < 0 || (code = dict_int_param(op, "lenIV", 0, (~(-1 << ((1 << 2) * 8 - 1))), 4, &state.lenIV)) < 0)
	    return code;
	state.cstate = cstate;
	code = 1;
    } else {
...
const op_def *zmisc1_op_defs(void)
{
    static const op_def op_defs_[] = { {"3.type1encrypt", ztype1encrypt}, {"3.type1decrypt", ztype1decrypt}, {"filterdict", 0}, {"2eexecEncode", zexE}, {"2eexecDecode", zexD}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
