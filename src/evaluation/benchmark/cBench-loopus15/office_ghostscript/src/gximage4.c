...
static const byte *sample_unpack_12(byte * bptr, int *pdata_x, const byte * data, int data_x, uint dsize, const sample_lookup_t * ignore_ptab, int spread)
{
    ...
    static const frac bits2frac_4[16] = { ((((frac) 0x7ff8) / 15) * (0)), ((((frac) 0x7ff8) / 15) * (1)), ((((frac) 0x7ff8) / 15) * (2)), ((((frac) 0x7ff8) / 15) * (3)), ((((frac) 0x7ff8) / 15) * (4)), ((((frac) 0x7ff8) / 15) * (5)), ((((frac) 0x7ff8) / 15) * (6)), ((((frac) 0x7ff8) / 15) * (7)), ((((frac) 0x7ff8) / 15) * (8)), ((((frac) 0x7ff8) / 15) * (9)), ((((frac) 0x7ff8) / 15) * (10)), ((((frac) 0x7ff8) / 15) * (11)), ((((frac) 0x7ff8) / 15) * (12)), ((((frac) 0x7ff8) / 15) * (13)), ((((frac) 0x7ff8) / 15) * (14)), ((((frac) 0x7ff8) / 15) * (15)) };
    if ((data_x & 1) && left > 0)
	switch (left) {
	default:
...
static irender_proc_t image_strategy_frac(gx_image_enum * penum)
{
    if (penum->bps > 8) {
	do {
	} while (0);
...
void gs_gximage4_init(gs_memory_t * mem)
{
...
static int image_render_frac(gx_image_enum * penum, const byte * buffer, int data_x, uint w, int h, gx_device * dev)
{
    ...
    gx_device_color *spdevc = &devc1;
    gx_device_color *spdevc_next = &devc2;
    int spp = penum->spp;
    ...
    cc.pattern = 0;
    (*remap_color) (&cc, pcs, ((gx_device_color *) spdevc), pis, dev, gs_color_select_source);
    run.v[0] = ~psrc[0];
    while (psrc < bufend) {
	next.v[0] = psrc[0];
	switch (spp) {
	case 4:
	    ...
	    next.v[3] = psrc[3];
	    psrc += 4;
	    if (((next).all[0] == (run).all[0] && (next).all[1] == (run).all[1]))
		goto inc;
	    if (device_color) {
		(*map_cmyk) (next.v[0], next.v[1], next.v[2], next.v[3], ((gx_device_color *) spdevc_next), pis, dev, gs_color_select_source);
		goto f;
	    }
	...
	case 3:
		...
		goto inc;
	    if (device_color) {
		(*map_rgb) (next.v[0], next.v[1], next.v[2], ((gx_device_color *) spdevc_next), pis, dev, gs_color_select_source);
		goto f;
	    }
	    cc.paint.values[0] = penum->map[0].decode_lookup[0] + (next.v[0]) * penum->map[0].decode_factor;
	    cc.paint.values[1] = penum->map[1].decode_lookup[0] + (next.v[1]) * penum->map[1].decode_factor;
	...
	case 1:
		...
		goto inc;
	    if (device_color) {
		(*map_rgb) (next.v[0], next.v[0], next.v[0], ((gx_device_color *) spdevc_next), pis, dev, gs_color_select_source);
		goto f;
	    }
	    cc.paint.values[0] = penum->map[0].decode_lookup[0] + (next.v[0]) * penum->map[0].decode_factor;
	    do {
	...
	(*remap_color) (&cc, pcs, ((gx_device_color *) spdevc_next), pis, dev, gs_color_select_source);
      f:do {
	} while (0);
	if (!(((&(devc1))->type == (&gx_dc_procs_pure)) ? ((&(devc2))->type == (&gx_dc_procs_pure)) && ((&(devc1))->colors.pure) == ((&(devc2))->colors.pure) : ((&(devc1))->type == (&gx_dc_procs_ht_binary)) ? ((&(devc2))->type == (&gx_dc_procs_ht_binary)) && ((&(devc1))->colors.binary.color[0]) == ((&(devc2))->colors.binary.color[0]) && ((&(devc1))->colors.binary.color[1]) == ((&(devc2))->colors.binary.color[1]) && (devc1).colors.binary.b_level == (devc2).colors.binary.b_level : ((bool) 0))) {
	    gx_device_color *sptemp;
	    if (posture != image_portrait) {
		code = (*((dev)->std_procs.fill_parallelogram)) (dev, xrun, yrun, xl - xrun, ytf - yrun, pdyx, pdyy, ((gx_device_color *) spdevc), lop);
		xrun = xl;
		yrun = ytf;
	    } else {
		int xi = irun;
		...
		if (wi < 0)
		    xi += wi, wi = -wi;
		code = (*(((gx_device_color *) spdevc))->type->fill_rectangle) (((gx_device_color *) spdevc), xi, yt, wi, iht, dev, lop, ((void *) 0));
	    } if (code < 0)
		return code;
	    sptemp = spdevc;
	    spdevc = spdevc_next;
	...
	run = next;
      inc:xl = ((pnext.x).state).Q += (((pnext.x).state).R >= ((pnext.x).step).dR ? (((pnext.x).state).R -= ((pnext.x).step).dR, ((pnext.x).step).dQ) : (((pnext.x).state).R += ((pnext.x).step).NdR, ((pnext.x).step).dQ + 1));
	ytf = ((pnext.y).state).Q += (((pnext.y).state).R >= ((pnext.y).step).dR ? (((pnext.y).state).R -= ((pnext.y).step).dR, ((pnext.y).step).dQ) : (((pnext.y).state).R += ((pnext.y).step).NdR, ((pnext.y).step).dQ + 1));
    }
    code = (*((dev)->std_procs.fill_parallelogram)) (dev, xrun, yrun, xl - xrun, ytf - yrun, pdyx, pdyy, ((gx_device_color *) spdevc), lop);
