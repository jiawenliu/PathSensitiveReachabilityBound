...
int gs_closepath(gs_state * pgs)
{
    ...
    if (code < 0)
	return code;
    if (((ppath)->state_flags != 0 && ((ppath)->start_flags & psf_outside_range) != 0))
	((ppath)->outside_position.x = (ppath->outside_start.x), (ppath)->outside_position.y = (ppath->outside_start.y), (ppath)->state_flags |= psf_outside_range);
    return code;
}

int gs_upmergepath(gs_state * pgs)
{
    return gx_path_add_path(pgs->saved->path, pgs->path);
}

gx_path *gx_current_path(const gs_state * pgs)
{
    return pgs->path;
}

static void clamp_point(gs_fixed_point * ppt, floatp x, floatp y)
{
    ppt->x = (x > ((((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12))) * (1.0 / (1 << 12))) ? ((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12)) : x < (((-((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12)))) * (1.0 / (1 << 12))) ? (-((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12))) : ((fixed) ((x) * (float) (1 << 12))));
    ppt->y = (y > ((((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12))) * (1.0 / (1 << 12))) ? ((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12)) : y < (((-((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12)))) * (1.0 / (1 << 12))) ? (-((~(-1L << ((1 << 2) * 8 - 1))) - ((fixed) (1000) << 12))) : ((fixed) ((y) * (float) (1 << 12))));
} int gs_currentpoint(const gs_state * pgs, gs_point * ppt)
{
    gx_path *ppath = pgs->path;
    int code;
    gs_fixed_point pt;
    if ((((ppath)->state_flags & psf_outside_range) != 0))
	return gs_itransform((gs_state *) pgs, ppath->outside_position.x, ppath->outside_position.y, ppt);
    code = gx_path_current_point(pgs->path, &pt);
    if (code < 0)
	return code;
    return gs_itransform((gs_state *) pgs, ((pt.x) * (1.0 / (1 << 12))), ((pt.y) * (1.0 / (1 << 12))), ppt);
}
...
int gs_moveto(gs_state * pgs, floatp x, floatp y)
{
	...
	if (pgs->clamp_coordinates) {
	    gs_point opt;
	    if (code != (-13) || (code = gs_transform(pgs, x, y, &opt)) < 0)
		return code;
	    clamp_point(&pt, opt.x, opt.y);
	    code = gx_path_add_point(ppath, pt.x, pt.y);
	    if (code < 0)
		return code;
	    ((ppath)->outside_position.x = (opt.x), (ppath)->outside_position.y = (opt.y), (ppath)->state_flags |= psf_outside_range);
...
int gs_rmoveto(gs_state * pgs, floatp x, floatp y)
{
    gs_fixed_point dpt;
    int code;
    if ((code = gs_distance_transform2fixed(&pgs->ctm, x, y, &dpt)) < 0 || (code = gx_path_add_relative_point(pgs->path, dpt.x, dpt.y)) < 0) {
	gs_point upt;
...
int gs_lineto(gs_state * pgs, floatp x, floatp y)
{
    ...
    return gx_path_add_line_notes(pgs->path, pt.x, pt.y, sn_none);
}

int gs_rlineto(gs_state * pgs, floatp x, floatp y)
{
    gx_path *ppath = pgs->path;
    gs_fixed_point dpt;
    fixed nx, ny;
    int code;
    if (!(((ppath)->state_flags & (psf_position_valid + psf_outside_range)) == psf_position_valid) || (code = gs_distance_transform2fixed(&pgs->ctm, x, y, &dpt)) < 0 || (((nx = ppath->position.x + dpt.x) ^ dpt.x) < 0 && (ppath->position.x ^ dpt.x) >= 0) || (((ny = ppath->position.y + dpt.y) ^ dpt.y) < 0 && (ppath->position.y ^ dpt.y) >= 0) || (code = gx_path_add_line_notes(ppath, nx, ny, sn_none)) < 0) {
	gs_point upt;
	if ((code = gs_currentpoint(pgs, &upt)) < 0)
	    return code;
...
int gs_curveto(gs_state * pgs, floatp x1, floatp y1, floatp x2, floatp y2, floatp x3, floatp y3)
{
	...
	if (pgs->clamp_coordinates) {
	    ...
	    int code;
	    if ((code1 < 0 && code1 != (-13)) || (code1 = gs_transform(pgs, x1, y1, &opt1)) < 0)
		return code1;
	    if ((code2 < 0 && code2 != (-13)) || (code2 = gs_transform(pgs, x2, y2, &opt2)) < 0)
		return code2;
	    if ((code3 < 0 && code3 != (-13)) || (code3 = gs_transform(pgs, x3, y3, &opt3)) < 0)
		return code3;
	    ...
	    if (code < 0)
		return code;
	    ((ppath)->outside_position.x = (opt3.x), (ppath)->outside_position.y = (opt3.y), (ppath)->state_flags |= psf_outside_range);
	    return code;
	} else
	    return (code1 < 0 ? code1 : code2 < 0 ? code2 : code3);
    }
    return gx_path_add_curve_notes(ppath, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, sn_none);
}

int gs_rcurveto(gs_state * pgs, floatp dx1, floatp dy1, floatp dx2, floatp dy2, floatp dx3, floatp dy3)
{
    gx_path *ppath = pgs->path;
    gs_fixed_point p1, p2, p3;
    fixed ptx, pty;
    int code;
    if (!(((ppath)->state_flags & (psf_position_valid + psf_outside_range)) == psf_position_valid) || (code = gs_distance_transform2fixed(&pgs->ctm, dx1, dy1, &p1)) < 0 || (code = gs_distance_transform2fixed(&pgs->ctm, dx2, dy2, &p2)) < 0 || (code = gs_distance_transform2fixed(&pgs->ctm, dx3, dy3, &p3)) < 0 || (ptx = ppath->position.x, pty = ppath->position.y, code = gx_path_add_curve_notes(ppath, ptx + p1.x, pty + p1.y, ptx + p2.x, pty + p2.y, ptx + p3.x, pty + p3.y, sn_none)) < 0) {
	gs_point upt;
	if ((code = gs_currentpoint(pgs, &upt)) < 0)
	    return code;
	return gs_curveto(pgs, upt.x + dx1, upt.y + dy1, upt.x + dx2, upt.y + dy2, upt.x + dx3, upt.y + dy3);
    }
    return code;
}

static int common_clip(gs_state *, int);
static int set_clip_path(gs_state *, gx_clip_path *, int);
int gs_clippath(gs_state * pgs)
{
    gx_path path, cpath;
    int code = gx_cpath_path(pgs->clip_path, &path);
    if (code < 0)
	return code;
    code = gx_path_copy_reducing(&path, &cpath, (~(-1L << ((1 << 2) * 8 - 1))), (1 ? pco_init : pco_none));
    if (code < 0)
	return code;
    gx_path_release(pgs->path);
    *pgs->path = cpath;
...
int gs_initclip(gs_state * pgs)
{
	...
	imat.tx += dev->Margins[0] * dev->HWResolution[0] / dev->MarginsHWResolution[0];
	imat.ty += dev->Margins[1] * dev->HWResolution[1] / dev->MarginsHWResolution[1];
    ...
    box.p.x = (((((fixed) ((bbox.p.x) * (float) (1 << 12)))) + ((1 << 12) >> 1)) & (-1L << 12));
    box.p.y = (((((fixed) ((bbox.p.y) * (float) (1 << 12)))) + ((1 << 12) >> 1)) & (-1L << 12));
    box.q.x = (((((fixed) ((bbox.q.x) * (float) (1 << 12)))) + ((1 << 12) >> 1)) & (-1L << 12));
    box.q.y = (((((fixed) ((bbox.q.y) * (float) (1 << 12)))) + ((1 << 12) >> 1)) & (-1L << 12));
...
static int common_clip(gs_state * pgs, int rule)
{
    gx_path fpath;
    int code = gx_path_copy_reducing(pgs->path, &fpath, ((fixed) ((pgs->flatness) * (float) (1 << 12))), (pgs->accurate_curves ? pco_init | pco_accurate : pco_init));
    if (code < 0)
	return code;
...
int gs_setclipoutside(gs_state * pgs, bool outside)
{
    return gx_cpath_set_outside(pgs->clip_path, outside);
}

bool gs_currentclipoutside(const gs_state * pgs)
{
    return gx_cpath_is_outside(pgs->clip_path);
}

int gx_clip_to_rectangle(gs_state * pgs, gs_fixed_rect * pbox)
{
    gx_clip_path cpath;
    ...
    return set_clip_path(pgs, &cpath, (-1));
}

int gx_clip_to_path(gs_state * pgs)
{
    gs_fixed_rect bbox;
    int code;
    if ((code = gx_path_bbox(pgs->path, &bbox)) < 0 || (code = gx_clip_to_rectangle(pgs, &bbox)) < 0)
	return code;
    return gs_clip(pgs);
}

static int set_clip_path(gs_state * pgs, gx_clip_path * pcpath, int rule)
{
