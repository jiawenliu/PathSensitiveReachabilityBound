...
static const char *type_strings[] = { "INVL", "bool", "dict", "file", "arry", "mpry", "spry", "u?ry", "STRC", "ASTR", "font", "int ", "mark", "name", "null", "oper", "real", "save", "str ", "devc", "opry" };

extern const int tx_next_index;
void debug_print_name(const ref * pnref)
{
    ref sref;
    name_string_ref(pnref, &sref);
    debug_print_string(sref.value.const_bytes, ((&sref)->tas.rsize));
} static void debug_print_full_ref(const ref * pref)
{
    unsigned size = ((pref)->tas.rsize);
    ref nref;
    fprintf(gs_debug_out, "(%x)", ((pref)->tas.type_attrs));
    switch ((((const byte *) &((pref)->tas.type_attrs))[1])) {
    case t_array:
	fprintf(gs_debug_out, "array(%u)0x%lx", size, (ulong) pref->value.refs);
	break;
    case t_astruct:
	goto strct;
    case t_boolean:
	fprintf(gs_debug_out, "boolean %x", pref->value.boolval);
	break;
    case t_device:
	fprintf(gs_debug_out, "device 0x%lx", (ulong) pref->value.pdevice);
	break;
    case t_dictionary:
	fprintf(gs_debug_out, "dict(%u/%u)0x%lx", dict_length(pref), dict_maxlength(pref), (ulong) pref->value.pdict);
	break;
    case t_file:
	fprintf(gs_debug_out, "file 0x%lx", (ulong) pref->value.pfile);
	break;
    case t_fontID:
	goto strct;
    case t_integer:
	fprintf(gs_debug_out, "int %ld", pref->value.intval);
	break;
    case t_mark:
	fprintf(gs_debug_out, "mark");
	break;
    case t_mixedarray:
	fprintf(gs_debug_out, "mixed packedarray(%u)0x%lx", size, (ulong) pref->value.packed);
	break;
    case t_name:
	fprintf(gs_debug_out, "name(0x%lx#%u)", (ulong) pref->value.pname, name_index(pref));
	debug_print_name(pref);
	break;
    case t_null:
	fprintf(gs_debug_out, "null");
	break;
    case t_oparray:
	fprintf(gs_debug_out, "op_array(%u)0x%lx:", size, (ulong) pref->value.const_refs); {
	    const op_array_table *opt = ((size) < op_array_table_local.base_index ? &op_array_table_global : &op_array_table_local);
	    name_index_ref(opt->nx_table[size - opt->base_index], &nref);
	} debug_print_name(&nref);
	break;
    case t_operator:
	fprintf(gs_debug_out, "op(%u", size);
	if (size > 0 && size < op_def_count)
	    fprintf(gs_debug_out, ":%s", (const char *) (op_def_table[size]->oname + 1));
	fprintf(gs_debug_out, ")0x%lx", (ulong) pref->value.opproc);
	break;
    case t_real:
	fprintf(gs_debug_out, "real %f", pref->value.realval);
	break;
    case t_save:
	fprintf(gs_debug_out, "save %lu", pref->value.saveid);
	break;
    case t_shortarray:
	fprintf(gs_debug_out, "short packedarray(%u)0x%lx", size, (ulong) pref->value.packed);
	break;
    case t_string:
	fprintf(gs_debug_out, "string(%u)0x%lx", size, (ulong) pref->value.bytes);
	break;
    case t_struct:
      strct:{
	    obj_header_t *obj = (obj_header_t *) pref->value.pstruct;
	    fprintf(gs_debug_out, "struct %s 0x%lx", (((avm_space) (((pref)->tas.type_attrs) & (((1 << 2) - 1) << 2)) == avm_foreign) ? "-foreign-" : ((const char *) gs_struct_type_name((*(((gs_memory_t *) (gs_imemory.current)))->procs.object_type) (((gs_memory_t *) (gs_imemory.current)), obj)))), (ulong) obj);
	} break;
    default:
	fprintf(gs_debug_out, "type 0x%x", (((const byte *) &((pref)->tas.type_attrs))[1]));
}} static void debug_print_packed_ref(const ref_packed * pref)
{
    ushort elt = *pref & ((1 << 12) - 1);
    ref nref;
    switch (*pref >> 13) {
    case pt_executable_operator:
	fprintf(gs_debug_out, "<op_name>");
	op_index_ref(elt, &nref);
	debug_print_ref(&nref);
	break;
    case pt_integer:
	fprintf(gs_debug_out, "<int> %d", (int) elt + (-(1 << (12 - 1))));
	break;
    case pt_literal_name:
	fprintf(gs_debug_out, "<lit_name>");
	goto ptn;
    case pt_executable_name:
	fprintf(gs_debug_out, "<exec_name>");
      ptn:name_index_ref(elt, &nref);
	fprintf(gs_debug_out, "(0x%lx#%u)", (ulong) nref.value.pname, elt);
	debug_print_name(&nref);
	break;
    default:
	fprintf(gs_debug_out, "<packed_%d?>0x%x", *pref >> 13, elt);
    }
}

void debug_print_ref(const ref * pref)
{
    if ((*(const ref_packed *) (pref) >= ((ref_packed) (2) << 13)))
...
void debug_dump_one_ref(const ref * p)
{
    uint attrs = ((p)->tas.type_attrs);
    uint type = (((const byte *) &((p)->tas.type_attrs))[1]);
    static const attr_print_mask apm[] = { {1, 1, 'm'}, {1, 0, '-'}, {2, 2, 'n'}, {2, 0, '-'}, {((1 << 2) - 1) << 2, avm_foreign, 'F'}, {((1 << 2) - 1) << 2, avm_system, 'S'}, {((1 << 2) - 1) << 2, avm_global, 'G'}, {((1 << 2) - 1) << 2, avm_local, 'L'}, {0x10, 0x10, 'w'}, {0x10, 0, '-'}, {0x20, 0x20, 'r'}, {0x20, 0, '-'}, {0x40, 0x40, 'x'}, {0x40, 0, '-'}, {0x80, 0x80, 'e'}, {0x80, 0, '-'}, {0x4000, 0x4000, '?'}, {0x4000, 0, '-'}, {0x8000, 0x8000, '?'}, {0x8000, 0, '-'}, {0, 0, 0} };
    const attr_print_mask *ap = apm;
    char buf[30 + 1];
    uint plen;
    if (type >= tx_next_index)
	fprintf(gs_debug_out, "0x%02x?? ", type);
    else if (type >= t_next_index)
	fprintf(gs_debug_out, "opr* ");
    else
	fprintf(gs_debug_out, "%s ", type_strings[type]);
    for (; ap->mask; ++ap)
	if ((attrs & ap->mask) == ap->value)
	    fprintf(gs_debug_out, "%c", ap->print);
    fprintf(gs_debug_out, " 0x%04x 0x%08lx", ((p)->tas.rsize), *(const ulong *) &p->value);
    if (obj_cvp(p, (byte *) buf, (sizeof(buf) / sizeof((buf)[0])) - 1, &plen, ((void *) 0), ((bool) 0)) >= 0 && ((buf[plen] = 0), strcmp(buf, "--nostringval--")))
	fprintf(gs_debug_out, " = %s", buf);
    fflush(gs_debug_out);
} void debug_dump_refs(const ref * from, uint size, const char *msg)
{
    const ref *p = from;
    uint count = size;
    if (size && msg)
	fprintf(gs_debug_out, "%s at 0x%lx:\n", msg, (ulong) from);
    while (count--) {
	fprintf(gs_debug_out, "..%04x: 0x%04x ", (uint) (ulong) p & 0xffff, ((p)->tas.type_attrs));
	debug_dump_one_ref(p);
	fprintf(gs_debug_out, "%c", '\n');
	p++;
    }
}
...
void debug_dump_stack(const ref_stack * pstack, const char *msg)
{
	...
	if (m)
	    fprintf(gs_debug_out, "%s at 0x%lx:\n", m, (ulong) pstack), m = ((void *) 0);
	fprintf(gs_debug_out, "0x%lx: 0x%02x ", (ulong) p, (((const byte *) &((p)->tas.type_attrs))[1]));
	debug_dump_one_ref(p);
	fprintf(gs_debug_out, "%c", '\n');
}} void debug_dump_array(const ref * array)
{
    const ref_packed *pp;
    unsigned int type = (((const byte *) &((array)->tas.type_attrs))[1]);
    uint len;
    switch (type) {
    default:
	fprintf(gs_debug_out, "%s at 0x%lx isn't an array.\n", (type < (sizeof(type_strings) / sizeof((type_strings)[0])) ? type_strings[type] : "????"), (ulong) array);
	return;
    case t_oparray:
    ...
    for (len = ((array)->tas.rsize), pp = array->value.packed; len > 0; len--, pp = ((*(const ref_packed *) (pp) >= ((ref_packed) (2) << 13)) ? pp + 1 : pp + (sizeof(ref) / sizeof(ref_packed)))) {
	ref temp;
	packed_get(pp, &temp);
	fprintf(gs_debug_out, "..%04x%c 0x%02x ", (uint) (ulong) pp & 0xffff, (((*(const ref_packed *) (pp) >= ((ref_packed) (2) << 13))) ? '*' : ':'), (((const byte *) &((&temp)->tas.type_attrs))[1]));
