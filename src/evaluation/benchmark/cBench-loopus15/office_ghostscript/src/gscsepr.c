...
extern gs_memory_t *gs_state_memory(const gs_state *);
static int gx_concretize_Separation(const gs_client_color *, const gs_color_space *, frac *, const gs_imager_state *);
static int gx_install_Separation(gs_color_space *, gs_state *);
static void gx_adjust_cspace_Separation(const gs_color_space *, gs_memory_t *, int);
static gs_ptr_type_t gx_enum_ptrs_Separation(void *ptr, uint size, uint index, const void **pep);
static void gx_reloc_ptrs_Separation(void *ptr, uint size, gc_state_t * gcst);
static const gs_color_space *gx_concrete_space_Separation(const gs_color_space *, const gs_imager_state *);
static int gx_remap_concrete_Separation(const frac *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void gx_init_Separation(gs_client_color *, const gs_color_space *);
const gs_color_space_type gs_color_space_type_Separation = { gs_color_space_index_Separation, 1, ((bool) 0), gx_init_Separation, gx_concrete_space_Separation, gx_concretize_Separation, gx_remap_concrete_Separation, gx_default_remap_color, gx_install_Separation, gx_adjust_cspace_Separation, gx_no_adjust_color_count, gx_enum_ptrs_Separation, gx_reloc_ptrs_Separation };

void gs_setoverprint(gs_state * pgs, bool ovp)
{
    pgs->overprint = ovp;
} bool gs_currentoverprint(const gs_state * pgs)
{
    return pgs->overprint;
}

static void gx_init_Separation(gs_client_color * pcc, const gs_color_space * pcs)
{
    pcc->paint.values[0] = 1.0;
} static const gs_color_space *gx_concrete_space_Separation(const gs_color_space * pcs, const gs_imager_state * pis)
{
    const gs_color_space *pacs = (const gs_color_space *) &pcs->params.separation.alt_space;
    return (*(pacs)->type->concrete_space) (pacs, pis);
}

static int gx_concretize_Separation(const gs_client_color * pc, const gs_color_space * pcs, frac * pconc, const gs_imager_state * pis)
{
...
static int gx_remap_concrete_Separation(const frac * pconc, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    return ((-15));
}

static int gx_install_Separation(gs_color_space * pcs, gs_state * pgs)
{
    return (*pcs->params.separation.alt_space.type->install_cspace) ((gs_color_space *) & pcs->params.separation.alt_space, pgs);
}

static void gx_adjust_cspace_Separation(const gs_color_space * pcs, gs_memory_t * mem, int delta)
{
    do {
	if ((pcs->params.separation.map) != 0 && !((pcs->params.separation.map)->rc.ref_count += delta)) {
	    (*(pcs->params.separation.map)->rc.free) ((pcs->params.separation.map)->rc.memory, (void *) (pcs->params.separation.map), "gx_adjust_Separation");
	    do {
	    } while (0);
	}
    } while (0);
    (*pcs->params.separation.alt_space.type->adjust_cspace_count) ((const gs_color_space *) &pcs->params.separation.alt_space, mem, delta);
} static gs_ptr_type_t gx_enum_ptrs_Separation(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
	    return (*((gs_color_space *) vptr)->params.separation.alt_space.type->enum_ptrs) (&((gs_color_space *) vptr)->params.separation.alt_space, sizeof(((gs_color_space *) vptr)->params.separation.alt_space), index - 1, pep);
	}
    case 0:
	do {
	    *pep = (const void *) (((gs_color_space *) vptr)->params.separation.map);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void gx_reloc_ptrs_Separation(void *vptr, uint size, gc_state_t * gcst)
{ {
	((gs_color_space *) vptr)->params.separation.map = gs_reloc_struct_ptr((const void *) ((const gs_color_space *) vptr)->params.separation.map, gcst);
	(*((gs_color_space *) vptr)->params.separation.alt_space.type->reloc_ptrs) (&((gs_color_space *) vptr)->params.separation.alt_space, sizeof(gs_base_color_space), gcst);
}
}
