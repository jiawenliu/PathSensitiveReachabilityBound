...
static gs_ptr_type_t device_color_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void device_color_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
const gs_memory_struct_type_t st_device_color = { sizeof(gx_device_color), "gx_device_color", 0, 0, device_color_enum_ptrs, device_color_reloc_ptrs, 0 };

static gs_ptr_type_t device_color_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
	    gs_ptr_type_t(*proc) (void *ptr, uint size, uint index, const void **pep) = ((gx_device_color *) vptr)->type->enum_ptrs;
	    if (proc == 0)
		return 0;
...
static void device_color_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	void (*proc) (void *ptr, uint size, gc_state_t * gcst) = ((gx_device_color *) vptr)->type->reloc_ptrs;
	if (proc != 0)
	    (*proc) (vptr, size, gcst);
}
}

gx_color_index gx_proc_map_rgb_color(gx_device * dev, gx_color_value vr, gx_color_value vg, gx_color_value vb)
{
    gx_color_index cindex = (*((dev)->std_procs.map_rgb_color)) (dev, vr, vg, vb);
    do {
    } while (0);
    return cindex;
}

gx_color_index gx_proc_map_rgb_alpha_color(gx_device * dev, gx_color_value vr, gx_color_value vg, gx_color_value vb, gx_color_value va)
{
    gx_color_index cindex = (*((dev)->std_procs.map_rgb_alpha_color)) (dev, vr, vg, vb, va);
    do {
    } while (0);
    return cindex;
}

gx_color_index gx_proc_map_cmyk_color(gx_device * dev, gx_color_value vc, gx_color_value vm, gx_color_value vy, gx_color_value vk)
{
    gx_color_index cindex = (*((dev)->std_procs.map_cmyk_color)) (dev, vc, vm, vy, vk);
    do {
    } while (0);
    return cindex;
}
...
static void cmap_gray_to_rgb_halftoned(frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_gray_to_rgb_direct(frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_gray_to_cmyk_halftoned(frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_gray_to_cmyk_direct(frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
...
static void cmap_rgb_to_gray_halftoned(frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_rgb_to_gray_direct(frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_rgb_to_cmyk(frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_cmyk_direct(frac, frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_cmyk_to_gray(frac, frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void cmap_cmyk_to_rgb(frac, frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static const gx_color_map_procs cmap_gray_few = { cmap_gray_halftoned, cmap_rgb_to_gray_halftoned, cmap_cmyk_to_gray }, cmap_gray_many = {
cmap_gray_direct, cmap_rgb_to_gray_direct, cmap_cmyk_to_gray}, cmap_rgb_few = {
cmap_gray_to_rgb_halftoned, cmap_rgb_halftoned, cmap_cmyk_to_rgb}, cmap_rgb_many = {
cmap_gray_to_rgb_direct, cmap_rgb_direct, cmap_cmyk_to_rgb}, cmap_cmyk_few = {
cmap_gray_to_cmyk_halftoned, cmap_rgb_to_cmyk, cmap_cmyk_direct}, cmap_cmyk_many = {
cmap_gray_to_cmyk_direct, cmap_rgb_to_cmyk, cmap_cmyk_direct};

const gx_color_map_procs *cmap_procs_default = &cmap_gray_many;
static const gx_color_map_procs *cmap_few[] = { 0, &cmap_gray_few, 0, &cmap_rgb_few, &cmap_cmyk_few };
static const gx_color_map_procs *cmap_many[] = { 0, &cmap_gray_many, 0, &cmap_rgb_many, &cmap_cmyk_many };

const gx_color_map_procs *gx_device_cmap_procs(const gx_device * dev)
{
    return ((((dev)->color_info.num_components > 1) ? dev->color_info.max_color : dev->color_info.max_gray) >= 31 ? cmap_many : cmap_few)[dev->color_info.num_components];
}

void gx_set_cmap_procs(gs_imager_state * pis, const gx_device * dev)
{
    pis->cmap_procs = gx_device_cmap_procs(dev);
} int gx_remap_color(gs_state * pgs)
{
    const gs_color_space *pcs = pgs->color_space;
    return (*pcs->type->remap_color) (pgs->ccolor, pcs, pgs->dev_color, (gs_imager_state *) pgs, pgs->device, gs_color_select_texture);
}

const gs_color_space *gx_no_concrete_space(const gs_color_space * pcs, const gs_imager_state * pis)
{
    return ((void *) 0);
} const gs_color_space *gx_same_concrete_space(const gs_color_space * pcs, const gs_imager_state * pis)
{
    return pcs;
}

int gx_no_concretize_color(const gs_client_color * pcc, const gs_color_space * pcs, frac * pconc, const gs_imager_state * pis)
{
    return ((-15));
}

int gx_default_remap_color(const gs_client_color * pcc, const gs_color_space * pcs, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac conc[4];
...
int gx_concretize_DeviceGray(const gs_client_color * pc, const gs_color_space * pcs, frac * pconc, const gs_imager_state * pis)
{
    float ftemp;
    pconc[0] = (ftemp = (pc->paint.values[0]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    return 0;
}

int gx_remap_concrete_DGray(const frac * pconc, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    (*pis->cmap_procs->map_gray) (pconc[0], pdc, pis, dev, select);
    return 0;
}

int gx_remap_DeviceGray(const gs_client_color * pc, const gs_color_space * pcs, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    float ftemp;
    (*pis->cmap_procs->map_gray) ((ftemp = (pc->paint.values[0]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), pdc, pis, dev, select);
    return 0;
}

int gx_concretize_DeviceRGB(const gs_client_color * pc, const gs_color_space * pcs, frac * pconc, const gs_imager_state * pis)
{
    float ftemp;
    pconc[0] = (ftemp = (pc->paint.values[0]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    pconc[1] = (ftemp = (pc->paint.values[1]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    pconc[2] = (ftemp = (pc->paint.values[2]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    return 0;
}

int gx_remap_concrete_DRGB(const frac * pconc, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    (*pis->cmap_procs->map_rgb) (pconc[0], pconc[1], pconc[2], pdc, pis, dev, select);
    return 0;
}

int gx_remap_DeviceRGB(const gs_client_color * pc, const gs_color_space * pcs, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    float ft0, ft1, ft2;
    (*pis->cmap_procs->map_rgb) ((ft0 = (pc->paint.values[0]), (((ft0) < 0.0) ? ((frac) 0) : ((ft0) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ft0) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), (ft1 = (pc->paint.values[1]), (((ft1) < 0.0) ? ((frac) 0) : ((ft1) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ft1) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), (ft2 = (pc->paint.values[2]), (((ft2) < 0.0) ? ((frac) 0) : ((ft2) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ft2) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), pdc, pis, dev, select);
    return 0;
}

int gx_concretize_DeviceCMYK(const gs_client_color * pc, const gs_color_space * pcs, frac * pconc, const gs_imager_state * pis)
{
    float ftemp;
    pconc[0] = (ftemp = (pc->paint.values[0]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    pconc[1] = (ftemp = (pc->paint.values[1]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    pconc[2] = (ftemp = (pc->paint.values[2]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    pconc[3] = (ftemp = (pc->paint.values[3]), (((ftemp) < 0.0) ? ((frac) 0) : ((ftemp) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ftemp) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8))))));
    return 0;
}

int gx_remap_concrete_DCMYK(const frac * pconc, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    (*pis->cmap_procs->map_cmyk) (pconc[0], pconc[1], pconc[2], pconc[3], pdc, pis, dev, select);
    return 0;
}

int gx_remap_DeviceCMYK(const gs_client_color * pc, const gs_color_space * pcs, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    float ft0, ft1, ft2, ft3;
    (*pis->cmap_procs->map_cmyk) ((ft0 = (pc->paint.values[0]), (((ft0) < 0.0) ? ((frac) 0) : ((ft0) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ft0) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), (ft1 = (pc->paint.values[1]), (((ft1) < 0.0) ? ((frac) 0) : ((ft1) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ft1) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), (ft2 = (pc->paint.values[2]), (((ft2) < 0.0) ? ((frac) 0) : ((ft2) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ft2) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), (ft3 = (pc->paint.values[3]), (((ft3) < 0.0) ? ((frac) 0) : ((ft3) >= 1.0) ? ((frac) 0x7ff8) : ((frac) (((ft3) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))))), pdc, pis, dev, select);
    return 0;
}

static void cmap_gray_halftoned(frac gray, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    if (gx_render_device_gray(gx_color_frac_map(gray, &pis->effective_transfer.colored.gray->values[0]), pis->alpha, pdc, dev, pis->dev_ht, &pis->screen_phase[select]) == 1)
	(*(pdc)->type->load) (pdc, pis, dev, select);
}

static void cmap_gray_direct(frac gray, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac mgray = gx_color_frac_map(gray, &pis->effective_transfer.colored.gray->values[0]);
    gx_color_value cv_gray = ((ushort) (((mgray) << (((1 << 1) * 8) - 15)) + ((mgray) >> (15 * 2 - ((1 << 1) * 8) - 3))));
    gx_color_index color = (pis->alpha == ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) ? (*((dev)->std_procs.map_rgb_color)) (dev, cv_gray, cv_gray, cv_gray) : (*((dev)->std_procs.map_rgb_alpha_color)) (dev, cv_gray, cv_gray, cv_gray, pis->alpha));
    if (color == ((unsigned long) (-1))) {
	if (gx_render_device_gray(mgray, pis->alpha, pdc, dev, pis->dev_ht, &pis->screen_phase[select]) == 1)
	    (*(pdc)->type->load) (pdc, pis, dev, select);
	return;
    }
    ((pdc)->colors.pure = (color), (pdc)->type = (&gx_dc_procs_pure));
}

static void cmap_gray_to_rgb_halftoned(frac gray, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    cmap_rgb_halftoned(gray, gray, gray, pdc, pis, dev, select);
} static void cmap_gray_to_rgb_direct(frac gray, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    cmap_rgb_direct(gray, gray, gray, pdc, pis, dev, select);
} static void cmap_gray_to_cmyk_halftoned(frac gray, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac mgray = gx_color_frac_map(gray, &pis->effective_transfer.colored.gray->values[0]);
    if (gx_render_device_gray(mgray, pis->alpha, pdc, dev, pis->dev_ht, &pis->screen_phase[select]) == 1)
	(*(pdc)->type->load) (pdc, pis, dev, select);
...
static void cmap_gray_to_cmyk_direct(frac gray, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac mgray = gx_color_frac_map(gray, &pis->effective_transfer.colored.gray->values[0]);
    frac mblack = ((frac) 0x7ff8) - mgray;
    gx_color_index color = (*((dev)->std_procs.map_cmyk_color)) (dev, ((ushort) (((((frac) 0)) << (((1 << 1) * 8) - 15)) + ((((frac) 0)) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((((frac) 0)) << (((1 << 1) * 8) - 15)) + ((((frac) 0)) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((((frac) 0)) << (((1 << 1) * 8) - 15)) + ((((frac) 0)) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((mblack) << (((1 << 1) * 8) - 15)) + ((mblack) >> (15 * 2 - ((1 << 1) * 8) - 3)))));
    if (color != ((unsigned long) (-1))) {
	((pdc)->colors.pure = (color), (pdc)->type = (&gx_dc_procs_pure));
	return;
    }
...
static void cmap_rgb_halftoned(frac r, frac g, frac b, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac mred = gx_color_frac_map(r, &pis->effective_transfer.colored.red->values[0]);
    frac mgreen = gx_color_frac_map(g, &pis->effective_transfer.colored.green->values[0]);
    frac mblue = gx_color_frac_map(b, &pis->effective_transfer.colored.blue->values[0]);
    if ((mred == mgreen && mred == mblue ? gx_render_device_gray(mred, pis->alpha, pdc, dev, pis->dev_ht, &pis->screen_phase[select]) : gx_render_device_color(mred, mgreen, mblue, ((frac) 0), ((bool) 0), pis->alpha, pdc, dev, pis->dev_ht, &pis->screen_phase[select])) == 1)
	(*(pdc)->type->load) (pdc, pis, dev, select);
}

static void cmap_rgb_direct(frac r, frac g, frac b, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac mred = gx_color_frac_map(r, &pis->effective_transfer.colored.red->values[0]);
    frac mgreen = gx_color_frac_map(g, &pis->effective_transfer.colored.green->values[0]);
    frac mblue = gx_color_frac_map(b, &pis->effective_transfer.colored.blue->values[0]);
    gx_color_index color = (pis->alpha == ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) ? (*((dev)->std_procs.map_rgb_color)) (dev, ((ushort) (((mred) << (((1 << 1) * 8) - 15)) + ((mred) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((mgreen) << (((1 << 1) * 8) - 15)) + ((mgreen) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((mblue) << (((1 << 1) * 8) - 15)) + ((mblue) >> (15 * 2 - ((1 << 1) * 8) - 3))))) : (*((dev)->std_procs.map_rgb_alpha_color)) (dev, ((ushort) (((mred) << (((1 << 1) * 8) - 15)) + ((mred) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((mgreen) << (((1 << 1) * 8) - 15)) + ((mgreen) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((mblue) << (((1 << 1) * 8) - 15)) + ((mblue) >> (15 * 2 - ((1 << 1) * 8) - 3)))), pis->alpha));
    if (color != ((unsigned long) (-1))) {
	((pdc)->colors.pure = (color), (pdc)->type = (&gx_dc_procs_pure));
	return;
    }
...
static void cmap_rgb_to_gray_halftoned(frac r, frac g, frac b, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    cmap_gray_halftoned(color_rgb_to_gray(r, g, b, pis), pdc, pis, dev, select);
} static void cmap_rgb_to_gray_direct(frac r, frac g, frac b, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    cmap_gray_direct(color_rgb_to_gray(r, g, b, pis), pdc, pis, dev, select);
} static void cmap_rgb_to_cmyk(frac r, frac g, frac b, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac cmyk[4];
    color_rgb_to_cmyk(r, g, b, pis, cmyk);
    (*pis->cmap_procs->map_cmyk) (cmyk[0], cmyk[1], cmyk[2], cmyk[3], pdc, pis, dev, select);
} static void cmap_cmyk_to_gray(frac c, frac m, frac y, frac k, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    (*pis->cmap_procs->map_gray) (color_cmyk_to_gray(c, m, y, k, pis), pdc, pis, dev, select);
} static void cmap_cmyk_direct(frac c, frac m, frac y, frac k, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac mcyan = ((frac) 0x7ff8) - gx_color_frac_map(((frac) 0x7ff8) - c, &pis->effective_transfer.colored.red->values[0]);
    frac mmagenta = ((frac) 0x7ff8) - gx_color_frac_map(((frac) 0x7ff8) - m, &pis->effective_transfer.colored.green->values[0]);
    frac myellow = ((frac) 0x7ff8) - gx_color_frac_map(((frac) 0x7ff8) - y, &pis->effective_transfer.colored.blue->values[0]);
    frac mblack = ((frac) 0x7ff8) - gx_color_frac_map(((frac) 0x7ff8) - k, &pis->effective_transfer.colored.gray->values[0]);
    if (dev->color_info.max_color >= 31) {
	gx_color_index color = (*((dev)->std_procs.map_cmyk_color)) (dev, ((ushort) (((mcyan) << (((1 << 1) * 8) - 15)) + ((mcyan) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((mmagenta) << (((1 << 1) * 8) - 15)) + ((mmagenta) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((myellow) << (((1 << 1) * 8) - 15)) + ((myellow) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((mblack) << (((1 << 1) * 8) - 15)) + ((mblack) >> (15 * 2 - ((1 << 1) * 8) - 3)))));
	if (color != ((unsigned long) (-1))) {
	    ((pdc)->colors.pure = (color), (pdc)->type = (&gx_dc_procs_pure));
	    return;
	}
    }
    if (gx_render_device_color(mcyan, mmagenta, myellow, mblack, ((bool) 1), pis->alpha, pdc, dev, pis->dev_ht, &pis->screen_phase[select]) == 1)
	(*(pdc)->type->load) (pdc, pis, dev, select);
}

static void cmap_cmyk_to_rgb(frac c, frac m, frac y, frac k, gx_device_color * pdc, const gs_imager_state * pis, gx_device * dev, gs_color_select_t select)
{
    frac rgb[3];
    color_cmyk_to_rgb(c, m, y, k, pis, rgb);
    (*pis->cmap_procs->map_rgb) (rgb[0], rgb[1], rgb[2], pdc, pis, dev, select);
} float gs_mapped_transfer(floatp value, const gx_transfer_map * pmap)
{
    return ((pmap)->values[(int) ((value) * (1 << 8) + 0.5)] / ((float) ((frac) 0x7ff8)));
} frac gx_color_frac_map(frac cv, const frac * values)
{
...
gx_color_index gx_default_w_b_map_rgb_color(gx_device * dev, gx_color_value r, gx_color_value g, gx_color_value b)
{
    return ((r | g | b) > ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) / 2 ? (gx_color_index) 1 : (gx_color_index) 0);
}

int gx_default_w_b_map_color_rgb(gx_device * dev, gx_color_index color, gx_color_value prgb[3])
{
    prgb[0] = prgb[1] = prgb[2] = -(gx_color_value) color;
    return 0;
}

gx_color_index gx_default_b_w_map_rgb_color(gx_device * dev, gx_color_value r, gx_color_value g, gx_color_value b)
{
    return ((r | g | b) > ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) / 2 ? (gx_color_index) 0 : (gx_color_index) 1);
}

int gx_default_b_w_map_color_rgb(gx_device * dev, gx_color_index color, gx_color_value prgb[3])
{
    prgb[0] = prgb[1] = prgb[2] = -((gx_color_value) color ^ 1);
    return 0;
}

gx_color_index gx_default_gray_map_rgb_color(gx_device * dev, gx_color_value r, gx_color_value g, gx_color_value b)
{
    gx_color_value gray = (((r * (ulong) 30) + (g * (ulong) 59) + (b * (ulong) 11) + ((30 + 59 + 11) / 2)) / (30 + 59 + 11) * dev->color_info.max_gray + (((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) / 2)) / ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1));
    return gray;
}

int gx_default_gray_map_color_rgb(gx_device * dev, gx_color_index color, gx_color_value prgb[3])
{
    gx_color_value gray = color * ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) / dev->color_info.max_gray;
    prgb[0] = gray;
    prgb[1] = gray;
    prgb[2] = gray;
    return 0;
...
gx_color_index gx_default_rgb_map_rgb_color(gx_device * dev, gx_color_value r, gx_color_value g, gx_color_value b)
{
    if (dev->color_info.depth == 24)
	return ((b) >> ((sizeof(gx_color_value) * 8) - 8)) + ((uint) ((g) >> ((sizeof(gx_color_value) * 8) - 8)) << 8) + ((ulong) ((r) >> ((sizeof(gx_color_value) * 8) - 8)) << 16);
    else {
	uint bits_per_color = dev->color_info.depth / 3;
	ulong max_value = (1 << bits_per_color) - 1;
	return ((r * max_value / ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1))) << (bits_per_color * 2)) + ((g * max_value / ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1))) << (bits_per_color)) + (b * max_value / ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)));
    }
}

int gx_default_rgb_map_color_rgb(gx_device * dev, gx_color_index color, gx_color_value prgb[3])
{
    if (dev->color_info.depth == 24) {
	...
	prgb[0] = ((color >> (bits_per_color * 2)) & color_mask) * (ulong) ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) / color_mask;
	prgb[1] = ((color >> (bits_per_color)) & color_mask) * (ulong) ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) / color_mask;
	prgb[2] = (color & color_mask) * (ulong) ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) / color_mask;
    }
    return 0;
}

gx_color_index gx_default_map_cmyk_color(gx_device * dev, gx_color_value c, gx_color_value m, gx_color_value y, gx_color_value k)
{
    frac rgb[3];
    color_cmyk_to_rgb(((frac) (((c) >> (((1 << 1) * 8) - 15)) - ((c) >> (((1 << 1) * 8) - 3)))), ((frac) (((m) >> (((1 << 1) * 8) - 15)) - ((m) >> (((1 << 1) * 8) - 3)))), ((frac) (((y) >> (((1 << 1) * 8) - 15)) - ((y) >> (((1 << 1) * 8) - 3)))), ((frac) (((k) >> (((1 << 1) * 8) - 15)) - ((k) >> (((1 << 1) * 8) - 3)))), ((void *) 0), rgb);
    return (*((dev)->std_procs.map_rgb_color)) (dev, ((ushort) (((rgb[0]) << (((1 << 1) * 8) - 15)) + ((rgb[0]) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((rgb[1]) << (((1 << 1) * 8) - 15)) + ((rgb[1]) >> (15 * 2 - ((1 << 1) * 8) - 3)))), ((ushort) (((rgb[2]) << (((1 << 1) * 8) - 15)) + ((rgb[2]) >> (15 * 2 - ((1 << 1) * 8) - 3)))));
}

gx_color_index gx_default_cmyk_map_cmyk_color(gx_device * dev, gx_color_value c, gx_color_value m, gx_color_value y, gx_color_value k)
{
    gx_color_index color = (((k) >> ((sizeof(gx_color_value) * 8) - 8)) + ((uint) ((y) >> ((sizeof(gx_color_value) * 8) - 8)) << 8)) + ((ulong) (((m) >> ((sizeof(gx_color_value) * 8) - 8)) + ((uint) ((c) >> ((sizeof(gx_color_value) * 8) - 8)) << 8)) << 16);
    return (color == ((unsigned long) (-1)) ? color ^ 1 : color);
} gx_color_index gx_default_map_rgb_alpha_color(gx_device * dev, gx_color_value r, gx_color_value g, gx_color_value b, gx_color_value alpha)
{
    gx_color_value white = ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1)) - alpha;
    if (white == 0)
	return (*((dev)->std_procs.map_rgb_color)) (dev, r, g, b);
    return (*((dev)->std_procs.map_rgb_color)) (dev, (gx_color_value) ((ulong) r * alpha / ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1))) + white, (gx_color_value) ((ulong) g * alpha / ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1))) + white, (gx_color_value) ((ulong) b * alpha / ((gx_color_value) ((1L << (sizeof(gx_color_value) * 8)) - 1))) + white);
}
