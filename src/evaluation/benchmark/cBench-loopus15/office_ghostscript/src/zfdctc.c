...
static int quant_params(const ref *, int, UINT16 *, floatp);
int zfdct_byte_params(const ref *, int, int, UINT8 *);
int zfdct_setup_quantization_tables(const ref * op, stream_DCT_state * pdct, bool is_encode)
{
	...
	if (((pa)->tas.rsize) != 64)
	    return ((-15));
	code = quant_params(pa, 64, this_table->quantval, pdct->QFactor);
	if (code < 0)
	    return code;
    }
    return 0;
...
int zfdct_setup_huffman_tables(const ref * op, stream_DCT_state * pdct, bool is_encode)
{
    ...
    ref *pdval;
    const ref *pa;
    const ref *DCArrays[4];
    const ref *ACArrays[4];
    int num_in_tables;
    int ndc, nac;
    int codes_size;
    jpeg_component_info *comp_info;
	...
	if (codes_size > 256 || ((pa)->tas.rsize) != codes_size + 16)
	    return ((-15));
...
int zfdct_setup(const ref * op, stream_DCT_state * pdct)
{
    ...
    if ((code = dict_int_param(dop, "Picky", 0, 1, 0, &pdct->data.common->Picky)) < 0 || (code = dict_int_param(dop, "Relax", 0, 1, 0, &pdct->data.common->Relax)) < 0 || (code = dict_int_param(dop, "ColorTransform", -1, 2, -1, &pdct->ColorTransform)) < 0 || (code = dict_float_param(dop, "QFactor", 1.0, &pdct->QFactor)) < 0)
	return code;
...
static int quant_params(const ref * op, int count, UINT16 * pvals, floatp QFactor)
{
    int i;
    const ref_packed *pref;
    double val;
    static const int natural_order[64] = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44, 51, 58, 59, 52, 45, 38, 31, 39, 46, 53, 60, 61, 54, 47, 55, 62, 63 };
	...
	for (i = 0; i < count; i++) {
	    ...
	    if (val > 255)
		val = 255;
	    pvals[natural_order[i]] = (UINT16) (val + 0.5);
	}
	return 0;
    case t_array:
	if (!(((&*op)->tas.type_attrs) & (0x20)))
    ...
    for (i = 0; i < count; pref = ((*(const ref_packed *) (pref) >= ((ref_packed) (2) << 13)) ? pref + 1 : pref + (sizeof(ref) / sizeof(ref_packed))), i++) {
	ref nref;
	packed_get(pref, &nref);
	switch ((((const byte *) &((&nref)->tas.type_attrs))[1])) {
	case t_integer:
...
int zfdct_byte_params(const ref * op, int start, int count, UINT8 * pvals)
{
	...
	pref = op->value.packed;
	for (i = 0; i < start; i++)
	    pref = ((*(const ref_packed *) (pref) >= ((ref_packed) (2) << 13)) ? pref + 1 : pref + (sizeof(ref) / sizeof(ref_packed)));
	break;
    default:
	return ((-20));
    }
    for (i = 0, pval = pvals; i < count; pref = ((*(const ref_packed *) (pref) >= ((ref_packed) (2) << 13)) ? pref + 1 : pref + (sizeof(ref) / sizeof(ref_packed))), i++, pval++) {
	ref nref;
	packed_get(pref, &nref);
	switch ((((const byte *) &((&nref)->tas.type_attrs))[1])) {
	case t_integer:
