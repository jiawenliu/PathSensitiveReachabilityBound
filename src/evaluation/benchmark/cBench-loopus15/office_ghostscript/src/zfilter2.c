...
int filter_write_predictor(os_ptr op, int npop, const stream_template * template, stream_state * st)
{
	...
	op = (o_stack.p);
	(*(&rfd) = *(op));
	code = (predictor == 2 ? filter_read(op, 0, &s_PDiffE_template, (stream_state *) & pds, 0) : filter_read(op, 0, &s_PNGPE_template, (stream_state *) & pps, 0));
	if (code < 0) {
	    (o_stack.p) = ++op;
	    (*(op - 1) = *(&rtarget));
	    (*(op) = *(&rdict));
...
static int zLZWE(os_ptr op)
{
    stream_LZW_state lzs;
    int code = zlz_setup(op, &lzs);
    if (code < 0)
	return code;
    return filter_write_predictor(op, code, &s_LZWE_template, (stream_state *) & lzs);
}

const op_def *zfilter2_op_defs(void)
{
    static const op_def op_defs_[] = { {"filterdict", 0}, {"2CCITTFaxEncode", zCFE}, {"1LZWEncode", zLZWE}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
