...
static int indexed_map1(os_ptr);
static void rc_free_indexed_map(gs_memory_t * mem, void *data, client_name_t cname)
{
    char *cdata = (char *) data;
    (*(mem)->procs.free_object) (mem, ((gs_indexed_map *) cdata)->values, cname);
    (*(mem)->procs.free_object) (mem, cdata, cname);
} static int lookup_indexed(const gs_indexed_params * params, int index, float *values)
{
...
static int zsetindexedspace(register os_ptr op)
{
    ref *pproc = &((int_gstate *) gs_state_client_data(igs))->colorspace.procs.special.index_proc;
    const ref *pcsa;
    gs_color_space cs;
    ...
    if (!((((const byte *) &((&pcsa[1])->tas.type_attrs))[1]) == (t_integer)))
	return ((-20));
    ...
    if (((((const byte *) &((&pcsa[2])->tas.type_attrs))[1]) == (t_string))) {
	int num_values = num_entries * cs.type->num_components;
	if (!(((&pcsa[2])->tas.type_attrs) & (0x20)))
	    return ((-7));
	if (((&pcsa[2])->tas.rsize) != num_values)
	    return ((-15));
	...
	code = zcs_begin_map(&map, &pcsa[2], num_entries, (const gs_base_color_space *) &cs, indexed_map1);
	if (code < 0)
	    return code;
...
static int indexed_map1(os_ptr op)
{
    es_ptr ep = (e_stack.p);
    ...
    ((ep + 1)->value.opproc = (indexed_map1), ((ep + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), ((ep + 1)->tas.rsize = (0)));
    ep[2] = ep[(-2)];
    (e_stack.p) = ep + 2;
    return 5;
}

const op_def *zcsindex_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"1.setindexedspace", zsetindexedspace}, {"1%indexed_map1", indexed_map1}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
...
int zcs_begin_map(gs_indexed_map ** pmap, const ref * pproc, int num_entries, const gs_base_color_space * base_space, int (*map1) (os_ptr))
{
    ...
    float *values;
    do {
	if (((map) = (gs_indexed_map *) (*(mem)->procs.alloc_struct) (mem, &st_indexed_map, "setcolorspace(mapped)")) == 0) {
	    return ((-25));
	} else {
	    (map)->rc.ref_count = 0;
	    (map)->rc.memory = mem;
	    (map)->rc.free = rc_free_struct_only;
	}
    } while (0);
    values = (float *) (*(mem)->procs.alloc_byte_array) (mem, num_values, sizeof(float), "setcolorspace(mapped)");
    if (values == 0) {
	(*(mem)->procs.free_object) (mem, map, "setcolorspace(mapped)");
	return ((-25));
    }
    ...
    map->values = values;
    *pmap = map;
    if ((e_stack.p) > (e_stack.top) - (5 + 1)) {
	int es_code_ = ref_stack_extend(&e_stack, 5 + 1);
	if (es_code_ < 0)
	    return es_code_;
    };
    ep = (e_stack.p) += 5;
    ((ep + (-4))->value.intval = (num_components), ((ep + (-4))->tas.type_attrs = ((t_integer) << 8) + (0)));
    ((ep + (-3))->value.pstruct = ((obj_header_t *) (map)), ((ep + (-3))->tas.type_attrs = ((t_struct) << 8) + (imemory_space((gs_ref_memory_t *) mem))));
    ep[(-2)] = *pproc;
    ((ep + (-1))->value.intval = (num_entries - 1), ((ep + (-1))->tas.type_attrs = ((t_integer) << 8) + (0)));
    ((ep + 0)->value.intval = (-1), ((ep + 0)->tas.type_attrs = ((t_integer) << 8) + (0)));
    (++(e_stack.p), (((e_stack.p))->value.opproc = (map1), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    return 5;
}
