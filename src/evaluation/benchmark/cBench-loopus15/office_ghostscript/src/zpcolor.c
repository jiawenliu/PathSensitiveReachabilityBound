...
static int pattern_paint_prepare(os_ptr);
static int pattern_paint_finish(os_ptr);
typedef struct int_pattern_s {
    ref dict;
} int_pattern;
static const gs_memory_struct_type_t st_int_pattern = { sizeof(int_pattern), "int_pattern", 0, ref_struct_clear_marks, ref_struct_enum_ptrs, ref_struct_reloc_ptrs, 0 };

static void zpcolor_init(void)
{
    gstate_set_pattern_cache(igs, gx_pattern_alloc_cache(((gs_memory_t *) (gs_imemory.spaces.named.system)), gx_pat_cache_default_tiles(), gx_pat_cache_default_bits()));
    zcolor2_st_pattern_instance_p = &st_pattern_instance;
} static int zbuildpattern(os_ptr op)
{
    os_ptr op1 = op - 1;
    int code;
    gs_matrix mat;
	...
	return ((-7));
    if ((code = read_matrix(op, &mat)) < 0 || (code = dict_uid_param(op1, &template.uid, 1, ((gs_memory_t *) (gs_imemory.current)))) != 1 || (code = dict_int_param(op1, "PatternType", 1, 1, 0, &PatternType)) < 0 || (code = dict_int_param(op1, "PaintType", 1, 2, 0, &template.PaintType)) < 0 || (code = dict_int_param(op1, "TilingType", 1, 3, 0, &template.TilingType)) < 0 || (code = dict_float_array_param(op1, "BBox", 4, BBox, ((void *) 0))) != 4 || (code = dict_float_param(op1, "XStep", 0.0, &template.XStep)) != 0 || (code = dict_float_param(op1, "YStep", 0.0, &template.YStep)) != 0 || (code = dict_find_string(op1, "PaintProc", &pPaintProc)) <= 0)
	return ((code < 0 ? code : (-15)));
    if (!(((&*pPaintProc)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&*pPaintProc));;
    ...
    pdata = (int_pattern *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_int_pattern, "int_pattern");
    if (pdata == 0)
	return ((-25));
    template.client_data = pdata;
    pdata->dict = *op1;
    code = gs_makepattern(&cc_instance, &template, &mat, igs, ((gs_memory_t *) (gs_imemory.current)));
    if (code < 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), pdata, "int_pattern");
	return code;
    }
    ((op)->value.pstruct = ((obj_header_t *) (cc_instance.pattern)), ((op)->tas.type_attrs = ((t_struct) << 8) + (((0x20 + 0x40)) | (((&gs_imemory))->current_space))));
    return code;
}

static int zsetpatternspace(register os_ptr op)
{
    gs_color_space cs;
    ...
    if (cs.type->num_components < 0)
	return ((-15));
    switch (((op)->tas.rsize)) {
    case 1:
	cs.params.pattern.has_base_space = ((bool) 0);
	break;
    default:
	return ((-15));
    case 2:{
...
const op_def *zpcolor_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"2.buildpattern", zbuildpattern}, {"1.setpatternspace", zsetpatternspace}, {"0%pattern_paint_prepare", pattern_paint_prepare}, {"0%pattern_paint_finish", pattern_paint_finish}, {(char *) 0, (op_proc_p) zpcolor_init} };
    return op_defs_;
}

static int pattern_paint_cleanup(os_ptr);
static int zPaintProc(const gs_client_color * pcc, gs_state * pgs)
{
    ...
    (e_stack.p)++;
    (++(e_stack.p), (((e_stack.p))->value.opproc = (pattern_paint_prepare), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    return (-103);
}

static int pattern_paint_prepare(os_ptr op)
{
    gs_state *pgs = igs;
    ...
    pdev = gx_pattern_accum_alloc(((gs_memory_t *) (gs_imemory.current)), "pattern_paint_prepare");
    if (pdev == 0)
	return ((-25));
    pdev->instance = pinst;
    pdev->bitmap_memory = gstate_pattern_cache(pgs)->memory;
    code = (*((pdev)->std_procs.open_device)) ((gx_device *) pdev);
    if (code < 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), pdev, "pattern_paint_prepare");
	return code;
    }
    code = gs_gsave(pgs);
    if (code < 0)
    ...
    (++(e_stack.p), (((e_stack.p))->value.opproc = (pattern_paint_cleanup), (((e_stack.p))->tas.type_attrs = ((t_null) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    ++(e_stack.p);
    (((e_stack.p))->value.pstruct = ((obj_header_t *) (pdev)), (((e_stack.p))->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&gs_imemory))->current_space))));
    (++(e_stack.p), (((e_stack.p))->value.opproc = (pattern_paint_finish), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
...
static int pattern_paint_finish(os_ptr op)
{
    gx_device_pattern_accum *pdev = ((gx_device_pattern_accum *) (((e_stack.p))->value.pstruct));
    gx_color_tile *ctile;
    int code = gx_pattern_cache_add_entry((gs_imager_state *) igs, pdev, &ctile);
    if (code < 0)
	return code;
    (e_stack.p) -= 2;
...
static int pattern_paint_cleanup(os_ptr op)
{
    gx_device_pattern_accum *pdev = ((gx_device_pattern_accum *) (((e_stack.p) + 2)->value.pstruct));
    gs_grestore(igs);
    (*((pdev)->std_procs.close_device)) ((gx_device *) pdev);
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), pdev, "pattern_paint_cleanup");
    return 0;
}
