...
int gs_setcmykcolor(gs_state *, floatp, floatp, floatp, floatp), gs_currentcmykcolor(const gs_state *, float[4]), gs_setblackgeneration(gs_state *, gs_mapping_proc), gs_setblackgeneration_remap(gs_state *, gs_mapping_proc, bool);
gs_mapping_proc gs_currentblackgeneration(const gs_state *);
int gs_setundercolorremoval(gs_state *, gs_mapping_proc), gs_setundercolorremoval_remap(gs_state *, gs_mapping_proc, bool);
gs_mapping_proc gs_currentundercolorremoval(const gs_state *);
int gs_setcolortransfer(gs_state *, gs_mapping_proc, gs_mapping_proc, gs_mapping_proc, gs_mapping_proc), gs_setcolortransfer_remap(gs_state *, gs_mapping_proc, gs_mapping_proc, gs_mapping_proc, gs_mapping_proc, bool);
...
static int zcurrentblackgeneration(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    *op = ((int_gstate *) ((igs)->client_data))->black_generation;
    return 0;
}

static int zcurrentcmykcolor(register os_ptr op)
{
    float par[4];
...
static int zcurrentcolortransfer(register os_ptr op)
{
    do {
	if ((op += (4)) > (o_stack.top)) {
	    o_stack.requested = (4);
    ...
    op[-3] = ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.red;
    op[-2] = ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.green;
    op[-1] = ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.blue;
    *op = ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.gray;
    return 0;
}

static int zcurrentundercolorremoval(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    *op = ((int_gstate *) ((igs)->client_data))->undercolor_removal;
    return 0;
}

static int zsetblackgeneration(register os_ptr op)
{
    int code;
    if (!(((&*op)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&*op));;
    if ((o_stack.top) - (o_stack.p) < (zcolor_remap_one_ostack - 1)) {
	o_stack.requested = (zcolor_remap_one_ostack - 1);
	return ((-16));
    };
    if ((e_stack.p) > (e_stack.top) - (1 + zcolor_remap_one_estack)) {
	int es_code_ = ref_stack_extend(&e_stack, 1 + zcolor_remap_one_estack);
	if (es_code_ < 0)
	    return es_code_;
    };
    code = gs_setblackgeneration_remap(igs, gs_mapped_transfer, ((bool) 0));
    if (code < 0)
	return code;
    ((int_gstate *) ((igs)->client_data))->black_generation = *op;
    ((o_stack.p) -= (1));
    op--;
    (++(e_stack.p), (((e_stack.p))->value.opproc = (zcolor_remap_color), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    return zcolor_remap_one(&((int_gstate *) ((igs)->client_data))->black_generation, op, igs->black_generation, igs, zcolor_remap_one_finish);
}

static int zsetcmykcolor(register os_ptr op)
{
    float par[4];
    int code;
    if ((code = num_params(op, 4, par)) < 0 || (code = gs_setcmykcolor(igs, par[0], par[1], par[2], par[3])) < 0)
	return code;
    (((&((int_gstate *) ((igs)->client_data))->colorspace.array)->tas.type_attrs = ((t_null) << 8) + (0)));
    ((o_stack.p) -= (4));
    return 0;
}

static int zsetcolortransfer(register os_ptr op)
{
    int code;
    if (!(((&op[-3])->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&op[-3]));;
    if (!(((&op[-2])->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&op[-2]));;
    if (!(((&op[-1])->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&op[-1]));;
    if (!(((&*op)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&*op));;
    if ((o_stack.top) - (o_stack.p) < (zcolor_remap_one_ostack * 4 - 4)) {
	o_stack.requested = (zcolor_remap_one_ostack * 4 - 4);
	return ((-16));
    };
    if ((e_stack.p) > (e_stack.top) - (1 + zcolor_remap_one_estack * 4)) {
	int es_code_ = ref_stack_extend(&e_stack, 1 + zcolor_remap_one_estack * 4);
	if (es_code_ < 0)
	    return es_code_;
    };
    ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.red = op[-3];
    ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.green = op[-2];
    ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.blue = op[-1];
    ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.gray = *op;
    if ((code = gs_setcolortransfer_remap(igs, gs_mapped_transfer, gs_mapped_transfer, gs_mapped_transfer, gs_mapped_transfer, ((bool) 0))) < 0)
	return code;
    ((o_stack.p) -= (4));
    op -= 4;
    (++(e_stack.p), (((e_stack.p))->value.opproc = (zcolor_reset_transfer), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    if ((code = zcolor_remap_one(&((int_gstate *) ((igs)->client_data))->transfer_procs.colored.red, (o_stack.p), igs->set_transfer.colored.red, igs, zcolor_remap_one_finish)) < 0 || (code = zcolor_remap_one(&((int_gstate *) ((igs)->client_data))->transfer_procs.colored.green, (o_stack.p), igs->set_transfer.colored.green, igs, zcolor_remap_one_finish)) < 0 || (code = zcolor_remap_one(&((int_gstate *) ((igs)->client_data))->transfer_procs.colored.blue, (o_stack.p), igs->set_transfer.colored.blue, igs, zcolor_remap_one_finish)) < 0)
	return code;
    return zcolor_remap_one(&((int_gstate *) ((igs)->client_data))->transfer_procs.colored.gray, (o_stack.p), igs->set_transfer.colored.gray, igs, zcolor_remap_one_finish);
}

static int zsetundercolorremoval(register os_ptr op)
{
    int code;
    if (!(((&*op)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&*op));;
    ...
    code = gs_setundercolorremoval_remap(igs, gs_mapped_transfer, ((bool) 0));
    if (code < 0)
	return code;
    ((int_gstate *) ((igs)->client_data))->undercolor_removal = *op;
    ((o_stack.p) -= (1));
    op--;
    (++(e_stack.p), (((e_stack.p))->value.opproc = (zcolor_remap_color), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    return zcolor_remap_one(&((int_gstate *) ((igs)->client_data))->undercolor_removal, op, igs->undercolor_removal, igs, zcolor_remap_one_signed_finish);
}

static int zcolorimage(register os_ptr op)
{
    int spp;
    int npop = 7;
    os_ptr procp = op - 2;
	...
	pcs = gs_color_space_DeviceCMYK();
      color:if (op[-1].value.boolval) {
	    npop += spp - 1;
	    procp -= spp - 1, multi = ((bool) 1);
	}
	break;
    default:
	return ((-15));
...
const op_def *zcolor1_op_defs(void)
{
    static const op_def op_defs_[] = { {"0currentblackgeneration", zcurrentblackgeneration}, {"0currentcmykcolor", zcurrentcmykcolor}, {"0currentcolortransfer", zcurrentcolortransfer}, {"0currentundercolorremoval", zcurrentundercolorremoval}, {"1setblackgeneration", zsetblackgeneration}, {"4setcmykcolor", zsetcmykcolor}, {"4setcolortransfer", zsetcolortransfer}, {"1setundercolorremoval", zsetundercolorremoval}, {"7colorimage", zcolorimage}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
