...
{
    exit(exit_status);
} const char *gp_strerror(int errnum)
{
    return ((void *) 0);
} void gp_get_realtime(long *pdt)
{
    struct timeval tp; {
	struct timezone tzp;
	if (gettimeofday(&tp, &tzp) == -1) {
	    (lprintf_file_and_line(gs_stderr, "gp_unix.c", 93), fprintf(gs_stderr, "Ghostscript: gettimeofday failed!\n"));
	    gs_exit(1);
	}
    }
    pdt[0] = tp.tv_sec;
...
FILE *gp_open_printer(char *fname, int binary_mode)
{
    return (strlen(fname) == 0 ? gp_open_scratch_file(gp_scratch_file_name_prefix, fname, "w") : fname[0] == '|' ? popen(fname + 1, "w") : fopen(fname, "w"));
}
