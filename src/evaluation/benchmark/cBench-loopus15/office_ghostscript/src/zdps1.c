...
static int gstate_unshare(os_ptr);
const gs_memory_struct_type_t st_igstate_obj = { sizeof(igstate_obj), "gstatetype", 0, ref_struct_clear_marks, ref_struct_enum_ptrs, ref_struct_reloc_ptrs, 0 };

static void zdps1_init(void)
{
    if (zcopy_procs[t_astruct] == zcopy_procs[t_struct])
	zcopy_procs[t_astruct] = zcopy_gstate;
}

static int zsetstrokeadjust(register os_ptr op)
{
...
static int zcurrentstrokeadjust(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.boolval = (gs_currentstrokeadjust(igs)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    return 0;
}

static int gstate_check_space(int_gstate * isp, uint space)
{ {
	register ref *rp_ = (ref *) (isp);
	register int i = sizeof(int_gstate) / sizeof(ref);
	do {
	    if ((avm_space) (((rp_)->tas.type_attrs) & (((1 << 2) - 1) << 2)) > (space))
		return ((-7));
	    ++rp_;
	} while (--i);
};
...
int zgstate(register os_ptr op)
{
    int code = gstate_check_space(((int_gstate *) gs_state_client_data(igs)), (((&gs_imemory))->current_space));
    igstate_obj *pigo;
    gs_state *pnew;
    int_gstate *isp;
    if (code < 0)
	return code;
    pigo = (igstate_obj *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_igstate_obj, "gstate");
    if (pigo == 0)
	return ((-25));
    pnew = gs_state_copy(igs, ((gs_memory_t *) (gs_imemory.current)));
    if (pnew == 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), pigo, "gstate");
	return ((-25));
    }
	...
	register int i = sizeof(int_gstate) / sizeof(ref);
	do {
	    ((rp_)->tas.type_attrs |= ((&gs_imemory)->new_mask));
	    ++rp_;
	} while (--i);
    };
    do {
    ...
    ((op)->value.pstruct = ((obj_header_t *) (pigo)), ((op)->tas.type_attrs = ((t_astruct) << 8) + (((0x10 + 0x20 + 0x40)) | (((&gs_imemory))->current_space))));
    (((&pigo->gstate)->tas.type_attrs = ((t_null) << 8) + (0)));
    ((void) ((((((&pigo->gstate)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), op, (ref_packed *) (&pigo->gstate), "gstate") : 0)));
    ((&pigo->gstate)->value.pstruct = ((obj_header_t *) (pnew)), ((&pigo->gstate)->tas.type_attrs = ((t_struct) << 8) + (((0) | (((&gs_imemory))->current_space)) | ((&gs_imemory)->new_mask))));
    return 0;
}

int zcopy_gstate(register os_ptr op)
{
    os_ptr op1 = op - 1;
    gs_state *pgs;
    gs_state *pgs1;
    ...
    gs_memory_t *mem;
    int code;
    if (!((((&*op)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *) (gs_imemory.current)))->procs.object_type) (((gs_memory_t *) (gs_imemory.current)), (&*op)->value.pstruct) == &st_igstate_obj))
	return (check_type_failed(&*op));
    if (!((((&*op1)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *) (gs_imemory.current)))->procs.object_type) (((gs_memory_t *) (gs_imemory.current)), (&*op1)->value.pstruct) == &st_igstate_obj))
    ...
    code = gstate_check_space(((int_gstate *) gs_state_client_data(pgs1)), (avm_space) (((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)));
    if (code < 0)
	return code; {
	register ref *rp_ = (ref *) (pistate);
	register int i = sizeof(int_gstate) / sizeof(ref);
	do {
	    ((void) ((((((rp_)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), op, (ref_packed *) (rp_), "copygstate") : 0)));
	    ++rp_;
	} while (--i);
	};
	mem = gs_state_swap_memory(pgs, ((gs_memory_t *) (gs_imemory.current)));
...
int zcurrentgstate(register os_ptr op)
{
    gs_state *pgs;
    int_gstate *pistate;
    int code;
    ...
    pgs = ((gs_state *) ((&((igstate_obj *) ((op)->value.pstruct))->gstate)->value.pstruct));
    pistate = ((int_gstate *) gs_state_client_data(pgs));
    code = gstate_check_space(((int_gstate *) gs_state_client_data(igs)), (avm_space) (((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)));
    if (code < 0)
	return code; {
	register ref *rp_ = (ref *) (pistate);
	register int i = sizeof(int_gstate) / sizeof(ref);
	do {
	    ((void) ((((((rp_)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), op, (ref_packed *) (rp_), "currentgstate") : 0)));
	    ++rp_;
	} while (--i);
	};
	mem = gs_state_swap_memory(pgs, ((gs_memory_t *) (gs_imemory.current)));
...
int zsetgstate(register os_ptr op)
{
    int code;
    if (!((((&*op)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *) (gs_imemory.current)))->procs.object_type) (((gs_memory_t *) (gs_imemory.current)), (&*op)->value.pstruct) == &st_igstate_obj))
	return (check_type_failed(&*op));
	...
	return ((-7));
    code = gs_setgstate(igs, ((gs_state *) ((&((igstate_obj *) ((op)->value.pstruct))->gstate)->value.pstruct)));
    if (code < 0)
	return code;
    ((o_stack.p) -= (1));
    return 0;
...
static int rect_get(local_rects *, os_ptr);
static void rect_release(local_rects *);
static int zrectappend(os_ptr op)
{
    local_rects lr;
...
static int zrectclip(os_ptr op)
{
    local_rects lr;
    int npop = rect_get(&lr, op);
    int code;
...
static int zrectfill(os_ptr op)
{
    local_rects lr;
    int npop = rect_get(&lr, op);
    int code;
...
static int zrectstroke(os_ptr op)
{
    gs_matrix mat;
    local_rects lr;
    int npop, code;
...
static int rect_get(local_rects * plr, os_ptr op)
{
    int format, code;
    uint n, count;
    gs_rect *pr;
	...
	pr = (gs_rect *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_byte_array) (((gs_memory_t *) (gs_imemory.current)), count, sizeof(gs_rect), "rect_get");
	if (pr == 0)
	    return ((-25));
    }
    plr->pr = pr;
	...
	int i;
	for (i = 0; i < 4; i++) {
	    code = num_array_get((const ref *) op, format, (n << 2) + i, &rnum);
	    switch (code) {
	    case t_integer:
...
static void rect_release(local_rects * plr)
{
    if (plr->pr != plr->rl)
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), plr->pr, "rect_release");
}

int zsetbbox(register os_ptr op)
{
    float box[4];
...
const op_def *zdps1_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"0currentstrokeadjust", zcurrentstrokeadjust}, {"1setstrokeadjust", zsetstrokeadjust}, {"1currentgstate", zcurrentgstate}, {"0gstate", zgstate}, {"1setgstate", zsetgstate}, {"1.rectappend", zrectappend}, {"1rectclip", zrectclip}, {"1rectfill", zrectfill}, {"1rectstroke", zrectstroke}, {"4setbbox", zsetbbox}, {(char *) 0, (op_proc_p) zdps1_init} };
    return op_defs_;
}

static int gstate_unshare(os_ptr op)
{
    ref *pgsref = &((igstate_obj *) ((op)->value.pstruct))->gstate;
    gs_state *pgs = ((gs_state *) ((pgsref)->value.pstruct));
    gs_state *pnew;
    int_gstate *isp;
    if (!((((pgsref)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0))
	return 0;
    pnew = gs_gstate(pgs);
    if (pnew == 0)
	return ((-25));
    ...
    alloc_save_change((&gs_imemory), op, (ref_packed *) (pgsref), "gstate_unshare");
    ((pgsref)->value.pstruct = ((obj_header_t *) (pnew)), ((pgsref)->tas.type_attrs = ((t_struct) << 8) + (((0) | (((&gs_imemory))->current_space)) | ((&gs_imemory)->new_mask))));
    return 0;
}
