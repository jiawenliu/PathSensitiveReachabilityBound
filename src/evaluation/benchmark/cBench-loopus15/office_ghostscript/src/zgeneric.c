...
extern int zcopy_dict(os_ptr);
static int zcopy_invalid(os_ptr);
static int zcopy_integer(os_ptr);
static int zcopy_interval(os_ptr);
static int copy_interval(os_ptr, uint, os_ptr, client_name_t);
op_proc_p zcopy_procs[t_next_index];
static void zgeneric_init(void)
{
    int i;
...
static int zcopy(register os_ptr op)
{
...
static int zcopy_invalid(os_ptr op)
{
    return (check_type_failed(op));
}

static int zcopy_integer(register os_ptr op)
{
    os_ptr op1 = op - 1;
    int count, i;
    int code;
    if ((ulong) op->value.intval > op - (o_stack.bot)) {
	if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	    return (check_type_failed(&*op));
	if ((ulong) (*op).value.intval >= (ref_stack_count(&o_stack)))
	    return ((-15));
	count = op->value.intval;
    } else if (op1 + (count = op->value.intval) <= (o_stack.top)) {
	memcpy((char *) op, (char *) (op - count), count * sizeof(ref));
	do {
	    if ((op += (count - 1)) > (o_stack.top)) {
		o_stack.requested = (count - 1);
		return ((-16));
	    } else
		(o_stack.p) = op;
	} while (0);
	...
	return code;
    for (i = 0; i < count; i++)
	*ref_stack_index(&o_stack, i) = *ref_stack_index(&o_stack, i + count);
    return 0;
}

static int zcopy_interval(register os_ptr op)
{
    os_ptr op1 = op - 1;
...
static int zlength(register os_ptr op)
{
...
static int zget(register os_ptr op)
{
    os_ptr op1 = op - 1;
    ref *pvalue;
	...
	break;
    case t_string:
	if (!(((&*op1)->tas.type_attrs) & (0x20)))
	    return ((-7));
	if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	    return (check_type_failed(&*op));
	if ((ulong) (*op).value.intval >= (((op1)->tas.rsize)))
	    return ((-15));
...
static int zput(register os_ptr op)
{
    os_ptr op1 = op - 1;
    os_ptr op2 = op1 - 1;
    switch ((((const byte *) &((op2)->tas.type_attrs))[1])) {
    case t_dictionary:
	if (!(((&*(&(&*op2)->value.pdict->values))->tas.type_attrs) & (0x10)))
	    return ((-7)); {
	    ...
	    break;
    case t_array:
	    if (!(((&*op2)->tas.type_attrs) & (0x10)))
		return ((-7));
	    if (!((((const byte *) &((&*op1)->tas.type_attrs))[1]) == (t_integer)))
		return (check_type_failed(&*op1));
	if ((ulong) (*op1).value.intval >= (((op2)->tas.rsize)))
	    return ((-15));
	if ((avm_space) (((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)) > ((avm_space) (((op2)->tas.type_attrs) & (((1 << 2) - 1) << 2))))
	    return ((-7)); {
	    ref *eltp = op2->value.refs + (uint) op1->value.intval;
	    (((void) ((((((eltp)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), op2, (ref_packed *) (eltp), "put") : 0))), ((void) (((*(eltp) = *(op)), ((eltp)->tas.type_attrs |= ((&gs_imemory)->new_mask))))));
	    } break;
    case t_mixedarray:
    case t_shortarray:
	    return ((-7));
...
static int zgetinterval(register os_ptr op)
{
    os_ptr op1 = op - 1;
    os_ptr op2 = op1 - 1;
    uint index;
    ...
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
    if ((ulong) (*op).value.intval > (((op2)->tas.rsize) - index))
	return ((-15));
    count = op->value.intval;
    switch ((((const byte *) &((op2)->tas.type_attrs))[1])) {
    case t_array:
...
static int zputinterval(register os_ptr op)
{
    os_ptr opindex = op - 1;
    ...
    if (!(((&*opto)->tas.type_attrs) & (0x10)))
	return ((-7));
    if (!((((const byte *) &((&*opindex)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*opindex));
    if ((ulong) (*opindex).value.intval > (((opto)->tas.rsize)))
	return ((-15));
...
static int array_continue(os_ptr), dict_continue(os_ptr), string_continue(os_ptr), packedarray_continue(os_ptr);
static int forall_cleanup(os_ptr);
static int zforall(register os_ptr op)
{
    os_ptr obj = op - 1;
    register es_ptr ep = (e_stack.p);
    uint index = 0;
    if ((e_stack.p) > (e_stack.top) - (6)) {
	int es_code_ = ref_stack_extend(&e_stack, 6);
	if (es_code_ < 0)
	    return es_code_;
    };
    ...
    case t_array:
	if (!(((&*obj)->tas.type_attrs) & (0x20)))
	    return ((-7));
	(((ep + 5))->value.opproc = (array_continue), (((ep + 5))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((ep + 5))->tas.rsize = (0)));
	break;
    case t_dictionary:
	if (!(((&*(&(&*obj)->value.pdict->values))->tas.type_attrs) & (0x20)))
	    return ((-7));
	(((ep + 5))->value.opproc = (dict_continue), (((ep + 5))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((ep + 5))->tas.rsize = (0)));
	index = dict_first(obj);
	break;
    case t_string:
	if (!(((&*obj)->tas.type_attrs) & (0x20)))
	    return ((-7));
	(((ep + 5))->value.opproc = (string_continue), (((ep + 5))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((ep + 5))->tas.rsize = (0)));
	break;
    case t_mixedarray:
    case t_shortarray:
	if (!(((&*obj)->tas.type_attrs) & (0x20)))
	    return ((-7));
	(((ep + 5))->value.opproc = (packedarray_continue), (((ep + 5))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((ep + 5))->tas.rsize = (0)));
	break;
    }
    ...
    ep[2] = *obj;
    ((ep + 3)->value.intval = (index), ((ep + 3)->tas.type_attrs = ((t_integer) << 8) + (0)));
    ep[4] = *op;
    (e_stack.p) += 4;
    ((o_stack.p) -= (2));
    op -= 2;
    return (*(((ep + 5))->value.opproc)) (op);
}

static int array_continue(register os_ptr op)
{
    es_ptr obj = (e_stack.p) - 2;
    if (((obj)->tas.rsize)) {
	do {
	    if ((op += (1)) > (o_stack.top)) {
		o_stack.requested = (1);
...
static int dict_continue(register os_ptr op)
{
    es_ptr obj = (e_stack.p) - 2;
    int index = (int) (e_stack.p)[-1].value.intval;
    do {
	if ((op += (2)) > (o_stack.top)) {
	    o_stack.requested = (2);
	    return ((-16));
	...
	(e_stack.p)[-1].value.intval = index;
	(e_stack.p) += 2;
	*(e_stack.p) = obj[2];
	return 5;
    } else {
...
static int string_continue(register os_ptr op)
{
    es_ptr obj = (e_stack.p) - 2;
    if (((obj)->tas.rsize)) {
	((obj)->tas.rsize -= (1));
...
static int packedarray_continue(register os_ptr op)
{
    es_ptr obj = (e_stack.p) - 2;
    if (((obj)->tas.rsize)) {
...
static int forall_cleanup(os_ptr op)
{
    return 0;
}

const op_def *zgeneric_op_defs(void)
{
    static const op_def op_defs_[] = { {"1copy", zcopy}, {"2forall", zforall}, {"2get", zget}, {"3getinterval", zgetinterval}, {"1length", zlength}, {"3put", zput}, {"3putinterval", zputinterval}, {"0%array_continue", array_continue}, {"0%dict_continue", dict_continue}, {"0%packedarray_continue", packedarray_continue}, {"0%string_continue", string_continue}, {(char *) 0, (op_proc_p) zgeneric_init} };
    return op_defs_;
}

static int copy_interval(os_ptr prto, uint index, os_ptr prfrom, client_name_t cname)
{
    int fromtype = (((const byte *) &((prfrom)->tas.type_attrs))[1]);
    uint fromsize = ((prfrom)->tas.rsize);
    if (!(fromtype == (((const byte *) &((prto)->tas.type_attrs))[1]) || ((fromtype == t_shortarray || fromtype == t_mixedarray) && (((const byte *) &((prto)->tas.type_attrs))[1]) == t_array)))
	return (check_type_failed(prfrom));
    if (!(((&*prfrom)->tas.type_attrs) & (0x20)))
	return ((-7));
    if (!(((&*prto)->tas.type_attrs) & (0x10)))
	return ((-7));
    if (fromsize > ((prto)->tas.rsize) - index)
	return ((-15));
    switch (fromtype) {
    case t_array:{
	    return refcpy_to_old(prto, index, prfrom->value.refs, fromsize, cname);
	}
    case t_string:{
    ...
    case t_shortarray:{
		...
		(((void) ((((((pdest)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), prto, (ref_packed *) (pdest), cname) : 0))), ((void) (((*(pdest) = *(&elt)), ((pdest)->tas.type_attrs |= ((&gs_imemory)->new_mask))))));
		packed = ((*(const ref_packed *) (packed) >= ((ref_packed) (2) << 13)) ? packed + 1 : packed + (sizeof(ref) / sizeof(ref_packed)));
	    }
	}
	break;
