...
static gs_ptr_type_t path_enum_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (((gs_path_enum *) vptr)->pseg);
	    return (&ptr_struct_procs);
	} while (0);
    case 1:
	do {
	    *pep = (const void *) (((gs_path_enum *) vptr)->pgs);
	    return (&ptr_struct_procs);
	} while (0);
    case 2:
	do {
	    *pep = (const void *) (((gs_path_enum *) vptr)->path);
	    return (&ptr_struct_procs);
	} while (0);
    case 3:
	do {
	    *pep = (const void *) (((gs_path_enum *) vptr)->copied_path);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void path_enum_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((gs_path_enum *) vptr)->pseg = gs_reloc_struct_ptr((const void *) ((const gs_path_enum *) vptr)->pseg, gcst);
    ((gs_path_enum *) vptr)->pgs = gs_reloc_struct_ptr((const void *) ((const gs_path_enum *) vptr)->pgs, gcst);
    ((gs_path_enum *) vptr)->path = gs_reloc_struct_ptr((const void *) ((const gs_path_enum *) vptr)->path, gcst);
    ((gs_path_enum *) vptr)->copied_path = gs_reloc_struct_ptr((const void *) ((const gs_path_enum *) vptr)->copied_path, gcst);
} static const gs_memory_struct_type_t st_gs_path_enum = { sizeof(gs_path_enum), "gs_path_enum", 0, 0, path_enum_enum_ptrs, path_enum_reloc_ptrs, 0 };

void gx_path_assign(gx_path * pto, const gx_path * pfrom)
{
    *pto = *pfrom;
} int gx_path_current_point(const gx_path * ppath, gs_fixed_point * ppt)
{
...
int gx_path_bbox(gx_path * ppath, gs_fixed_rect * pbox)
{
	...
	*pbox = ppath->bbox;
	...
	while (pseg) {
	    switch (pseg->type) {
	    case s_curve:
		if ((((curve_segment *) pseg)->p1).x < box.p.x)
		    box.p.x = (((curve_segment *) pseg)->p1).x;
		else if ((((curve_segment *) pseg)->p1).x > box.q.x)
		    box.q.x = (((curve_segment *) pseg)->p1).x;
		if ((((curve_segment *) pseg)->p1).y < box.p.y)
		    box.p.y = (((curve_segment *) pseg)->p1).y;
		else if ((((curve_segment *) pseg)->p1).y > box.q.y)
		    box.q.y = (((curve_segment *) pseg)->p1).y;
		if ((((curve_segment *) pseg)->p2).x < box.p.x)
		    box.p.x = (((curve_segment *) pseg)->p2).x;
		else if ((((curve_segment *) pseg)->p2).x > box.q.x)
		    box.q.x = (((curve_segment *) pseg)->p2).x;
		if ((((curve_segment *) pseg)->p2).y < box.p.y)
		    box.p.y = (((curve_segment *) pseg)->p2).y;
		else if ((((curve_segment *) pseg)->p2).y > box.q.y)
		    box.q.y = (((curve_segment *) pseg)->p2).y;
	    default:
		if ((pseg->pt).x < box.p.x)
		    box.p.x = (pseg->pt).x;
		else if ((pseg->pt).x > box.q.x)
		    box.q.x = (pseg->pt).x;
		if ((pseg->pt).y < box.p.y)
		    box.p.y = (pseg->pt).y;
		else if ((pseg->pt).y > box.q.y)
		    box.q.y = (pseg->pt).y;
	    }
	    pseg = pseg->next;
	}
...
bool gx_path_has_curves(const gx_path * ppath)
{
    return ppath->curve_count != 0;
}

bool gx_path_is_void(const gx_path * ppath)
{
    return ppath->first_subpath == 0;
}

bool gx_path_is_null(const gx_path * ppath)
{
    return ppath->first_subpath == 0 && !(((ppath)->state_flags & psf_position_valid) != 0);
}

bool gx_subpath_is_rectangle(const subpath * pseg0, gs_fixed_rect * pbox, const subpath ** ppnext)
{
    const segment *pseg1, *pseg2, *pseg3, *pseg4;
    if (pseg0->curve_count == 0 && (pseg1 = pseg0->next) != 0 && (pseg2 = pseg1->next) != 0 && (pseg3 = pseg2->next) != 0 && ((pseg4 = pseg3->next) == 0 || pseg4->type != s_line || (pseg4->pt.x == pseg0->pt.x && pseg4->pt.y == pseg0->pt.y && (pseg4->next == 0 || pseg4->next->type != s_line)))) {
	fixed x0 = pseg0->pt.x, y0 = pseg0->pt.y;
	fixed x2 = pseg2->pt.x, y2 = pseg2->pt.y;
	if ((x0 == pseg1->pt.x && pseg1->pt.y == y2 && x2 == pseg3->pt.x && pseg3->pt.y == y0) || (x0 == pseg3->pt.x && pseg3->pt.y == y2 && x2 == pseg1->pt.x && pseg1->pt.y == y0)) {
	    if (x0 < x2)
...
bool gx_path_is_rectangle(const gx_path * ppath, gs_fixed_rect * pbox)
{
    const subpath *pnext;
    return (ppath->subpath_count == 1 && gx_subpath_is_rectangle(ppath->first_subpath, pbox, &pnext));
}

int gx_path_translate(gx_path * ppath, fixed dx, fixed dy)
{
    segment *pseg;
    if (ppath->box_last != 0) {
    ...
    for (pseg = (segment *) (ppath->first_subpath); pseg != 0; pseg = pseg->next)
	switch (pseg->type) {
	case s_curve:
...
void gx_point_scale_exp2(gs_fixed_point * pt, int sx, int sy)
{
    if (sx >= 0)
	pt->x <<= sx;
    else
...
void gx_rect_scale_exp2(gs_fixed_rect * pr, int sx, int sy)
{
    gx_point_scale_exp2(&pr->p, sx, sy);
    gx_point_scale_exp2(&pr->q, sx, sy);
} int gx_path_scale_exp2(gx_path * ppath, int log2_scale_x, int log2_scale_y)
{
    segment *pseg;
...
int gx_path_copy_reversed(const gx_path * ppath_old, gx_path * ppath, bool init)
{
  ...
  nsp:while (psub) {
	...
	const segment *prev;
	segment_notes prev_notes = (pseg == (const segment *) psub ? sn_none : psub->next->notes);
	segment_notes notes;
	...
	for (;; pseg = prev, prev_notes = notes) {
	    prev = pseg->prev;
	    notes = pseg->notes;
	    prev_notes = (prev_notes & sn_not_first) | (notes & ~sn_not_first);
	    switch (pseg->type) {
	    case s_start:
	      endsp:if (psub->is_closed) {
		    code = gx_path_close_subpath_notes(ppath, prev_notes);
		    if (code < 0)
			goto fx;
		}
	    ...
	    case s_curve:{
		    ...
		    code = gx_path_add_curve_notes(ppath, pc->p2.x, pc->p2.y, pc->p1.x, pc->p1.y, prev->pt.x, prev->pt.y, prev_notes);
		    break;
		}
	    case s_line:
	    case s_line_close:
		...
		code = gx_path_add_line_notes(ppath, prev->pt.x, prev->pt.y, prev_notes);
		break;
	    }
	    if (code < 0)
		goto fx;
	...
	code = gx_path_add_point(ppath, ppath_old->position.x, ppath_old->position.y);
	if (code < 0)
	    goto fx;
    }
    return 0;
...
gs_path_enum *gs_path_enum_alloc(gs_memory_t * mem, client_name_t cname)
{
    return (gs_path_enum *) (*(mem)->procs.alloc_struct) (mem, &st_gs_path_enum, cname);
}

int gx_path_enum_init(gs_path_enum * penum, const gx_path * ppath)
{
    penum->path = ppath;
    penum->copied_path = 0;
    penum->pgs = 0;
...
int gx_path_enum_next(gs_path_enum * penum, gs_fixed_point ppts[3])
{
	...
	return 3;
    default:
	(lprintf_file_and_line(gs_stderr, "gxpath2.c", 391), fprintf(gs_stderr, "bad type %x in gx_path_enum_next!\n", pseg->type));
	return ((-100));
    }
}

segment_notes gx_path_enum_notes(const gs_path_enum * penum)
{
    return penum->notes;
}

bool gx_path_enum_backup(gs_path_enum * penum)
{
    const segment *pseg = penum->pseg;
    if (pseg != 0) {
	if ((pseg = pseg->prev) == 0)
