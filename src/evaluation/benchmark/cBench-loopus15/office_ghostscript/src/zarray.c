...
int zarray(register os_ptr op)
{
    uint size;
    int code;
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
    if ((ulong) (*op).value.intval > ((((unsigned short) 0xffff + (unsigned short) 0) & (((unsigned int) 0xffffffff + (unsigned int) 0) / (uint) sizeof(ref)))))
	return ((-15));
    size = op->value.intval;
    code = gs_alloc_ref_array((gs_imemory.current), (ref *) op, (0x10 + 0x20 + 0x40), size, "array");
    if (code < 0)
	return code;
...
static int zaload(register os_ptr op)
{
    ref aref;
    (*(&aref) = *(op));
    if (!(((&aref)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
	return (check_type_failed(op));
    if (!(((&aref)->tas.type_attrs) & (0x20)))
	return ((-7));
    if (((&aref)->tas.rsize) > (o_stack.top) - op) {
	int code = ref_stack_push(&o_stack, ((&aref)->tas.rsize));
	uint i;
	const ref_packed *packed = aref.value.packed;
	if (code < 0)
	    return code;
	for (i = ((&aref)->tas.rsize); i > 0; i--, packed = ((*(const ref_packed *) (packed) >= ((ref_packed) (2) << 13)) ? packed + 1 : packed + (sizeof(ref) / sizeof(ref_packed))))
	    packed_get(packed, ref_stack_index(&o_stack, i));
	*(o_stack.p) = aref;
	return 0;
    }
    if (((((const byte *) &((&aref)->tas.type_attrs))[1]) == (t_array)))
	memcpy((char *) op, (const char *) aref.value.refs, ((&aref)->tas.rsize) * sizeof(ref));
    else {
	register ushort i;
	const ref_packed *packed = aref.value.packed;
	os_ptr pdest = op;
	for (i = 0; i < ((&aref)->tas.rsize); i++, pdest++)
	    packed_get(packed, pdest), packed = ((*(const ref_packed *) (packed) >= ((ref_packed) (2) << 13)) ? packed + 1 : packed + (sizeof(ref) / sizeof(ref_packed)));
    }
    do {
	if ((op += (((&aref)->tas.rsize))) > (o_stack.top)) {
	    o_stack.requested = (((&aref)->tas.rsize));
	    return ((-16));
	} else
	    (o_stack.p) = op;
    } while (0);
...
static int zastore(register os_ptr op)
{
    uint size;
    int code;
    ...
    size = ((op)->tas.rsize);
    if (size > op - (o_stack.bot)) {
	ref arr;
	if (size >= ref_stack_count(&o_stack))
	    return ((-17));
	arr = *op;
	code = ref_stack_store(&o_stack, &arr, size, 1, 0, ((bool) 1), "astore");
	if (code < 0)
	    return code;
...
const op_def *zarray_op_defs(void)
{
    static const op_def op_defs_[] = { {"1aload", zaload}, {"1array", zarray}, {"1astore", zastore}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
