...
int gs_jpeg_create_decompress(stream_DCT_state * st)
{
    ...
    jpeg_CreateDecompress((&st->data.decompress->dinfo), 61, (size_t) sizeof(struct jpeg_decompress_struct));
    return 0;
}
...
} int gs_jpeg_read_scanlines(stream_DCT_state * st, JSAMPARRAY scanlines, int max_lines)
{
    if (_setjmp(st->data.common->exit_jmpbuf))
	return (gs_jpeg_log_error(st));
    return (int) jpeg_read_scanlines(&st->data.decompress->dinfo, scanlines, (JDIMENSION) max_lines);
} int gs_jpeg_finish_decompress(stream_DCT_state * st)
{
    if (_setjmp(st->data.common->exit_jmpbuf))
	return (gs_jpeg_log_error(st));
