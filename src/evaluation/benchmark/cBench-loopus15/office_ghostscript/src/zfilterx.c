...
static int bhc_setup(os_ptr op, stream_BHC_state * pbhcs)
{
    int code;
    int num_counts;
    int data[16 + 1 + 256 + 100 + 1];
    uint dsize;
    int i;
	...
	return ((-7));
    if ((code = dict_bool_param(op, "FirstBitLowOrder", ((bool) 0), &pbhcs->FirstBitLowOrder)) < 0 || (code = dict_int_param(op, "MaxCodeLength", 1, 16, 16, &num_counts)) < 0 || (code = dict_bool_param(op, "EndOfData", ((bool) 1), &pbhcs->EndOfData)) < 0 || (code = dict_uint_param(op, "EncodeZeroRuns", 2, 256, 256, &pbhcs->EncodeZeroRuns)) < 0 || (code = dict_int_array_param(op, "Tables", (sizeof(data) / sizeof((data)[0])), data)) <= 0)
    ...
    if (dsize != num_counts + 1 + num_values || accum != 1 << (num_counts + 1) || pbhcs->EncodeZeroRuns > (pbhcs->EndOfData ? num_values - 1 : num_values))
	return ((-15));
    ...
    pbhcs->definition.counts = counts = (ushort *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_byte_array) (((gs_memory_t *) (gs_imemory.current)), num_counts + 1, sizeof(ushort), "bhc_setup(counts)");
    pbhcs->definition.values = values = (ushort *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_byte_array) (((gs_memory_t *) (gs_imemory.current)), num_values, sizeof(ushort), "bhc_setup(values)");
    if (counts == 0 || values == 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), values, "bhc_setup(values)");
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), counts, "bhc_setup(counts)");
	return ((-25));
    }
...
static int zcomputecodes(os_ptr op)
{
    os_ptr op1 = op - 1;
    uint asize;
    hc_definition def;
	...
	return (check_type_failed(&*op));
    if (!(((&*op1)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_array) << 8) + (0x10))))
	return ((!((((const byte *) &((&*op1)->tas.type_attrs))[1]) == (t_array)) ? check_type_failed(&*op1) : (-7)));
    asize = ((op1)->tas.rsize);
    if (op->value.intval < 1 || op->value.intval > 16)
	return ((-15));
    def.num_counts = op->value.intval;
    if (asize < def.num_counts + 2)
	return ((-15));
    def.num_values = asize - (def.num_counts + 1);
    data = (ushort *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_byte_array) (((gs_memory_t *) (gs_imemory.current)), asize, sizeof(ushort), "zcomputecodes");
    freqs = (long *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_byte_array) (((gs_memory_t *) (gs_imemory.current)), def.num_values, sizeof(long), "zcomputecodes(freqs)");
    if (data == 0 || freqs == 0)
	code = ((-25));
    else {
	uint i;
	...
	for (i = 0; i < def.num_values; i++) {
	    ...
	    if (!((((const byte *) &((pf)->tas.type_attrs))[1]) == (t_integer))) {
		code = ((-20));
		break;
	    }
	...
	if (!code) {
	    code = hc_compute(&def, freqs, ((gs_memory_t *) (gs_imemory.current)));
	    if (code >= 0) {
		for (i = 0; i < asize; i++)
		    ((op1->value.refs + i)->value.intval = (data[i]), ((op1->value.refs + i)->tas.type_attrs = ((t_integer) << 8) + (0)));
	    }
	}
    }
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), freqs, "zcomputecodes(freqs)");
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), data, "zcomputecodes");
    if (code < 0)
	return code;
    ((o_stack.p) -= (1));
    return code;
...
static int bwbs_setup(os_ptr op, stream_BWBS_state * pbwbss)
{
    int code;
    if ((code = dict_int_param(op, "BlockSize", 1, (~(-1 << ((1 << 2) * 8 - 1))) / sizeof(int) - 10, 16384, &pbwbss->BlockSize)) < 0)
	return code;
    return 0;
}

static int zBWBSE(os_ptr op)
{
...
static int bt_setup(os_ptr op, stream_BT_state * pbts)
{
    int npop = 1;
    if (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary)))
	++npop, --op;
...
static int zBTE(os_ptr op)
{
    stream_BT_state bts;
    int code = bt_setup(op, &bts);
    if (code < 0)
	return code;
    return filter_write(op, code, &s_BT_template, (stream_state *) & bts, 0);
}

static int zBTD(os_ptr op)
{
    stream_BT_state bts;
    int code = bt_setup(op, &bts);
    if (code < 0)
	return code;
    return filter_read(op, code, &s_BT_template, (stream_state *) & bts, 0);
}

static int zMTFE(os_ptr op)
{
    return filter_write_simple(op, &s_MTFE_template);
}

static int zMTFD(os_ptr op)
{
    return filter_read_simple(op, &s_MTFD_template);
}

static int zPCXD(os_ptr op)
{
    return filter_read_simple(op, &s_PCXD_template);
}

const op_def *zfilterx_op_defs(void)
{
    static const op_def op_defs_[] = { {"2.computecodes", zcomputecodes}, {"filterdict", 0}, {"2BoundedHuffmanEncode", zBHCE}, {"2BoundedHuffmanDecode", zBHCD}, {"2BWBlockSortEncode", zBWBSE}, {"2BWBlockSortDecode", zBWBSD}, {"2ByteTranslateEncode", zBTE}, {"2ByteTranslateDecode", zBTD}, {"1MoveToFrontEncode", zMTFE}, {"1MoveToFrontDecode", zMTFD}, {"1PCXDecode", zPCXD}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
