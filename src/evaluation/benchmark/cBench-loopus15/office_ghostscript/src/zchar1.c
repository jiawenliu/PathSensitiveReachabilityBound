...
int zchar_exec_char_proc(os_ptr);
typedef enum { metricsNone = 0, metricsWidthOnly = 1, metricsSideBearingAndWidth = 2 } metrics_present;
int zchar_get_metrics(const gs_font_base * pbfont, const ref * pcnref, float psbw[4]);
int zchar_set_cache(os_ptr op, const gs_font_base * pbfont, const ref * pcnref, const float psb[2], const float pwidth[2], const gs_rect * pbbox, int (*cont_fill) (os_ptr), int (*cont_stroke) (os_ptr));
struct dict_s {
    ref values;
    ref keys;
    ref count;
...
static int type1addpath_continue(os_ptr);
static int type1_call_OtherSubr(gs_type1_state *, int (*)(os_ptr), const ref *);
static int type1_continue_dispatch(gs_type1_state *, const ref *, ref *);
static int op_type1_cleanup(os_ptr);
static void op_type1_free(os_ptr);
static void type1_cis_get_metrics(const gs_type1_state * pcis, float psbw[4]);
static int type1getsbw_continue(os_ptr);
static int bbox_fill(os_ptr);
static int bbox_stroke(os_ptr);
static int nobbox_continue(os_ptr);
static int nobbox_fill(os_ptr);
static int nobbox_stroke(os_ptr);
static int ztype1execchar(register os_ptr op)
{
    gs_font *pfont;
    ...
    if (code < 0)
	return code;
    if (penum == 0 || !((pfont)->FontType == ft_encrypted || (pfont)->FontType == ft_disk_based))
	return ((-21));
    pdata = &((gs_font_type1 *) pfont)->data;
    if (pfont->PaintType)
    ...
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_string)))
	return (check_type_failed(&*op));
    if (((op)->tas.rsize) <= (((pdata->lenIV) > (0)) ? (pdata->lenIV) : (0)))
	return ((-10));
    if ((o_stack.top) - (o_stack.p) < (3)) {
	o_stack.requested = (3);
	return ((-16));
    };
    present = zchar_get_metrics(((gs_font_base *) pfont), op - 1, sbw);
    if (present < 0)
	return present;
    code = gs_moveto(igs, 0.0, 0.0);
    if (code < 0)
	return code;
    code = (gs_type1_interp_init((&cis), (gs_imager_state *) ((penum)->pgs), (penum)->pgs->path, &(penum)->log2_current_scale, gs_show_in_charpath(penum) != cpm_show, ((gs_font_type1 *) pfont)->PaintType, ((gs_font_type1 *) pfont)) | ((((void *) 0)) == 0 ? 0 : (gs_type1_set_lsb((&cis), ((void *) 0)), 0)));
    if (code < 0)
	return code;
    if (((gs_font_type1 *) pfont)->FontBBox.q.x > ((gs_font_type1 *) pfont)->FontBBox.p.x && ((gs_font_type1 *) pfont)->FontBBox.q.y > ((gs_font_type1 *) pfont)->FontBBox.p.y) {
	if (present == metricsNone) {
	    ref cnref;
	    (*(&cnref) = *(op - 1));
	    code = type1_continue_dispatch((&cis), op, &other_subr);
	    switch (code) {
	    default:
		return ((code < 0 ? code : ((-10))));
	    case 2:
		return type1_call_OtherSubr((&cis), type1getsbw_continue, &other_subr);
	    case 1:
		break;
	    }
	    type1_cis_get_metrics((&cis), sbw);
	    return zchar_set_cache((o_stack.p), ((gs_font_base *) pfont), &cnref, ((void *) 0), sbw + 2, &((gs_font_type1 *) pfont)->FontBBox, bbox_fill, bbox_stroke);
	} else {
	    return zchar_set_cache(op, ((gs_font_base *) pfont), op - 1, (present == metricsSideBearingAndWidth ? sbw : ((void *) 0)), sbw + 2, &((gs_font_type1 *) pfont)->FontBBox, bbox_fill, bbox_stroke);
    }} else {
	const ref *opstr = op;
	if (present == metricsSideBearingAndWidth) {
	    gs_point sbpt;
	    sbpt.x = sbw[0], sbpt.y = sbw[1];
	    gs_type1_set_lsb((&cis), &sbpt);
	}
      icont:code = type1_continue_dispatch((&cis), opstr, &other_subr);
	switch (code) {
	case 0:
	    break;
	default:
	    return code;
	case 2:
	    (++(e_stack.p), (((e_stack.p))->value.opproc = (nobbox_continue), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
	    return type1_call_OtherSubr((&cis), type1addpath_continue, &other_subr);
	case 1:
	    opstr = 0;
	    goto icont;
	}
...
static int type1_continue_dispatch(gs_type1_state * pcis, const ref * pcref, ref * pos)
{
    int value;
    int code;
	...
	charstring.size = ((pcref)->tas.rsize);
	pchars = &charstring;
    }
...
static int type1_call_OtherSubr(gs_type1_state * pcis, int (*cont) (os_ptr), const ref * pos)
{
    gs_type1_state *hpcis = (gs_type1_state *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_gs_type1_state, ".type1addpath");
    if (hpcis == 0)
	return ((-25));
    *hpcis = *pcis;
    (++(e_stack.p), (((e_stack.p))->value.opproc = (op_type1_cleanup), (((e_stack.p))->tas.type_attrs = ((t_null) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (1))));
    ++(e_stack.p);
    (((e_stack.p))->value.pstruct = ((obj_header_t *) (hpcis)), (((e_stack.p))->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&gs_imemory))->current_space))));
    (++(e_stack.p), (((e_stack.p))->value.opproc = (cont), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    ++(e_stack.p);
    *(e_stack.p) = *pos;
    return 5;
}

static int type1getsbw_continue(os_ptr op)
{
    ref other_subr;
    gs_type1_state *pcis = ((gs_type1_state *) (((e_stack.p))->value.pstruct));
    int code;
    if ((o_stack.top) - (o_stack.p) < (3)) {
	o_stack.requested = (3);
	return ((-16));
    };
    code = type1_continue_dispatch(pcis, ((void *) 0), &other_subr);
    op = (o_stack.p);
    switch (code) {
    default:
	op_type1_free(op);
	return ((code < 0 ? code : ((-10))));
    case 2:
	(++(e_stack.p), (((e_stack.p))->value.opproc = (type1getsbw_continue), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
	++(e_stack.p);
	*(e_stack.p) = other_subr;
	return 5;
    case 1:{
	    float sbw[4];
	    const gs_font_base *pbfont = (const gs_font_base *) pcis->pfont;
	    type1_cis_get_metrics(pcis, sbw);
	    op_type1_free(op);
	    return zchar_set_cache(op, pbfont, op, sbw, sbw + 2, &pbfont->FontBBox, bbox_fill, bbox_stroke);
	}
    }
}

static int op_type1_cleanup(os_ptr op)
{
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), ((void *) (((e_stack.p) + 2)->value.pstruct)), "op_type1_cleanup");
    return 0;
}

static void op_type1_free(os_ptr op)
{
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), ((void *) (((e_stack.p))->value.pstruct)), "op_type1_free");
    (((e_stack.p) - 1)->value.const_refs = ((const ref *) ((void *) 0)), (((e_stack.p) - 1)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) + 0x80)), (((e_stack.p) - 1)->tas.rsize = (0)));
    (((e_stack.p))->value.const_refs = ((const ref *) ((void *) 0)), (((e_stack.p))->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) + 0x80)), (((e_stack.p))->tas.rsize = (0)));
} static int bbox_finish(os_ptr, int (*)(os_ptr));

static int bbox_fill(os_ptr op)
{
    return bbox_finish(op, nobbox_fill);
}

static int bbox_stroke(os_ptr op)
{
    return bbox_finish(op, nobbox_stroke);
}

static int bbox_finish(os_ptr op, int (*cont) (os_ptr))
{
    gs_font *pfont;
    gs_type1_data *pdata;
    int code;
	...
	return ((-21));
    pdata = &((gs_font_type1 *) pfont)->data;
    if (((opc)->tas.rsize) <= (((pdata->lenIV) > (0)) ? (pdata->lenIV) : (0)))
	return ((-10));
    ...
    code = (gs_type1_interp_init((&cis), (gs_imager_state *) ((penum)->pgs), (penum)->pgs->path, &(penum)->log2_current_scale, gs_show_in_charpath(penum) != cpm_show, ((gs_font_type1 *) pfont)->PaintType, ((gs_font_type1 *) pfont)) | ((psbpt) == 0 ? 0 : (gs_type1_set_lsb((&cis), psbpt), 0)));
    if (code < 0)
	return code;
    opstr = opc;
  icont:code = type1_continue_dispatch((&cis), opstr, &other_subr);
    switch (code) {
    case 0:
	((o_stack.p) -= ((psbpt == 0 ? 1 : 3)));
	return (*cont) ((o_stack.p));
    case 2:
	(++(e_stack.p), (((e_stack.p))->value.opproc = (cont), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
	return type1_call_OtherSubr((&cis), type1addpath_continue, &other_subr);
    case 1:
...
static int type1addpath_continue(os_ptr op)
{
    ref other_subr;
    gs_type1_state *pcis = ((gs_type1_state *) (((e_stack.p))->value.pstruct));
    int code;
  cont:code = type1_continue_dispatch(pcis, ((void *) 0), &other_subr);
    op = (o_stack.p);
    switch (code) {
    case 0:{
	    int npop = (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_string)) ? 1 : 3);
	    ((o_stack.p) -= (npop));
	    op -= npop;
	    op_type1_free(op);
	    return 0;
	...
	return ((code < 0 ? code : ((-10))));
    case 2:
	(++(e_stack.p), (((e_stack.p))->value.opproc = (type1addpath_continue), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
	++(e_stack.p);
	*(e_stack.p) = other_subr;
	return 5;
    case 1:
...
static int nobbox_continue(os_ptr op)
{
    int code;
    gs_rect bbox;
    gs_font *pfont;
    float sbw[4];
    if ((code = gs_upathbbox(igs, &bbox, ((bool) 0))) < 0 || (code = font_param(op - 2, &pfont)) < 0 || (code = zchar_get_metrics(((gs_font_base *) pfont), op, sbw)) < 0)
	return code;
    if (code == metricsNone) {
	gs_point endpt;
    ...
    return zchar_set_cache(op, ((gs_font_base *) pfont), op, ((void *) 0), sbw + 2, &bbox, nobbox_fill, nobbox_stroke);
} static int nobbox_fill(os_ptr op)
{
    ((o_stack.p) -= (3));
    return gs_fill(igs);
}

static int nobbox_stroke(os_ptr op)
{
    ((o_stack.p) -= (3));
...
static void type1_cis_get_metrics(const gs_type1_state * pcis, float psbw[4])
{
    psbw[0] = ((pcis->lsb.x) * (1.0 / (1 << 12)));
    psbw[1] = ((pcis->lsb.y) * (1.0 / (1 << 12)));
    psbw[2] = ((pcis->width.x) * (1.0 / (1 << 12)));
    psbw[3] = ((pcis->width.y) * (1.0 / (1 << 12)));
} const op_def *zchar1_op_defs(void)
{
    static const op_def op_defs_[] = { {"4.type1execchar", ztype1execchar}, {"0%type1addpath_continue", type1addpath_continue}, {"3%nobbox_fill", nobbox_fill}, {"3%nobbox_stroke", nobbox_stroke}, {"0%type1getsbw_continue", type1getsbw_continue}, {"4%bbox_fill", bbox_fill}, {"4%bbox_stroke", bbox_stroke}, {"3%nobbox_continue", nobbox_continue}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}

int z1_subr_proc(gs_font_type1 * pfont, int index, bool global, gs_const_string * pstr)
{
    const font_data *pfdata = ((font_data *) ((pfont)->client_data));
    ref subr;
    int code;
    code = array_get((global ? &pfdata->u.type1.GlobalSubrs : &pfdata->u.type1.Subrs), index, &subr);
    if (code < 0)
	return code;
    if (!((((const byte *) &((&subr)->tas.type_attrs))[1]) == (t_string)))
	return ((-20));
    pstr->data = subr.value.const_bytes;
    pstr->size = ((&subr)->tas.rsize);
    return 0;
}

int z1_seac_proc(gs_font_type1 * pfont, int index, gs_const_string * pstr)
{
    const font_data *pfdata = ((font_data *) ((pfont)->client_data));
    ref *pcstr;
    ref enc_entry;
    int code = array_get(&(registered_Encodings.value.refs[0]), (long) index, &enc_entry);
    if (code < 0)
	return code;
    ...
    pstr->size = ((pcstr)->tas.rsize);
    return 0;
}

int z1_push_proc(gs_font_type1 * ignore, const fixed * pf, int count)
{
    const fixed *p = pf + count - 1;
    int i;
    if ((o_stack.top) - (o_stack.p) < (count)) {
	o_stack.requested = (count);
	return ((-16));
    };
    for (i = 0; i < count; i++, p--) {
	(o_stack.p)++;
	(((o_stack.p))->value.realval = (((*p) * (1.0 / (1 << 12)))), (((o_stack.p))->tas.type_attrs = ((t_real) << 8) + (0)));
    }
    return 0;
}

int z1_pop_proc(gs_font_type1 * ignore, fixed * pf)
{
    float val;
    int code = num_params((o_stack.p), 1, &val);
    if (code < 0)
	return code;
    *pf = ((fixed) ((val) * (float) (1 << 12)));
    (o_stack.p)--;
    return 0;
}
