...
static gs_ptr_type_t vm_save_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (((vm_save_t *) vptr)->gsave);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void vm_save_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((vm_save_t *) vptr)->gsave = gs_reloc_struct_ptr((const void *) ((const vm_save_t *) vptr)->gsave, gcst);
} static const gs_memory_struct_type_t st_vm_save = { sizeof(vm_save_t), "savetype", 0, 0, vm_save_enum_ptrs, vm_save_reloc_ptrs, 0 };

int zsave(register os_ptr op)
{
    ...
    vmsave = (vm_save_t *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_vm_save, "zsave");
    ialloc_set_space((&gs_imemory), space);
    if (vmsave == 0)
	return ((-25));
    sid = alloc_save_state((&gs_imemory), vmsave);
    if (sid == 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), vmsave, "zsave");
	return ((-25));
    }
    do {
    } while (0);
...
static int restore_check_operand(os_ptr, alloc_save_t **);
static int restore_check_stack(const ref_stack *, const alloc_save_t *, bool);
static void restore_fix_stack(ref_stack *, const alloc_save_t *, bool);
int zrestore(register os_ptr op)
{
    alloc_save_t *asave;
    bool last;
    vm_save_t *vmsave;
    ...
    (o_stack.p)--; {
	int code;
	if ((code = restore_check_stack(&o_stack, asave, ((bool) 0))) < 0 || (code = restore_check_stack(&e_stack, asave, ((bool) 1))) < 0 || (code = restore_check_stack(&d_stack, asave, ((bool) 0))) < 0) {
	    (o_stack.p)++;
	    return code;
	}
    }
    restore_fix_stack(&o_stack, asave, ((bool) 0));
    restore_fix_stack(&e_stack, asave, ((bool) 1));
    restore_fix_stack(&d_stack, asave, ((bool) 0));
    do {
	vmsave = alloc_save_client_data(alloc_save_current((&gs_imemory)));
	gs_grestoreall(igs);
	...
	uint space = (((&gs_imemory))->current_space);
	ialloc_set_space((&gs_imemory), avm_local);
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), vmsave, "zrestore");
	ialloc_set_space((&gs_imemory), space);
    }
    dict_set_top();
    return 0;
...
static int restore_check_operand(os_ptr op, alloc_save_t ** pasave)
{
    ...
    ulong sid;
    alloc_save_t *asave;
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_save)))
	return (check_type_failed(&*op));
    vmsave = ((vm_save_t *) ((op)->value.pstruct));
    if (vmsave == 0)
	return ((-11));
    sid = op->value.saveid;
    asave = alloc_find_save((&gs_imemory), sid);
    if (asave == 0)
	return ((-11));
    *pasave = asave;
    return 0;
...
static int restore_check_stack(const ref_stack * pstack, const alloc_save_t * asave, bool is_estack)
{ {
	...
	for (;;) { {
		...
		for (stkp = bot; size; stkp++, size--) {
		    ...
		    case t_file:{
			    stream *s;
			    if (is_estack && ((((stkp)->tas.type_attrs) & (0x80)) || !(s = (stkp)->value.pfile, (s->read_id | s->write_id) == ((stkp)->tas.rsize))))
				continue;
			}
			...
			continue;
		    case t_string:
			if (((stkp)->tas.rsize) == 0 && (((stkp)->tas.type_attrs) & (0x80)) && is_estack)
			    continue;
...
static void restore_fix_stack(ref_stack * pstack, const alloc_save_t * asave, bool is_estack)
{ {
	ref_stack_block *pblock_ = (ref_stack_block *) (pstack)->current.value.refs;
	ref *bot = (pstack)->bot;
	uint size = (pstack)->p + 1 - (pstack)->bot;
	for (;;) { {
		...
		for (stkp = bot; size; stkp++, size--) {
			...
			switch ((((const byte *) &((stkp)->tas.type_attrs))[1])) {
			case t_string:
			    if (((stkp)->tas.rsize) == 0 && alloc_is_since_save(stkp->value.bytes, asave)) {
				((stkp)->value.const_bytes = ((const byte *) ((void *) 0)), ((stkp)->tas.type_attrs = ((t_string) << 8) + (avm_foreign)), ((stkp)->tas.rsize = (0)));
				break;
			    }
			    continue;
			case t_file:
			    if (alloc_is_since_save(stkp->value.pfile, asave)) {
...
static int zvmstatus(register os_ptr op)
{
    gs_memory_status_t mstat, dstat;
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.status) (((gs_memory_t *) (gs_imemory.current)), &mstat);
    if (((gs_memory_t *) (gs_imemory.current)) == ((gs_memory_t *) (gs_imemory.spaces.named.global))) {
	gs_memory_status_t sstat;
	(*(((gs_memory_t *) (gs_imemory.spaces.named.system)))->procs.status) (((gs_memory_t *) (gs_imemory.spaces.named.system)), &sstat);
	mstat.allocated += sstat.allocated;
	mstat.used += sstat.used;
    }
    (*(&gs_memory_default)->procs.status) (&gs_memory_default, &dstat);
    do {
	if ((op += (3)) > (o_stack.top)) {
	    o_stack.requested = (3);
	    return ((-16));
    ...
    ((op - 2)->value.intval = (alloc_save_level((&gs_imemory))), ((op - 2)->tas.type_attrs = ((t_integer) << 8) + (0)));
...
static int zforgetsave(register os_ptr op)
{
    alloc_save_t *asave;
    vm_save_t *vmsave;
    int code = restore_check_operand(op, &asave);
    ...
    restore_fix_stack(&o_stack, asave, ((bool) 0));
    restore_fix_stack(&e_stack, asave, ((bool) 0));
    restore_fix_stack(&d_stack, asave, ((bool) 0)); {
	gs_state *pgs = igs;
	gs_state *last;
...
const op_def *zvmem_op_defs(void)
{
    static const op_def op_defs_[] = { {"1.forgetsave", zforgetsave}, {"1restore", zrestore}, {"0save", zsave}, {"0vmstatus", zvmstatus}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
