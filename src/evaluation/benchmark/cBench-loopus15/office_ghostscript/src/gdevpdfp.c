...
static const int CoreDistVersion = 3000;
int gdev_pdf_get_params(gx_device * dev, gs_param_list * plist)
{
    gx_device_pdf *pdev = (gx_device_pdf *) dev;
    int code = gdev_psdf_get_params(dev, plist);
    if (code < 0 || (code = (*(plist)->procs->xmit_float) (plist, "CompatibilityLevel", &pdev->CompatibilityLevel)) < 0 || (code = (*(plist)->procs->xmit_int) (plist, "CoreDistVersion", (int *) &CoreDistVersion)) < 0 || (code = (*(plist)->procs->xmit_bool) (plist, "DoThumbnails", &pdev->DoThumbnails)) < 0 || (code = (*(plist)->procs->xmit_long) (plist, "FirstObjectNumber", &pdev->FirstObjectNumber)) < 0);
    return code;
}
...
int gdev_pdf_put_params(gx_device * dev, gs_param_list * plist)
{
    ...
    switch (code = (*(plist)->procs->xmit_float) (plist, (param_name = "CompatibilityLevel"), &cl)) {
    default:
	ecode = code;
    ...
    switch (code = (*(plist)->procs->xmit_long) (plist, (param_name = "FirstObjectNumber"), &fon)) {
    case 0:
	if (fon <= 0 || fon > 0x7fff0000 || (pdev->next_id != 0 && pdev->next_id != pdev->FirstObjectNumber + 2))
	    ecode = (-15);
	else
	    break;
    default:
	...
	switch (code = (*(plist)->procs->xmit_string_array) (plist, (param_name = "pdfmark"), &ppa)) {
	case 0:
    ...
    {
	gs_param_dict ppd;
	switch (code = (*(plist)->procs->begin_xmit_dict) (plist, (param_name = "show"), &ppd, ((bool) 0))) {
	case 0:
	    pdf_open_document(pdev);
	    code = pdfshow_process(pdev, &ppd);
	    (*(plist)->procs->end_xmit_dict) (plist, param_name, &ppd);
	    if (code >= 0)
		break;
	default:
	    ecode = code;
