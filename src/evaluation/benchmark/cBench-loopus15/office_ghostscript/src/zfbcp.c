...
static int no_bcp_signal_interrupt(stream_state * st)
{
    return 0;
}

static int no_bcp_request_status(stream_state * st)
{
    return 0;
}

static int zBCPE(os_ptr op)
{
    return filter_write_simple(op, &s_BCPE_template);
}

static int zBCPD(os_ptr op)
{
    stream_BCPD_state state;
    state.signal_interrupt = no_bcp_signal_interrupt;
    state.request_status = no_bcp_request_status;
    return filter_read(op, (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary)) ? 1 : 0), &s_BCPD_template, (stream_state *) & state, 0);
} static int zTBCPE(os_ptr op)
{
    return filter_write_simple(op, &s_TBCPE_template);
}

static int zTBCPD(os_ptr op)
{
    stream_BCPD_state state;
    state.signal_interrupt = no_bcp_signal_interrupt;
    state.request_status = no_bcp_request_status;
    return filter_read(op, (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary)) ? 1 : 0), &s_TBCPD_template, (stream_state *) & state, 0);
} const op_def *zfbcp_op_defs(void)
{
    static const op_def op_defs_[] = { {"filterdict", 0}, {"1BCPEncode", zBCPE}, {"1BCPDecode", zBCPD}, {"1TBCPEncode", zTBCPE}, {"1TBCPDecode", zTBCPD}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
