...
static int gs_stack_modal_fonts(gs_show_enum * penum)
{
    int fdepth = penum->fstack.depth;
    gs_font *cfont = penum->fstack.items[fdepth].font;
    while (cfont->FontType == ft_composite && ((((gs_font_type0 *) cfont)->data.FMapType) == fmap_escape || (((gs_font_type0 *) cfont)->data.FMapType) == fmap_double_escape || (((gs_font_type0 *) cfont)->data.FMapType) == fmap_shift)) {
	if (fdepth == 5)
	    return ((-10));
	fdepth++;
	cfont = ((gs_font_type0 *) cfont)->data.FDepVector[((gs_font_type0 *) cfont)->data.Encoding[0]];
...
int gs_type0_init_fstack(gs_show_enum * penum, gs_font * pfont)
{
    do {
    } while (0);
...
int gs_type0_next_glyph(register gs_show_enum * penum, gs_char * pchr, gs_glyph * pglyph)
{
	...
	for (; pfont->FontType == ft_composite;) {
	    ...
	    switch (fmt) {
	    case fmap_escape:
		if (chr != (((gs_font_type0 *) (penum->fstack.items[0].font))->data.EscChar))
		    break;
		if (end - p < 2)
		    return ((-15));
		fidx = p[1];
		...
		do {
	      ...
	      rdown:if (fidx >= pdata->encoding_size)
		    return ((-15));
		if (idepth == 5)
		    return ((-10));
		pfont = pdata->FDepVector[pdata->Encoding[fidx]];
		if (++idepth > orig_depth || pfont != penum->fstack.items[idepth].font)
		    penum->fstack.items[idepth].font = pfont, changed = 1;
		penum->fstack.items[idepth].index = fidx;
		do {
		} while (0);
		continue;
  ...
  up:if (p == end)
	...
	case fmap_escape:
	  ...
	  down:if (++p == end)
		...
		if (++fdepth > orig_depth || pfont != penum->fstack.items[fdepth].font)
		    penum->fstack.items[fdepth].font = pfont, changed = 1;
	...
	case fmap_SubsVector:{
		...
		case 1:{
			...
			while (subs_count != 0 && tchr >= (schr = *psv))
			    subs_count--, tchr -= schr, psv += 1;
			chr = tchr;
			p += 1 - 1;
			break;
		    }
		case 2:
			...
			while (subs_count != 0 && tchr >= (schr = (((ushort) * psv << 8) + psv[1])))
			    subs_count--, tchr -= schr, psv += 2;
			chr = tchr;
			p += 2 - 1;
			break;
			}
		case 3:
			if (end - p < 2)
			    return ((-15)); {
			    ulong tchr = ((ulong) chr << 16) + (((ushort) * p << 8) + p[1]), schr;
			    while (subs_count != 0 && tchr >= (schr = (((ulong) * psv << 16) + ((uint) psv[1] << 8) + psv[2])))
				subs_count--, tchr -= schr, psv += 3;
			    chr = tchr;
			    p += 3 - 1;
			    break;
			    }
		case 4:
			    if (end - p < 3)
				return ((-15)); {
				ulong tchr = ((ulong) chr << 24) + (((ulong) * p << 16) + ((uint) p[1] << 8) + p[2]), schr;
				while (subs_count != 0 && tchr >= (schr = (((ulong) * psv << 24) + ((ulong) psv[1] << 16) + ((uint) psv[2] << 8) + psv[3])))
				    subs_count--, tchr -= schr, psv += 4;
				chr = tchr;
				p += 4 - 1;
				break;
				}
	...
	case fmap_CMap:{
		...
		cstr.data = str;
		cstr.size = end - str;
		code = gs_cmap_decode_next(pdata->CMap, &cstr, &mindex, &fidx, &glyph);
		if (code < 0)
		    return code;
		p = str + mindex;
		do {
