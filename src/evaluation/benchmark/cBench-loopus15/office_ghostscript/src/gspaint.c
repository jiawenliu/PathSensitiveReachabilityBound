...
int gs_fillpage(gs_state * pgs)
{
    ...
    code = (*(pgs->dev_color)->type->fill_rectangle) (pgs->dev_color, 0, 0, dev->width, dev->height, (pgs)->device, (pgs)->log_op, ((void *) 0));
    pgs->log_op = save_lop;
    if (code < 0)
	return code;
...
static int alpha_buffer_bits(gs_state * pgs)
{
    gx_device *dev;
    if (!((pgs->dev_color)->type == (&gx_dc_procs_pure)))
	return 0;
    dev = ((pgs)->device);
    if (gs_device_is_abuf(dev)) {
	return 0;
    }
    return (*((dev)->std_procs.get_alpha_bits)) (dev, go_graphics);
}

static int alpha_buffer_init(gs_state * pgs, fixed extra_x, fixed extra_y, int alpha_bits)
{
    ...
    ibox.p.x = ((int) ((bbox.p.x - extra_x) >> (12))) - 1;
    ibox.p.y = ((int) ((bbox.p.y - extra_y) >> (12))) - 1;
    ibox.q.x = ((int) (((bbox.q.x + extra_x) + ((1 << 12) - 1)) >> (12))) + 1;
    ibox.q.y = ((int) (((bbox.q.y + extra_y) + ((1 << 12) - 1)) >> (12))) + 1;
    width = (ibox.q.x - ibox.p.x) << log2_scale.x;
    raster = ((uint) (((width + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2));
    band_space = raster << log2_scale.y;
    height = ((0 ? 500 : 2000) / band_space) << log2_scale.y;
    if (height == 0)
    ...
    if (mdev == 0)
	return 0;
    gs_make_mem_abuf_device(mdev, mem, dev, &log2_scale, alpha_bits, ibox.p.x << log2_scale.x);
    mdev->width = width;
    mdev->height = height;
    mdev->bitmap_memory = mem;
    if ((*((mdev)->std_procs.open_device)) ((gx_device *) mdev) < 0) {
...
static void alpha_buffer_release(gs_state * pgs, bool newpath)
{
    gx_device_memory *mdev = (gx_device_memory *) ((pgs)->device);
    gx_device *target = mdev->target;
    (*((mdev)->std_procs.close_device)) ((gx_device *) mdev);
    (*(mdev->memory)->procs.free_object) (mdev->memory, mdev, "alpha_buffer_release");
    gx_set_device_only(pgs, target);
    gx_cpath_scale_exp2(pgs->clip_path, -mdev->log2_scale.x, -mdev->log2_scale.y);
    if (!(newpath && !pgs->path->shares_segments))
	gx_path_scale_exp2(pgs->path, -mdev->log2_scale.x, -mdev->log2_scale.y);
}

static int fill_with_rule(gs_state * pgs, int rule)
{
    int code;
    if (pgs->in_charpath)
	code = gx_path_add_char_path(pgs->show_gstate->path, pgs->path, pgs->in_charpath);
    else {
	int abits, acode;
	if (!(((pgs)->dev_color)->type != (&gx_dc_procs_none))) {
	    int code_dc = gx_remap_color(pgs);
	...
	if (abits > 1) {
	    acode = alpha_buffer_init(pgs, pgs->fill_adjust.x, pgs->fill_adjust.y, abits);
	    if (acode < 0)
		return acode;
	} else
	    acode = 0;
	code = gx_fill_path(pgs->path, pgs->dev_color, pgs, rule, pgs->fill_adjust.x, pgs->fill_adjust.y);
	if (acode > 0)
...
int gs_stroke(gs_state * pgs)
{
	...
	if (abits > 1) {
	    ...
	    float new_width = (orig_width = gs_currentlinewidth(pgs)) * (1 << (abits / 2));
	    fixed extra_adjust = ((fixed) (((((xxyy) > (xyyx)) ? (xxyy) : (xyyx)) * new_width / 2) * (float) (1 << 12)));
	    if (extra_adjust < (1L << 12))
		extra_adjust = (1L << 12);
	    acode = alpha_buffer_init(pgs, pgs->fill_adjust.x + extra_adjust, pgs->fill_adjust.y + extra_adjust, abits);
	    if (acode < 0)
		return acode;
...
int gs_strokepath(gs_state * pgs)
{
    gx_path spath;
    int code;
