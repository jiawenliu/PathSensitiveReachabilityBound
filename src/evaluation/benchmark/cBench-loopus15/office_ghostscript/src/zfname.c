...
int parse_file_name(const ref * op, parsed_file_name * pfn)
{
    const byte *pname;
    uint len, dlen;
    const byte *pdelim;
...
int parse_real_file_name(const ref * op, parsed_file_name * pfn, client_name_t cname)
{
...
int terminate_file_name(parsed_file_name * pfn, client_name_t cname)
{
    uint len = pfn->len;
    ref fnref;
    const char *fname;
    if (pfn->iodev == ((void *) 0))
	pfn->iodev = (gs_getiodevice(0));
    fnref.value.const_bytes = (const byte *) pfn->fname;
    ((&fnref)->tas.rsize = (len));
    fname = ref_to_string(&fnref, ((gs_memory_t *) (gs_imemory.current)), cname);
    if (fname == 0)
	return ((-25));
    pfn->fname = fname;
    pfn->len = len + 1;
...
void free_file_name(parsed_file_name * pfn, client_name_t cname)
{
    if (pfn->fname != 0)
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_string) (((gs_memory_t *) (gs_imemory.current)), (byte *) pfn->fname, pfn->len, cname);
}
