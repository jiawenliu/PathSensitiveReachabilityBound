...
static int set_language_level(int);
static int zlanguagelevel(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    (*(op) = *(&ref_language_level));
    return 0;
}

static int zsetlanguagelevel(register os_ptr op)
{
    int code = 0;
    ...
    ((o_stack.p) -= (1));
    (((void) ((((((&ref_language_level)->tas.type_attrs) & ((&gs_imemory)->test_mask)) == 0) ? alloc_save_change((&gs_imemory), ((void *) 0), (ref_packed *) (&ref_language_level), "setlanguagelevel") : 0))), ((void) (((*(&ref_language_level) = *(op)), ((&ref_language_level)->tas.type_attrs |= ((&gs_imemory)->new_mask))))));
    return code;
}

static int z2where(register os_ptr op)
{
    ref rkns, rfh;
    const ref *pdref = (d_stack.p);
    ref *pvalue;
    if (!((((const byte *) &((op)->tas.type_attrs))[1]) == (t_name)) || (name_string_ref(op, &rkns), ((&rkns)->tas.rsize)) != 8 || memcmp(rkns.value.bytes, "setcolor", 8) != 0 || name_ref((const byte *) "FreeHandDict", 12, &rfh, -1) < 0 || (pvalue = dict_find_name_by_index(name_index(&rfh))) == 0 || !obj_eq(pvalue, pdref))
	return zwhere(op);
...
const op_def *zmisc2_op_defs(void)
{
    static const op_def op_defs_[] = { {"0.languagelevel", zlanguagelevel}, {"1.setlanguagelevel", zsetlanguagelevel}, {"level2dict", 0}, {"1where", z2where}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}

static int swap_entry(ref elt[2], ref * pdict, ref * pdict2);
static int set_language_level(int level)
{
    ref *pgdict = ref_stack_index(&d_stack, ref_stack_count(&d_stack) - 2);
    ref *level2dict;
    if (dict_find_string((&ref_systemdict), "level2dict", &level2dict) <= 0)
	return ((-21));
    if (level == 2) {
	ref *pdict;
	...
	*pgdict = *(&ref_systemdict);
	dict_auto_expand = ((bool) 0);
    } {
	int index = dict_first(level2dict);
	ref elt[2];
	ref *subdict;
	while ((index = dict_next(level2dict, index, &elt[0])) >= 0)
	    if (((((const byte *) &((&elt[1])->tas.type_attrs))[1]) == (t_dictionary)) && dict_find(&elt[1], &elt[0], &subdict) > 0 && obj_eq(&elt[1], subdict)) {
		int isub = dict_first(&elt[1]);
		ref subelt[2];
		int found = dict_find((&ref_systemdict), &elt[0], &subdict);
		if (found <= 0)
		    continue;
		...
		int code = swap_entry(elt, (&ref_systemdict), level2dict);
		if (code < 0)
		    return code;
	    }
    }
