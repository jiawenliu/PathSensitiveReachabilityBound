...
static const gs_memory_struct_type_t st_RLD_state = { sizeof(stream_RLD_state), "RunLengthDecode state", 0, 0, 0, 0, 0 };

static void s_RLD_set_defaults(stream_state * st)
{
    ((((stream_RLD_state *) st))->EndOfData = ((bool) 1));
} static int s_RLD_init(stream_state * st)
{
    return ((((stream_RLD_state *) st))->copy_left = 0);
}

static int s_RLD_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * pw, bool last)
{
    ...
    int left;
    int status = 0;
  top:if ((left = ((stream_RLD_state *) st)->copy_left) > 0) {
	uint avail = wlimit - q;
	int copy_status = 1;
	if (left > avail)
	    left = avail;
	if (((stream_RLD_state *) st)->copy_data >= 0)
	    memset(q + 1, ((stream_RLD_state *) st)->copy_data, left);
	else {
	    avail = rlimit - p;
	    if (left >= avail) {
		copy_status = 0;
	...
	q += left;
	if ((((stream_RLD_state *) st)->copy_left -= left) > 0) {
	    status = copy_status;
	    goto x;
	}
    }
	...
	if (b < 128) {
		...
		((stream_RLD_state *) st)->copy_left = b;
		((stream_RLD_state *) st)->copy_data = -1;
		goto top;
	    }
	    memcpy(q + 1, p + 1, b);
	    p += b;
	    ...
	    if (((stream_RLD_state *) st)->EndOfData) {
		status = ((int) (-1));
		break;
	    }
	} else if (p == rlimit) {
	    ...
	    ((stream_RLD_state *) st)->copy_left = b;
	    ((stream_RLD_state *) st)->copy_data = *++p;
	    goto top;
	} else {
	    memset(q + 1, *++p, b);
	    q += b;
...
const stream_template s_RLD_template = { &st_RLD_state, s_RLD_init, s_RLD_process, 1, 1, ((void *) 0), s_RLD_set_defaults };
