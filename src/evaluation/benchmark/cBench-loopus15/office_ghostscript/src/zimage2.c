...
static int image_dict_unpack(os_ptr op, image_params * pip, int max_bits_per_component)
{
    int code;
    int num_components;
    int decode_size;
	...
	return ((-7));
    num_components = gs_color_space_num_components(gs_currentcolorspace(igs));
    if (num_components < 1)
	return ((-15));
    if (max_bits_per_component == 1)
	num_components = 1;
    if ((code = dict_int_param(op, "ImageType", 1, 1, 1, &code)) < 0 || (code = dict_int_param(op, "Width", 0, 0x7fff, -1, &(&pip->image)->Width)) < 0 || (code = dict_int_param(op, "Height", 0, 0x7fff, -1, &(&pip->image)->Height)) < 0 || (code = dict_matrix_param(op, "ImageMatrix", &(&pip->image)->ImageMatrix)) < 0 || (code = dict_bool_param(op, "MultipleDataSources", ((bool) 0), &pip->MultipleDataSources)) < 0 || (code = dict_int_param(op, "BitsPerComponent", 0, max_bits_per_component, -1, &(&pip->image)->BitsPerComponent)) < 0 || (code = decode_size = dict_float_array_param(op, "Decode", num_components * 2, &(&pip->image)->Decode[0], ((void *) 0))) < 0 || (code = dict_bool_param(op, "Interpolate", ((bool) 0), &(&pip->image)->Interpolate)) < 0 || (code = dict_bool_param(op, "CombineWithColor", ((bool) 0), &(&pip->image)->CombineWithColor)) < 0)
	return code;
    if (decode_size == 0)
	pip->pDecode = 0;
	...
	pip->pDecode = &(&pip->image)->Decode[0];
    if ((code = dict_find_string(op, "DataSource", &pds)) < 0)
	return code;
    if (pip->MultipleDataSources) {
	if (!((((const byte *) &((&*pds)->tas.type_attrs))[1]) == (t_array)))
	    return ((-20));
	if (((pds)->tas.rsize) != num_components)
	    return ((-15));
...
static int z2image(register os_ptr op)
{
	...
	if (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary))) {
	    ...
	    return zimage_setup(&ip.image, ip.MultipleDataSources, &ip.DataSource[0], 1);
	}
    }
    if (op < (o_stack.bot) + ((5) - 1))
	return ((-17));
    return zimage(op);
}

static int z2imagemask(register os_ptr op)
{
    if (((int) ref_language_level.value.intval == 2)) {
	if (op < (o_stack.bot) + ((1) - 1))
	    return ((-17));
	if (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_dictionary))) {
	    image_params ip;
	    int code;
	    gs_image_t_init_mask(&ip.image, ((bool) 0));
	    code = image_dict_unpack(op, &ip, 1);
	    if (code < 0)
		return code;
	    if (ip.MultipleDataSources)
		return ((-15));
	    return zimage_setup(&ip.image, ((bool) 0), &ip.DataSource[0], 1);
	}
    }
    if (op < (o_stack.bot) + ((5) - 1))
	return ((-17));
...
const op_def *zimage2_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"1image", z2image}, {"1imagemask", z2imagemask}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
