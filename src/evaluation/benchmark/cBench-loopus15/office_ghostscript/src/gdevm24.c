...
static int mem_true24_copy_mono(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1);
static int mem_true24_copy_color(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height);
static int mem_true24_fill_rectangle(gx_device * dev, int x, int y, int width, int height, gx_color_index color);
...
const gx_device_memory mem_true24_device = { sizeof(gx_device_memory), 0, "image24", 0, 0, 0, 0, {(24 ? 3 : 0) + (0 ? 1 : 0), 24 + 0, (0 ? (1 << 0) - 1 : (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1)), (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1), (0 ? (1 << 0) - 1 : (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1)) + 1, (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1) + 1}
...
static int mem_true24_fill_rectangle(gx_device * dev, int x, int y, int w, int h, gx_color_index color)
{
    ...
    dest = (byte *) (((((gx_device_memory *) dev))->line_ptrs[y]) + (((x) * 3) & -(sizeof(byte) == 1 ? 1 : sizeof(byte) == sizeof(short) ? 2 : sizeof(byte) == sizeof(int) ? 4 : 4)));
    if (w >= 5) {
	if (r == g && r == b) {
	    int bcntm7 = w * 3 - 7;
		...
		*(bits32 *) pptr = ((bits32 *) pptr)[1] = ((bits32 *) pptr)[2] = cword;
		pptr += 12;
		while (pptr < limit) {
		    *(bits32 *) pptr = ((bits32 *) pptr)[1] = cword;
		    pptr += 8;
		}
		switch (pptr - limit) {
		case 0:
	    ...
	    int x3 = -x & 3, ww = w - x3;
	    bits32 rgbr, gbrg, brgb;
	    if (((gx_device_memory *) dev)->color24.rgb == color)
		rgbr = ((gx_device_memory *) dev)->color24.rgbr, gbrg = ((gx_device_memory *) dev)->color24.gbrg, brgb = ((gx_device_memory *) dev)->color24.brgb;
	    else
		((gx_device_memory *) dev)->color24.rgbr = rgbr = ((bits32) (r) << 24) | ((bits32) (b) << 16) | ((bits16) (g) << 8) | (r), ((gx_device_memory *) dev)->color24.brgb = brgb = (rgbr << 8) | (b), ((gx_device_memory *) dev)->color24.gbrg = gbrg = (brgb << 8) | (g), ((gx_device_memory *) dev)->color24.rgb = (color);
	    while (h-- > 0) {
		register byte *pptr = dest;
		int w1 = ww;
		switch (x3) {
...
static int mem_true24_copy_mono(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h, gx_color_index zero, gx_color_index one)
{
    const byte *line;
    int sbit;
    int first_bit;
	...
	byte r0 = (byte) (zero >> 16);
	byte g0 = (byte) ((uint) zero >> 8);
	byte b0 = (byte) zero;
	byte r1 = (byte) (one >> 16);
	byte g1 = (byte) ((uint) one >> 8);
	byte b1 = (byte) one;
	while (h-- > 0) {
	    register byte *pptr = dest;
...
static int mem_true24_copy_color(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
    if ((x | y) < 0) {
	if (x < 0)
	    w += x, sourcex -= x, x = 0;
    ...
    if (w <= 0 || h <= 0)
	return 0;
    bytes_copy_rectangle(((((gx_device_memory *) dev))->line_ptrs[y]) + ((x) * 3), (((gx_device_memory *) dev))->raster, base + ((sourcex) * 3), sraster, ((w) * 3), h);
    return 0;
}

static int mem_true24_copy_alpha(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h, gx_color_index color, int depth)
{
    const byte *line;
    register byte *dest;
    uint draster;
...
static int mem24_word_copy_mono(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1);
static int mem24_word_copy_color(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height);
static int mem24_word_fill_rectangle(gx_device * dev, int x, int y, int width, int height, gx_color_index color);
const gx_device_memory mem_true24_word_device = { sizeof(gx_device_memory), 0, "image24w", 0, 0, 0, 0, {(24 ? 3 : 0) + (0 ? 1 : 0), 24 + 0, (0 ? (1 << 0) - 1 : (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1)), (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1), (0 ? (1 << 0) - 1 : (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1)) + 1, (24 >= 8 ? 255 : 24 == 4 ? 15 : 24 == 2 ? 3 : 24 == 1 ? 1 : (1 << 0) - 1) + 1}
, 0, 0, {(((0) * 72.0 + 0.5) - 0.5) / (72), (((0) * 72.0 + 0.5) - 0.5) / (72)}
, {0, 0, 0, 0}
, 0, {72, 72}
, {72, 72}
...
static int mem24_word_copy_mono(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h, gx_color_index zero, gx_color_index one)
{
    byte *row;
    uint raster;
    bool store;
    ...
    mem_true24_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero, one);
    mem_swap_byte_rect(row, raster, x * 24, w * 24, h, ((bool) 0));
    return 0;
}

static int mem24_word_copy_color(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
    byte *row;
    uint raster;
    if ((x | y) < 0) {
    ...
    bytes_copy_rectangle(row + x * 3, raster, base + sourcex * 3, sraster, w * 3, h);
    mem_swap_byte_rect(row, raster, x * 24, w * 24, h, ((bool) 0));
    return 0;
}
