...
int gx_fill_path(gx_path * ppath, gx_device_color * pdevc, gs_state * pgs, int rule, fixed adjust_x, fixed adjust_y)
{
    ...
    return (*((dev)->std_procs.fill_path)) (dev, (const gs_imager_state *) pgs, ppath, &params, pdevc, pgs->clip_path);
} int gx_stroke_fill(gx_path * ppath, gs_state * pgs)
{
    gx_device *dev = ((pgs)->device);
    gx_stroke_params params;
    params.flatness = (pgs->in_cachedevice > 1 ? 0.0 : pgs->flatness);
    return (*((dev)->std_procs.stroke_path)) (dev, (const gs_imager_state *) pgs, ppath, &params, pgs->dev_color, pgs->clip_path);
} int gx_stroke_add(gx_path * ppath, gx_path * to_path, gs_state * pgs)
{
    gx_stroke_params params;
    params.flatness = (pgs->in_cachedevice > 1 ? 0.0 : pgs->flatness);
    return gx_stroke_path_only(ppath, to_path, pgs->device, (const gs_imager_state *) pgs, &params, ((void *) 0), ((void *) 0));
}
