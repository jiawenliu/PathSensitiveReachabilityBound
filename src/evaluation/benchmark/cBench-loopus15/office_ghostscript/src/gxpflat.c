...
int gx_curve_log2_samples(fixed x0, fixed y0, const curve_segment * pc, fixed fixed_flat)
{
    fixed x03 = pc->pt.x - x0, y03 = pc->pt.y - y0;
    int k;
    if (x03 < 0)
	x03 = -x03;
    if (y03 < 0)
	y03 = -y03;
    if ((x03 | y03) < ((fixed) (16) << 12))
	fixed_flat >>= 1;
    if (fixed_flat == 0) {
	...
	const fixed x12 = pc->p1.x - pc->p2.x, y12 = pc->p1.y - pc->p2.y, dx0 = x0 - pc->p1.x - x12, dy0 = y0 - pc->p1.y - y12, dx1 = x12 - pc->p2.x + pc->pt.x, dy1 = y12 - pc->p2.y + pc->pt.y, adx0 = ((dx0) < 0 ? -(dx0) : (dx0)), ady0 = ((dy0) < 0 ? -(dy0) : (dy0)), adx1 = ((dx1) < 0 ? -(dx1) : (dx1)), ady1 = ((dy1) < 0 ? -(dy1) : (dy1));
	fixed d = (((adx0) > (adx1)) ? (adx0) : (adx1)) + (((ady0) > (ady1)) ? (ady0) : (ady1));
	uint q = (d - (d >> 2) + fixed_flat - 1) / fixed_flat;
	do {
	} while (0);
	do {
	} while (0);
...
static void split_curve_midpoint(fixed x0, fixed y0, const curve_segment * pc, curve_segment * pc1, curve_segment * pc2)
{
    fixed x12 = (((pc->p1.x) >> 1) + ((pc->p2.x) >> 1) + ((pc->p1.x) & (pc->p2.x) & 1) + 1);
    fixed y12 = (((pc->p1.y) >> 1) + ((pc->p2.y) >> 1) + ((pc->p1.y) & (pc->p2.y) & 1) + 1);
    pc1->p1.x = (((x0) >> 1) + ((pc->p1.x) >> 1) + ((x0) & (pc->p1.x) & 1) + 1);
    pc1->p1.y = (((y0) >> 1) + ((pc->p1.y) >> 1) + ((y0) & (pc->p1.y) & 1) + 1);
    pc2->p2.x = (((pc->p2.x) >> 1) + ((pc->pt.x) >> 1) + ((pc->p2.x) & (pc->pt.x) & 1) + 1);
    pc2->p2.y = (((pc->p2.y) >> 1) + ((pc->pt.y) >> 1) + ((pc->p2.y) & (pc->pt.y) & 1) + 1);
    pc1->p2.x = (((pc1->p1.x) >> 1) + ((x12) >> 1) + ((pc1->p1.x) & (x12) & 1) + 1);
    pc1->p2.y = (((pc1->p1.y) >> 1) + ((y12) >> 1) + ((pc1->p1.y) & (y12) & 1) + 1);
    pc2->p1.x = (((x12) >> 1) + ((pc2->p2.x) >> 1) + ((x12) & (pc2->p2.x) & 1) + 1);
    pc2->p1.y = (((y12) >> 1) + ((pc2->p2.y) >> 1) + ((y12) & (pc2->p2.y) & 1) + 1);
    if (pc2 != pc)
	pc2->pt.x = pc->pt.x, pc2->pt.y = pc->pt.y;
    pc1->pt.x = (((pc1->p2.x) >> 1) + ((pc2->p1.x) >> 1) + ((pc1->p2.x) & (pc2->p1.x) & 1) + 1);
    pc1->pt.y = (((pc1->p2.y) >> 1) + ((pc2->p1.y) >> 1) + ((pc1->p2.y) & (pc2->p1.y) & 1) + 1);
}

int gx_flatten_sample(gx_path * ppath, int k, curve_segment * pc, segment_notes notes)
{
    fixed x0, y0;
  ...
  top:x0 = ppath->position.x;
	...
	(x01 = (pc->p1.x) - (x0), cx = (x01 << 1) + x01, x12 = (pc->p2.x) - (pc->p1.x), bx = (x12 << 1) + x12 - cx, ax = (pc->pt.x) - bx - cx - (x0));
	(y01 = (pc->p1.y) - (y0), cy = (y01 << 1) + y01, y12 = (pc->p2.y) - (pc->p1.y), by = (y12 << 1) + y12 - cy, ay = (pc->pt.y) - by - cy - (y0));
    }
    do {
    } while (0);
    if (k == 0) {
	return gx_path_add_line_notes(ppath, pc->pt.x, pc->pt.y, notes);
    }
    if (k <= ((((((int) (sizeof(int))) * 8 - 1) / 3) < (10)) ? ((((int) (sizeof(int))) * 8 - 1) / 3) : (10)) && (ax < ((~(-1L << ((1 << 2) * 8 - 1))) / 6) && ax > (-((~(-1L << ((1 << 2) * 8 - 1))) / 6))) && (ay < ((~(-1L << ((1 << 2) * 8 - 1))) / 6) && ay > (-((~(-1L << ((1 << 2) * 8 - 1))) / 6))) && (bx < ((~(-1L << ((1 << 2) * 8 - 1))) / 6) && bx > (-((~(-1L << ((1 << 2) * 8 - 1))) / 6))) && (by < ((~(-1L << ((1 << 2) * 8 - 1))) / 6) && by > (-((~(-1L << ((1 << 2) * 8 - 1))) / 6))) && (cx < ((~(-1L << ((1 << 2) * 8 - 1))) / 6) && cx > (-((~(-1L << ((1 << 2) * 8 - 1))) / 6))) && (cy < ((~(-1L << ((1 << 2) * 8 - 1))) / 6) && cy > (-((~(-1L << ((1 << 2) * 8 - 1))) / 6)))) {
	x = x0, y = y0;
	rx = ry = 0;
	ppt = points;
	if (k == 1) {
	    ...
	    if (((x ^ x0) | (y ^ y0)) & ((fixed) ((-0.5) * (float) (1 << 12))))
		ppt->x = ptx = x, ppt->y = pty = y, ppt++;
	    goto last;
	} else {
	    ...
	    rmask = (1 << k3) - 1;
	    idx = ((cx) >> (k)), idy = ((cy) >> (k));
	    rdx = ((uint) cx << k2) & rmask, rdy = ((uint) cy << k2) & rmask;
	    id2x = ((bx2) >> (k2)), id2y = ((by2) >> (k2));
	    rd2x = ((uint) bx2 << k) & rmask, rd2y = ((uint) by2 << k) & rmask;
	    idx += ((id2x) >> 1), idy += ((id2y) >> 1);
	    rdx += ((uint) bx << k) & rmask, rdy += ((uint) by << k) & rmask;
	    if (rdx > rmask)
		idx++, rdx &= rmask;
	    if (rdy > rmask)
		idy++, rdy &= rmask;
	    idx += ((ax) >> (k3)), idy += ((ay) >> (k3));
	    rdx += (uint) ax & rmask, rdy += (uint) ay & rmask;
	    if (rdx > rmask)
		idx++, rdx &= rmask;
	    if (rdy > rmask)
		idy++, rdy &= rmask;
	    id2x += id3x = ((ax6) >> (k3)), id2y += id3y = ((ay6) >> (k3));
	    rd2x += rd3x = (uint) ax6 & rmask, rd2y += rd3y = (uint) ay6 & rmask;
	    if (rd2x > rmask)
		id2x++, rd2x &= rmask;
	    if (rd2y > rmask)
		id2y++, rd2y &= rmask;
	...
	if ((!(((x) ^ (ptx)) & ((fixed) ((-0.5) * (float) (1 << 12)))))) {
	    if ((!(((y) ^ (pty)) & ((fixed) ((-0.5) * (float) (1 << 12))))))
		goto skip;
	    if (ppt > points + 1 && (!(((x) ^ (ppt[-2].x)) & ((fixed) ((-0.5) * (float) (1 << 12))))) && ((((ptx) - (ppt[-2].x)) ^ ((x) - (ptx))) >= 0) && ((((pty) - (ppt[-2].y)) ^ ((y) - (pty))) >= 0))
		--ppt;
	} else if ((!(((y) ^ (pty)) & ((fixed) ((-0.5) * (float) (1 << 12)))))) {
	    if (ppt > points + 1 && (!(((y) ^ (ppt[-2].y)) & ((fixed) ((-0.5) * (float) (1 << 12))))) && ((((ptx) - (ppt[-2].x)) ^ ((x) - (ptx))) >= 0) && ((((pty) - (ppt[-2].y)) ^ ((y) - (pty))) >= 0))
		--ppt;
	}
	if (ppt == &points[50]) {
	    if (notes & sn_not_first)
		code = gx_path_add_lines_notes(ppath, points, 50, notes);
	    else {
		code = gx_path_add_line_notes(ppath, points[0].x, points[0].y, notes);
		if (code < 0)
		    return code;
		code = gx_path_add_lines_notes(ppath, points, 50 - 1, notes | sn_not_first);
	    }
	    if (code < 0)
		return code;
	    ppt = points;
	...
	ppt++;
      skip:if (--i == 0)
	    break;
	if ((rdx += rd2x) > rmask)
	    rdx &= rmask, idx += id2x + 1;
	else
	    idx += id2x;
	if ((rd2x += rd3x) > rmask)
	    rd2x &= rmask, id2x += id3x + 1;
	else
	    id2x += id3x;
	if ((rdy += rd2y) > rmask)
	    rdy &= rmask, idy += id2y + 1;
	else
	    idy += id2y;
	if ((rd2y += rd3y) > rmask)
	    rd2y &= rmask, id2y += id3y + 1;
	else
	    id2y += id3y;
    }
  last:do {
	...
	if (!(notes & sn_not_first)) {
	    int code = gx_path_add_line_notes(ppath, points[0].x, points[0].y, notes);
	    if (code < 0)
		return code;
	    ++pts, --count;
	    notes |= sn_not_first;
