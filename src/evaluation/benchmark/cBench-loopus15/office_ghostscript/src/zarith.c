...
int zop_add(register os_ptr op)
{
	...
	case t_integer:{
		...
		if (((op[-1].value.intval += int2) ^ int2) < 0 && ((op[-1].value.intval - int2) ^ int2) >= 0) {
		    ((op - 1)->value.realval = ((float) (op[-1].value.intval - int2) + int2), ((op - 1)->tas.type_attrs = ((t_real) << 8) + (0)));
		}
    }}} return 0;
}
...
static int zdiv(register os_ptr op)
{
...
static int zmul(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    default:
	return (check_type_failed(op));
	...
	case t_integer:{
		...
		float fprod;
		if ((abs1 > ((1 << (((int) (sizeof(long))) / 2 - 1)) - 1) || abs2 > ((1 << (((int) (sizeof(long))) / 2 - 1)) - 1)) && abs1 != 0 && abs2 > (~(-1L << ((1 << 2) * 8 - 1))) / abs1 && (fprod = (float) int1 * int2, (int1 * int2 != (-1L << ((1 << 2) * 8 - 1)) || fprod != (float) (-1L << ((1 << 2) * 8 - 1)))))
		    ((op - 1)->value.realval = (fprod), ((op - 1)->tas.type_attrs = ((t_real) << 8) + (0)));
		else
...
int zop_sub(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    default:
	return (check_type_failed(op));
	...
	case t_integer:{
		long int1 = op[-1].value.intval;
		if ((int1 ^ (op[-1].value.intval = int1 - op->value.intval)) < 0 && (int1 ^ op->value.intval) < 0) {
...
static int zidiv(register os_ptr op)
{
    register os_ptr op1 = op - 1;
    ...
    if (op->value.intval == 0)
	return ((-23));
    if ((op1->value.intval /= op->value.intval) == (-1L << ((1 << 2) * 8 - 1)) && op->value.intval == -1) {
	return ((-15));
    }
    ((o_stack.p) -= (1));
    return 0;
...
static int zmod(register os_ptr op)
{
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
...
static int zneg(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    default:
	return (check_type_failed(op));
	...
	break;
    case t_integer:
	if (op->value.intval == (-1L << ((1 << 2) * 8 - 1)))
	    ((op)->value.realval = (-(float) (-1L << ((1 << 2) * 8 - 1))), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
	else
...
static int zceiling(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    default:
	return (check_type_failed(op));
...
static int zfloor(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    default:
	return (check_type_failed(op));
...
static int zround(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    default:
	return (check_type_failed(op));
...
static int ztruncate(register os_ptr op)
{
    switch ((((const byte *) &((op)->tas.type_attrs))[1])) {
    default:
	return (check_type_failed(op));
...
const op_def *zarith_op_defs(void)
{
    static const op_def op_defs_[] = { {"2add", zadd}, {"1ceiling", zceiling}, {"2div", zdiv}, {"2idiv", zidiv}, {"1floor", zfloor}, {"2mod", zmod}, {"2mul", zmul}, {"1neg", zneg}, {"1round", zround}, {"2sub", zsub}, {"1truncate", ztruncate}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
