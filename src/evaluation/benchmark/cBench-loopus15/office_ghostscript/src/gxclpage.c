...
int gdev_prn_render_pages(gx_device_printer * pdev, const gx_placed_page * ppages, int count);
int gdev_prn_save_page(gx_device_printer * pdev, gx_saved_page * page, int num_copies)
{
    int code;
	...
	return ((-13));
    if ((code = clist_end_page(((gx_device_clist_writer *) pdev))) < 0 || (code = clist_fclose(((gx_device_clist_writer *) pdev)->page_info.cfile, ((gx_device_clist_writer *) pdev)->page_info.cfname, ((bool) 0))) < 0 || (code = clist_fclose(((gx_device_clist_writer *) pdev)->page_info.bfile, ((gx_device_clist_writer *) pdev)->page_info.bfname, ((bool) 0))) < 0)
	return code;
    memcpy(&page->device, pdev, sizeof(gx_device));
    strcpy(page->dname, pdev->dname);
    page->info = ((gx_device_clist_writer *) pdev)->page_info;
    page->info.cfile = 0;
    page->info.bfile = 0;
...
{ {
	...
	for (i = 0; i < count; ++i) {
	    ...
	    if (strcmp(page->dname, pdev->dname) != 0 || memcmp(&page->device.color_info, &pdev->color_info, sizeof(pdev->color_info)) != 0)
		return ((-15));
	    if (ppages[i].offset.y != 0)
		return ((-15));
	    if (page->info.band_params.BandBufferSpace != pdev->buffer_space || page->info.band_params.BandWidth != pdev->width)
		return ((-15));
	    if (i == 0)
		params = page->info.band_params;
	    else if (page->info.band_params.BandHeight != params.BandHeight)
		return ((-15));
	}
}
((gx_device_clist_reader *) pdev)->ymin = ((gx_device_clist_reader *) pdev)->ymax = 0;
((gx_device_clist_reader *) pdev)->pages = ppages;
((gx_device_clist_reader *) pdev)->num_pages = count; {
    int code = (*((pdev)->std_procs.output_page)) ((gx_device *) pdev, ppages[0].page->num_copies, ((bool) 1));
    int i;
    for (i = 0; i < count; ++i) {
	const gx_saved_page *page = ppages[i].page;
