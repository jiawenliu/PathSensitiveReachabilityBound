...
int zexch(register os_ptr op)
{
    ref next;
    if (op < (o_stack.bot) + ((2) - 1))
	return ((-17));
    ((&next)->value = (op - 1)->value, (&next)->tas = (op - 1)->tas);
    ((op - 1)->value = (op)->value, (op - 1)->tas = (op)->tas);
    ((op)->value = (&next)->value, (op)->tas = (&next)->tas);
    return 0;
}

int zdup(register os_ptr op)
{
    if (op < (o_stack.bot) + ((1) - 1))
	return ((-17));
    do {
    ...
    ((op)->value = (op - 1)->value, (op)->tas = (op - 1)->tas);
    return 0;
}

int zindex(register os_ptr op)
{
    register os_ptr opn;
    ...
    ((op)->value = (opn)->value, (op)->tas = (opn)->tas);
    return 0;
}

int zroll(register os_ptr op)
{
    os_ptr op1 = op - 1;
    int count, mod;
    ...
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
    if ((ulong) op1->value.intval > op1 - (o_stack.bot)) {
	int left, i;
	if (op1->value.intval < 0 || op1->value.intval + 2 > ref_stack_count(&o_stack))
	    return ((-15));
	...
	op -= 2; {
	    ref top;
	    ((&top)->value = (op)->value, (&top)->tas = (op)->tas);
	    for (from = op, n = count; --n; from--)
		((from)->value = (from - 1)->value, (from)->tas = (from - 1)->tas);
	    ((from)->value = (&top)->value, (from)->tas = (&top)->tas);
	}
	return 0;
    case -1:
	((o_stack.p) -= (2));
	op -= 2; {
	    ref bot;
	    to = op - count + 1;
	    ((&bot)->value = (to)->value, (&bot)->tas = (to)->tas);
	    for (n = count; --n; to++)
		(*(to) = *(to + 1));
	    ((to)->value = (&bot)->value, (to)->tas = (&bot)->tas);
	}
	return 0;
    }
    if (mod < 0) {
...
static int zclear_stack(os_ptr op)
{
    ref_stack_pop(&o_stack, ref_stack_count(&o_stack));
    return 0;
}

static int zcount(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.intval = (ref_stack_count(&o_stack) - 1), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
    return 0;
}

static int zmark(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
...
int zcleartomark(register os_ptr op)
{
...
static int zcounttomark(os_ptr op)
{
    uint count = ref_stack_counttomark(&o_stack);
    if (count == 0)
	return ((-24));
...
const op_def *zstack_op_defs(void)
{
    static const op_def op_defs_[] = { {"0clear", zclear_stack}, {"0cleartomark", zcleartomark}, {"0count", zcount}, {"0counttomark", zcounttomark}, {"1dup", zdup}, {"2exch", zexch}, {"2index", zindex}, {"0mark", zmark}, {"1pop", zpop}, {"2roll", zroll}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
