...
static int common_to(os_ptr, int (*)(gs_state *, floatp, floatp));
static int common_curve(os_ptr, int (*)(gs_state *, floatp, floatp, floatp, floatp, floatp, floatp));
static int znewpath(register os_ptr op)
{
    return gs_newpath(igs);
}

static int zcurrentpoint(register os_ptr op)
{
    gs_point pt;
...
int zmoveto(os_ptr op)
{
    return common_to(op, gs_moveto);
}

int zrmoveto(os_ptr op)
{
    return common_to(op, gs_rmoveto);
}

int zlineto(os_ptr op)
{
    return common_to(op, gs_lineto);
}

int zrlineto(os_ptr op)
{
    return common_to(op, gs_rlineto);
}

static int common_to(os_ptr op, int (*add_proc) (gs_state *, floatp, floatp))
{
    float opxy[2];
    int code;
    if ((code = num_params(op, 2, opxy)) < 0 || (code = (*add_proc) (igs, opxy[0], opxy[1])) < 0)
	return code;
    ((o_stack.p) -= (2));
    return 0;
}

int zcurveto(register os_ptr op)
{
    return common_curve(op, gs_curveto);
}

int zrcurveto(register os_ptr op)
{
    return common_curve(op, gs_rcurveto);
}

static int common_curve(os_ptr op, int (*add_proc) (gs_state *, floatp, floatp, floatp, floatp, floatp, floatp))
{
    float opxy[6];
    int code;
...
int zclosepath(register os_ptr op)
{
    return gs_closepath(igs);
}

static int zinitclip(register os_ptr op)
{
    return gs_initclip(igs);
}

static int zclip(register os_ptr op)
{
    return gs_clip(igs);
}

static int zeoclip(register os_ptr op)
{
    return gs_eoclip(igs);
}

static int zsetclipoutside(register os_ptr op)
{
    int code;
...
static int zcurrentclipoutside(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.boolval = (gs_currentclipoutside(igs)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    return 0;
}

const op_def *zpath_op_defs(void)
{
    static const op_def op_defs_[] = { {"0clip", zclip}, {"0closepath", zclosepath}, {"0.currentclipoutside", zcurrentclipoutside}, {"0currentpoint", zcurrentpoint}, {"6curveto", zcurveto}, {"0eoclip", zeoclip}, {"0initclip", zinitclip}, {"2lineto", zlineto}, {"2moveto", zmoveto}, {"0newpath", znewpath}, {"6rcurveto", zrcurveto}, {"2rlineto", zrlineto}, {"2rmoveto", zrmoveto}, {"1.setclipoutside", zsetclipoutside}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
