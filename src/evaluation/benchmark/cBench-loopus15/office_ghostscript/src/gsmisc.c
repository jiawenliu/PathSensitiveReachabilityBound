...
char gs_debug[128];
FILE *gs_debug_out;
void eprintf_program_name(FILE * f, const char *program_name)
{
...
} int gs_log_error(int err, const char *file, int line)
{
    if (gs_debug['#']) {
	if (file == ((void *) 0))
	    fprintf(gs_stderr, "Returning error %d.\n", err);
	else
	    fprintf(gs_stderr, "%s(%d): Returning error %d.\n", (const char *) file, line, err);
    }
    return err;
}
...
int gs_return_check_interrupt(int code)
{
	...
	return code; {
	int icode = 0;
	return (icode == 0 ? 0 : gs_log_error((icode > 0 ? (-6) : icode), "gsmisc.c", 72));
	}
}

void *gs_realloc(void *old_ptr, size_t old_size, size_t new_size)
{
    void *new_ptr;
    if (new_size) {
...
void debug_dump_bytes(const byte * from, const byte * to, const char *msg)
{
	...
	fprintf(gs_stderr, "0x%lx:", (ulong) p);
	while (p != q)
	    fprintf(gs_stderr, " %02x", *p++);
	fprintf(gs_stderr, "%c", '\n');
    }
}

void debug_dump_bitmap(const byte * bits, uint raster, uint height, const char *msg)
{
    uint y;
    const byte *data = bits;
    for (y = 0; y < height; ++y, data += raster)
	debug_dump_bytes(data, data + raster, (y == 0 ? msg : ((void *) 0)));
} void debug_print_string(const byte * chrs, uint len)
{
    uint i;
    for (i = 0; i < len; i++)
	fprintf(gs_stderr, "%c", chrs[i]);
    fflush(gs_stderr);
}
...
int set_fmul2fixed_(fixed * pr, long a, long b)
{
    ulong ma = ((ushort) ((a) >> 8)) | 0x8000;
    ulong mb = ((ushort) ((b) >> 8)) | 0x8000;
    int e = 260 + 12 - (((((uint) ((ulong) a >> 16)) & 0x7f80) + (((uint) ((ulong) b >> 16)) & 0x7f80)) >> 7);
    ulong p1 = ma * (b & 0xff);
    ulong p = (ulong) ma * mb;
    if ((byte) a) {
	ulong p2 = (a & 0xff) * mb;
	...
	p += p1 >> 8;
    if ((uint) e < (((int) (sizeof(p))) * 8))
	p >>= e;
    else if (e >= (((int) (sizeof(p))) * 8)) {
	*pr = 0L;
	return 0;
    } else if (e >= -((((int) (sizeof(p))) * 8) - 1) || p >= 1L << ((((int) (sizeof(p))) * 8) - 1 + e))
	return gs_log_error((-13), "gsmisc.c", 296);
    else
	p <<= -e;
    *pr = ((a ^ b) < 0 ? -p : p);
    return 0;
...
int set_dfmul2fixed_(fixed * pr, ulong xalo, long b, long xahi)
{
    return set_fmul2fixed_(pr, (xahi & 0xc0000000) + (((xahi) << 3) & 0x3ffffff8) + ((xalo) >> 29), b);
}

extern double sqrt(double);
double gs_sqrt(double x, const char *file, int line)
{
    if ((('~') >= 'a' && ('~') <= 'z' ? gs_debug['~'] | gs_debug[('~') ^ 32] : gs_debug['~'])) {
	fprintf(gs_stdout, "[~]sqrt(%g) at %s:%d\n", x, (const char *) file, line);
	fflush(gs_stdout);
    }
    return sqrt(x);
}
...
double gs_sin_degrees(double ang)
{
    double quot = ang / 90;
    if (floor(quot) == quot) {
	int quads = (int) fmod(quot, 4) & 3;
	return isincos[quads];
    }
    return sin(ang * (3.14159265358979323846 / 180));
}

double gs_cos_degrees(double ang)
{
    double quot = ang / 90;
    if (floor(quot) == quot) {
	int quads = (int) fmod(quot, 4) & 3;
...
void gs_sincos_degrees(double ang, gs_sincos_t * psincos)
{
    double quot = ang / 90;
    if (floor(quot) == quot) {
	int quads = (int) fmod(quot, 4) & 3;
