...
static int s_file_available(stream *, long *), s_file_read_seek(stream *, long), s_file_read_close(stream *), s_file_read_process(stream_state *, stream_cursor_read *, stream_cursor_write *, bool);
static int s_file_write_seek(stream *, long), s_file_write_flush(stream *), s_file_write_close(stream *), s_file_write_process(stream_state *, stream_cursor_read *, stream_cursor_write *, bool);
static int s_file_switch(stream *, bool);
void sread_file(register stream * s, FILE * file, byte * buf, uint len)
{
    static const stream_procs p = { s_file_available, s_file_read_seek, s_std_read_reset, s_std_read_flush, s_file_read_close, s_file_read_process, s_file_switch };
    int had_error = ferror(file);
    long curpos = ftell(file);
    bool seekable = (curpos != -1L && fseek(file, curpos, 0) == 0);
    if (!had_error)
	clearerr(file);
    s_std_init(s, buf, len, &p, (seekable ? 1 + 4 : 1));
    do {
    } while (0);
    s->file = file;
...
static int s_file_available(register stream * s, long *pl)
{
...
static int s_file_read_seek(register stream * s, long pos)
{
...
static int s_file_read_close(stream * s)
{
    FILE *file = s->file;
    if (file != 0) {
	s->file = 0;
...
{
    static const stream_procs p = { s_std_noavailable, s_file_write_seek, s_std_write_reset, s_file_write_flush, s_file_write_close, s_file_write_process, s_file_switch };
    s_std_init(s, buf, len, &p, (file == stdout ? 2 : 2 + 4));
    do {
    } while (0);
    s->file = file;
    s->file_modes = s->modes;
...
void sappend_file(register stream * s, FILE * file, byte * buf, uint len)
{
...
} static int s_file_write_seek(stream * s, long pos)
{
...
static int s_file_write_flush(register stream * s)
{
...
static int s_file_write_close(register stream * s)
{
    s_process_write_buf(s, ((bool) 1));
    return s_file_read_close(s);
}

static int s_file_read_process(stream_state * st, stream_cursor_read * ignore_pr, stream_cursor_write * pw, bool last)
{
    ...
    return (ferror(file) ? ((int) (-2)) : feof(file) ? ((int) (-1)) : 1);
} static int s_file_write_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * ignore_pw, bool last)
{
...
static int s_file_switch(stream * s, bool writing)
{
