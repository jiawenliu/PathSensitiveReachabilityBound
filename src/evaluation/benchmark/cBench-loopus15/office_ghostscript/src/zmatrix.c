...
static int common_transform(os_ptr, int (*)(gs_state *, floatp, floatp, gs_point *), int (*)(floatp, floatp, const gs_matrix *, gs_point *));
static int zinitmatrix(os_ptr op)
{
    return gs_initmatrix(igs);
}

static int zdefaultmatrix(register os_ptr op)
{
    gs_matrix mat;
...
static int zcurrentmatrix(register os_ptr op)
{
    gs_matrix mat;
...
static int zsetmatrix(register os_ptr op)
{
    gs_matrix mat;
...
static int zsetdefaultmatrix(register os_ptr op)
{
    int code;
    if (((((const byte *) &((op)->tas.type_attrs))[1]) == (t_null)))
	code = gs_setdefaultmatrix(igs, ((void *) 0));
    else {
	gs_matrix mat;
...
static int ztranslate(register os_ptr op)
{
    int code;
    float trans[2];
	...
	if (op < (o_stack.bot) + ((2) - 1))
	    return ((-17));
	if ((code = num_params(op - 1, 2, trans)) < 0 || (code = gs_make_translation(trans[0], trans[1], &mat)) < 0 || (code = write_matrix(op, &mat)) < 0) {
...
static int zscale(register os_ptr op)
{
    int code;
    float scale[2];
	...
	if (op < (o_stack.bot) + ((2) - 1))
	    return ((-17));
	if ((code = num_params(op - 1, 2, scale)) < 0 || (code = gs_make_scaling(scale[0], scale[1], &mat)) < 0 || (code = write_matrix(op, &mat)) < 0) {
	    if (op < (o_stack.bot) + ((3) - 1))
		return ((-17));
	    return code;
	}
...
static int zrotate(register os_ptr op)
{
    int code;
    float ang;
	...
	if (op < (o_stack.bot) + ((1) - 1))
	    return ((-17));
	if ((code = num_params(op - 1, 1, &ang)) < 0 || (code = gs_make_rotation(ang, &mat)) < 0 || (code = write_matrix(op, &mat)) < 0) {
	    if (op < (o_stack.bot) + ((2) - 1))
		return ((-17));
	    return code;
	}
...
static int zconcat(register os_ptr op)
{
    gs_matrix mat;
    int code = read_matrix(op, &mat);
    if (code < 0)
...
static int zconcatmatrix(register os_ptr op)
{
    gs_matrix m1, m2, mp;
    int code;
    if ((code = read_matrix(op - 2, &m1)) < 0 || (code = read_matrix(op - 1, &m2)) < 0 || (code = gs_matrix_multiply(&m1, &m2, &mp)) < 0 || (code = write_matrix(op, &mp)) < 0)
	return code;
    op[-2] = *op;
    ((o_stack.p) -= (2));
    return code;
...
static int ztransform(register os_ptr op)
{
    return common_transform(op, gs_transform, gs_point_transform);
}

static int zdtransform(register os_ptr op)
{
    return common_transform(op, gs_dtransform, gs_distance_transform);
}

static int zitransform(register os_ptr op)
{
    return common_transform(op, gs_itransform, gs_point_transform_inverse);
}

static int zidtransform(register os_ptr op)
{
    return common_transform(op, gs_idtransform, gs_distance_transform_inverse);
}

static int common_transform(register os_ptr op, int (*ptproc) (gs_state *, floatp, floatp, gs_point *), int (*matproc) (floatp, floatp, const gs_matrix *, gs_point *))
{
    float opxy[2];
    gs_point pt;
    int code;
    ...
    case t_mixedarray:{
	    gs_matrix mat;
	    gs_matrix *pmat = &mat;
	    if ((code = read_matrix(op, pmat)) < 0 || (code = num_params(op - 1, 2, opxy)) < 0 || (code = (*matproc) (opxy[0], opxy[1], pmat, &pt)) < 0) {
		if (op < (o_stack.bot) + ((3) - 1))
		    return ((-17));
		return code;
	    }
	...
	return code;
  out:((op - 1)->value.realval = (pt.x), ((op - 1)->tas.type_attrs = ((t_real) << 8) + (0)));
    ((op)->value.realval = (pt.y), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
    return 0;
}

static int zinvertmatrix(register os_ptr op)
{
    gs_matrix m;
    int code;
    if ((code = read_matrix(op - 1, &m)) < 0 || (code = gs_matrix_invert(&m, &m)) < 0 || (code = write_matrix(op, &m)) < 0)
	return code;
    op[-1] = *op;
    ((o_stack.p) -= (1));
    return code;
...
const op_def *zmatrix_op_defs(void)
{
    static const op_def op_defs_[] = { {"1concat", zconcat}, {"2dtransform", zdtransform}, {"3concatmatrix", zconcatmatrix}, {"1currentmatrix", zcurrentmatrix}, {"1defaultmatrix", zdefaultmatrix}, {"2idtransform", zidtransform}, {"0initmatrix", zinitmatrix}, {"2invertmatrix", zinvertmatrix}, {"2itransform", zitransform}, {"1rotate", zrotate}, {"2scale", zscale}, {"1setmatrix", zsetmatrix}, {"1.setdefaultmatrix", zsetdefaultmatrix}, {"2transform", ztransform}, {"2translate", ztranslate}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
