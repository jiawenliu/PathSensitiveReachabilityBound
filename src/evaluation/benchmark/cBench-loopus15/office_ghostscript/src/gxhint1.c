...
static void compute_snaps(const gs_matrix_fixed *, const a_stem_table *, stem_snap_table *, int, int, const char *);
static alignment_zone *compute_zones(const gs_matrix_fixed *, const font_hints *, const a_zone_table *, const a_zone_table *, alignment_zone *, int);
static int transform_zone(const gs_matrix_fixed *, const font_hints *, const float *, alignment_zone *);
void reset_font_hints(font_hints * pfh, const gs_log2_scale_point * plog2_scale)
{
    (&pfh->scale.x)->unit = ((&pfh->scale.x)->half = ((1L << 12) >> 1) << ((&pfh->scale.x)->log2_unit = plog2_scale->x)) << 1;
    (&pfh->scale.y)->unit = ((&pfh->scale.y)->half = ((1L << 12) >> 1) << ((&pfh->scale.y)->log2_unit = plog2_scale->y)) << 1;
    pfh->axes_swapped = pfh->x_inverted = pfh->y_inverted = ((bool) 0);
    pfh->use_x_hints = pfh->use_y_hints = ((bool) 0);
    pfh->snap_h.count = pfh->snap_v.count = 0;
    pfh->a_zone_count = 0;
} void compute_font_hints(font_hints * pfh, const gs_matrix_fixed * pmat, const gs_log2_scale_point * plog2_scale, const gs_type1_data * pdata)
{
    alignment_zone *zp = &pfh->a_zones[0];
    reset_font_hints(pfh, plog2_scale);
    if (((pmat->xy) == 0.0))
	pfh->y_inverted = ((pmat->yy) < 0.0), pfh->use_y_hints = ((bool) 1);
    else if (((pmat->xx) == 0.0))
	pfh->y_inverted = ((pmat->xy) < 0.0), pfh->axes_swapped = ((bool) 1), pfh->use_y_hints = ((bool) 1);
    if (((pmat->yx) == 0.0))
	pfh->x_inverted = ((pmat->xx) < 0.0), pfh->use_x_hints = ((bool) 1);
    else if (((pmat->yy) == 0.0))
	pfh->x_inverted = ((pmat->yx) < 0.0), pfh->axes_swapped = ((bool) 1), pfh->use_x_hints = ((bool) 1);
    do {
    } while (0);
    do {
    } while (0);
    if (pfh->use_x_hints) {
	compute_snaps(pmat, (const a_stem_table *) &pdata->StdHW, &pfh->snap_h, 0, pfh->axes_swapped, "h");
	compute_snaps(pmat, (const a_stem_table *) &pdata->StemSnapH, &pfh->snap_h, 0, pfh->axes_swapped, "h");
    }
    if (pfh->use_y_hints) {
	gs_fixed_point vw;
	fixed *vp = (pfh->axes_swapped ? &vw.x : &vw.y);
	pixel_scale *psp = (pfh->axes_swapped ? &pfh->scale.x : &pfh->scale.y);
	if (gs_distance_transform2fixed(pmat, 0.0, (float) pdata->BlueFuzz, &vw) < 0)
	    vw.x = vw.y = 0L;
	pfh->blue_fuzz = ((*vp) < 0 ? -(*vp) : (*vp));
	if (gs_distance_transform2fixed(pmat, 0.0, 1.0, &vw) < 0)
	    vw.x = vw.y = 0L;
	pfh->suppress_overshoot = ((((*vp) < 0 ? -(*vp) : (*vp)) >> psp->log2_unit) * (1.0 / (1 << 12))) < pdata->BlueScale;
	if (gs_distance_transform2fixed(pmat, 0.0, pdata->BlueShift, &vw) < 0)
	    vw.x = vw.y = 0L;
	pfh->blue_shift = ((*vp) < 0 ? -(*vp) : (*vp));
	if (pfh->blue_shift > psp->half)
	    pfh->blue_shift = psp->half;
	do {
	} while (0);
	zp = compute_zones(pmat, pfh, (const a_zone_table *) &pdata->BlueValues, (const a_zone_table *) &pdata->FamilyBlues, zp, 1);
	zp = compute_zones(pmat, pfh, (const a_zone_table *) &pdata->OtherBlues, (const a_zone_table *) &pdata->FamilyOtherBlues, zp, 5);
	compute_snaps(pmat, (const a_stem_table *) &pdata->StdVW, &pfh->snap_v, 1, !pfh->axes_swapped, "v");
	compute_snaps(pmat, (const a_stem_table *) &pdata->StemSnapV, &pfh->snap_v, 1, !pfh->axes_swapped, "v");
    }
    pfh->a_zone_count = zp - &pfh->a_zones[0];
} static void compute_snaps(const gs_matrix_fixed * pmat, const a_stem_table * pst, stem_snap_table * psst, int from_y, int to_y, const char *tname)
{
    gs_fixed_point wxy;
	...
	int code = (from_y ? gs_distance_transform2fixed(pmat, 0.0, w, &wxy) : gs_distance_transform2fixed(pmat, w, 0.0, &wxy));
	if (code < 0)
	    continue;
	psst->data[j] = ((*wp) < 0 ? -(*wp) : (*wp));
	do {
	} while (0);
	j++;
    }
...
static alignment_zone *compute_zones(const gs_matrix_fixed * pmat, const font_hints * pfh, const a_zone_table * blues, const a_zone_table * family_blues, alignment_zone * zp, int bottom_count)
{
    int i;
    fixed fuzz = pfh->blue_fuzz;
    int inverted = (pfh->axes_swapped ? pfh->x_inverted : pfh->y_inverted);
	...
	if (i < family_blues->count) {
	    ...
	    fixed unit = (pfh->axes_swapped ? pfh->scale.x.unit : pfh->scale.y.unit);
	    fixed diff;
...
static int transform_zone(const gs_matrix_fixed * pmat, const font_hints * pfh, const float *vp, alignment_zone * zp)
{
    gs_fixed_point p0, p1;
    fixed v0, v1;
    int code;
    if ((code = gs_point_transform2fixed(pmat, 0.0, vp[0], &p0)) < 0 || (code = gs_point_transform2fixed(pmat, 0.0, vp[1], &p1)) < 0)
	return code;
    if (pfh->axes_swapped)
	v0 = p0.x, v1 = p1.x;
    else
