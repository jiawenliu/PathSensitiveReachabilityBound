...
static const char *standard_font_names[] = { "Courier", "Courier-Bold", "Courier-Oblique", "Courier-BoldOblique", "Helvetica", "Helvetica-Bold", "Helvetica-Oblique", "Helvetica-BoldOblique", "Symbol", "Times-Roman", "Times-Bold", "Times-Italic", "Times-BoldItalic", "ZapfDingbats", 0 };
...
int pdfshow_process(gx_device_pdf * pdev, const gs_param_dict * ptd)
{
    ...
    bool re_encode = ((bool) 0);
    float size;
    byte strbuf[200];
    if ((code = (*((ptd->list))->procs->xmit_string) ((ptd->list), "String", &str)) || (code = (*((ptd->list))->procs->xmit_float_array) ((ptd->list), "Values", &va)) || va.size != 7 || (code = (*((ptd->list))->procs->xmit_float_array) ((ptd->list), "Matrix", &ma)) || ma.size != 6 || (code = (*((ptd->list))->procs->xmit_string) ((ptd->list), "FontName", &fnstr)) || (code = (*((ptd->list))->procs->xmit_name_array) ((ptd->list), "Encoding", &ea)) || ea.size != 256 || (code = (*((ptd->list))->procs->xmit_name_array) ((ptd->list), "BaseEncoding", &bea)) || bea.size != 256)
	return ((-15));
    if (va.data[1] != 0 || ((int) va.data[2] != 32 && va.data[0] != 0) || va.data[4] != 0)
	return ((-21)); {
	uint i;
	...
	for (ppf = (pdf_font *) pdev->resources[resourceFont]; ppf != 0; ppf = ppf->next)
	    if (!bytes_compare(ppf->fname.data, ppf->fname.size, fnstr.data, fnstr.size))
		break;
    size = ((*(const gs_matrix *) ma.data).xx != 0 ? (*(const gs_matrix *) ma.data).xx / sx : 1);
	if (ppf == 0) {
	    const char **ppfn;
	    for (ppfn = standard_font_names; *ppfn; ++ppfn)
		if (strlen(*ppfn) == fnstr.size && !strncmp(*ppfn, (const char *) fnstr.data, fnstr.size))
		    break;
	    if (!*ppfn)
		return ((-21));
	    code = pdf_begin_resource(pdev, resourceFont, (pdf_resource **) & ppf);
	    if (code < 0)
		return ((-21));
	...
	gs_matrix tmat;
	gs_point spacing;
	tmat.xx = (*(const gs_matrix *) ma.data).xx / sx;
	tmat.xy = (*(const gs_matrix *) ma.data).xy / sy;
	tmat.yx = (*(const gs_matrix *) ma.data).yx / sx;
	tmat.yy = (*(const gs_matrix *) ma.data).yy / sy;
	tmat.tx = (va.data[5] + (*(const gs_matrix *) ma.data).tx) / pdev->scale.x;
	tmat.ty = (va.data[6] + (*(const gs_matrix *) ma.data).ty) / pdev->scale.y;
	pprintg2(s, pprintg4(s, "%g %g %g %g %g %g Tm\n", tmat.xx, tmat.xy, tmat.yx, tmat.yy), tmat.tx, tmat.ty);
	gs_distance_transform_inverse(va.data[3], va.data[4], &tmat, &spacing); {
	...
	gs_distance_transform_inverse(va.data[0], va.data[1], &tmat, &spacing); {
	...
	do {
	    w.ptr = buf - 1;
	    status = (*s_PSSE_template.process) (((void *) 0), &r, &w, ((bool) 1));
