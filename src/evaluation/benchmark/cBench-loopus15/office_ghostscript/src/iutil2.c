...
int param_read_password(gs_param_list * plist, const char *kstr, password * ppass)
{
    gs_param_string ps;
    long ipass;
    int code;
    ps.data = (const byte *) ppass->data, ps.size = ppass->size, ps.persistent = ((bool) 0);
    code = (*(plist)->procs->xmit_string) (plist, kstr, &ps);
    switch (code) {
    case 0:
	if (ps.size > 64)
	    return ((-13));
    ...
    if (code != (-20))
	return code;
    code = (*(plist)->procs->xmit_long) (plist, kstr, &ipass);
    if (code != 0)
	return code;
...
int param_write_password(gs_param_list * plist, const char *kstr, const password * ppass)
{
    gs_param_string ps;
    ps.data = (const byte *) ppass->data, ps.size = ppass->size, ps.persistent = ((bool) 0);
    if (ps.size > 64)
	return ((-13));
    return (*(plist)->procs->xmit_string) (plist, kstr, &ps);
}

int param_check_password(gs_param_list * plist, const password * ppass)
{
    if (ppass->size != 0) {
	password pass;
	...
	if (code)
	    return code;
	if (pass.size != ppass->size || bytes_compare(&pass.data[0], pass.size, &ppass->data[0], ppass->size) != 0)
	    return 1;
    }
    return 0;
}
