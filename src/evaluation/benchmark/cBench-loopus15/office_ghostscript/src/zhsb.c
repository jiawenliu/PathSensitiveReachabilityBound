...
static int zcurrenthsbcolor(register os_ptr op)
{
    float par[3];
...
static int zsethsbcolor(register os_ptr op)
{
    float par[3];
    int code;
    if ((code = num_params(op, 3, par)) < 0 || (code = gs_sethsbcolor(igs, par[0], par[1], par[2])) < 0)
	return code;
...
const op_def *zhsb_op_defs(void)
{
    static const op_def op_defs_[] = { {"0currenthsbcolor", zcurrenthsbcolor}, {"3sethsbcolor", zsethsbcolor}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
