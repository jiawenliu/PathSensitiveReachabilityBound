...
static int zcurrentalpha(register os_ptr op)
{
    ...
    ((op)->value.realval = (gs_currentalpha(igs)), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
    return 0;
}

static int zcurrentgray(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.realval = (gs_currentgray(igs)), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
    return 0;
}

static int zcurrentrgbcolor(register os_ptr op)
{
    float par[3];
...
static int zcurrenttransfer(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
...
static int zprocesscolors(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.intval = (gs_currentdevice(igs)->color_info.num_components), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
    return 0;
}

static int zsetalpha(register os_ptr op)
{
    float alpha;
    int code;
...
static int zsetgray(register os_ptr op)
{
    float gray;
    int code;
...
static int zsetrgbcolor(register os_ptr op)
{
    float par[3];
    int code;
    if ((code = num_params(op, 3, par)) < 0 || (code = gs_setrgbcolor(igs, par[0], par[1], par[2])) < 0)
	return code;
    (((&((int_gstate *) ((igs)->client_data))->colorspace.array)->tas.type_attrs = ((t_null) << 8) + (0)));
    ((o_stack.p) -= (3));
    return 0;
...
static int zsettransfer(register os_ptr op)
{
    int code;
    ...
    ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.red = ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.green = ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.blue = ((int_gstate *) ((igs)->client_data))->transfer_procs.colored.gray = *op;
    code = gs_settransfer_remap(igs, gs_mapped_transfer, ((bool) 0));
    if (code < 0)
	return code;
    (++(e_stack.p), (((e_stack.p))->value.opproc = (zcolor_reset_transfer), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    ((o_stack.p) -= (1));
    op--;
    return zcolor_remap_one(&((int_gstate *) ((igs)->client_data))->transfer_procs.colored.gray, op, igs->set_transfer.colored.gray, igs, zcolor_remap_one_finish);
}

int zcolor_remap_one(const ref * pproc, register os_ptr op, gx_transfer_map * pmap, const gs_state * pgs, int (*finish_proc) (os_ptr))
{
    (o_stack.p) = op += 4;
    ((op - 3)->value.intval = (0), ((op - 3)->tas.type_attrs = ((t_integer) << 8) + (0)));
    ((op - 2)->value.intval = (1), ((op - 2)->tas.type_attrs = ((t_integer) << 8) + (0)));
    ((op - 1)->value.intval = ((1 << 8) - 1), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)));
    *op = *pproc;
    ++(e_stack.p);
    (((e_stack.p))->value.pstruct = ((obj_header_t *) (pmap)), (((e_stack.p))->tas.type_attrs = ((t_struct) << 8) + (imemory_space((gs_ref_memory_t *) pgs->memory))));
    (++(e_stack.p), (((e_stack.p))->value.opproc = (finish_proc), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    (++(e_stack.p), (((e_stack.p))->value.opproc = (zfor_fraction), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    return 5;
}

static int zcolor_remap_one_store(os_ptr op, floatp min_value)
{
    int i;
    gx_transfer_map *pmap = ((gx_transfer_map *) (((e_stack.p))->value.pstruct));
    if (ref_stack_count(&o_stack) < (1 << 8))
	return ((-17));
    for (i = 0; i < (1 << 8); i++) {
	float v;
	int code = real_param(ref_stack_index(&o_stack, (1 << 8) - 1 - i), &v);
	if (code < 0)
	    return code;
	pmap->values[i] = (v < min_value ? ((frac) (((min_value) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))) : v >= 1.0 ? ((frac) 0x7ff8) : ((frac) (((v) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))));
    } ref_stack_pop(&o_stack, (1 << 8));
    (e_stack.p)--;
    return 14;
}

int zcolor_remap_one_finish(os_ptr op)
{
    return zcolor_remap_one_store(op, 0.0);
}

int zcolor_remap_one_signed_finish(os_ptr op)
{
    return zcolor_remap_one_store(op, -1.0);
}

int zcolor_reset_transfer(os_ptr op)
{
    gx_set_effective_transfer(igs);
    return zcolor_remap_color(op);
}

int zcolor_remap_color(os_ptr op)
{
    (((igs)->dev_color)->type = (&gx_dc_procs_none));
    return 0;
}

const op_def *zcolor_op_defs(void)
{
    static const op_def op_defs_[] = { {"0currentalpha", zcurrentalpha}, {"0currentgray", zcurrentgray}, {"0currentrgbcolor", zcurrentrgbcolor}, {"0currenttransfer", zcurrenttransfer}, {"0processcolors", zprocesscolors}, {"1setalpha", zsetalpha}, {"1setgray", zsetgray}, {"3setrgbcolor", zsetrgbcolor}, {"1settransfer", zsettransfer}, {"1%zcolor_remap_one_finish", zcolor_remap_one_finish}, {"1%zcolor_remap_one_signed_finish", zcolor_remap_one_signed_finish}, {"0%zcolor_reset_transfer", zcolor_reset_transfer}, {"0%zcolor_remap_color", zcolor_remap_color}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
