...
int alloc_save_change(gs_dual_memory_t *, const ref * pcont, ref_packed * ptr, client_name_t cname);
int alloc_save_level(const gs_dual_memory_t *);
static int make_font(os_ptr, const gs_matrix *);
static void make_uint_array(os_ptr, const uint *, int);
gs_font_dir *ifont_dir = 0;
static gs_gc_root_t font_dir_root;
bool zfont_mark_glyph_name(gs_glyph glyph, void *ignore_data)
{
    return (glyph >= ((gs_glyph) 0x80000000) || glyph == ((gs_glyph) 0x7fffffff) ? ((bool) 0) : name_mark_index((uint) glyph));
}

static void zfont_init(void)
{
    ifont_dir = gs_font_dir_alloc(((gs_memory_t *) (gs_imemory.current)));
    ifont_dir->ccache.mark_glyph = zfont_mark_glyph_name;
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &font_dir_root, (&ptr_struct_procs), (void **) &ifont_dir, "ifont_dir");
} static int zscalefont(register os_ptr op)
{
    int code;
    float scale;
    gs_matrix mat;
...
static int zmakefont(register os_ptr op)
{
    int code;
    gs_matrix mat;
...
int zsetfont(register os_ptr op)
{
    gs_font *pfont;
...
static int zcurrentfont(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    *op = *(&((font_data *) ((gs_currentfont(igs))->client_data))->dict);
    return 0;
}

static int zcachestatus(register os_ptr op)
{
    uint status[7];
...
static int zsetcachelimit(register os_ptr op)
{
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
    if ((ulong) (*op).value.intval > (((unsigned int) 0xffffffff + (unsigned int) 0)))
	return ((-15));
    gs_setcacheupper(ifont_dir, (uint) op->value.intval);
    ((o_stack.p) -= (1));
    return 0;
}

static int zsetcacheparams(register os_ptr op)
{
    uint params[3];
    int i, code;
    os_ptr opp = op;
    for (i = 0; i < 3 && !((((const byte *) &((opp)->tas.type_attrs))[1]) == (t_mark)); i++, opp--) {
	if (!((((const byte *) &((&*opp)->tas.type_attrs))[1]) == (t_integer)))
	    return (check_type_failed(&*opp));
	if ((ulong) (*opp).value.intval > (((unsigned int) 0xffffffff + (unsigned int) 0)))
	    return ((-15));
...
static int zcurrentcacheparams(register os_ptr op)
{
    uint params[3];
...
const op_def *zfont_op_defs(void)
{
    static const op_def op_defs_[] = { {"0currentfont", zcurrentfont}, {"2makefont", zmakefont}, {"2scalefont", zscalefont}, {"1setfont", zsetfont}, {"0cachestatus", zcachestatus}, {"1setcachelimit", zsetcachelimit}, {"1setcacheparams", zsetcacheparams}, {"0currentcacheparams", zcurrentcacheparams}, {(char *) 0, (op_proc_p) zfont_init} };
    return op_defs_;
}
...
int font_param(const ref * pfdict, gs_font ** ppfont)
{
    ...
    if (!((((const byte *) &((&*pfdict)->tas.type_attrs))[1]) == (t_dictionary)))
	return (check_type_failed(&*pfdict));
    if (dict_find_string(pfdict, "FID", &pid) <= 0 || !((((const byte *) &((pid)->tas.type_attrs))[1]) == (t_fontID)))
	return ((-10));
...
int add_FID(ref * fp, gs_font * pfont)
{
    ref fid;
    ((&fid)->value.pstruct = ((void *) pfont), ((&fid)->tas.type_attrs = ((t_fontID) << 8) + (((0x20 + 0x40) | (((&gs_imemory))->current_space)) | ((&gs_imemory)->new_mask))));
    return dict_put_string(fp, "FID", &fid);
}

static int make_font(os_ptr op, const gs_matrix * pmat)
{
    os_ptr fp = op - 1;
	...
	ialloc_set_space((&gs_imemory), (avm_space) (((fp)->tas.type_attrs) & (((1 << 2) - 1) << 2)));
	if (dict_find_string(fp, "Encoding", &pencoding) > 0 && !(((pencoding)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
	    code = ((-10));
	else {
	    ref olddict;
	...
	if (code < 0)
	    return code;
    if (pencoding != 0 && !obj_eq(pencoding, &((font_data *) ((newfont)->client_data))->Encoding)) {
...
int zbase_make_font(gs_font_dir * pdir, const gs_font * oldfont, const gs_matrix * pmat, gs_font ** ppfont)
{
...
int zdefault_make_font(gs_font_dir * pdir, const gs_font * oldfont, const gs_matrix * pmat, gs_font ** ppfont)
{
    ...
    if (dlen < mlen)
	dlen = mlen;
    if ((pdata = (font_data *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_font_data, "make_font(font_data)")) == 0)
	return ((-25));
    if ((code = dict_create(dlen, &newdict)) < 0 || (code = dict_copy_entries(fp, &newdict, ((bool) 0))) < 0 || (code = gs_alloc_ref_array((gs_imemory.current), &newmat, (0x10 + 0x20 + 0x40), 12, "make_font(matrices)")) < 0)
	return code;
	...
	ref *ppsm;
	if (!(dict_find_string(fp, "ScaleMatrix", &ppsm) > 0 && read_matrix(ppsm, &prev_scale) >= 0 && gs_matrix_multiply(pmat, &prev_scale, &scale) >= 0))
	    scale = *pmat;
    ...
    if ((code = dict_put_string(&newdict, "FontMatrix", &newmat)) < 0 || (code = dict_put_string(&newdict, "OrigFont", (&((font_data *) ((oldfont->base)->client_data))->dict))) < 0 || (code = dict_put_string(&newdict, "ScaleMatrix", &scalemat)) < 0 || (code = add_FID(&newdict, newfont)) < 0)
	return code;
...
static void make_uint_array(register os_ptr op, const uint * intp, int count)
{
    int i;
...
static bool purge_if_name_removed(cached_char * cc, void *vsave)
{
...
void font_restore(const alloc_save_t * save)
{
	...
	return; {
	gs_font *pfont;
      otop:for (pfont = pdir->orig_fonts; pfont != 0; pfont = pfont->next) {
	...
	}
	{
	    cached_fm_pair *pair;
	    uint n;
	    for (pair = pdir->fmcache.mdata, n = pdir->fmcache.mmax; n > 0; pair++, n--)
		if (!((pair)->font == 0 && !((&(pair)->UID)->id != 0x1000000))) {
		    if ((pair->font != 0 && alloc_is_since_save((char *) pair->font, save)) || (((&pair->UID)->id < 0) && alloc_is_since_save((char *) pair->UID.xvalues, save)))
			gs_purge_fm_pair(pdir, pair, 0);
		    else if (pair->xfont != 0 && alloc_is_since_save((char *) pair->xfont, save))
			gs_purge_fm_pair(pdir, pair, 1);
		}
	} if (alloc_any_names_since_save(save))
	    gx_purge_selected_cached_chars(pdir, purge_if_name_removed, (void *) save);
}
