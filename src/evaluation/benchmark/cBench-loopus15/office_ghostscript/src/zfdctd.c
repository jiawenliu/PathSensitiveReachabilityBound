...
static int zDCTD(os_ptr op)
{
    ...
    const ref *dop;
    uint dspace;
    jddp = (void *) (*(&gs_memory_default)->procs.alloc_byte_array) (&gs_memory_default, 1, sizeof(*jddp), "zDCTD");
    if (jddp == 0)
	return ((-25));
    ...
    if ((code = zfdct_setup_huffman_tables(dop, &state, ((bool) 0))) < 0 || (code = zfdct_setup_quantization_tables(dop, &state, ((bool) 0))) < 0)
	goto fail;
    jddp->template = s_DCTD_template;
    code = filter_read(op, npop, &jddp->template, (stream_state *) & state, dspace);
    if (code >= 0)
	return code;
  fail:gs_jpeg_destroy(&state);
    (*(&gs_memory_default)->procs.free_object) (&gs_memory_default, jddp, "zDCTD fail");
    return code;
}

const op_def *zfdctd_op_defs(void)
{
    static const op_def op_defs_[] = { {"filterdict", 0}, {"2DCTDecode", zDCTD}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
