...
static int s_A85E_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * pw, bool last)
{
...
const stream_template s_A85E_template = { &st_stream_state, ((void *) 0), s_A85E_process, 4, 6 };
static const gs_memory_struct_type_t st_A85D_state = { sizeof(stream_A85D_state), "ASCII85Decode state", 0, 0, 0, 0, 0 };

static int s_A85D_init(stream_state * st)
{
    return ((((stream_A85D_state *) st))->word = 0, (((stream_A85D_state *) st))->odd = 0);
}

static int a85d_finish(int, ulong, stream_cursor_write *);
static int s_A85D_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * pw, bool last)
{
    register const byte *p = pr->ptr;
    register byte *q = pw->ptr;
    const byte *rlimit = pr->limit;
    byte *wlimit = pw->limit;
    int ccount = ((stream_A85D_state *) st)->odd;
    ulong word = ((stream_A85D_state *) st)->word;
    int status = 0;
    while (p < rlimit) {
	int ch = *++p;
	uint ccode = ch - '!';
    ...
    pr->ptr = p;
    ((stream_A85D_state *) st)->odd = ccount;
    ((stream_A85D_state *) st)->word = word;
    return status;
}

static int a85d_finish(int ccount, ulong word, stream_cursor_write * pw)
{
    byte *q = pw->ptr;
    int status = ((int) (-1));
    switch (ccount) {
...
const stream_template s_A85D_template = { &st_A85D_state, s_A85D_init, s_A85D_process, 2, 4 };
static const gs_memory_struct_type_t st_BT_state = { sizeof(stream_BT_state), "ByteTranslateEncode/Decode state", 0, 0, 0, 0, 0 };

static int s_BT_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * pw, bool last)
{
    const byte *p = pr->ptr;
    byte *q = pw->ptr;
    ...
    while (count--)
	*++q = ((stream_BT_state *) st)->table[*++p];
    pr->ptr = p;
    pw->ptr = q;
    return status;
}
const stream_template s_BT_template = { &st_BT_state, ((void *) 0), s_BT_process, 1, 1 };
