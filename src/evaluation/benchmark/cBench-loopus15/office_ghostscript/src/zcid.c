...
static int build_gs_cid_font(os_ptr op, font_type ftype, const build_proc_refs * pbuild)
{
    int code;
    gs_font_base *pfont;
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_dictionary)))
	return (check_type_failed(&*op));
    code = build_gs_simple_font(op, &pfont, ftype, &st_gs_font_base, pbuild, bf_Encoding_optional | bf_FontBBox_required | bf_UniqueID_ignored);
    if (code < 0)
	return code;
    return define_gs_font((gs_font *) pfont);
}

static int zbuildfont9(os_ptr op)
{
    build_proc_refs build;
    int code = build_proc_name_refs(&build, ((void *) 0), "%Type9BuildGlyph");
    if (code < 0)
	return code;
    return build_gs_cid_font(op, ft_CID_encrypted, &build);
}

static int zbuildfont10(os_ptr op)
{
    build_proc_refs build;
...
static int zbuildfont11(os_ptr op)
{
    return build_gs_TrueType_font(op, ft_CID_TrueType, (const char *) 0, "%Type11BuildGlyph", bf_Encoding_optional | bf_FontBBox_required | bf_UniqueID_ignored | bf_CharStrings_optional);
} const op_def *zcid_op_defs(void)
{
    static const op_def op_defs_[] = { {"2.buildfont9", zbuildfont9}, {"2.buildfont10", zbuildfont10}, {"2.buildfont11", zbuildfont11}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
