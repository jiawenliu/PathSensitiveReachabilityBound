...
static irender_proc_t image_strategy_interpolate(gx_image_enum * penum)
{
    ...
    iss.memory = mem;
    gs_distance_transform((float) penum->rect.w, (float) penum->rect.h, &penum->matrix, &dst_xy);
    if (penum->bps <= 8 && penum->device_color)
	iss.BitsPerComponentIn = 8, iss.MaxValueIn = 0xff;
    else
	iss.BitsPerComponentIn = sizeof(frac) * 8, iss.MaxValueIn = ((frac) 0x7ff8);
    ...
    iss.Colors = (*(pcs)->type->concrete_space) (pcs, pis)->type->num_components; {
	uint in_size = iss.WidthIn * iss.Colors * (iss.BitsPerComponentIn / 8);
	uint out_size = iss.WidthOut * iss.Colors * (((iss.BitsPerComponentOut / 8) > (sizeof(gx_color_index))) ? (iss.BitsPerComponentOut / 8) : (sizeof(gx_color_index)));
	line = (*(mem)->procs.alloc_bytes) (mem, (((in_size) > (out_size)) ? (in_size) : (out_size)), "image scale src line");
    }
    pss = (stream_IScale_state *) (*(mem)->procs.alloc_struct) (mem, &st_IScale_state, "image scale state");
    if (line == 0 || pss == 0 || (*pss = iss, (*s_IScale_template.init) ((stream_state *) pss) < 0)) {
	(*(mem)->procs.free_object) (mem, pss, "image scale state");
	(*(mem)->procs.free_object) (mem, line, "image scale src line");
	penum->interpolate = ((bool) 0);
	return 0;
    }
    penum->line = line;
    penum->scaler = pss;
    penum->line_xy = 0;
    penum->xyi.x = ((int) ((((((penum->dda.pixel0.x).state).Q) + (((1 << 12) >> 1) - 1L))) >> (12)));
    penum->xyi.y = ((int) ((((((penum->dda.pixel0.y).state).Q) + (((1 << 12) >> 1) - 1L))) >> (12)));
    do {
    } while (0);
    return image_render_interpolate;
}

void gs_gximage5_init(gs_memory_t * mem)
{
    image_strategies.interpolate = image_strategy_interpolate;
} static int image_render_interpolate(gx_image_enum * penum, const byte * buffer, int data_x, uint iw, int h, gx_device * dev)
{
	...
	if (pss->sizeofPixelIn == 1) {
		...
		if (bps <= 8)
			...
			switch (penum->map[j].decoding) {
			case sd_none:
			    cc.paint.values[j] = (*pdata) * (1.0 / 255.0);
			    break;
			case sd_lookup:
			    cc.paint.values[j] = penum->map[j].decode_lookup[(*pdata) >> 4];
			    break;
			case sd_compute:
			    cc.paint.values[j] = penum->map[j].decode_lookup[0] + (*pdata) * penum->map[j].decode_factor;
			};
		} else
		    for (j = 0; j < dc; pdata += sizeof(frac), ++j) {
			cc.paint.values[j] = penum->map[j].decode_lookup[0] + (*(const frac *) pdata) * penum->map[j].decode_factor;
	...
	int dy;
	const gs_color_space *pconcs = (*(pcs)->type->concrete_space) (pcs, pis);
	int bpp = dev->color_info.depth;
	uint raster = ((uint) (((width * bpp + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2));
	...
	for (;;) {
	    ...
	    int code;
	    byte *l_dst = (penum->line);
	    uint l_bits = 0;
	    int l_shift = 8 - (bpp);
	    int l_xprev = (xo);
	    w.limit = penum->line + width * c * sizeof(gx_color_index) - 1;
	    w.ptr = w.limit - width * c * (sizeof(gx_color_index) - pss->sizeofPixelOut);
	    psrc = (const frac *) (w.ptr + 1);
	    code = (*s_IScale_template.process) ((stream_state *) pss, &r, &w, ((bool) 0));
	    if (code < 0 && code != ((int) (-1)))
		return ((-12));
	    if (w.ptr == w.limit) {
		for (x = xo; x < xo + width; x++, psrc += c) {
		    (*pconcs->type->remap_concrete_color) (psrc, &devc, pis, dev, gs_color_select_source);
		    if (((&devc)->type == (&gx_dc_procs_pure))) {
			...
			if ((x) > l_xprev) {
			    ...
			    if (l_shift != 8 - (bpp))
				*l_dst = (byte) l_bits;
			    code = (*((dev)->std_procs.copy_color)) (dev, penum->line, l_xprev - (xo), raster, 0L, l_xprev, ry, (x) - l_xprev, 1);
			    if (code < 0)
				return code;
			};
			code = (*(&devc)->type->fill_rectangle) (&devc, x, ry, 1, 1, dev, lop, ((void *) 0));
			if (code < 0)
			    return code;
			l_xprev = x + 1;
		    }
