...
extern int gx_remap_DeviceCMYK(const gs_client_color *, const gs_color_space *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
extern int gx_concretize_DeviceCMYK(const gs_client_color *, const gs_color_space *, frac *, const gs_imager_state *);
extern int gx_remap_concrete_DCMYK(const frac *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
const gs_color_space_type gs_color_space_type_DeviceCMYK = { gs_color_space_index_DeviceCMYK, 4, ((bool) 1), gx_init_paint_4, gx_same_concrete_space, gx_concretize_DeviceCMYK, gx_remap_concrete_DCMYK, gx_remap_DeviceCMYK, gx_no_install_cspace, gx_no_adjust_cspace_count, gx_no_adjust_color_count, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs };

extern const gs_color_space *gs_cs_static_DeviceCMYK;
void gs_gscolor1_init(gs_memory_t * mem)
{
    gs_color_space *pcs = (gs_color_space *) (*(mem)->procs.alloc_struct) (mem, &st_color_space, "gs_cs_static_DeviceCMYK");
...
int gs_currentcmykcolor(const gs_state * pgs, float pr4[4])
{
    ...
    case gs_color_space_index_DeviceRGB:{
	    frac fcmyk[4];
	    color_rgb_to_cmyk(((frac) (((pcc->paint.values[0]) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))), ((frac) (((pcc->paint.values[1]) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))), ((frac) (((pcc->paint.values[2]) + 0.5 / ((float) ((frac) 0x7ff8))) * ((float) ((frac) 0x7ff8)))), (const gs_imager_state *) pgs, fcmyk);
	    pr4[0] = ((fcmyk[0]) / ((float) ((frac) 0x7ff8)));
	    pr4[1] = ((fcmyk[1]) / ((float) ((frac) 0x7ff8)));
	    pr4[2] = ((fcmyk[2]) / ((float) ((frac) 0x7ff8)));
	    pr4[3] = ((fcmyk[3]) / ((float) ((frac) 0x7ff8)));
	} break;
...
int gs_setblackgeneration(gs_state * pgs, gs_mapping_proc proc)
{
    return gs_setblackgeneration_remap(pgs, proc, ((bool) 1));
}

int gs_setblackgeneration_remap(gs_state * pgs, gs_mapping_proc proc, bool remap)
{
    do {
	if ((pgs->black_generation) == 0 || (pgs->black_generation)->rc.ref_count > 1 || (pgs->black_generation)->rc.memory != (pgs->memory)) {
	    gx_transfer_map *new;
	    do {
		if (((new) = (gx_transfer_map *) (*(pgs->memory)->procs.alloc_struct) (pgs->memory, &st_transfer_map, "gs_setblackgeneration")) == 0) {
		    return ((-25));
		} else {
		    (new)->rc.ref_count = 1;
	    ...
	    if (pgs->black_generation)
		(pgs->black_generation)->rc.ref_count--;
	    (pgs->black_generation) = new;
	}
    } while (0);
...
gs_mapping_proc gs_currentblackgeneration(const gs_state * pgs)
{
    return pgs->black_generation->proc;
}

int gs_setundercolorremoval(gs_state * pgs, gs_mapping_proc proc)
{
    return gs_setundercolorremoval_remap(pgs, proc, ((bool) 1));
}

int gs_setundercolorremoval_remap(gs_state * pgs, gs_mapping_proc proc, bool remap)
{
    do {
	if ((pgs->undercolor_removal) == 0 || (pgs->undercolor_removal)->rc.ref_count > 1 || (pgs->undercolor_removal)->rc.memory != (pgs->memory)) {
	    gx_transfer_map *new;
	    do {
		if (((new) = (gx_transfer_map *) (*(pgs->memory)->procs.alloc_struct) (pgs->memory, &st_transfer_map, "gs_setundercolorremoval")) == 0) {
		    return ((-25));
		} else {
		    (new)->rc.ref_count = 1;
		    (new)->rc.memory = pgs->memory;
	    ...
	    if (pgs->undercolor_removal)
		(pgs->undercolor_removal)->rc.ref_count--;
	    (pgs->undercolor_removal) = new;
	}
    } while (0);
...
gs_mapping_proc gs_currentundercolorremoval(const gs_state * pgs)
{
    return pgs->undercolor_removal->proc;
}

int gs_setcolortransfer_remap(gs_state * pgs, gs_mapping_proc red_proc, gs_mapping_proc green_proc, gs_mapping_proc blue_proc, gs_mapping_proc gray_proc, bool remap)
{
	...
	if ((ptran->gray) == 0 || (ptran->gray)->rc.ref_count > 1 || (ptran->gray)->rc.memory != (pgs->memory)) {
	    gx_transfer_map *new;
	    do {
		if (((new) = (gx_transfer_map *) (*(pgs->memory)->procs.alloc_struct) (pgs->memory, &st_transfer_map, "gs_setcolortransfer")) == 0) {
		    goto fgray;
		} else {
		    (new)->rc.ref_count = 1;
		    (new)->rc.memory = pgs->memory;
    ...
    do {
	if ((ptran->red) == 0 || (ptran->red)->rc.ref_count > 1 || (ptran->red)->rc.memory != (pgs->memory)) {
	    gx_transfer_map *new;
	    do {
		if (((new) = (gx_transfer_map *) (*(pgs->memory)->procs.alloc_struct) (pgs->memory, &st_transfer_map, "gs_setcolortransfer")) == 0) {
		    goto fred;
    ...
    do {
	if ((ptran->green) == 0 || (ptran->green)->rc.ref_count > 1 || (ptran->green)->rc.memory != (pgs->memory)) {
	    gx_transfer_map *new;
	    do {
		if (((new) = (gx_transfer_map *) (*(pgs->memory)->procs.alloc_struct) (pgs->memory, &st_transfer_map, "gs_setcolortransfer")) == 0) {
		    goto fgreen;
    ...
    do {
	if ((ptran->blue) == 0 || (ptran->blue)->rc.ref_count > 1 || (ptran->blue)->rc.memory != (pgs->memory)) {
	    gx_transfer_map *new;
	    do {
		if (((new) = (gx_transfer_map *) (*(pgs->memory)->procs.alloc_struct) (pgs->memory, &st_transfer_map, "gs_setcolortransfer")) == 0) {
		    goto fblue;
    ...
    return 0;
  fblue:do {
	if ((ptran->green) != (old.green)) {
	    do {
		if ((ptran->green) != 0 && !((ptran->green)->rc.ref_count += -1)) {
		    (*(ptran->green)->rc.free) ((ptran->green)->rc.memory, (void *) (ptran->green), "setcolortransfer");
		    do {
		    } while (0);
		}
	    } while (0);
	    (ptran->green) = (old.green);
	    do {
		if ((ptran->green) != 0)
		    (ptran->green)->rc.ref_count++;
	    } while (0);
  ...
  fgreen:do {
	if ((ptran->red) != (old.red)) {
	    do {
		if ((ptran->red) != 0 && !((ptran->red)->rc.ref_count += -1)) {
		    (*(ptran->red)->rc.free) ((ptran->red)->rc.memory, (void *) (ptran->red), "setcolortransfer");
		    do {
		    } while (0);
		}
	    } while (0);
  ...
  fred:do {
	if ((ptran->gray) != (old.gray)) {
	    do {
		if ((ptran->gray) != 0 && !((ptran->gray)->rc.ref_count += -1)) {
		    (*(ptran->gray)->rc.free) ((ptran->gray)->rc.memory, (void *) (ptran->gray), "setcolortransfer");
		    do {
		    } while (0);
		}
	    } while (0);
...
int gs_setcolortransfer(gs_state * pgs, gs_mapping_proc red_proc, gs_mapping_proc green_proc, gs_mapping_proc blue_proc, gs_mapping_proc gray_proc)
{
    return gs_setcolortransfer_remap(pgs, red_proc, green_proc, blue_proc, gray_proc, ((bool) 1));
}

void gs_currentcolortransfer(const gs_state * pgs, gs_mapping_proc procs[4])
{
