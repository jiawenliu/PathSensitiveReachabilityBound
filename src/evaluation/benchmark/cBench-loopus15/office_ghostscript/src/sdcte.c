...
static void dcte_init_destination(j_compress_ptr cinfo)
{
} static boolean dcte_empty_output_buffer(j_compress_ptr cinfo)
{
    return 0;
}

static void dcte_term_destination(j_compress_ptr cinfo)
{
} static int s_DCTE_init(stream_state * st)
{
    struct jpeg_destination_mgr *dest = &((stream_DCT_state *) st)->data.compress->destination;
    ...
    ((stream_DCT_state *) st)->data.compress->cinfo.dest = dest;
    ((stream_DCT_state *) st)->phase = 0;
    return 0;
}

static int s_DCTE_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * pw, bool last)
{
    jpeg_compress_data *jcdp = ((stream_DCT_state *) st)->data.compress;
    ...
    case 0:
	if (gs_jpeg_start_compress(((stream_DCT_state *) st), 1) < 0)
	    return ((int) (-2));
	pw->ptr = dest->next_output_byte - 1;
	((stream_DCT_state *) st)->phase = 1;
    case 1:
	if (pw->limit - pw->ptr < ((stream_DCT_state *) st)->Markers.size)
	    return 1;
	memcpy(pw->ptr + 1, ((stream_DCT_state *) st)->Markers.data, ((stream_DCT_state *) st)->Markers.size);
	pw->ptr += ((stream_DCT_state *) st)->Markers.size;
	((stream_DCT_state *) st)->phase = 2;
    case 2:
	if (!((stream_DCT_state *) st)->NoMarker) {
	    static const byte Adobe[] = { 0xFF, 0xE0 + 14, 0, 14, 'A', 'd', 'o', 'b', 'e', 0, 100, 0, 0, 0, 0, 0 };
	    if (pw->limit - pw->ptr < sizeof(Adobe))
		return 1;
	    memcpy(pw->ptr + 1, Adobe, sizeof(Adobe));
	    pw->ptr += sizeof(Adobe);
	    *pw->ptr = ((stream_DCT_state *) st)->ColorTransform;
	}
	dest->next_output_byte = pw->ptr + 1;
	dest->free_in_buffer = pw->limit - pw->ptr;
	...
	while (jcdp->cinfo.image_height > jcdp->cinfo.next_scanline) {
	    ...
	    if ((uint) (pr->limit - pr->ptr) < ((stream_DCT_state *) st)->scan_line_size) {
		if (last)
		    return ((int) (-2));
		return 0;
	    }
	    written = gs_jpeg_write_scanlines(((stream_DCT_state *) st), &samples, 1);
	    if (written < 0)
		return ((int) (-2));
	    pw->ptr = dest->next_output_byte - 1;
	    if (!written)
		return 1;
	    pr->ptr += ((stream_DCT_state *) st)->scan_line_size;
	}
	((stream_DCT_state *) st)->phase = 4;
    case 4:
	if (pw->limit - pw->ptr < 100)
	    return 1;
	if (gs_jpeg_finish_compress(((stream_DCT_state *) st)) < 0)
	    return ((int) (-2));
	pw->ptr = dest->next_output_byte - 1;
...
} static void s_DCTE_release(stream_state * st)
{
    gs_jpeg_destroy(((stream_DCT_state *) st));
    (*(&gs_memory_default)->procs.free_object) (&gs_memory_default, ((stream_DCT_state *) st)->data.compress, "s_DCTE_release");
    st->template = &s_DCTE_template;
} const stream_template s_DCTE_template = { &st_DCT_state, s_DCTE_init, s_DCTE_process, 1000, 4000, s_DCTE_release };
