...
extern void file_gc_prepare(void);
extern void dstack_gc_cleanup(void);
static void gs_vmreclaim(gs_dual_memory_t *, bool);
static int ireclaim(gs_dual_memory_t *, int);
static void ireclaim_init(void)
{
    gs_imemory.reclaim = ireclaim;
} static int ireclaim(gs_dual_memory_t * dmem, int space)
{
    bool global;
    gs_ref_memory_t *mem;
    if (space < 0) {
	...
	for (i = 0; i < (sizeof(dmem->spaces.indexed) / sizeof((dmem->spaces.indexed)[0])); ++i) {
	...
	mem = dmem->spaces.indexed[space >> 2];
    }
    do {
    } while (0);
...
static void set_ref_chunk(chunk_t *, ref *, ref *, gs_ref_memory_t *);
static void gs_vmreclaim(gs_dual_memory_t * dmem, bool global)
{
    gs_ref_memory_t *lmem = dmem->spaces.named.local;
    gs_ref_memory_t *gmem = dmem->spaces.named.global;
    gs_ref_memory_t *smem = dmem->spaces.named.system;
    struct ir_ {
	chunk_head_t head;
	obj_header_t prefix;
	ref refs[5 + 1];
    ...
    iroot_refs.refs[0 + 2] = ref_ref_stacks[0], *((ref_stack *) ((&iroot_refs.refs[0 + 2])->value.pstruct)) = d_stack;
    ref_stack_cleanup(&e_stack);
    iroot_refs.refs[1 + 2] = ref_ref_stacks[1], *((ref_stack *) ((&iroot_refs.refs[1 + 2])->value.pstruct)) = e_stack;
    ref_stack_cleanup(&o_stack);
    iroot_refs.refs[2 + 2] = ref_ref_stacks[2], *((ref_stack *) ((&iroot_refs.refs[2 + 2])->value.pstruct)) = o_stack;
    iroot_refs.refs[1] = ref_static_stacks;
    ((&iroot_refs.refs[0])->value.refs = (&iroot_refs.refs[1]), ((&iroot_refs.refs[0])->tas.type_attrs = ((t_array) << 8) + (avm_system)), ((&iroot_refs.refs[0])->tas.rsize = (4)));
    set_ref_chunk(&cir, &iroot_refs.refs[0], &iroot_refs.refs[5], smem);
    (*((gs_memory_t *) smem)->procs.register_root) ((gs_memory_t *) smem, &iroot, (&ptr_ref_procs), &piroot, "gs_gc_main");
    if (iroot_refs.refs[1].value.refs != ((void *) 0))
	set_ref_chunk(&css, iroot_refs.refs[1].value.refs, iroot_refs.refs[1].value.refs + ((&iroot_refs.refs[1])->tas.rsize), smem);
    file_gc_prepare();
    gs_reclaim(&dmem->spaces, global);
    if (iroot_refs.refs[1].value.refs != ((void *) 0))
	alloc_unlink_chunk(&css, smem);
    (*((gs_memory_t *) smem)->procs.unregister_root) ((gs_memory_t *) smem, &iroot, "gs_gc_main");
    alloc_unlink_chunk(&cir, smem);
    ref_ref_stacks[0].value.pstruct = iroot_refs.refs[0 + 2].value.pstruct, d_stack = *((ref_stack *) ((&iroot_refs.refs[0 + 2])->value.pstruct));
    ref_ref_stacks[1].value.pstruct = iroot_refs.refs[1 + 2].value.pstruct, e_stack = *((ref_stack *) ((&iroot_refs.refs[1 + 2])->value.pstruct));
    ref_ref_stacks[2].value.pstruct = iroot_refs.refs[2 + 2].value.pstruct, o_stack = *((ref_stack *) ((&iroot_refs.refs[2 + 2])->value.pstruct));
    dstack_gc_cleanup();
...
static void set_ref_chunk(chunk_t * cp, ref * bot, ref * top, gs_ref_memory_t * mem)
{
    ...
    pre->d.o.size = (byte *) (top + 1) - (byte *) bot;
    pre->d.o.t.type = &st_refs;
    alloc_init_chunk(cp, (byte *) head, (byte *) (top + 1), ((bool) 0), ((void *) 0));
    cp->cbot = cp->ctop;
    alloc_link_chunk(cp, mem);
    ((top)->value.intval = (0), ((top)->tas.type_attrs = ((t_integer) << 8) + (0)));
} const op_def *ireclaim_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {(char *) 0, (op_proc_p) ireclaim_init} };
    return op_defs_;
}
