...
extern int mem_gray8_rgb24_strip_copy_rop(gx_device * dev, const byte * sdata, int sourcex, uint sraster, gx_bitmap_id id, const gx_color_index * scolors, const gx_strip_bitmap * textures, const gx_color_index * tcolors, int x, int y, int width, int height, int phase_x, int phase_y, gs_logical_operation_t lop);
static int mem_mapped8_copy_mono(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1);
static int mem_mapped8_copy_color(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height);
static int mem_mapped8_fill_rectangle(gx_device * dev, int x, int y, int width, int height, gx_color_index color);
const gx_device_memory mem_mapped8_device = { sizeof(gx_device_memory), 0, "image8", 0, 0, 0, 0, {(8 ? 3 : 0) + (0 ? 1 : 0), 8 + 0, (0 ? (1 << 0) - 1 : (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1)), (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1), (0 ? (1 << 0) - 1 : (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1)) + 1, (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1) + 1}
...
static int mem_mapped8_fill_rectangle(gx_device * dev, int x, int y, int w, int h, gx_color_index color)
{
    if ((x | y) < 0) {
	if (x < 0)
	    w += x, x = 0;
    ...
    if (w <= 0 || h <= 0)
	return 0;
    bytes_fill_rectangle(((((gx_device_memory *) dev))->line_ptrs[y]) + x, ((gx_device_memory *) dev)->raster, (byte) color, w, h);
    return 0;
}

static void mapped8_copy01(byte *, const byte *, int, int, uint, int, int, byte, byte);
static void mapped8_copyN1(byte *, const byte *, int, int, uint, int, int, byte);
static void mapped8_copy0N(byte *, const byte *, int, int, uint, int, int, byte);
static int mem_mapped8_copy_mono(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h, gx_color_index zero, gx_color_index one)
{
    const byte *line;
    int first_bit;
    register byte *dest;
    ...
    dest = (byte *) (((((gx_device_memory *) dev))->line_ptrs[y]) + ((x) & -(sizeof(byte) == 1 ? 1 : sizeof(byte) == sizeof(short) ? 2 : sizeof(byte) == sizeof(int) ? 4 : 4)));
    line = base + (sourcex >> 3);
    first_bit = 0x80 >> (sourcex & 7);
    if (((int) (one) != (int) ((unsigned long) (-1)))) {
	if (((int) (zero) != (int) ((unsigned long) (-1))))
	    mapped8_copy01(dest, line, first_bit, sraster, draster, w, h, (byte) zero, (byte) one);
	else
	    mapped8_copyN1(dest, line, first_bit, sraster, draster, w, h, (byte) one);
    } else if (((int) (zero) != (int) ((unsigned long) (-1))))
	mapped8_copy0N(dest, line, first_bit, sraster, draster, w, h, (byte) zero);
    return 0;
}

static void mapped8_copy01(byte * dest, const byte * line, int first_bit, int sraster, uint draster, int w, int h, byte b0, byte b1)
{
    while (h-- > 0) {
...
}} static void mapped8_copyN1(byte * dest, const byte * line, int first_bit, int sraster, uint draster, int w, int h, byte b1)
{
    while (h-- > 0) {
	register byte *pptr = dest;
	const byte *sptr = line;
	...
	line += sraster;
	dest = (void *) ((byte *) dest + (draster));
}} static void mapped8_copy0N(byte * dest, const byte * line, int first_bit, int sraster, uint draster, int w, int h, byte b0)
{
    while (h-- > 0) {
	register byte *pptr = dest;
	const byte *sptr = line;
	...
	line += sraster;
	dest = (void *) ((byte *) dest + (draster));
}} static int mem_mapped8_copy_color(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
    if ((x | y) < 0) {
	if (x < 0)
	    w += x, sourcex -= x, x = 0;
    ...
    if (w <= 0 || h <= 0)
	return 0;
    bytes_copy_rectangle(((((gx_device_memory *) dev))->line_ptrs[y]) + (x), (((gx_device_memory *) dev))->raster, base + (sourcex), sraster, (w), h);
    return 0;
}

static int mem8_word_copy_mono(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1);
static int mem8_word_copy_color(gx_device * dev, const byte * data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height);
static int mem8_word_fill_rectangle(gx_device * dev, int x, int y, int width, int height, gx_color_index color);
const gx_device_memory mem_mapped8_word_device = { sizeof(gx_device_memory), 0, "image8w", 0, 0, 0, 0, {(8 ? 3 : 0) + (0 ? 1 : 0), 8 + 0, (0 ? (1 << 0) - 1 : (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1)), (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1), (0 ? (1 << 0) - 1 : (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1)) + 1, (8 >= 8 ? 255 : 8 == 4 ? 15 : 8 == 2 ? 3 : 8 == 1 ? 1 : (1 << 0) - 1) + 1}
, 0, 0, {(((0) * 72.0 + 0.5) - 0.5) / (72), (((0) * 72.0 + 0.5) - 0.5) / (72)}
, {0, 0, 0, 0}
, 0, {72, 72}
, {72, 72}
...
, {0, 0, 0, 0}
, 0, 0, 0, {gx_default_install, gx_default_begin_page, gx_default_end_page}
, {mem_open, mem_get_initial_matrix, gx_default_sync_output, gx_default_output_page, mem_close, mem_mapped_map_rgb_color, mem_mapped_map_color_rgb, mem8_word_fill_rectangle, gx_default_tile_rectangle, mem8_word_copy_mono, mem8_word_copy_color, gx_default_draw_line, mem_word_get_bits, gx_default_get_params, gx_default_put_params, gx_default_map_cmyk_color, gx_forward_get_xfont_procs, gx_forward_get_xfont_device, gx_default_map_rgb_alpha_color, gx_forward_get_page_device, gx_default_get_alpha_bits, gx_default_copy_alpha, gx_default_get_band, gx_default_copy_rop, gx_default_fill_path, gx_default_stroke_path, gx_default_fill_mask, gx_default_fill_trapezoid, gx_default_fill_parallelogram, gx_default_fill_triangle, gx_default_draw_thin_line, gx_default_begin_image, gx_default_image_data, gx_default_end_image, gx_default_strip_tile_rectangle, gx_no_strip_copy_rop, gx_default_get_clipping_box}
, 0, {(float) (1), (float) (0), (float) (0), (float) (1), (float) (0), (float) (0)}, 0, ((bool) 1), (byte *) 0, (byte **) 0, 0, {(byte *) 0, 0}, {((unsigned long) (-1))}, {0, 0}, 0, 0, 0, 0, 0, ((unsigned long) (-1)) };

static int mem8_word_fill_rectangle(gx_device * dev, int x, int y, int w, int h, gx_color_index color)
{
    byte *base;
    uint raster;
    if ((x | y) < 0) {
...
static int mem8_word_copy_mono(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h, gx_color_index zero, gx_color_index one)
{
    byte *row;
    uint raster;
    bool store;
    ...
    mem_mapped8_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero, one);
    mem_swap_byte_rect(row, raster, x << 3, w << 3, h, ((bool) 0));
    return 0;
}

static int mem8_word_copy_color(gx_device * dev, const byte * base, int sourcex, int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
    byte *row;
    uint raster;
    if ((x | y) < 0) {
