...
static int process_spot(gx_ht_order *, gs_state *, gs_spot_halftone *, gs_memory_t *);
static int process_threshold(gx_ht_order *, gs_state *, gs_threshold_halftone *, gs_memory_t *);
static gs_ptr_type_t halftone_component_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void halftone_component_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
const gs_memory_struct_type_t st_halftone_component = { sizeof(gs_halftone_component), "gs_halftone_component", 0, 0, halftone_component_enum_ptrs, halftone_component_reloc_ptrs, 0 };

static gs_ptr_type_t ht_comp_elt_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    uint count = size / (uint) sizeof(gs_halftone_component);
    if (count == 0)
	return 0;
    return (*st_halftone_component.enum_ptrs) ((char *) vptr + (index % count) * sizeof(gs_halftone_component), sizeof(gs_halftone_component), index / count, pep);
}

static void ht_comp_elt_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	uint count = size / (uint) sizeof(gs_halftone_component);
	for (; count; count--, vptr = (char *) vptr + sizeof(gs_halftone_component))
	    (*st_halftone_component.reloc_ptrs) (vptr, sizeof(gs_halftone_component), gcst);
}
}
const gs_memory_struct_type_t st_ht_component_element = { sizeof(gs_halftone_component), "gs_halftone_component[]", 0, 0, ht_comp_elt_enum_ptrs, ht_comp_elt_reloc_ptrs, 0 };

static gs_ptr_type_t halftone_component_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	if (((gs_halftone_component *) vptr)->type != ht_type_threshold)
	    return 0;
	do {
	    *pep = (const void *) &((gs_halftone_component *) vptr)->params.threshold.thresholds;
	    return (&ptr_const_string_procs);
	} while (0);
    }
}

static void halftone_component_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	if (((gs_halftone_component *) vptr)->type == ht_type_threshold)
	    gs_reloc_const_string(&((gs_halftone_component *) vptr)->params.threshold.thresholds, gcst);
}
}

int gs_setcolorscreen(gs_state * pgs, gs_colorscreen_halftone * pht)
{
    gs_halftone ht;
...
int gs_currentcolorscreen(gs_state * pgs, gs_colorscreen_halftone * pht)
{
    int code;
...
int gs_sethalftone(gs_state * pgs, gs_halftone * pht)
{
    gs_halftone ht;
    ht = *pht;
...
int gs_sethalftone_allocated(gs_state * pgs, gs_halftone * pht)
{
    gx_device_halftone dev_ht;
...
int gs_sethalftone_prepare(gs_state * pgs, gs_halftone * pht, gx_device_halftone * pdht)
{
    ...
    switch (pht->type) {
    case ht_type_colorscreen:{
	    gs_screen_halftone *phc = pht->params.colorscreen.screens.indexed;
	    static const gs_ht_separation_name cnames[4] = { gs_ht_separation_Default, gs_ht_separation_Red, gs_ht_separation_Green, gs_ht_separation_Blue };
		...
		code = gx_ht_process_screen_memory(&senum, pgs, &phc[ci], gs_currentaccuratescreens(), mem);
		if (code < 0)
		    break;
		...
		if (i == 0)
		    ...
		    uint num_tiles = (0 ? 512 : 4096) / tile_bytes + 1;
		    gx_ht_cache *pcache = gx_ht_alloc_cache(mem, num_tiles, tile_bytes * num_tiles);
		    if (pcache == 0) {
			code = ((-25));
			break;
		    }
	...
	code = process_threshold(&pdht->order, pgs, &pht->params.threshold, mem);
	if (code < 0)
	    return code;
	pdht->components = 0;
	break;
    ...
    case ht_type_multiple_colorscreen:{
		...
		switch (phc->type) {
		case ht_type_spot:
		    code = process_spot(&poc->corder, pgs, &phc->params.spot, mem);
		    break;
		case ht_type_threshold:
		    code = process_threshold(&poc->corder, pgs, &phc->params.threshold, mem);
		    break;
		default:
		    code = ((-15));
		    break;
		...
		if (code < 0)
		    break;
		if (poc != pocs) {
		    gx_ht_cache *pcache = gx_ht_alloc_cache(mem, 1, poc->corder.raster * (poc->corder.num_bits / poc->corder.width));
		    if (pcache == 0) {
			code = ((-25));
			break;
		    }
...
static int process_transfer(gx_ht_order * porder, gs_state * pgs, gs_mapping_proc tproc, gs_memory_t * mem)
{
    gx_transfer_map *pmap;
    if (tproc == 0)
	return 0;
    pmap = (gx_transfer_map *) (*(mem)->procs.alloc_struct) (mem, &st_transfer_map, "process_transfer");
    if (pmap == 0)
	return ((-25));
    pmap->proc = tproc;
...
static int process_spot(gx_ht_order * porder, gs_state * pgs, gs_spot_halftone * phsp, gs_memory_t * mem)
{
    gs_screen_enum senum;
    int code = gx_ht_process_screen_memory(&senum, pgs, &phsp->screen, phsp->accurate_screens, mem);
    if (code < 0)
	return code;
    *porder = senum.order;
...
static int process_threshold(gx_ht_order * porder, gs_state * pgs, gs_threshold_halftone * phtp, gs_memory_t * mem)
{
    int code;
    ...
    porder->params.R1 = 1;
    code = gx_ht_alloc_order(porder, phtp->width, phtp->height, 0, 256, mem);
    if (code < 0)
	return code;
...
void gx_ht_construct_threshold_order(gx_ht_order * porder, const byte * thresholds)
{
