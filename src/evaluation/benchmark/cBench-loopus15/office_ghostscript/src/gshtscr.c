...
static gs_ptr_type_t screen_enum_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void screen_enum_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
static const gs_memory_struct_type_t st_gs_screen_enum = { sizeof(gs_screen_enum), "gs_screen_enum", 0, 0, screen_enum_enum_ptrs, screen_enum_reloc_ptrs, 0 };

static gs_ptr_type_t screen_enum_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
	    if (index < 2 + 4) {
		gs_ptr_type_t ret = (*st_ht_order.enum_ptrs) (&((gs_screen_enum *) vptr)->order, sizeof(((gs_screen_enum *) vptr)->order), index - 2, pep);
		if (ret == 0)
		    ret = (&ptr_struct_procs), *pep = 0;
		return ret;
	    }
	    return (*st_halftone.enum_ptrs) (&((gs_screen_enum *) vptr)->halftone, sizeof(((gs_screen_enum *) vptr)->halftone), index - (2 + 4), pep);
	}
    case 0:
	do {
	    *pep = (const void *) (((gs_screen_enum *) vptr)->pgs);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void screen_enum_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	((gs_screen_enum *) vptr)->pgs = gs_reloc_struct_ptr((const void *) ((const gs_screen_enum *) vptr)->pgs, gcst);
	(*st_halftone.reloc_ptrs) (&((gs_screen_enum *) vptr)->halftone, sizeof(gs_halftone), gcst);
	(*st_ht_order.reloc_ptrs) (&((gs_screen_enum *) vptr)->order, sizeof(gx_ht_order), gcst);
}
}

static bool screen_accurate_screens = ((bool) 0);
void gs_setaccuratescreens(bool accurate)
{
...
static uint screen_min_screen_levels = 1;
void gs_setminscreenlevels(uint levels)
{
...
void gx_compute_cell_values(gx_ht_cell_params_t * phcp)
{
...
static int pick_cell_size(gs_screen_halftone * ph, const gs_matrix * pmat, ulong max_size, uint min_levels, bool accurate, gx_ht_cell_params_t * phcp);
gs_screen_enum *gs_screen_enum_alloc(gs_memory_t * mem, client_name_t cname)
{
    return (gs_screen_enum *) (*(mem)->procs.alloc_struct) (mem, &st_gs_screen_enum, cname);
}

int gs_screen_init(gs_screen_enum * penum, gs_state * pgs, gs_screen_halftone * phsp)
{
    return gs_screen_init_memory(penum, pgs, phsp, screen_accurate_screens, pgs->memory);
}

int gs_screen_init_memory(gs_screen_enum * penum, gs_state * pgs, gs_screen_halftone * phsp, bool accurate, gs_memory_t * mem)
{
    int code = gs_screen_order_init_memory(&penum->order, pgs, phsp, accurate, mem);
    if (code < 0)
	return code;
    return gs_screen_enum_init_memory(penum, &penum->order, pgs, phsp, mem);
}

int gs_screen_order_init_memory(gx_ht_order * porder, const gs_state * pgs, gs_screen_halftone * phsp, bool accurate, gs_memory_t * mem)
{
    gs_matrix imat;
    ...
    code = pick_cell_size(phsp, &imat, max_size, screen_min_screen_levels, accurate, &porder->params);
    if (code < 0)
	return code;
    gx_compute_cell_values(&porder->params);
    if (porder->params.W1 <= max_size / ((uint) (((porder->params.W + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2))) {
	code = gx_ht_alloc_order(porder, porder->params.W, porder->params.W1, 0, porder->params.W * porder->params.D, mem);
	porder->height = porder->orig_height = porder->params.D;
	porder->shift = porder->orig_shift = porder->params.S;
    } else {
	code = gx_ht_alloc_order(porder, porder->params.W, porder->params.D, porder->params.S, porder->params.W * porder->params.D, mem);
    }
    if (code < 0)
	return code;
    return 0;
...
static int pick_cell_size(gs_screen_halftone * ph, const gs_matrix * pmat, ulong max_size, uint min_levels, bool accurate, gx_ht_cell_params_t * phcp)
{
    ...
    int rt = 1;
    double f = 0, a = 0;
    double f_best = (~(-1 << ((1 << 2) * 8 - 1))), a_best = 360, e_best = 1000;
  ...
  try_size:better = ((bool) 0); {
	...
	for (p.M = m0 + 1; p.M >= m0; p.M--)
		...
		if (wt >= (~(-1 << ((1 << 1) * 8 - 1))))
		    continue;
		raster = ((uint) (((wt + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2));
		if (raster > max_size / p.D || raster > (~(-1L << ((1 << 2) * 8 - 1))) / wt)
		    continue;
		wt_size = raster * wt;
		if (landscape)
...
int gs_screen_enum_init_memory(gs_screen_enum * penum, const gx_ht_order * porder, gs_state * pgs, gs_screen_halftone * phsp, gs_memory_t * mem)
{
    penum->pgs = pgs;
    penum->order = *porder;
...
int gs_screen_currentpoint(gs_screen_enum * penum, gs_point * ppt)
{
    gs_point pt;
    int code;
...
int gs_screen_next(gs_screen_enum * penum, floatp value)
{
    ht_sample_t sample;
    int width = penum->order.width;
    if (value < -1.0 || value > 1.0)
	return ((-15));
    sample = (ht_sample_t) (value * (ht_sample_t) (((1 << ((sizeof(ht_mask_t) * 8) - 2)) - 1) * 2 + 1));
    sample += (ht_sample_t) (((1 << ((sizeof(ht_mask_t) * 8) - 2)) - 1) * 2 + 1);
...
int gs_screen_install(gs_screen_enum * penum)
{
    gx_device_halftone dev_ht;
