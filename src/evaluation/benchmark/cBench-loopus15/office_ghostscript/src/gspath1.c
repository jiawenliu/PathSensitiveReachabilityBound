...
typedef enum { arc_nothing, arc_moveto, arc_lineto } arc_action;
static int arc_add(gs_state *, arc_action, segment_notes, floatp, floatp, floatp, floatp, floatp, floatp, floatp);
int gs_arc(gs_state * pgs, floatp xc, floatp yc, floatp r, floatp ang1, floatp ang2)
{
    return gs_arc_add(pgs, ((bool) 0), xc, yc, r, ang1, ang2, ((bool) 1));
}

int gs_arcn(gs_state * pgs, floatp xc, floatp yc, floatp r, floatp ang1, floatp ang2)
{
    return gs_arc_add(pgs, ((bool) 1), xc, yc, r, ang1, ang2, ((bool) 1));
}

int gs_arc_add(gs_state * pgs, bool clockwise, floatp axc, floatp ayc, floatp arad, floatp aang1, floatp aang2, bool add_line)
{
    double ar = arad;
    fixed ang1 = ((fixed) ((aang1) * (float) (1 << 12))), ang2 = ((fixed) ((aang2) * (float) (1 << 12))), adiff;
    double ang1r;
    gs_sincos_t sincos;
    ...
    ang1r = ((ang1 % ((fixed) (360) << 12)) * (1.0 / (1 << 12)));
	...
	while (ang1 < ang2)
	    ang2 -= ((fixed) (360) << 12);
	while ((adiff = ang2 - ang1) < -((fixed) (90) << 12)) {
	    double w = sin0;
	    sin0 = -cos0;
	    cos0 = w;
	    x3r = axc + cos0, y3r = ayc + sin0;
	    code = arc_add(pgs, action, notes, ar, x0, y0, x3r, y3r, x0 + cos0, y0 + sin0);
	    if (code < 0)
		return code;
	    x0 = x3r, y0 = y3r;
	    ang1 -= ((fixed) (90) << 12);
    ...
    {
	double trad = tan(((adiff) * (1.0 / (1 << 12))) * ((3.14159265358979323846 / 180.0) / 2));
	double xt = x0 - trad * sin0, yt = y0 + trad * cos0;
	gs_sincos_degrees(((ang2) * (1.0 / (1 << 12))), &sincos);
	code = arc_add(pgs, action, notes, ar, x0, y0, (axc + ar * sincos.cos), (ayc + ar * sincos.sin), xt, yt);
    } return code;
}

int gs_arcto(gs_state * pgs, floatp ax1, floatp ay1, floatp ax2, floatp ay2, floatp arad, float retxy[4])
{
	...
	if (fabs(num) < 1.0e-6 || fabs(denom) < 1.0e-6) {
	    ...
	    if (code >= 0)
		code = gx_path_add_line_notes(pgs->path, pt.x, pt.y, sn_none);
	    xt0 = xt2 = ax1;
	    yt0 = yt2 = ay1;
	} else {
	    ...
	    xt2 = ax1 + dx2 * l2;
	    yt2 = ay1 + dy2 * l2;
	    code = arc_add(pgs, arc_lineto, sn_none, arad, xt0, yt0, xt2, yt2, ax1, ay1);
	}} if (retxy != 0) {
	    retxy[0] = xt0;
	    retxy[1] = yt0;
	    retxy[2] = xt2;
...
static int arc_add(gs_state * pgs, arc_action action, segment_notes notes, floatp r, floatp x0, floatp y0, floatp x3, floatp y3, floatp xt, floatp yt)
{
    ...
    if ((code = gs_point_transform2fixed(&pgs->ctm, x0, y0, &p0)) < 0 || (code = gs_point_transform2fixed(&pgs->ctm, x3, y3, &p3)) < 0 || (code = gs_point_transform2fixed(&pgs->ctm, xt, yt, &pt)) < 0 || (code = (action == arc_nothing ? 0 : action == arc_lineto && gx_path_current_point(path, &cpt) >= 0 ? gx_path_add_line_notes(path, p0.x, p0.y, sn_none) : gx_path_add_point(path, p0.x, p0.y))) < 0)
	return code;
    return gx_path_add_partial_arc_notes(path, p3.x, p3.y, pt.x, pt.y, fraction, notes);
}

int gs_dashpath(gs_state * pgs)
{
    gx_path fpath;
    int code;
    ...
    if (code < 0)
	return code;
    code = gx_path_expand_dashes(pgs->path, &fpath, (const gs_imager_state *) pgs);
    if (code < 0)
	return code;
...
int gs_flattenpath(gs_state * pgs)
{
    gx_path fpath;
    int code;
    if (!pgs->path->curve_count)
	return 0;
    code = gx_path_copy_reducing(pgs->path, &fpath, ((fixed) ((pgs->flatness) * (float) (1 << 12))), (pgs->accurate_curves ? pco_init | pco_accurate : pco_init));
    if (code < 0)
	return code;
    gx_path_release(pgs->path);
    *pgs->path = fpath;
...
int gs_reversepath(gs_state * pgs)
{
    gx_path rpath;
...
int gs_upathbbox(gs_state * pgs, gs_rect * pbox, bool include_moveto)
{
    gs_fixed_rect fbox;
    gs_rect dbox;
	...
	gs_fixed_point pt;
	(!(((pgs->path)->state_flags & psf_position_valid) != 0) ? ((-14)) : ((&pt)->x = pgs->path->position.x, (&pt)->y = pgs->path->position.y, 0));
	if (pt.x < fbox.p.x)
	    fbox.p.x = pt.x;
	if (pt.y < fbox.p.y)
	    fbox.p.y = pt.y;
    ...
    dbox.p.x = ((fbox.p.x) * (1.0 / (1 << 12)));
    dbox.p.y = ((fbox.p.y) * (1.0 / (1 << 12)));
    dbox.q.x = ((fbox.q.x) * (1.0 / (1 << 12)));
    dbox.q.y = ((fbox.q.y) * (1.0 / (1 << 12)));
    return gs_bbox_transform_inverse(&dbox, &(*(const gs_matrix *) &(pgs)->ctm), pbox);
} int gs_path_enum_init(gs_path_enum * penum, const gs_state * pgs)
{
    gx_path *copied_path = gx_path_alloc(pgs->memory, "gs_path_enum_init");
    int code;
    if (copied_path == 0)
	return ((-25));
    code = gx_path_copy_reducing(pgs->path, copied_path, (~(-1L << ((1 << 2) * 8 - 1))), (1 ? pco_init : pco_none));
    if (code < 0) {
	(*(pgs->memory)->procs.free_object) (pgs->memory, copied_path, "gs_path_enum_init");
	return code;
    }
...
int gs_path_enum_next(gs_path_enum * penum, gs_point ppts[3])
{
    gs_fixed_point fpts[3];
    gs_state *pgs = (gs_state *) penum->pgs;
	...
	break;
    case 3:
	if ((code = gs_itransform(pgs, ((fpts[1].x) * (1.0 / (1 << 12))), ((fpts[1].y) * (1.0 / (1 << 12))), &ppts[1])) < 0 || (code = gs_itransform(pgs, ((fpts[2].x) * (1.0 / (1 << 12))), ((fpts[2].y) * (1.0 / (1 << 12))), &ppts[2])) < 0)
	    return code;
    case 1:
    case 2:
	if ((code = gs_itransform(pgs, ((fpts[0].x) * (1.0 / (1 << 12))), ((fpts[0].y) * (1.0 / (1 << 12))), &ppts[0])) < 0)
	    return code;
    default:
	break;
    }
...
void gs_path_enum_cleanup(gs_path_enum * penum)
{
    if (penum->copied_path != 0) {
	gx_path_release(penum->copied_path);
	(*(penum->pgs->memory)->procs.free_object) (penum->pgs->memory, penum->copied_path, "gs_path_enum_cleanup");
	penum->path = 0;
	penum->copied_path = 0;
    }
}
