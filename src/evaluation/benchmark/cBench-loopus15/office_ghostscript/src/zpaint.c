...
extern gs_state *igs;
static int zfill(register os_ptr op)
{
    return gs_fill(igs);
}

static int zeofill(register os_ptr op)
{
    return gs_eofill(igs);
}

static int zstroke(register os_ptr op)
{
    return gs_stroke(igs);
}

static int zfillpage(register os_ptr op)
{
    return gs_fillpage(igs);
}

static int zimagepath(register os_ptr op)
{
    int code;
	...
	return ((-15));
    code = gs_imagepath(igs, (int) op[-2].value.intval, (int) op[-1].value.intval, op->value.const_bytes);
    if (code >= 0)
	((o_stack.p) -= (3));
    return code;
}

const op_def *zpaint_op_defs(void)
{
    static const op_def op_defs_[] = { {"0eofill", zeofill}, {"0fill", zfill}, {"0stroke", zstroke}, {"0.fillpage", zfillpage}, {"3.imagepath", zimagepath}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
