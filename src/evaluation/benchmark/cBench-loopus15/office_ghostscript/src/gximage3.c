...
static int image_render_color(gx_image_enum * penum, const byte * buffer, int data_x, uint w, int h, gx_device * dev);
static irender_proc_t image_strategy_color(gx_image_enum * penum)
{
    return image_render_color;
}

void gs_gximage3_init(gs_memory_t * mem)
{
...
static int image_render_color(gx_image_enum * penum, const byte * buffer, int data_x, uint w, int h, gx_device * dev)
{
    ...
    void (*map_cmyk) (frac, frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t) = cmap_procs->map_cmyk;
    ...
    color_samples run;
    color_samples next;
    bool small = ((int) ((((penum->x_extent.x) < 0 ? -(penum->x_extent.x) : (penum->x_extent.x))) >> (12))) < penum->rect.w && ((int) ((((penum->x_extent.y) < 0 ? -(penum->x_extent.y) : (penum->x_extent.y))) >> (12))) < penum->rect.w;
	...
	irun = ((int) (((xrun) + ((1 << 12) >> 1)) >> (12)));
	break;
    case image_landscape:
	vci = penum->xci, vdi = penum->wci;
	irun = ((int) (((yrun) + ((1 << 12) >> 1)) >> (12)));
	break;
    }
    do {
    } while (0);
    ...
    next.all = 0;
    if (use_cache) {
	((&empty_clue.dev_color)->colors.pure = (((unsigned long) (-1))), (&empty_clue.dev_color)->type = (&gx_dc_procs_pure));
	pic = &empty_clue;
    }
    cc.paint.values[0] = cc.paint.values[1] = cc.paint.values[2] = cc.paint.values[3] = 0;
    cc.pattern = 0;
    run.v[0] = ~psrc[0];
    while (psrc < bufend) {
	((pnext.x).state).Q += (((pnext.x).state).R >= ((pnext.x).step).dR ? (((pnext.x).state).R -= ((pnext.x).step).dR, ((pnext.x).step).dQ) : (((pnext.x).state).R += ((pnext.x).step).NdR, ((pnext.x).step).dQ + 1));
	((pnext.y).state).Q += (((pnext.y).state).R >= ((pnext.y).step).dR ? (((pnext.y).state).R -= ((pnext.y).step).dR, ((pnext.y).step).dQ) : (((pnext.y).state).R += ((pnext.y).step).NdR, ((pnext.y).step).dQ + 1));
	if (spp == 4) {
	    next.v[0] = psrc[0];
	    next.v[1] = psrc[1];
	    next.v[2] = psrc[2];
	    next.v[3] = psrc[3];
	    psrc += 4;
	    if (next.all == run.all || (small && !(((xl < ((pnext.x).state).Q ? (xl - (((1L << 12) >> 1) + 1L)) ^ (((pnext.x).state).Q - ((1L << 12) >> 1)) : (((pnext.x).state).Q - (((1L << 12) >> 1) + 1L)) ^ (xl - ((1L << 12) >> 1))) & (-1L << 12)) != 0) && !(((ytf < ((pnext.y).state).Q ? (ytf - (((1L << 12) >> 1) + 1L)) ^ (((pnext.y).state).Q - ((1L << 12) >> 1)) : (((pnext.y).state).Q - (((1L << 12) >> 1) + 1L)) ^ (ytf - ((1L << 12) >> 1))) & (-1L << 12)) != 0) && psrc <= bufend))
		goto inc;
	    if (use_cache) {
		pic_next = &penum->clues[(next.v[0] + (next.v[1] << 2) + (next.v[2] << 4) + (next.v[3] << 6)) & 255];
	    ...
	    if (device_color) {
		(*map_cmyk) (((frac) (((frac) (next.v[0]) << (15 - (8))) + ((next.v[0]) >> ((8) * 2 - 15)) - ((next.v[0]) >> ((8) - 3)))), ((frac) (((frac) (next.v[1]) << (15 - (8))) + ((next.v[1]) >> ((8) * 2 - 15)) - ((next.v[1]) >> ((8) - 3)))), ((frac) (((frac) (next.v[2]) << (15 - (8))) + ((next.v[2]) >> ((8) * 2 - 15)) - ((next.v[2]) >> ((8) - 3)))), ((frac) (((frac) (next.v[3]) << (15 - (8))) + ((next.v[3]) >> ((8) * 2 - 15)) - ((next.v[3]) >> ((8) - 3)))), (&pic_next->dev_color), pis, dev, gs_color_select_source);
		goto mapped;
	    }
	    switch (penum->map[3].decoding) {
	    case sd_none:
		cc.paint.values[3] = (next.v[3]) * (1.0 / 255.0);
		break;
	    case sd_lookup:
		cc.paint.values[3] = penum->map[3].decode_lookup[(next.v[3]) >> 4];
		break;
	    case sd_compute:
		...
		goto inc;
	    if (use_cache) {
		pic_next = &penum->clues[(next.v[0] + (next.v[1] << 2) + (next.v[2] << 4)) & 255];;
		if (pic_next->key == next.all)
		    goto f;
		if (pic_next == pic) {
		    clue_temp = *pic;
	    ...
	    if (device_color) {
		(*map_rgb) (((frac) (((frac) (next.v[0]) << (15 - (8))) + ((next.v[0]) >> ((8) * 2 - 15)) - ((next.v[0]) >> ((8) - 3)))), ((frac) (((frac) (next.v[1]) << (15 - (8))) + ((next.v[1]) >> ((8) * 2 - 15)) - ((next.v[1]) >> ((8) - 3)))), ((frac) (((frac) (next.v[2]) << (15 - (8))) + ((next.v[2]) >> ((8) * 2 - 15)) - ((next.v[2]) >> ((8) - 3)))), (&pic_next->dev_color), pis, dev, gs_color_select_source);
		goto mapped;
	    }
	}
	switch (penum->map[0].decoding) {
	case sd_none:
	    cc.paint.values[0] = (next.v[0]) * (1.0 / 255.0);
	    break;
	case sd_lookup:
	    cc.paint.values[0] = penum->map[0].decode_lookup[(next.v[0]) >> 4];
	    break;
	case sd_compute:
	    cc.paint.values[0] = penum->map[0].decode_lookup[0] + (next.v[0]) * penum->map[0].decode_factor;
	};
	switch (penum->map[1].decoding) {
	case sd_none:
	    cc.paint.values[1] = (next.v[1]) * (1.0 / 255.0);
	    break;
	case sd_lookup:
	    cc.paint.values[1] = penum->map[1].decode_lookup[(next.v[1]) >> 4];
	    break;
	case sd_compute:
	    cc.paint.values[1] = penum->map[1].decode_lookup[0] + (next.v[1]) * penum->map[1].decode_factor;
	};
	switch (penum->map[2].decoding) {
	case sd_none:
	    cc.paint.values[2] = (next.v[2]) * (1.0 / 255.0);
	    break;
	case sd_lookup:
	    cc.paint.values[2] = penum->map[2].decode_lookup[(next.v[2]) >> 4];
	    break;
	case sd_compute:
	...
	(*remap_color) (&cc, pcs, (&pic_next->dev_color), pis, dev, gs_color_select_source);
      mapped:if (pic == pic_next)
	    goto fill;
      f:do {
	} while (0);
	if ((((&(*(&pic->dev_color)))->type == (&gx_dc_procs_pure)) ? ((&(*(&pic_next->dev_color)))->type == (&gx_dc_procs_pure)) && ((&(*(&pic->dev_color)))->colors.pure) == ((&(*(&pic_next->dev_color)))->colors.pure) : ((&(*(&pic->dev_color)))->type == (&gx_dc_procs_ht_binary)) ? ((&(*(&pic_next->dev_color)))->type == (&gx_dc_procs_ht_binary)) && ((&(*(&pic->dev_color)))->colors.binary.color[0]) == ((&(*(&pic_next->dev_color)))->colors.binary.color[0]) && ((&(*(&pic->dev_color)))->colors.binary.color[1]) == ((&(*(&pic_next->dev_color)))->colors.binary.color[1]) && (*(&pic->dev_color)).colors.binary.b_level == (*(&pic_next->dev_color)).colors.binary.b_level : ((bool) 0)))
	    goto set;
      fill:{
	    switch (posture) {
	    case image_portrait:{
		    int xi = irun;
		    int wi = (irun = ((int) (((xl) + ((1 << 12) >> 1)) >> (12)))) - xi;
		    if (wi < 0)
			xi += wi, wi = -wi;
		    code = (*((&pic->dev_color))->type->fill_rectangle) ((&pic->dev_color), xi, vci, wi, vdi, dev, lop, ((void *) 0));
		    xrun = xl;
		} break;
	    case image_landscape:{
		    int yi = irun;
		    int hi = (irun = ((int) (((ytf) + ((1 << 12) >> 1)) >> (12)))) - yi;
		    if (hi < 0)
			yi += hi, hi = -hi;
		    code = (*((&pic->dev_color))->type->fill_rectangle) ((&pic->dev_color), vci, yi, vdi, hi, dev, lop, ((void *) 0));
		    yrun = ytf;
		} break;
	    default:{
		    code = (*((dev)->std_procs.fill_parallelogram)) (dev, xrun, yrun, xl - xrun, ytf - yrun, pdyx, pdyy, (&pic->dev_color), lop);
		    xrun = xl;
		    yrun = ytf;
		}
	    }
