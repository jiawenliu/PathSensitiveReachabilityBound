...
static int z2copy_gstate(os_ptr);
static int push_callout(const char *);
static void zdevice2_init(void)
{
    zcopy_procs[t_astruct] = z2copy_gstate;
} static int zcurrentshowpagecount(register os_ptr op)
{
    gx_device *dev = gs_currentdevice(igs);
    if ((*((dev)->std_procs.get_page_device)) (dev) == 0) {
	do {
	    if ((op += (1)) > (o_stack.top)) {
		o_stack.requested = (1);
		return ((-16));
...
static int zdoneshowpage(register os_ptr op)
{
    gx_device *dev = gs_currentdevice(igs);
    if ((*((dev)->std_procs.get_page_device)) (dev) != 0)
	dev->ShowpageCount++;
...
static int zcurrentpagedevice(register os_ptr op)
{
    gx_device *dev = gs_currentdevice(igs);
    do {
	if ((op += (2)) > (o_stack.top)) {
    ...
    if ((*((dev)->std_procs.get_page_device)) (dev) != 0) {
	op[-1] = ((int_gstate *) gs_state_client_data(igs))->pagedevice;
	((op)->value.boolval = (1), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    } else {
...
static int zsetpagedevice(register os_ptr op)
{
    int code;
    ...
    if (code < 0)
	return code;
    ((int_gstate *) gs_state_client_data(igs))->pagedevice = *op;
    ((o_stack.p) -= (1));
    return 0;
}

static int zcallinstall(os_ptr op)
{
    gx_device *dev = gs_currentdevice(igs);
    if ((dev = (*((dev)->std_procs.get_page_device)) (dev)) != 0) {
...
static int zcallbeginpage(os_ptr op)
{
    gx_device *dev = gs_currentdevice(igs);
...
static int zcallendpage(os_ptr op)
{
    gx_device *dev = gs_currentdevice(igs);
    int code;
...
static int z2gsave(os_ptr op)
{
    if (!(((int_gstate *) gs_state_client_data(igs))->pagedevice.value.pdict == i_null_pagedevice.value.pdict && (*((gs_currentdevice(igs))->std_procs.get_page_device)) (gs_currentdevice(igs)) != 0))
	return gs_gsave(igs);
    return push_callout("%gsavepagedevice");
}

static int z2save(os_ptr op)
{
    if (!(((int_gstate *) gs_state_client_data(igs))->pagedevice.value.pdict == i_null_pagedevice.value.pdict && (*((gs_currentdevice(igs))->std_procs.get_page_device)) (gs_currentdevice(igs)) != 0))
	return zsave(op);
...
static int z2copy_gstate(os_ptr op)
{
    if (!(((int_gstate *) gs_state_client_data(igs))->pagedevice.value.pdict == i_null_pagedevice.value.pdict && (*((gs_currentdevice(igs))->std_procs.get_page_device)) (gs_currentdevice(igs)) != 0))
...
static int z2currentgstate(os_ptr op)
{
    if (!(((int_gstate *) gs_state_client_data(igs))->pagedevice.value.pdict == i_null_pagedevice.value.pdict && (*((gs_currentdevice(igs))->std_procs.get_page_device)) (gs_currentdevice(igs)) != 0))
...
static bool restore_page_device(const gs_state * pgs_old, const gs_state * pgs_new)
{
    gx_device *dev_old = gs_currentdevice(pgs_old);
    gx_device *dev_new;
    gx_device *dev_t1;
    gx_device *dev_t2;
    if ((dev_t1 = (*((dev_old)->std_procs.get_page_device)) (dev_old)) == 0)
	return ((bool) 0);
    dev_new = gs_currentdevice(pgs_new);
    if (dev_old != dev_new) {
	if ((dev_t2 = (*((dev_new)->std_procs.get_page_device)) (dev_new)) == 0)
	    return ((bool) 0);
	if (dev_t1 != dev_t2)
	    return ((bool) 1);
    }
    {
	const ref *pdict1 = &((int_gstate *) gs_state_client_data(pgs_old))->pagedevice;
	const ref *pdict2 = &((int_gstate *) gs_state_client_data(pgs_new))->pagedevice;
	return pdict1->value.pdict != pdict2->value.pdict;
    }
}

static int z2grestore(os_ptr op)
{
...
static int z2grestoreall(os_ptr op)
{
    for (;;) {
	if (!restore_page_device(igs, gs_state_saved(igs))) {
...
static int z2restore(os_ptr op)
{
    for (;;) {
	if (!restore_page_device(igs, gs_state_saved(igs))) {
	    zgrestore(op);
...
static int z2setgstate(os_ptr op)
{
	...
	return (check_type_failed(&*op));
    if (!restore_page_device(igs, ((gs_state *) ((&((igstate_obj *) ((op)->value.pstruct))->gstate)->value.pstruct))))
	return zsetgstate(op);
    return push_callout("%setgstatepagedevice");
}

const op_def *zdevice2_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"0.currentshowpagecount", zcurrentshowpagecount}, {"0.currentpagedevice", zcurrentpagedevice}, {"0.doneshowpage", zdoneshowpage}, {"1.setpagedevice", zsetpagedevice}, {"0gsave", z2gsave}, {"0save", z2save}, {"0gstate", z2gstate}, {"1currentgstate", z2currentgstate}, {"0grestore", z2grestore}, {"0grestoreall", z2grestoreall}, {"1restore", z2restore}, {"1setgstate", z2setgstate}, {"0.callinstall", zcallinstall}, {"1.callbeginpage", zcallbeginpage}, {"2.callendpage", zcallendpage}, {(char *) 0, (op_proc_p) zdevice2_init} };
    return op_defs_;
}

static int push_callout(const char *callout_name)
{
    int code;
    ...
    code = name_enter_string(callout_name, (e_stack.p) + 1);
    if (code < 0)
	return code;
    ++(e_stack.p);
    (((e_stack.p))->tas.type_attrs |= (0x80));
    return 5;
}
