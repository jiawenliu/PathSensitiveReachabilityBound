...
static int num_param(const_os_ptr, int (*)(gs_state *, floatp));
static const gs_memory_struct_type_t st_int_gstate = { sizeof(int_gstate), "int_gstate", 0, ref_struct_clear_marks, ref_struct_enum_ptrs, ref_struct_reloc_ptrs, 0 };

gs_state *igs;
static gs_gc_root_t igs_root;
ref i_null_pagedevice;
static ref *npd_p;
static gs_gc_root_t npd_root;
...
static const gs_state_client_procs istate_procs = { gs_istate_alloc, gs_istate_copy, gs_istate_free };

void igs_init(void)
{
    int_gstate *iigs;
    ref proc0;
    igs = gs_state_alloc(((gs_memory_t *) (gs_imemory.current)));
    iigs = (int_gstate *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_int_gstate, "igs_init"); {
	register ref *rp_ = (ref *) (iigs);
	register int i = sizeof(int_gstate) / sizeof(ref);
	do {
	    (((rp_)->tas.type_attrs = ((t_null) << 8) + (0)));
	    ++rp_;
	} while (--i);
    };
    ((&iigs->dash_pattern)->value.refs = ((ref *) ((void *) 0)), ((&iigs->dash_pattern)->tas.type_attrs = ((t_array) << 8) + ((0x10 + 0x20 + 0x40))), ((&iigs->dash_pattern)->tas.rsize = (0)));
    gs_alloc_ref_array((gs_imemory.current), &proc0, (0x20 + 0x40) + 0x80, 2, "igs_init");
    ...
    ((&i_null_pagedevice)->tas.type_attrs &= ~(0x10));
    iigs->pagedevice = i_null_pagedevice;
    npd_p = &i_null_pagedevice;
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &npd_root, (&ptr_ref_procs), (void **) &npd_p, "igs(npd)");
    gs_state_set_client(igs, iigs, &istate_procs);
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.register_root) (((gs_memory_t *) (gs_imemory.current)), &igs_root, (&ptr_struct_procs), (void **) &igs, "igs");
    gs_setlimitclamp(igs, ((bool) 1));
} int zgsave(register os_ptr op)
{
    return gs_gsave(igs);
}

int zgrestore(register os_ptr op)
{
    return gs_grestore(igs);
}

int zgrestoreall(register os_ptr op)
{
    return gs_grestoreall(igs);
}

static int zinitgraphics(register os_ptr op)
{
    int code = gs_initgraphics(igs);
    if (code >= 0)
	(((&((int_gstate *) gs_state_client_data(igs))->colorspace.array)->tas.type_attrs = ((t_null) << 8) + (0)));
    return code;
}

static int zsetlinewidth(register os_ptr op)
{
    return num_param(op, gs_setlinewidth);
}

static int zcurrentlinewidth(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.realval = (gs_currentlinewidth(igs)), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
    return 0;
}

static int zsetlinecap(register os_ptr op)
{
    int param;
...
static int zcurrentlinecap(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.intval = ((int) gs_currentlinecap(igs)), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
    return 0;
}

static int zsetlinejoin(register os_ptr op)
{
    int param;
    int code = int_param(op, (~(-1 << ((1 << 2) * 8 - 1))), &param);
...
static int zcurrentlinejoin(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.intval = ((int) gs_currentlinejoin(igs)), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)));
    return 0;
}

static int zsetmiterlimit(register os_ptr op)
{
    return num_param(op, gs_setmiterlimit);
}

static int zcurrentmiterlimit(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.realval = (gs_currentmiterlimit(igs)), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
    return 0;
}

static int zsetdash(register os_ptr op)
{
    os_ptr op1 = op - 1;
    float offset;
    int code = real_param(op, &offset);
    uint i, n;
    gs_memory_t *mem = ((gs_memory_t *) (gs_imemory.current));
    float *pattern;
    if (code < 0)
    ...
    n = ((op1)->tas.rsize);
    pattern = (float *) (*(mem)->procs.alloc_byte_array) (mem, n, sizeof(float), "setdash");
    if (pattern == 0)
	return ((-25));
    ...
    if (code < 0)
	return code;
    (*(&((int_gstate *) gs_state_client_data(igs))->dash_pattern) = *(op1));
    ((o_stack.p) -= (2));
    return code;
}

static int zcurrentdash(register os_ptr op)
{
    do {
	if ((op += (2)) > (o_stack.top)) {
	    o_stack.requested = (2);
    ...
    (*(op - 1) = *(&((int_gstate *) gs_state_client_data(igs))->dash_pattern));
    ((op)->value.realval = (gs_currentdash_offset(igs)), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
    return 0;
}

static int zsetflat(register os_ptr op)
{
    return num_param(op, gs_setflat);
}

static int zcurrentflat(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.realval = (gs_currentflat(igs)), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
    return 0;
}

static int zsetaccuratecurves(register os_ptr op)
{
...
static int zcurrentaccuratecurves(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.boolval = (gs_currentaccuratecurves(igs)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    return 0;
}

static int zsetfilladjust2(register os_ptr op)
{
    float adjust[2];
...
static int zcurrentfilladjust2(register os_ptr op)
{
    gs_point adjust;
    do {
	if ((op += (2)) > (o_stack.top)) {
...
static int zsetdashadapt(register os_ptr op)
{
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_boolean)))
	return (check_type_failed(&*op));
...
static int zcurrentdashadapt(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.boolval = (gs_currentdashadapt(igs)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    return 0;
}

static int zsetdotlength(register os_ptr op)
{
    float length;
...
static int zcurrentdotlength(register os_ptr op)
{
    do {
	if ((op += (2)) > (o_stack.top)) {
	    o_stack.requested = (2);
    ...
    ((op - 1)->value.realval = (gs_currentdotlength(igs)), ((op - 1)->tas.type_attrs = ((t_real) << 8) + (0)));
    ((op)->value.boolval = (gs_currentdotlength_absolute(igs)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    return 0;
}

const op_def *zgstate_op_defs(void)
{
    static const op_def op_defs_[] = { {"0.currentaccuratecurves", zcurrentaccuratecurves}, {"0currentdash", zcurrentdash}, {"0.currentdashadapt", zcurrentdashadapt}, {"0.currentdotlength", zcurrentdotlength}, {"0.currentfilladjust2", zcurrentfilladjust2}, {"0currentflat", zcurrentflat}, {"0currentlinecap", zcurrentlinecap}, {"0currentlinejoin", zcurrentlinejoin}, {"0currentlinewidth", zcurrentlinewidth}, {"0currentmiterlimit", zcurrentmiterlimit}, {"0grestore", zgrestore}, {"0grestoreall", zgrestoreall}, {"0gsave", zgsave}, {"0initgraphics", zinitgraphics}, {"1.setaccuratecurves", zsetaccuratecurves}, {"2setdash", zsetdash}, {"1.setdashadapt", zsetdashadapt}, {"2.setdotlength", zsetdotlength}, {"2.setfilladjust2", zsetfilladjust2}, {"1setflat", zsetflat}, {"1.setlinecap", zsetlinecap}, {"1.setlinejoin", zsetlinejoin}, {"1setlinewidth", zsetlinewidth}, {"1setmiterlimit", zsetmiterlimit}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
...
} static int num_param(const_os_ptr op, int (*pproc) (gs_state *, floatp))
{
    float param;
