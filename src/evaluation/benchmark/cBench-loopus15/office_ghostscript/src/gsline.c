...
{
    (((&(pgs)->line_params))->half_width = (width) / 2);
    return 0;
}

float gs_currentlinewidth(const gs_state * pgs)
{
...
gs_line_cap gs_currentlinecap(const gs_state * pgs)
{
...
gs_line_join gs_currentlinejoin(const gs_state * pgs)
{
    return (&(pgs)->line_params)->join;
}

int gx_set_miter_limit(gx_line_params * plp, floatp limit)
{
    if (limit < 1.0)
	return ((-15));
	...
	if (limit_squared < 2.0001 && limit_squared > 1.9999)
	    ...
	    plp->miter_check = sqrt(limit_squared - 1) * 2 / (limit_squared - 2);
    }
    return 0;
}
...
float gs_currentmiterlimit(const gs_state * pgs)
{
    return (&(pgs)->line_params)->miter_limit;
}

int gx_set_dash(gx_dash_params * dash, const float *pattern, uint length, floatp offset, gs_memory_t * mem)
{
    uint n = length;
	...
	while ((dist_left -= pattern[index]) >= 0 && (dist_left > 0 || pattern[index] != 0))
	    ink = !ink, index++;
	if (mem) {
	    if (ppat == 0)
...
int gs_setdash(gs_state * pgs, const float *pattern, uint length, floatp offset)
{
    return gx_set_dash(&(&(pgs)->line_params)->dash, pattern, length, offset, pgs->memory);
}

uint gs_currentdash_length(const gs_state * pgs)
{
    return (&(pgs)->line_params)->dash.pattern_size;
}

const float *gs_currentdash_pattern(const gs_state * pgs)
{
    return (&(pgs)->line_params)->dash.pattern;
}

float gs_currentdash_offset(const gs_state * pgs)
{
    return (&(pgs)->line_params)->dash.offset;
}

const gx_line_params *gs_currentlineparams(const gs_imager_state * pis)
{
    return (&(pis)->line_params);
}

int gs_imager_setflat(gs_imager_state * pis, floatp flat)
{
    if (flat <= 0.2)
	flat = 0.2;
    else if (flat > 100)
...
float gs_currentflat(const gs_state * pgs)
{
    return pgs->flatness;
}
...
bool gs_currentstrokeadjust(const gs_state * pgs)
{
    return pgs->stroke_adjust;
}

void gs_setdashadapt(gs_state * pgs, bool adapt)
{
    (&(pgs)->line_params)->dash.adapt = adapt;
} bool gs_imager_currentdashadapt(const gs_imager_state * pis)
{
    return (&(pis)->line_params)->dash.adapt;
}

bool gs_currentdashadapt(const gs_state * pgs)
{
    return gs_imager_currentdashadapt((const gs_imager_state *) pgs);
} void gs_setaccuratecurves(gs_state * pgs, bool accurate)
{
    pgs->accurate_curves = accurate;
} bool gs_imager_currentaccuratecurves(const gs_imager_state * pis)
{
    return pis->accurate_curves;
}

bool gs_currentaccuratecurves(const gs_state * pgs)
{
    return gs_imager_currentaccuratecurves((const gs_imager_state *) pgs);
} int gx_set_dot_length(gx_line_params * plp, floatp length, bool absolute)
{
    if (length < 0)
	return ((-15));
...
int gs_setdotlength(gs_state * pgs, floatp length, bool absolute)
{
    return gx_set_dot_length((&(pgs)->line_params), length, absolute);
}

float gs_currentdotlength(const gs_state * pgs)
{
    return (&(pgs)->line_params)->dot_length;
}

bool gs_currentdotlength_absolute(const gs_state * pgs)
{
