...
static void sproc_clear_marks(void *vptr, uint size)
{
    ((&((stream_proc_state *) vptr)->proc)->tas.type_attrs &= ~(1));
    ((&((stream_proc_state *) vptr)->data)->tas.type_attrs &= ~(1));
} static gs_ptr_type_t sproc_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	*pep = &((stream_proc_state *) vptr)->proc;
	return (&ptr_ref_procs);
    case 1:
	*pep = &((stream_proc_state *) vptr)->data;
	return (&ptr_ref_procs);
    }
}

static void sproc_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    gs_reloc_refs((ref_packed *) & ((stream_proc_state *) vptr)->proc, (ref_packed *) (&((stream_proc_state *) vptr)->proc + 1), gcst);
    ((&((stream_proc_state *) vptr)->proc)->tas.type_attrs &= ~(1));
    gs_reloc_refs((ref_packed *) & ((stream_proc_state *) vptr)->data, (ref_packed *) (&((stream_proc_state *) vptr)->data + 1), gcst);
    ((&((stream_proc_state *) vptr)->data)->tas.type_attrs &= ~(1));
} static const gs_memory_struct_type_t st_sproc_state = { sizeof(stream_proc_state), "procedure stream state", 0, sproc_clear_marks, sproc_enum_ptrs, sproc_reloc_ptrs, 0 };

static int s_proc_init(ref * sop, stream ** psstrm, uint mode, const stream_template * temp)
{
    stream *sstrm;
    stream_proc_state *state;
    static const stream_procs procs = { s_std_noavailable, s_std_noseek, s_std_read_reset, s_std_read_flush, s_std_null, ((void *) 0) };
    sstrm = file_alloc_stream(((gs_memory_t *) (gs_imemory.current)), "s_proc_init(stream)");
    state = (stream_proc_state *) s_alloc_state(((gs_memory_t *) (gs_imemory.current)), &st_sproc_state, "s_proc_init(state)");
    if (sstrm == 0 || state == 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), state, "s_proc_init(state)");
	return ((-25));
    }
    s_std_init(sstrm, ((void *) 0), 0, &procs, mode);
    sstrm->procs.process = temp->process;
    state->template = temp;
    state->memory = ((gs_memory_t *) (gs_imemory.current));
    state->eof = 0;
    state->proc = *sop;
...
static int s_handle_intc(const ref * pstate, int nstate, int (*cont) (os_ptr))
{
    int npush = nstate + 2;
    if ((e_stack.p) > (e_stack.top) - (npush)) {
	int es_code_ = ref_stack_extend(&e_stack, npush);
	if (es_code_ < 0)
	    return es_code_;
    };
    if (nstate)
	memcpy((e_stack.p) + 2, pstate, nstate * sizeof(ref));
    npush--;
    (((e_stack.p) + 1)->value.opproc = (cont), (((e_stack.p) + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) + 1)->tas.rsize = (0)));
    (e_stack.p) += npush;
    return 5;
}

static int s_proc_read_process(stream_state *, stream_cursor_read *, stream_cursor_write *, bool);
static int s_proc_read_continue(os_ptr);
static const stream_template s_proc_read_template = { &st_sproc_state, ((void *) 0), s_proc_read_process, 1, 1, ((void *) 0) };

int sread_proc(ref * sop, stream ** psstrm)
{
    int code = s_proc_init(sop, psstrm, 1, &s_proc_read_template);
    if (code < 0)
	return code;
...
static int s_proc_read_process(stream_state * st, stream_cursor_read * ignore_pr, stream_cursor_write * pw, bool last)
{
    uint count = ((&((stream_proc_state *) st)->data)->tas.rsize) - ((stream_proc_state *) st)->index;
    if (count > 0) {
	uint wcount = pw->limit - pw->ptr;
	if (wcount < count)
	    count = wcount;
	memcpy(pw->ptr + 1, ((stream_proc_state *) st)->data.value.bytes + ((stream_proc_state *) st)->index, count);
	pw->ptr += count;
	((stream_proc_state *) st)->index += count;
	return 1;
    }
    return (((stream_proc_state *) st)->eof ? ((int) (-1)) : ((int) (-4)));
} int s_handle_read_exception(int status, const ref * fop, const ref * pstate, int nstate, int (*cont) (os_ptr))
{
    int npush = nstate + 4;
    stream *ps;
    switch (status) {
    ...
    (((e_stack.p) + 1)->value.opproc = (cont), (((e_stack.p) + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) + 1)->tas.rsize = (0)));
    (e_stack.p) += npush;
    (((e_stack.p) - 2)->value.opproc = (s_proc_read_continue), (((e_stack.p) - 2)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) - 2)->tas.rsize = (0)));
    (e_stack.p)[-1] = *fop;
    (((e_stack.p) - 1)->tas.type_attrs &= ~(0x80));
    *(e_stack.p) = ((stream_proc_state *) ps->state)->proc;
    return 5;
}

static int s_proc_read_continue(os_ptr op)
{
    os_ptr opbuf = op - 1;
    stream *ps;
	...
	if (!(ps = (op)->value.pfile, (ps->read_id | ps->write_id) == ((op)->tas.rsize)))
	    return ((-7));
    } while (0);
    if (!(((&*opbuf)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))))
	return ((!((((const byte *) &((&*opbuf)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*opbuf) : (-7)));
...
static int s_proc_write_continue(os_ptr);
static const stream_template s_proc_write_template = { &st_sproc_state, ((void *) 0), s_proc_write_process, 1, 1, ((void *) 0) };

int swrite_proc(ref * sop, stream ** psstrm)
{
    return s_proc_init(sop, psstrm, 2, &s_proc_write_template);
}

static int s_proc_write_process(stream_state * st, stream_cursor_read * pr, stream_cursor_write * ignore_pw, bool last)
{
    uint rcount = pr->limit - pr->ptr;
    if (rcount > 0) {
	uint wcount = ((&((stream_proc_state *) st)->data)->tas.rsize) - ((stream_proc_state *) st)->index;
	uint count = (((rcount) < (wcount)) ? (rcount) : (wcount));
	memcpy(((stream_proc_state *) st)->data.value.bytes + ((stream_proc_state *) st)->index, pr->ptr + 1, count);
	pr->ptr += count;
	((stream_proc_state *) st)->index += count;
	if (rcount > wcount)
	    return ((int) (-4));
	else if (last) {
	    ((stream_proc_state *) st)->eof = ((bool) 1);
	    return ((int) (-4));
	} else
	    return 0;
    }
    return ((((stream_proc_state *) st)->eof = last) ? ((int) (-1)) : 0);
} int s_handle_write_exception(int status, const ref * fop, const ref * pstate, int nstate, int (*cont) (os_ptr))
{
    stream *ps;
    switch (status) {
    case ((int) (-3)):
    ...
    for (ps = (fop)->value.pfile; ps->strm != 0;)
	ps = ps->strm;
    if (((stream_proc_state *) ps->state)->eof) {
	...
	(e_stack.p) += 5;
	(((e_stack.p) - 4)->value.opproc = (zpop), (((e_stack.p) - 4)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) - 4)->tas.rsize = (0)));
	(((e_stack.p) - 3)->value.opproc = (zpop), (((e_stack.p) - 3)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) - 3)->tas.rsize = (0)));
	(((e_stack.p) - 1)->value.boolval = (0), (((e_stack.p) - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    } else {
	int npush = nstate + 6;
	if ((e_stack.p) > (e_stack.top) - (npush)) {
	    int es_code_ = ref_stack_extend(&e_stack, npush);
	...
	(((e_stack.p) + 1)->value.opproc = (cont), (((e_stack.p) + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) + 1)->tas.rsize = (0)));
	(e_stack.p) += npush;
	(((e_stack.p) - 4)->value.opproc = (s_proc_write_continue), (((e_stack.p) - 4)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) - 4)->tas.rsize = (0)));
	(e_stack.p)[-3] = *fop;
	(((e_stack.p) - 3)->tas.type_attrs &= ~(0x80));
	(((e_stack.p) - 1)->value.boolval = (1), (((e_stack.p) - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    }
    (e_stack.p)[-2] = ((stream_proc_state *) ps->state)->proc;
    *(e_stack.p) = ((stream_proc_state *) ps->state)->data;
    (((e_stack.p))->tas.rsize = (((stream_proc_state *) ps->state)->index));
    return 5;
}

static int s_proc_write_continue(os_ptr op)
{
    os_ptr opbuf = op - 1;
    stream *ps;
    stream_proc_state *ss;
    ...
    if (!(((&*opbuf)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_string) << 8) + (0x10))))
	return ((!((((const byte *) &((&*opbuf)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*opbuf) : (-7)));
    while ((ps->end_status = 0, ps->strm) != 0)
	ps = ps->strm;
    ss = (stream_proc_state *) ps->state;
...
const op_def *zfproc_op_defs(void)
{
    static const op_def op_defs_[] = { {"2%s_proc_read_continue", s_proc_read_continue}, {"2%s_proc_write_continue", s_proc_write_continue}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
