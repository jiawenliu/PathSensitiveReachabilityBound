...
static int zsetglobal(register os_ptr op)
{
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_boolean)))
	return (check_type_failed(&*op));
    ialloc_set_space((&gs_imemory), (op->value.boolval ? avm_global : avm_local));
    ((o_stack.p) -= (1));
    return 0;
}

static int zcurrentglobal(register os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    ((op)->value.boolval = ((((&gs_imemory))->current_space) != avm_local), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)));
    return 0;
}

static int zgcheck(register os_ptr op)
{
...
static int zsetvmthreshold(register os_ptr op)
{
    int code;
...
int set_vm_reclaim(long val)
{
    if (val >= -2 && val <= 0) {
	gs_memory_gc_status_t stat;
	gs_memory_gc_status((&gs_imemory)->spaces.named.system, &stat);
	stat.enabled = val >= -1;
	gs_memory_set_gc_status((&gs_imemory)->spaces.named.system, &stat);
	gs_memory_gc_status((&gs_imemory)->spaces.named.global, &stat);
	stat.enabled = val >= -1;
	gs_memory_set_gc_status((&gs_imemory)->spaces.named.global, &stat);
	gs_memory_gc_status((&gs_imemory)->spaces.named.local, &stat);
...
static int zvmreclaim(register os_ptr op)
{
    if (!((((const byte *) &((&*op)->tas.type_attrs))[1]) == (t_integer)))
	return (check_type_failed(&*op));
...
const op_def *zvmem2_op_defs(void)
{
    static const op_def op_defs_[] = { {"0.currentglobal", zcurrentglobal}, {"1.gcheck", zgcheck}, {"1.setglobal", zsetglobal}, {"level2dict", 0}, {"1setvmthreshold", zsetvmthreshold}, {"1vmreclaim", zvmreclaim}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
