...
void gs_setrasterop(gs_state * pgs, gs_rop3_t rop)
{
    (pgs)->log_op = ((rop & rop3_1) | (pgs->log_op & ~rop3_1));
} gs_rop3_t gs_currentrasterop(const gs_state * pgs)
{
    return ((gs_rop3_t) ((pgs->log_op) & 0xff));
}

void gs_setsourcetransparent(gs_state * pgs, bool transparent)
{
    (pgs)->log_op = ((transparent ? pgs->log_op | 0x100 : pgs->log_op & ~0x100));
} bool gs_currentsourcetransparent(const gs_state * pgs)
{
    return (pgs->log_op & 0x100) != 0;
}

void gs_settexturetransparent(gs_state * pgs, bool transparent)
{
    (pgs)->log_op = ((transparent ? pgs->log_op | 0x200 : pgs->log_op & ~0x200));
} bool gs_currenttexturetransparent(const gs_state * pgs)
{
    return (pgs->log_op & 0x200) != 0;
}

int gs_setrenderalgorithm(gs_state * pgs, int render_algorithm)
{
    if (render_algorithm < 0 || render_algorithm > 14)
	return ((-15));
    (pgs)->log_op = ((render_algorithm << 10) | (pgs->log_op & ~(0xf << 10)));
    return 0;
}

int gs_currentrenderalgorithm(const gs_state * pgs)
{
    return (pgs->log_op >> 10) & 0xf;
}

void gs_set_logical_op(gs_state * pgs, gs_logical_operation_t lop)
{
    pgs->log_op = lop;
} gs_logical_operation_t gs_current_logical_op(const gs_state * pgs)
{
    return pgs->log_op;
}
