...
static int xshow_continue(os_ptr);
static int yshow_continue(os_ptr);
static int xyshow_continue(os_ptr);
static const op_proc_p xyshow_continues[4] = { 0, xshow_continue, yshow_continue, xyshow_continue };

static int cshow_continue(os_ptr);
static int moveshow(os_ptr, int);
static int moveshow_continue(os_ptr, int);
static int zcshow(os_ptr op)
{
    os_ptr proc_op = op - 1;
    os_ptr str_op = op;
    gs_show_enum *penum;
    int code;
    if ((((proc_op)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))));
    else if ((((op)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80)))) {
	proc_op = op;
	str_op = op - 1;
    } else {
	...
	return code;
    if ((code = gs_cshow_n_init(penum, igs, (char *) str_op->value.bytes, ((str_op)->tas.rsize))) < 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), penum, "op_show_enum_setup");
	return code;
    }
    op_show_finish_setup(penum, 2, ((void *) 0));
    (((e_stack.p))[-1]) = *proc_op;
    return cshow_continue(op - 2);
}

static int cshow_continue(os_ptr op)
{
	...
	if (code == 5) {
	    (((e_stack.p) - 1)->value.opproc = (cshow_continue), (((e_stack.p) - 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) - 1)->tas.rsize = (0)));
	}
	return code;
    }
    {
	...
	((op - 2)->value.intval = (gs_show_current_char(penum)), ((op - 2)->tas.type_attrs = ((t_integer) << 8) + (0)));
	((op - 1)->value.realval = (wpt.x), ((op - 1)->tas.type_attrs = ((t_real) << 8) + (0)));
	((op)->value.realval = (wpt.y), ((op)->tas.type_attrs = ((t_real) << 8) + (0)));
	(++(e_stack.p), (((e_stack.p))->value.opproc = (cshow_continue), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
	(++(e_stack.p), (((e_stack.p))->value.opproc = (zsetfont), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
	*++(e_stack.p) = *(&((font_data *) ((gs_currentfont(igs))->client_data))->dict);
...
static int zglyphshow(os_ptr op)
{
    gs_glyph glyph;
    gs_show_enum *penum;
    int code;
	...
	if ((ulong) (*op).value.intval > (((unsigned long) 0xffffffffL + (unsigned long) 0) - ((gs_glyph) 0x80000000)))
	    return ((-15));
	glyph = (gs_glyph) op->value.intval + ((gs_glyph) 0x80000000);
	break;
    default:
	...
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), penum, "op_show_glyph");
	return code;
    }
    op_show_finish_setup(penum, 1, ((void *) 0));
    return op_show_continue(op - 1);
}

static int zrootfont(os_ptr op)
{
    do {
	if ((op += (1)) > (o_stack.top)) {
	    o_stack.requested = (1);
    ...
    *op = *(&((font_data *) ((gs_rootfont(igs))->client_data))->dict);
    return 0;
}

static int zxshow(os_ptr op)
{
    return moveshow(op, 1);
}

static int zyshow(os_ptr op)
{
    return moveshow(op, 2);
}

static int zxyshow(os_ptr op)
{
    return moveshow(op, 3);
}

static int moveshow(os_ptr op, int xymask)
{
    gs_show_enum *penum;
    int code = op_show_setup(op - 1, &penum);
    if (code != 0)
	return code;
    if ((code = gs_xyshow_n_init(penum, igs, (char *) op[-1].value.bytes, ((op - 1)->tas.rsize)) < 0)) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), penum, "op_show_enum_setup");
	return code;
    }
...
static int xshow_continue(os_ptr op)
{
    return moveshow_continue(op, 1);
}

static int yshow_continue(os_ptr op)
{
    return moveshow_continue(op, 2);
}

static int xyshow_continue(os_ptr op)
{
    return moveshow_continue(op, 3);
}

static int sget_real(const ref * nsp, int format, uint index, ref * pvalue)
{
    int code;
...
static int moveshow_continue(os_ptr op, int xymask)
{
    const ref *nsp = &(((e_stack.p))[-1]);
    int format = num_array_format(nsp);
    int code;
    gs_show_enum *penum = ((gs_show_enum *) (((e_stack.p))->value.pstruct));
    uint index = (((e_stack.p))[-2]).value.intval;
    for (;;) {
	ref rwx, rwy;
	code = gs_show_next(penum);
	if (code != 3) {
	    (((e_stack.p))[-2]).value.intval = index;
	    code = op_show_continue_dispatch(op, code);
	    if (code == 5) {
		(((e_stack.p) - 1)->value.opproc = (xyshow_continues[xymask]), (((e_stack.p) - 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p) - 1)->tas.rsize = (0)));
	    }
	    return code;
	}
	if (xymask & 1) {
...
const op_def *zchar2_op_defs(void)
{
    static const op_def op_defs_[] = { {"2cshow", zcshow}, {"0rootfont", zrootfont}, {"0%cshow_continue", cshow_continue}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}

const op_def *zchar2_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"1glyphshow", zglyphshow}, {"2xshow", zxshow}, {"2xyshow", zxyshow}, {"2yshow", zyshow}, {"0%xshow_continue", xshow_continue}, {"0%yshow_continue", yshow_continue}, {"0%xyshow_continue", xyshow_continue}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
