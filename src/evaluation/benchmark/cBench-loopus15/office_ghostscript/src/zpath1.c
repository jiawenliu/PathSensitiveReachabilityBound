...
static int common_arc(os_ptr, int (*)(gs_state *, floatp, floatp, floatp, floatp, floatp));
static int common_arct(os_ptr, float *);
int zarc(os_ptr op)
{
...
static int common_arc(os_ptr op, int (*aproc) (gs_state *, floatp, floatp, floatp, floatp, floatp))
{
    float xyra[5];
    int code;
...
static int zarcto(register os_ptr op)
{
    float tanxy[4];
...
static int common_arct(os_ptr op, float *tanxy)
{
    float args[5];
    int code;
...
static int zdashpath(register os_ptr op)
{
    return gs_dashpath(igs);
}

static int zflattenpath(register os_ptr op)
{
    return gs_flattenpath(igs);
}

static int zreversepath(register os_ptr op)
{
    return gs_reversepath(igs);
}

static int zstrokepath(register os_ptr op)
{
    return gs_strokepath(igs);
}

static int zclippath(register os_ptr op)
{
    return gs_clippath(igs);
}

static int zpathbbox(register os_ptr op)
{
    gs_rect box;
    int code;
...
static int path_continue(os_ptr);
static int path_cleanup(os_ptr);
static int zpathforall(register os_ptr op)
{
    gs_path_enum *penum;
    int code;
    ...
    if ((e_stack.p) > (e_stack.top) - (8)) {
	int es_code_ = ref_stack_extend(&e_stack, 8);
	if (es_code_ < 0)
	    return es_code_;
    };
    if ((penum = gs_path_enum_alloc(((gs_memory_t *) (gs_imemory.current)), "pathforall")) == 0)
	return ((-25));
    code = gs_path_enum_init(penum, igs);
    if (code < 0) {
	(*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), penum, "path_cleanup");
	return code;
    }
    (++(e_stack.p), (((e_stack.p))->value.opproc = (path_cleanup), (((e_stack.p))->tas.type_attrs = ((t_null) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (2))));
    memcpy((e_stack.p) + 1, op - 3, 4 * sizeof(ref));
    (e_stack.p) += 5;
    (((e_stack.p))->value.pstruct = ((obj_header_t *) (penum)), (((e_stack.p))->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&gs_imemory))->current_space))));
    (++(e_stack.p), (((e_stack.p))->value.opproc = (path_continue), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    ((o_stack.p) -= (4));
    op -= 4;
    return 5;
}

static void pf_push(gs_point *, int, os_ptr);
static int path_continue(register os_ptr op)
{
    gs_path_enum *penum = ((gs_path_enum *) (((e_stack.p))->value.pstruct));
    gs_point ppts[3];
    int code;
    if ((o_stack.top) - (o_stack.p) < (6)) {
	o_stack.requested = (6);
	return ((-16));
    };
	...
	return code;
    case 1:
	(e_stack.p)[2] = (e_stack.p)[-4];
	pf_push(ppts, 1, op);
	break;
    case 2:
	(e_stack.p)[2] = (e_stack.p)[-3];
	pf_push(ppts, 1, op);
	break;
    case 3:
	(e_stack.p)[2] = (e_stack.p)[-2];
	pf_push(ppts, 3, op);
	break;
    case 4:
	(e_stack.p)[2] = (e_stack.p)[-1];
	break;
    }
    (++(e_stack.p), (((e_stack.p))->value.opproc = (path_continue), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    ++(e_stack.p);
...
static void pf_push(gs_point * ppts, int n, os_ptr op)
{
    while (n--) {
	op += 2;
...
static int path_cleanup(os_ptr op)
{
    gs_path_enum *penum = ((gs_path_enum *) (((e_stack.p) + 6)->value.pstruct));
    gs_path_enum_cleanup(penum);
    (*(((gs_memory_t *) (gs_imemory.current)))->procs.free_object) (((gs_memory_t *) (gs_imemory.current)), penum, "path_cleanup");
    return 0;
}

const op_def *zpath1_op_defs(void)
{
    static const op_def op_defs_[] = { {"5arc", zarc}, {"5arcn", zarcn}, {"5arct", zarct}, {"5arcto", zarcto}, {"0clippath", zclippath}, {"0.dashpath", zdashpath}, {"0flattenpath", zflattenpath}, {"4pathforall", zpathforall}, {"0reversepath", zreversepath}, {"0strokepath", zstrokepath}, {"0.pathbbox", zpathbbox}, {"0%path_continue", path_continue}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}
