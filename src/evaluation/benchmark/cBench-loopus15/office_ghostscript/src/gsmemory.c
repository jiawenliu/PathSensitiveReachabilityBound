...
const gs_memory_struct_type_t st_free = { sizeof(byte), "(free)", 0, 0, 0, 0, 0 };
const gs_memory_struct_type_t st_bytes = { sizeof(byte), "bytes", 0, 0, 0, 0, 0 };

void gs_alloc_memset(void *ptr, int fill, ulong lsize)
{
    ulong msize = lsize;
    char *p = ptr;
    int isize;
...
static gs_memory_type_ptr_t gs_heap_object_type(gs_memory_t * mem, const void *obj);
...
static void gs_heap_register_root(gs_memory_t * mem, gs_gc_root_t * root, gs_ptr_type_t ptype, void **pp, client_name_t cname);
...
gs_memory_t gs_memory_default = { {gs_heap_alloc_bytes, gs_heap_alloc_bytes, gs_heap_alloc_struct, gs_heap_alloc_struct, gs_heap_alloc_byte_array, gs_heap_alloc_byte_array, gs_heap_alloc_struct_array, gs_heap_alloc_struct_array, gs_heap_resize_object, gs_heap_object_size, gs_heap_object_type, gs_heap_free_object, gs_heap_alloc_string, gs_heap_alloc_string, gs_heap_resize_string, gs_heap_free_string, gs_heap_register_root, gs_heap_unregister_root, gs_heap_status, gs_heap_enable_free} };
...
struct malloc_block_s {
    ...
    gs_memory_type_ptr_t type;
    client_name_t cname;
    byte _pad[((-((int) (sizeof(struct malloc_block_data_s))) & 7) == 0 ? 8 : (-((int) (sizeof(struct malloc_block_data_s))) & 7))];
};
static malloc_block *malloc_list;
static long malloc_used;
long gs_malloc_max = 0;
long gs_malloc_limit = (~(-1L << ((1 << 2) * 8 - 1)));
static long heap_available(void);
void gs_malloc_init(void)
{
    malloc_list = 0;
    malloc_used = 0;
    gs_malloc_max = 0;
...
static byte *gs_heap_alloc_bytes(gs_memory_t * mem, uint size, client_name_t cname)
{
    byte *ptr = 0;
    const char *msg;
...
static void *gs_heap_resize_object(gs_memory_t * mem, void *obj, uint new_num_elements, client_name_t cname)
{
    ...
    uint old_size = (*(mem)->procs.object_size) (mem, obj) + sizeof(malloc_block);
    uint new_size = gs_struct_type_size(pstype) * new_num_elements + sizeof(malloc_block);
...
} static gs_memory_type_ptr_t gs_heap_object_type(gs_memory_t * mem, const void *ptr)
{
...
} static void gs_heap_free_object(gs_memory_t * mem, void *ptr, client_name_t cname)
{
    ...
    if ((('a') >= 'a' && ('a') <= 'z' ? gs_debug['a'] | gs_debug[('a') ^ 32] : gs_debug['a']))
	fprintf(gs_stderr, "[a-]gs_free(%s) 0x%lx(%u)\n", ((const char *) (cname)), (ulong) ptr, (ptr == 0 ? 0 : ((malloc_block *) ptr)[-1].size));
    if (ptr == 0)
	return;
    if (ptr == bp + 1) {
	malloc_list = bp->next;
	...
	(lprintf_file_and_line(gs_stderr, "gsmemory.c", 290), fprintf(gs_stderr, "%s: free 0x%lx not found!\n", ((const char *) (cname)), (ulong) ptr));
...
static byte *gs_heap_resize_string(gs_memory_t * mem, byte * data, uint old_num, uint new_num, client_name_t cname)
{
	...
	(lprintf_file_and_line(gs_stderr, "gsmemory.c", 303), fprintf(gs_stderr, "%s: resizing non-string 0x%lx!\n", ((const char *) (cname)), (ulong) data));
    }
...
} static void gs_heap_register_root(gs_memory_t * mem, gs_gc_root_t * rp, gs_ptr_type_t ptype, void **up, client_name_t cname)
{
} static void gs_heap_unregister_root(gs_memory_t * mem, gs_gc_root_t * rp, client_name_t cname)
{
...
{
    if (enable)
	mem->procs.free_object = gs_heap_free_object, mem->procs.free_string = gs_heap_free_string;
    else
...
void gs_malloc_release(void)
{
	...
	np = bp->next;
	if ((('a') >= 'a' && ('a') <= 'z' ? gs_debug['a'] | gs_debug[('a') ^ 32] : gs_debug['a']))
	    fprintf(gs_stderr, "[a]gs_malloc_release(%s) 0x%lx(%u)\n", ((const char *) (bp->cname)), (ulong) (bp + 1), bp->size);
	do {
	} while (0);
	free(bp);
    }
...
{
} gs_ptr_type_t gs_no_struct_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    return 0;
}

void gs_no_struct_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
}} uint gs_struct_type_size(gs_memory_type_ptr_t pstype)
{
    return pstype->ssize;
}

struct_name_t gs_struct_type_name(gs_memory_type_ptr_t pstype)
{
    return pstype->sname;
}

void rc_free_struct_only(gs_memory_t * mem, void *data, client_name_t cname)
{
