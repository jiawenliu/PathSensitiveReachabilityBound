...
int zcolor_remap_one(const ref *, os_ptr, gx_transfer_map *, const gs_state *, int (*)(os_ptr));
int zcolor_remap_one_finish(os_ptr);
int zcolor_remap_one_signed_finish(os_ptr);
int zcolor_reset_transfer(os_ptr);
int zcolor_remap_color(os_ptr);
int zscreen_params(os_ptr op, gs_screen_halftone * phs);
int zscreen_enum_init(os_ptr op, const gx_ht_order * porder, gs_screen_halftone * phs, ref * pproc, int npop, int (*finish_proc) (os_ptr), gs_memory_t * mem);
...
static int dict_spot_results(ref *, const gs_spot_halftone *);
static int dict_threshold_params(const ref *, gs_threshold_halftone *, ref *);
float spot_dummy(floatp, floatp);
int spot_sample_finish(os_ptr);
static int sethalftone_finish(os_ptr);
static int sethalftone_cleanup(os_ptr);
static int zsethalftone5(register os_ptr op)
{
    uint count;
    ...
    static const char *color_names[] = { "Default", "Gray", "Red", "Green", "Blue", "Cyan", "Magenta", "Yellow", "Black" };
    ref sprocs[(sizeof(color_names) / sizeof((color_names)[0]))];
    ref tprocs[(sizeof(color_names) / sizeof((color_names)[0]))];
    gs_memory_t *mem;
    int npop = 2;
    ...
    if (!(((&*(&(&op[-1])->value.pdict->values))->tas.type_attrs) & (0x20)))
	return ((-7));
    count = 0;
    ...
    mem = (gs_memory_t *) (&gs_imemory)->spaces.indexed[((int) (avm_space) (((op - 1)->tas.type_attrs) & (((1 << 2) - 1) << 2)) >> 2)];
    if ((e_stack.p) > (e_stack.top) - (5)) {
	int es_code_ = ref_stack_extend(&e_stack, 5);
	if (es_code_ < 0)
	    return es_code_;
    };
    refset_null(sprocs, (sizeof(sprocs) / sizeof((sprocs)[0])));
    refset_null(tprocs, (sizeof(tprocs) / sizeof((tprocs)[0])));
    do {
	if (((pht) = (gs_halftone *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_halftone, ".sethalftone5")) == 0) {
	    pht = 0;
	} else {
	    (pht)->rc.ref_count = 0;
	    (pht)->rc.memory = ((gs_memory_t *) (gs_imemory.current));
	    (pht)->rc.free = rc_free_struct_only;
	}
    } while (0);
    phtc = (gs_halftone_component *) (*(mem)->procs.alloc_struct_array) (mem, count, &st_ht_component_element, ".sethalftone5");
    do {
	if (((pdht) = (gx_device_halftone *) (*(((gs_memory_t *) (gs_imemory.current)))->procs.alloc_struct) (((gs_memory_t *) (gs_imemory.current)), &st_device_halftone, ".sethalftone5")) == 0) {
	    pdht = 0;
	} else {
	    (pdht)->rc.ref_count = 0;
	    (pdht)->rc.memory = ((gs_memory_t *) (gs_imemory.current));
	    (pdht)->rc.free = rc_free_struct_only;
	}
    } while (0);
	...
	for (i = 0, j = 0, pc = phtc; i < (sizeof(color_names) / sizeof((color_names)[0])); i++) {
	    ...
	    ref *pvalue;
	    if (dict_find_string(op, color_names[i], &pvalue) > 0) {
		if (!((((const byte *) &((&*pvalue)->tas.type_attrs))[1]) == (t_dictionary)))
		    return ((-20));
		if (!(((&*(&(&*pvalue)->value.pdict->values))->tas.type_attrs) & (0x20)))
		    return ((-7));
		if (dict_int_param(pvalue, "HalftoneType", 1, 5, 0, &type) < 0) {
		    code = ((-20));
		    break;
		}
		switch (type) {
		default:
		    ...
		    break;
		case 1:
		    code = dict_spot_params(pvalue, &pc->params.spot, sprocs + j, tprocs + j);
		    pc->params.spot.screen.spot_function = spot_dummy;
		    pc->type = ht_type_spot;
		    break;
		case 3:
		    code = dict_threshold_params(pvalue, &pc->params.threshold, tprocs + j);
	...
	pht->type = (type == 2 || type == 4 ? ht_type_multiple_colorscreen : ht_type_multiple);
	...
	op = (o_stack.p);
	(e_stack.p) += 5;
	(((e_stack.p) - 4)->value.opproc = (sethalftone_cleanup), (((e_stack.p) - 4)->tas.type_attrs = ((t_null) << 8) + (0x80)), (((e_stack.p) - 4)->tas.rsize = (0)));
	(e_stack.p)[-3] = odict;
	(((e_stack.p) - 2)->value.pstruct = ((obj_header_t *) (pht)), (((e_stack.p) - 2)->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&gs_imemory))->current_space))));
	(((e_stack.p) - 1)->value.pstruct = ((obj_header_t *) (pdht)), (((e_stack.p) - 1)->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&gs_imemory))->current_space))));
	(((e_stack.p))->value.opproc = (sethalftone_finish), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0)));
	for (j = 0; j < count; j++) {
	    gx_ht_order *porder = (pdht->components == 0 ? &pdht->order : &pdht->components[j].corder);
	    switch (phtc[j].type) {
	    case ht_type_spot:
		code = zscreen_enum_init(op, porder, &phtc[j].params.spot.screen, &sprocs[j], 0, 0, mem);
		if (code < 0)
		    break;
	    case ht_type_threshold:
		if (!((((const byte *) &((tprocs + j)->tas.type_attrs))[1]) == (t__invalid))) {
		    if ((o_stack.top) - (o_stack.p) < (zcolor_remap_one_ostack)) {
			o_stack.requested = (zcolor_remap_one_ostack);
			return ((-16));
		    };
		    if ((e_stack.p) > (e_stack.top) - (zcolor_remap_one_estack)) {
			int es_code_ = ref_stack_extend(&e_stack, zcolor_remap_one_estack);
			if (es_code_ < 0)
			    return es_code_;
		    };
		    code = zcolor_remap_one(tprocs + j, op, porder->transfer, igs, zcolor_remap_one_finish);
		    op = (o_stack.p);
		}
		break;
	    default:;
	    ...
	    if (code < 0) {
		ref_stack_pop(&o_stack, ref_stack_count(&o_stack) - (odepth));
    ...
    if (code < 0) {
	(*(mem)->procs.free_object) (mem, pdht, ".sethalftone5");
	(*(mem)->procs.free_object) (mem, phtc, ".sethalftone5");
	(*(mem)->procs.free_object) (mem, pht, ".sethalftone5");
	return code;
    }
    ((o_stack.p) -= (npop));
    return 5;
...
static int sethalftone_finish(os_ptr op)
{
    gx_device_halftone *pdht = ((gx_device_halftone *) (((e_stack.p))->value.pstruct));
    int code;
    if (pdht->components)
	pdht->order = pdht->components[0].corder;
    code = gx_ht_install(igs, ((gs_halftone *) (((e_stack.p) - 1)->value.pstruct)), pdht);
    if (code < 0)
	return code;
    ((int_gstate *) gs_state_client_data(igs))->halftone = (e_stack.p)[-2];
    (e_stack.p) -= 4;
    sethalftone_cleanup(op);
    return 14;
}

static int sethalftone_cleanup(os_ptr op)
{
    gx_device_halftone *pdht = ((gx_device_halftone *) ((&(e_stack.p)[4])->value.pstruct));
    gs_halftone *pht = ((gs_halftone *) ((&(e_stack.p)[3])->value.pstruct));
    (*(pdht->rc.memory)->procs.free_object) (pdht->rc.memory, pdht, "sethalftone_cleanup(device halftone)");
    (*(pht->rc.memory)->procs.free_object) (pht->rc.memory, pht, "sethalftone_cleanup(halftone)");
    return 0;
}

const op_def *zht2_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"2.sethalftone5", zsethalftone5}, {"0%sethalftone_finish", sethalftone_finish}, {(char *) 0, (op_proc_p) 0} };
    return op_defs_;
}

static int dict_spot_params(const ref * pdict, gs_spot_halftone * psp, ref * psproc, ref * ptproc)
{
    int code;
    if (!(((&*(&(&*pdict)->value.pdict->values))->tas.type_attrs) & (0x20)))
	return ((-7));
    if ((code = dict_float_param(pdict, "Frequency", 0.0, &psp->screen.frequency)) != 0 || (code = dict_float_param(pdict, "Angle", 0.0, &psp->screen.angle)) != 0 || (code = dict_proc_param(pdict, "SpotFunction", psproc, ((bool) 0))) != 0 || (code = dict_bool_param(pdict, "AccurateScreens", gs_currentaccuratescreens(), &psp->accurate_screens)) < 0 || (code = dict_proc_param(pdict, "TransferFunction", ptproc, ((bool) 0))) < 0)
	return (code < 0 ? code : (-21));
    psp->transfer = (code > 0 ? (gs_mapping_proc) 0 : gs_mapped_transfer);
    return 0;
}

static int dict_real_result(ref * pdict, const char *kstr, floatp val)
{
    int code = 0;
    ref *ignore;
	...
	if (!(((&*(&(&*pdict)->value.pdict->values))->tas.type_attrs) & (0x10)))
	    return ((-7));
	((&rval)->value.realval = (val), ((&rval)->tas.type_attrs = ((t_real) << 8) + (0)));
	code = dict_put_string(pdict, kstr, &rval);
    }
    return code;
}

static int dict_spot_results(ref * pdict, const gs_spot_halftone * psp)
{
    int code;
    code = dict_real_result(pdict, "ActualFrequency", psp->screen.actual_frequency);
    if (code < 0)
	return code;
    return dict_real_result(pdict, "ActualAngle", psp->screen.actual_angle);
}
...
static int dict_threshold_params(const ref * pdict, gs_threshold_halftone * ptp, ref * ptproc)
{
    ...
    if (!(((&*(&(&*pdict)->value.pdict->values))->tas.type_attrs) & (0x20)))
	return ((-7));
    if ((code = dict_int_param(pdict, "Width", 1, 0x7fff, -1, &ptp->width)) < 0 || (code = dict_int_param(pdict, "Height", 1, 0x7fff, -1, &ptp->height)) < 0 || (code = dict_find_string(pdict, "Thresholds", &tstring)) <= 0 || (code = dict_proc_param(pdict, "TransferFunction", ptproc, ((bool) 0))) < 0)
	return (code < 0 ? code : (-21));
    if (!(((&*tstring)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))))
	return ((!((((const byte *) &((&*tstring)->tas.type_attrs))[1]) == (t_string)) ? (-20) : (-7)));
    if (((tstring)->tas.rsize) != (long) ptp->width * ptp->height)
	return ((-15));
    ptp->thresholds.data = tstring->value.const_bytes;
    ptp->thresholds.size = ((tstring)->tas.rsize);
