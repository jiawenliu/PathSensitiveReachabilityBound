...
static void zfdecode_init(void)
{
    scan_ascii85_template = &s_A85D_template;
} static int zA85E(os_ptr op)
{
    return filter_write_simple(op, &s_A85E_template);
}

static int zA85D(os_ptr op)
{
    return filter_read_simple(op, &s_A85D_template);
}

int zcf_setup(os_ptr op, stream_CF_state * pcfs)
{
    int code;
    if ((code = dict_bool_param(op, "Uncompressed", ((bool) 0), &pcfs->Uncompressed)) < 0 || (code = dict_int_param(op, "K", -32000, 32000, 0, &pcfs->K)) < 0 || (code = dict_bool_param(op, "EndOfLine", ((bool) 0), &pcfs->EndOfLine)) < 0 || (code = dict_bool_param(op, "EncodedByteAlign", ((bool) 0), &pcfs->EncodedByteAlign)) < 0 || (code = dict_int_param(op, "Columns", 0, (2560 * 32000 * 2 / 3), 1728, &pcfs->Columns)) < 0 || (code = dict_int_param(op, "Rows", 0, 32000, 0, &pcfs->Rows)) < 0 || (code = dict_bool_param(op, "EndOfBlock", ((bool) 1), &pcfs->EndOfBlock)) < 0 || (code = dict_bool_param(op, "BlackIs1", ((bool) 0), &pcfs->BlackIs1)) < 0 || (code = dict_int_param(op, "DamagedRowsBeforeError", 0, 32000, 0, &pcfs->DamagedRowsBeforeError)) < 0 || (code = dict_bool_param(op, "FirstBitLowOrder", ((bool) 0), &pcfs->FirstBitLowOrder)) < 0 || (code = dict_int_param(op, "DecodedByteAlign", 1, 16, 1, &pcfs->DecodedByteAlign)) < 0)
	return code;
...
int filter_read_predictor(os_ptr op, int npop, const stream_template * template, stream_state * st)
{
	...
	op = (o_stack.p);
	(*(&rfd) = *(op));
	code = (predictor == 2 ? filter_read(op, 0, &s_PDiffD_template, (stream_state *) & pds, 0) : filter_read(op, 0, &s_PNGPD_template, (stream_state *) & pps, 0));
	if (code < 0) {
	    (o_stack.p) = ++op;
	    (*(op - 1) = *(&rsource));
	    (*(op) = *(&rdict));
...
int zlz_setup(os_ptr op, stream_LZW_state * plzs)
{
	...
	dop = 0, npop = 0;
    if ((code = dict_int_param(dop, "EarlyChange", 0, 1, 1, &plzs->EarlyChange)) < 0 || (code = dict_int_param(dop, "InitialCodeLength", 2, 11, 8, &plzs->InitialCodeLength)) < 0 || (code = dict_bool_param(dop, "FirstBitLowOrder", ((bool) 0), &plzs->FirstBitLowOrder)) < 0 || (code = dict_bool_param(dop, "BlockData", ((bool) 0), &plzs->BlockData)) < 0)
	return code;
    return npop;
}

static int zLZWD(os_ptr op)
{
    stream_LZW_state lzs;
    int code = zlz_setup(op, &lzs);
    if (code < 0)
	return code;
    return filter_read_predictor(op, code, &s_LZWD_template, (stream_state *) & lzs);
}
...
int zpd_setup(os_ptr op, stream_PDiff_state * ppds)
{
    ...
    if (!(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)))
	return ((-7));
    if ((code = dict_int_param(op, "Colors", 1, 4, 1, &ppds->Colors)) < 0 || (code = dict_int_param(op, "BitsPerComponent", 1, 8, 8, &bpc)) < 0 || (bpc & (bpc - 1)) != 0 || (code = dict_int_param(op, "Columns", 1, (~(-1 << ((1 << 2) * 8 - 1))), 1, &ppds->Columns)) < 0)
	return (code < 0 ? code : ((-15)));
    ppds->BitsPerComponent = bpc;
    return 0;
}

static int zPDiffE(os_ptr op)
{
    stream_PDiff_state pds;
...
int zpp_setup(os_ptr op, stream_PNGP_state * ppps)
{
    ...
    if (!(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)))
	return ((-7));
    if ((code = dict_int_param(op, "Colors", 1, 16, 1, &ppps->Colors)) < 0 || (code = dict_int_param(op, "BitsPerComponent", 1, 16, 8, &bpc)) < 0 || (bpc & (bpc - 1)) != 0 || (code = dict_uint_param(op, "Columns", 1, ((unsigned int) 0xffffffff + (unsigned int) 0), 1, &ppps->Columns)) < 0 || (code = dict_int_param(op, "Predictor", 10, 15, 15, &ppps->Predictor)) < 0)
	return (code < 0 ? code : ((-15)));
    ppps->BitsPerComponent = bpc;
    return 0;
}

static int zPNGPE(os_ptr op)
{
    stream_PNGP_state pps;
...
const op_def *zfdecode_op_defs(void)
{
    static const op_def op_defs_[] = { {"filterdict", 0}, {"1ASCII85Encode", zA85E}, {"1ASCII85Decode", zA85D}, {"2CCITTFaxDecode", zCFD}, {"1LZWDecode", zLZWD}, {"2PixelDifferenceDecode", zPDiffD}, {"2PixelDifferenceEncode", zPDiffE}, {"2PNGPredictorDecode", zPNGPD}, {"2PNGPredictorEncode", zPNGPE}, {(char *) 0, (op_proc_p) zfdecode_init} };
    return op_defs_;
}
