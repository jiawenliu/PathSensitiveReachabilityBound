...
gx_io_device gs_iodev_os = { "%os%", "FileSystem", {iodev_no_init, iodev_no_open_device, ((void *) 0), iodev_os_fopen, iodev_os_fclose, os_delete, os_rename, os_status, os_enumerate, gp_enumerate_files_next, gp_enumerate_files_close, os_get_params, iodev_no_put_params} };

void gs_iodev_init(gs_memory_t * mem)
{
...
file_enum *iodev_no_enumerate_files(gx_io_device * iodev, const char *pat, uint patlen, gs_memory_t * memory)
{
    return ((void *) 0);
} int iodev_no_get_params(gx_io_device * iodev, gs_param_list * plist)
{
...
static int os_get_params(gx_io_device * iodev, gs_param_list * plist)
{
    int code;
    bool btrue = ((bool) 1);
    if ((code = (*(plist)->procs->xmit_bool) (plist, "HasNames", &btrue)) < 0)
	return code;
    return 0;
}

gx_io_device *gs_getiodevice(int index)
{
...
gx_io_device *gs_findiodevice(const byte * str, uint len)
{
    gx_io_device **pftab;
    if (len > 1 && str[len - 1] == '%')
	len--;
    for (pftab = gx_io_device_table; *pftab != ((void *) 0); pftab++) {
...
int gs_getdevparams(gx_io_device * iodev, gs_param_list * plist)
{
    gs_param_string ts;
    int code;
    (ts).data = (const byte *) (iodev->dtype), (ts).size = strlen((const char *) (ts).data), (ts).persistent = ((bool) 1);
    code = (*(plist)->procs->xmit_name) (plist, "Type", &ts);
    if (code < 0)
	return code;
    return (*iodev->procs.get_params) (iodev, plist);
}

int gs_putdevparams(gx_io_device * iodev, gs_param_list * plist)
{
    return (*iodev->procs.put_params) (iodev, plist);
}

int gs_fopen_errno_to_code(int eno)
{
    switch (eno) {
    case 2:
	return ((-22));
