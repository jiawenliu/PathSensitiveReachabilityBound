...
int alloc_save_change(gs_dual_memory_t *, const ref * pcont, ref_packed * ptr, client_name_t cname);
int alloc_save_level(const gs_dual_memory_t *);
static int cache_common(gs_cie_common *, const ref_cie_procs *, void *, const gs_state *);
static gs_ptr_type_t cie_defg_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (((gs_cie_defg *) vptr)->Table.table);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void cie_defg_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((gs_cie_defg *) vptr)->Table.table = gs_reloc_struct_ptr((const void *) ((const gs_cie_defg *) vptr)->Table.table, gcst);
} static const gs_memory_struct_type_t st_cie_defg = { sizeof(gs_cie_defg), "gs_cie_defg", 0, 0, cie_defg_enum_ptrs, cie_defg_reloc_ptrs, 0 };

static gs_ptr_type_t cie_def_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) (((gs_cie_def *) vptr)->Table.table);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void cie_def_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((gs_cie_def *) vptr)->Table.table = gs_reloc_struct_ptr((const void *) ((const gs_cie_def *) vptr)->Table.table, gcst);
} static const gs_memory_struct_type_t st_cie_def = { sizeof(gs_cie_def), "gs_cie_def", 0, 0, cie_def_enum_ptrs, cie_def_reloc_ptrs, 0 };
static const gs_memory_struct_type_t st_cie_abc = { sizeof(gs_cie_abc), "gs_cie_abc", 0, 0, 0, 0, 0 };
static const gs_memory_struct_type_t st_cie_a = { sizeof(gs_cie_a), "gs_cie_a", 0, 0, 0, 0, 0 };

static ref empty_procs[4];
extern const gs_color_space_type gs_color_space_type_CIEDEFG, gs_color_space_type_CIEDEF, gs_color_space_type_CIEABC, gs_color_space_type_CIEA;
gs_color_space_type cs_type_zCIEDEFG, cs_type_zCIEDEF, cs_type_zCIEABC, cs_type_zCIEA;
static int cs_install_zCIEDEFG(gs_color_space *, gs_state *);
static int cs_install_zCIEDEF(gs_color_space *, gs_state *);
static int cs_install_zCIEABC(gs_color_space *, gs_state *);
static int cs_install_zCIEA(gs_color_space *, gs_state *);
static void zcie_init(void)
{
    ((&empty_procs[0])->value.const_refs = ((const ref *) ((void *) 0)), ((&empty_procs[0])->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) + 0x80)), ((&empty_procs[0])->tas.rsize = (0)));
    ((&empty_procs[1])->value.const_refs = ((const ref *) ((void *) 0)), ((&empty_procs[1])->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) + 0x80)), ((&empty_procs[1])->tas.rsize = (0)));
    ((&empty_procs[2])->value.const_refs = ((const ref *) ((void *) 0)), ((&empty_procs[2])->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) + 0x80)), ((&empty_procs[2])->tas.rsize = (0)));
    ((&empty_procs[3])->value.const_refs = ((const ref *) ((void *) 0)), ((&empty_procs[3])->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) + 0x80)), ((&empty_procs[3])->tas.rsize = (0)));
...
{
    int code = dict_float_array_param(pdref, kstr, count * 2, (float *) prange, ((void *) 0));
    if (code < 0)
	return code;
    else if (code == 0)
...
int dict_proc_array_param(const ref * pdict, const char *kstr, uint count, ref * pparray)
{
	...
	uint i;
	if (!((((const byte *) &((&*pvalue)->tas.type_attrs))[1]) == (t_array)))
	    return ((-20));
...
{
    int code;
    if ((code = dict_float_array_param(pdref, "WhitePoint", 3, (float *) &pwb->WhitePoint, ((void *) 0))) != 3 || (code = dict_float_array_param(pdref, "BlackPoint", 3, (float *) &pwb->BlackPoint, (const float *) &BlackPoint_default)) != 3)
	return (code < 0 ? code : ((-15)));
    if (pwb->WhitePoint.u <= 0 || pwb->WhitePoint.v != 1 || pwb->WhitePoint.w <= 0 || pwb->BlackPoint.u < 0 || pwb->BlackPoint.v < 0 || pwb->BlackPoint.w < 0)
	return ((-15));
    return 0;
}

static int cie_3d_table_param(const ref * ptable, uint count, uint nbytes, gs_const_string * strings);
int cie_table_param(const ref * ptref, gx_color_lookup_table * pclt, gs_memory_t * mem)
{
    ...
    gs_const_string *table;
    for (i = 0; i < n; ++i) {
	if (!((((const byte *) &((&pta[i])->tas.type_attrs))[1]) == (t_integer)))
	    return ((-20));
    ...
    if (n == 3) {
	table = (gs_const_string *) (*(mem)->procs.alloc_byte_array) (mem, pclt->dims[0], sizeof(gs_const_string), "cie_table_param");
	if (table == 0)
	    return ((-25));
	...
	uint ntables = d0 * d1;
	const ref *psuba;
	if (!(((&pta[4])->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_array) << 8) + (0x20))))
	    return ((!((((const byte *) &((&pta[4])->tas.type_attrs))[1]) == (t_array)) ? check_type_failed(&pta[4]) : (-7)));
	if (((pta + 4)->tas.rsize) != d0)
	    return ((-15));
	table = (gs_const_string *) (*(mem)->procs.alloc_byte_array) (mem, ntables, sizeof(gs_const_string), "cie_table_param");
	if (table == 0)
	    return ((-25));
    ...
    if (code < 0) {
	(*(mem)->procs.free_object) (mem, table, "cie_table_param");
	return code;
    }
    pclt->table = table;
    return 0;
...
static int cie_3d_table_param(const ref * ptable, uint count, uint nbytes, gs_const_string * strings)
{
    const ref *rstrings;
    uint i;
    if (!(((&*ptable)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_array) << 8) + (0x20))))
	return ((!((((const byte *) &((&*ptable)->tas.type_attrs))[1]) == (t_array)) ? check_type_failed(&*ptable) : (-7)));
    if (((ptable)->tas.rsize) != count)
	return ((-15));
    rstrings = ptable->value.const_refs;
    for (i = 0; i < count; ++i) {
	const ref *prt2 = rstrings + i;
	if (!(((&*prt2)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))))
	    return ((!((((const byte *) &((&*prt2)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*prt2) : (-7)));
...
static int cie_lmnp_param(const ref * pdref, gs_cie_common * pcie, ref_cie_procs * pcprocs)
{
    int code;
    if ((code = dict_ranges_param(pdref, "RangeLMN", 3, (&pcie->RangeLMN)->ranges)) < 0 || (code = dict_proc_array_param(pdref, "DecodeLMN", 3, &pcprocs->DecodeLMN)) < 0 || (code = dict_float_array_param(pdref, "MatrixLMN", 9, (float *) &pcie->MatrixLMN, (const float *) &Matrix3_default)) != 9 || (code = cie_points_param(pdref, &pcie->points)) < 0)
	return (code < 0 ? code : ((-15)));
    pcie->DecodeLMN = DecodeLMN_default;
    return 0;
}

static int cie_abc_param(const ref * pdref, gs_cie_abc_common * pcie, ref_cie_procs * pcprocs)
{
    int code;
    if ((code = dict_ranges_param(pdref, "RangeABC", 3, (&pcie->RangeABC)->ranges)) < 0 || (code = dict_proc_array_param(pdref, "DecodeABC", 3, &pcprocs->Decode.ABC)) < 0 || (code = dict_float_array_param(pdref, "MatrixABC", 9, (float *) &pcie->MatrixABC, (const float *) &Matrix3_default)) != 9 || (code = cie_lmnp_param(pdref, &pcie->common, pcprocs)) < 0)
	return (code < 0 ? code : ((-15)));
    pcie->DecodeABC = DecodeABC_default;
    return 0;
}

static int set_cie_finish(os_ptr op, gs_color_space * pcs, const ref_cie_procs * pcprocs)
{
    ref_colorspace cspace_old;
    uint edepth = ref_stack_count(&e_stack);
    int code;
    cspace_old = ((int_gstate *) gs_state_client_data(igs))->colorspace;
    ((int_gstate *) gs_state_client_data(igs))->colorspace.procs.cie = *pcprocs;
...
static int zsetcieabcspace(register os_ptr op)
{
	...
	return ((-7));
    procs = ((int_gstate *) gs_state_client_data(igs))->colorspace.procs;
    do {
	if (((pcie) = (gs_cie_abc *) (*(mem)->procs.alloc_struct) (mem, &st_cie_abc, "setcolorspace(CIEBasedABC)")) == 0) {
	    return ((-25));
	} else {
	    (pcie)->rc.ref_count = 0;
	    (pcie)->rc.memory = mem;
    ...
    if (code < 0) {
	(*(pcie)->rc.free) ((pcie)->rc.memory, (void *) (pcie), "setcolorspace(CIEBasedABC)");
	return code;
    }
    cs.params.abc = pcie;
...
static int zsetcieaspace(register os_ptr op)
{
    gs_memory_t *mem = gs_state_memory(igs);
    gs_color_space cs;
    ref_color_procs procs;
	...
	return ((-7));
    procs = ((int_gstate *) gs_state_client_data(igs))->colorspace.procs;
    if ((code = dict_proc_param(op, "DecodeA", &procs.cie.Decode.A, ((bool) 1))) < 0)
	return code;
    do {
	if (((pcie) = (gs_cie_a *) (*(mem)->procs.alloc_struct) (mem, &st_cie_a, "setcolorspace(CIEBasedA)")) == 0) {
	    return ((-25));
	} else {
	    (pcie)->rc.ref_count = 0;
	    (pcie)->rc.memory = mem;
    ...
    if ((code = dict_float_array_param(op, "RangeA", 2, (float *) &pcie->RangeA, (const float *) &RangeA_default)) != 2 || (code = dict_float_array_param(op, "MatrixA", 3, (float *) &pcie->MatrixA, (const float *) &MatrixA_default)) != 3 || (code = cie_lmnp_param(op, &pcie->common, &procs.cie)) < 0) {
	(*(pcie)->rc.free) ((pcie)->rc.memory, (void *) (pcie), "setcolorspace(CIEBasedA)");
	return (code < 0 ? code : ((-15)));
    }
...
static int cs_install_zCIEDEFG(gs_color_space * pcs, gs_state * pgs)
{
    return ((-21));
}

static int cs_install_zCIEDEF(gs_color_space * pcs, gs_state * pgs)
{
    return ((-21));
}

static int cie_abc_finish(os_ptr);
static int cs_install_zCIEABC(gs_color_space * pcs, gs_state * pgs)
{
    es_ptr ep = (e_stack.p);
    ...
    int code = (*gs_color_space_type_CIEABC.install_cspace) (pcs, pgs);
    if (code < 0 || (code = cie_cache_joint(&pigs->colorrendering.procs, pgs)) < 0 || (code = cie_cache_push_finish(cie_abc_finish, pgs, pcie)) < 0 || (code = cie_prepare_caches_3(&pcie->RangeABC, pcprocs->Decode.ABC.value.const_refs, &(&pcie->caches.DecodeABC[0])->floats, &(&pcie->caches.DecodeABC[0])[1].floats, &(&pcie->caches.DecodeABC[0])[2].floats, pcie, pgs, "Decode.ABC")) < 0 || (code = cache_common(&pcie->common, pcprocs, pcie, pgs)) < 0) {
	(e_stack.p) = ep;
	return code;
    }
    return 5;
...
static int cie_abc_finish(os_ptr op)
{
    gs_cie_abc_complete(((gs_cie_abc *) ((op)->value.pstruct)));
    ((o_stack.p) -= (1));
    return 0;
}

static int cie_a_finish(os_ptr);
static int cs_install_zCIEA(gs_color_space * pcs, gs_state * pgs)
{
    es_ptr ep = (e_stack.p);
    gs_cie_a *pcie = pcs->params.a;
    const int_gstate *pigs = ((int_gstate *) gs_state_client_data(pgs));
    const ref_cie_procs *pcprocs = &pigs->colorspace.procs.cie;
    int code = (*gs_color_space_type_CIEA.install_cspace) (pcs, pgs);
    if (code < 0 || (code = cie_cache_joint(&pigs->colorrendering.procs, pgs)) < 0 || (code = cie_cache_push_finish(cie_a_finish, pgs, pcie)) < 0 || (code = cie_prepare_cache(&pcie->RangeA, &pcprocs->Decode.A, &pcie->caches.DecodeA.floats, pcie, pgs, "Decode.A")) < 0 || (code = cache_common(&pcie->common, pcprocs, pcie, pgs)) < 0) {
	(e_stack.p) = ep;
	return code;
    }
    return 5;
...
static int cie_a_finish(os_ptr op)
{
    gs_cie_a_complete(((gs_cie_a *) ((op)->value.pstruct)));
    ((o_stack.p) -= (1));
    return 0;
}

static int cache_common(gs_cie_common * pcie, const ref_cie_procs * pcprocs, void *container, const gs_state * pgs)
{
    return cie_prepare_caches_3(&pcie->RangeLMN, pcprocs->DecodeLMN.value.const_refs, &(&pcie->caches.DecodeLMN[0])->floats, &(&pcie->caches.DecodeLMN[0])[1].floats, &(&pcie->caches.DecodeLMN[0])[2].floats, container, pgs, "Decode.LMN");
}

static int cie_cache_finish(os_ptr);
int cie_prepare_cache(const gs_range * domain, const ref * proc, cie_cache_floats * pcache, void *container, const gs_state * pgs, client_name_t cname)
{
    int space = imemory_space((gs_ref_memory_t *) gs_state_memory(pgs));
    gs_for_loop_params flp;
    register es_ptr ep;
    if ((e_stack.p) > (e_stack.top) - (9)) {
	int es_code_ = ref_stack_extend(&e_stack, 9);
	if (es_code_ < 0)
	    return es_code_;
    };
    ep = (e_stack.p);
    ...
    ((ep + 9)->value.realval = (flp.init), ((ep + 9)->tas.type_attrs = ((t_real) << 8) + (0)));
    ((ep + 8)->value.realval = (flp.step), ((ep + 8)->tas.type_attrs = ((t_real) << 8) + (0)));
    ((ep + 7)->value.realval = (flp.limit), ((ep + 7)->tas.type_attrs = ((t_real) << 8) + (0)));
    ep[6] = *proc;
    ((ep + 6)->tas.type_attrs &= ~(0x80));
    ((ep + 5)->value.opproc = (zcvx), ((ep + 5)->tas.type_attrs = ((t_operator) << 8) + (0x80)), ((ep + 5)->tas.rsize = (0)));
    ((ep + 4)->value.opproc = (zfor), ((ep + 4)->tas.type_attrs = ((t_operator) << 8) + (0x80)), ((ep + 4)->tas.rsize = (0)));
...
static int cie_prepare_caches(const gs_range * domains, const ref * procs, cie_cache_floats ** ppc, int count, void *container, const gs_state * pgs, client_name_t cname)
{
    int i, code = 0;
    for (i = 0; i < count; ++i)
	if ((code = cie_prepare_cache(domains + i, procs + i, ppc[i], container, pgs, cname)) < 0)
	    return code;
    return code;
}

int cie_prepare_caches_3(const gs_range3 * domains, const ref * procs, cie_cache_floats * pc0, cie_cache_floats * pc1, cie_cache_floats * pc2, void *container, const gs_state * pgs, client_name_t cname)
{
    cie_cache_floats *pc3[3];
    pc3[0] = pc0, pc3[1] = pc1, pc3[2] = pc2;
    return cie_prepare_caches((const gs_range *) domains, procs, pc3, 3, container, pgs, cname);
} static int cie_cache_finish(os_ptr op)
{
    cie_cache_floats *pcache;
    int code;
    if ((e_stack.p) < (e_stack.bot) + ((2) - 1)) {
	e_stack.requested = (2);
	return ((-104));
    };
    pcache = (cie_cache_floats *) (((char *) (((e_stack.p) - 1)->value.pstruct)) + (e_stack.p)->value.intval);
    code = num_params(op, (1 << 9), &pcache->values[0]);
    do {
    } while (0);
    if (code < 0) {
	uint i;
	for (i = 0; i < (1 << 9); i++) {
	    code = real_param(ref_stack_index(&o_stack, (1 << 9) - 1 - i), &pcache->values[i]);
	    if (code < 0)
		return code;
	}
    }
    ref_stack_pop(&o_stack, (1 << 9));
    (e_stack.p) -= 2;
    return 14;
}

int cie_cache_push_finish(int (*proc) (os_ptr), gs_state * pgs, void *ptr)
{
    ...
    (++(e_stack.p), (((e_stack.p))->value.opproc = (proc), (((e_stack.p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((e_stack.p))->tas.rsize = (0))));
    ++(e_stack.p);
    (((e_stack.p))->value.pstruct = ((obj_header_t *) (ptr)), (((e_stack.p))->tas.type_attrs = ((t_struct) << 8) + (imemory_space((gs_ref_memory_t *) gs_state_memory(pgs)))));
    return 5;
}

const op_def *zcie_l2_op_defs(void)
{
    static const op_def op_defs_[] = { {"level2dict", 0}, {"1.setcieaspace", zsetcieaspace}, {"1.setcieabcspace", zsetcieabcspace}, {"1%cie_abc_finish", cie_abc_finish}, {"1%cie_a_finish", cie_a_finish}, {"0%cie_cache_finish", cie_cache_finish}, {(char *) 0, (op_proc_p) zcie_init} };
    return op_defs_;
}
