...
int dict_int_param(const ref * pdict, const char *kstr, int minval, int maxval, int defaultval, int *pvalue)
{
    int code = dict_int_null_param(pdict, kstr, minval, maxval, defaultval, pvalue);
...
int dict_uint_param(const ref * pdict, const char *kstr, uint minval, uint maxval, uint defaultval, uint * pvalue)
{
	...
	if (!((((const byte *) &((&*pdval)->tas.type_attrs))[1]) == (t_integer)))
	    return ((-20));
...
int dict_int_array_param(const ref * pdict, const char *kstr, uint maxlen, int *ivec)
{
    ...
    if (pdict == 0 || dict_find_string(pdict, kstr, &pdval) <= 0)
	return 0;
    if (!((((const byte *) &((pdval)->tas.type_attrs))[1]) == (t_array)))
	return ((-20));
	...
	case t_integer:
	    ...
	    *pi = (int) pa->value.intval;
	    break;
	case t_real:
	    if (pa->value.realval < (-1 << ((1 << 2) * 8 - 1)) || pa->value.realval > (~(-1 << ((1 << 2) * 8 - 1))) || pa->value.realval != (int) pa->value.realval)
		return ((-15));
...
int dict_proc_param(const ref * pdict, const char *kstr, ref * pproc, bool defaultval)
{
    ...
    if (!(((&*pdval)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 + 0x80))) == (((t_array) << 8) + (0x40 + 0x80))))
	return (check_proc_failed(&*pdval));;
    *pproc = *pdval;
    return 0;
}
...
int dict_uid_param(const ref * pdict, gs_uid * puid, int defaultval, gs_memory_t * mem)
{
    ref *puniqueid;
    if (pdict == 0) {
	((puid)->id = 0x1000000, (puid)->xvalues = 0);
	return defaultval;
    }
    if (((int) ref_language_level.value.intval == 2) && dict_find_string(pdict, "XUID", &puniqueid) > 0) {
	long *xvalues;
	uint size, i;
	...
	if (!((((const byte *) &((puniqueid)->tas.type_attrs))[1]) == (t_integer)) || puniqueid->value.intval < 0 || puniqueid->value.intval > 0xffffffL)
	    return ((-15));
...
bool dict_check_uid_param(const ref * pdict, const gs_uid * puid)
{
	...
	if (dict_find_string(pdict, "XUID", &puniqueid) <= 0)
	    return ((bool) 0);
	if (!((((const byte *) &((puniqueid)->tas.type_attrs))[1]) == (t_array)) || ((puniqueid)->tas.rsize) != size)
	    return ((bool) 0);
	for (i = 0; i < size; i++) {
	    const ref *pvalue = puniqueid->value.const_refs + i;
	    if (!((((const byte *) &((pvalue)->tas.type_attrs))[1]) == (t_integer)))
	...
	if (dict_find_string(pdict, "UniqueID", &puniqueid) <= 0)
	    return ((bool) 0);
	return (((((const byte *) &((puniqueid)->tas.type_attrs))[1]) == (t_integer)) && puniqueid->value.intval == puid->id);
}}
