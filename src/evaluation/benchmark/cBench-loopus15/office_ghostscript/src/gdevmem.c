...
static gs_ptr_type_t device_memory_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void device_memory_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
const gs_memory_struct_type_t st_device_memory = { sizeof(gx_device_memory), "gx_device_memory", 0, 0, device_memory_enum_ptrs, device_memory_reloc_ptrs, 0 };

static gs_ptr_type_t device_memory_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
    ...
    case 0:
	do {
	    *pep = (const void *) ((((gx_device_memory *) vptr)->foreign_bits ? ((void *) 0) : (void *) ((gx_device_memory *) vptr)->base));
	    return (&ptr_struct_procs);
	} while (0);
    case 1:
	do {
	    *pep = (const void *) &((gx_device_memory *) vptr)->palette;
	    return (&ptr_string_procs);
	} while (0);
    }
}

static void device_memory_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	if (!((gx_device_memory *) vptr)->foreign_bits) {
	    byte *base_old = ((gx_device_memory *) vptr)->base;
	    long reloc;
	    int y;
	    ((gx_device_memory *) vptr)->base = gs_reloc_struct_ptr((const void *) ((const gx_device_memory *) vptr)->base, gcst);
	    reloc = base_old - ((gx_device_memory *) vptr)->base;
	    for (y = 0; y < ((gx_device_memory *) vptr)->height; y++)
		((gx_device_memory *) vptr)->line_ptrs[y] -= reloc;
	    ((gx_device_memory *) vptr)->line_ptrs = (byte **) ((byte *) ((gx_device_memory *) vptr)->line_ptrs - reloc);
	}
	gs_reloc_const_string(&((gx_device_memory *) vptr)->palette, gcst);
	(*st_device_forward.reloc_ptrs) (vptr, sizeof(gx_device_forward), gcst);
}
}
static const byte b_w_palette_string[6] = { 0xff, 0xff, 0xff, 0, 0, 0 };
const gs_const_string mem_mono_b_w_palette = { b_w_palette_string, 6 };
static const byte w_b_palette_string[6] = { 0, 0, 0, 0xff, 0xff, 0xff };
...
void gs_make_mem_device(gx_device_memory * dev, const gx_device_memory * mdproto, gs_memory_t * mem, int page_device, gx_device * target)
{
    *dev = *mdproto;
	...
	gdev_mem_mono_set_inverted(dev, (target == 0 || (*((target)->std_procs.map_rgb_color)) (target, (gx_color_value) 0, (gx_color_value) 0, (gx_color_value) 0) != 0));
}
...
void gs_make_mem_mono_device(gx_device_memory * dev, gs_memory_t * mem, gx_device * target)
{
    *dev = mem_mono_device;
    dev->memory = mem;
    dev->std_procs.get_page_device = gx_default_get_page_device;
    ((gx_device_memory *) dev)->target = target;
...
static ulong mem_bitmap_bits_size(const gx_device_memory * dev, int width, int height)
{
    return ((((((1 << 2)) > (4)) ? ((1 << 2)) : (4))) & ((((((1 << 2)) > (4)) ? ((1 << 2)) : (4))) - 1) ? (((ulong) height * ((uint) (((width * dev->color_info.depth + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2))) + ((((((1 << 2)) > (4)) ? ((1 << 2)) : (4))) - 1)) / (((((1 << 2)) > (4)) ? ((1 << 2)) : (4))) * (((((1 << 2)) > (4)) ? ((1 << 2)) : (4))) : (((ulong) height * ((uint) (((width * dev->color_info.depth + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2))) + ((((((1 << 2)) > (4)) ? ((1 << 2)) : (4))) - 1)) & -(((((1 << 2)) > (4)) ? ((1 << 2)) : (4))));
}

ulong gdev_mem_data_size(const gx_device_memory * dev, int width, int height)
{
    return mem_bitmap_bits_size(dev, width, height) + (ulong) height *sizeof(byte *);
}

int gdev_mem_max_height(const gx_device_memory * dev, int width, ulong size)
{
    ulong max_height = size / (((uint) (((width * dev->color_info.depth + ((1 << 2) * 8 - 1)) >> (2 + 3)) << 2)) + sizeof(byte *));
    int height = (int) (((max_height) < ((~(-1 << ((1 << 2) * 8 - 1))))) ? (max_height) : ((~(-1 << ((1 << 2) * 8 - 1)))));
    while (gdev_mem_data_size(dev, width, height) > size)
	--height;
    return height;
}

static void mem_set_line_ptrs(gx_device_memory *, byte **, byte *);
int mem_open(gx_device * dev)
{
    if (((gx_device_memory *) dev)->bitmap_memory != 0) {
	ulong size = gdev_mem_data_size(((gx_device_memory *) dev), (((gx_device_memory *) dev))->width, (((gx_device_memory *) dev))->height);
	if ((uint) size != size)
	    return ((-13));
	((gx_device_memory *) dev)->base = (*(((gx_device_memory *) dev)->bitmap_memory)->procs.alloc_bytes) (((gx_device_memory *) dev)->bitmap_memory, (uint) size, "mem_open");
	if (((gx_device_memory *) dev)->base == 0)
	    return ((-25));
	((gx_device_memory *) dev)->foreign_bits = ((bool) 0);
    }
    mem_set_line_ptrs(((gx_device_memory *) dev), ((void *) ((byte *) (((gx_device_memory *) dev)->base) + (mem_bitmap_bits_size(((gx_device_memory *) dev), ((gx_device_memory *) dev)->width, ((gx_device_memory *) dev)->height)))), ((gx_device_memory *) dev)->base);
    return 0;
}

static void mem_set_line_ptrs(gx_device_memory * devm, byte ** line_ptrs, byte * base)
{
    byte **pptr = devm->line_ptrs = line_ptrs;
    byte **pend = pptr + devm->height;
    byte *scan_line = devm->base = base;
    uint raster = devm->raster = gx_device_raster((const gx_device *) (devm), ((bool) 1));
    while (pptr < pend) {
	*pptr++ = scan_line;
	scan_line = ((void *) ((byte *) (scan_line) + (raster)));
}} void mem_get_initial_matrix(gx_device * dev, gs_matrix * pmat)
{
    pmat->xx = ((gx_device_memory *) dev)->initial_matrix.xx;
    pmat->xy = ((gx_device_memory *) dev)->initial_matrix.xy;
    pmat->yx = ((gx_device_memory *) dev)->initial_matrix.yx;
    pmat->yy = ((gx_device_memory *) dev)->initial_matrix.yy;
    pmat->tx = ((gx_device_memory *) dev)->initial_matrix.tx;
    pmat->ty = ((gx_device_memory *) dev)->initial_matrix.ty;
} bool gs_device_is_memory(const gx_device * dev)
{
    const gx_device_memory *bdev = gdev_mem_device_for_bits(dev->color_info.depth);
...
{
    if (((gx_device_memory *) dev)->bitmap_memory != 0)
	(*(((gx_device_memory *) dev)->bitmap_memory)->procs.free_object) (((gx_device_memory *) dev)->bitmap_memory, ((gx_device_memory *) dev)->base, "mem_close");
    return 0;
}

int mem_get_bits(gx_device * dev, int y, byte * str, byte ** actual_data)
{
    byte *src;
    if (y < 0 || y >= dev->height)
	return ((-15));
    src = ((((gx_device_memory *) dev))->line_ptrs[y]);
    if (actual_data == 0)
...
int mem_word_get_bits(gx_device * dev, int y, byte * str, byte ** actual_data)
{
    byte *src;
    uint raster = gx_device_raster(dev, 0);
    if (y < 0 || y >= dev->height)
	return ((-15));
    src = ((((gx_device_memory *) dev))->line_ptrs[y]);
    mem_swap_byte_rect(src, raster, 0, raster << 3, 1, ((bool) 0));
...
gx_color_index mem_mapped_map_rgb_color(gx_device * dev, gx_color_value r, gx_color_value g, gx_color_value b)
{
    byte br = ((r) >> ((sizeof(gx_color_value) * 8) - 8));
    byte bg = ((g) >> ((sizeof(gx_color_value) * 8) - 8));
    byte bb = ((b) >> ((sizeof(gx_color_value) * 8) - 8));
    register const byte *pptr = ((gx_device_memory *) dev)->palette.data;
    int cnt = ((gx_device_memory *) dev)->palette.size;
    const byte *which = 0;
    int best = 256 * 3;
    while ((cnt -= 3) >= 0) {
    ...
    return (gx_color_index) ((which - ((gx_device_memory *) dev)->palette.data) / 3);
}

int mem_mapped_map_color_rgb(gx_device * dev, gx_color_index color, gx_color_value prgb[3])
{
    const byte *pptr = ((gx_device_memory *) dev)->palette.data + (int) color * 3;
