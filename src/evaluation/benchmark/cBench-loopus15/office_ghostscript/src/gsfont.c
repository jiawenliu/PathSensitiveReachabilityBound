...
static gs_ptr_type_t font_dir_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void font_dir_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
static const gs_memory_struct_type_t st_font_dir = { sizeof(gs_font_dir), "gs_font_dir", 0, 0, font_dir_enum_ptrs, font_dir_reloc_ptrs, 0 };

static gs_ptr_type_t font_enum_ptrs(void *ptr, uint size, uint index, const void **pep);
static void font_reloc_ptrs(void *ptr, uint size, gc_state_t * gcst);
const gs_memory_struct_type_t st_gs_font = { sizeof(gs_font), "gs_font", 0, 0, font_enum_ptrs, font_reloc_ptrs, gs_font_finalize };

static gs_ptr_type_t font_base_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return (*st_gs_font.enum_ptrs) (vptr, size, index - (1), pep);
    case 0:
	do {
	    *pep = (const void *) (((gs_font_base *) vptr)->UID.xvalues);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void font_base_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{
    (*st_gs_font.reloc_ptrs) (vptr, size, gcst);
    ((gs_font_base *) vptr)->UID.xvalues = gs_reloc_struct_ptr((const void *) ((const gs_font_base *) vptr)->UID.xvalues, gcst);
} const gs_memory_struct_type_t st_gs_font_base = { sizeof(gs_font_base), "gs_font_base", 0, 0, font_base_enum_ptrs, font_base_reloc_ptrs, gs_font_finalize };

static gs_ptr_type_t font_ptr_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	*pep = (const void *) *(gs_font * *)vptr;
	return (&ptr_struct_procs);
    }
}

static void font_ptr_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    *(gs_font * *)vptr = gs_reloc_struct_ptr((const void *) *(gs_font * *)vptr, gcst);
} static const gs_memory_struct_type_t st_gs_font_ptr = { sizeof(gs_font *), "gs_font *", 0, 0, font_ptr_enum_ptrs, font_ptr_reloc_ptrs, 0 };

static gs_ptr_type_t font_ptr_element_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    uint count = size / (uint) sizeof(gs_font *);
    if (count == 0)
	return 0;
    return (*st_gs_font_ptr.enum_ptrs) ((char *) vptr + (index % count) * sizeof(gs_font *), sizeof(gs_font *), index / count, pep);
}

static void font_ptr_element_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{ {
	uint count = size / (uint) sizeof(gs_font *);
	for (; count; count--, vptr = (char *) vptr + sizeof(gs_font *))
	    (*st_gs_font_ptr.reloc_ptrs) (vptr, sizeof(gs_font *), gcst);
}
}
const gs_memory_struct_type_t st_gs_font_ptr_element = { sizeof(gs_font *), "gs_font *[]", 0, 0, font_ptr_element_enum_ptrs, font_ptr_element_reloc_ptrs, 0 };

static gs_ptr_type_t font_dir_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:{
	    uint cci = index - 4;
	    uint offset, count;
	    uint tmask = ((gs_font_dir *) vptr)->ccache.table_mask;
	    if (cci == 0)
		offset = 0, count = 1;
	    else if (cci == ((gs_font_dir *) vptr)->enum_index + 1)
		offset = ((gs_font_dir *) vptr)->enum_offset + 1, count = 1;
	    else
		offset = 0, count = cci;
	    for (; offset <= tmask; ++offset) {
		cached_char *cc = ((gs_font_dir *) vptr)->ccache.table[offset];
		if (cc != 0 && !--count) {
		    (*((gs_font_dir *) vptr)->ccache.mark_glyph) (cc->code, ((gs_font_dir *) vptr)->ccache.mark_glyph_data);
		    ((gs_font_dir *) vptr)->enum_index = cci;
		    ((gs_font_dir *) vptr)->enum_offset = offset;
		    do {
			*pep = (const void *) (((cc)->pair) - cc->pair_index);
			return (&ptr_struct_procs);
		    } while (0);
		}
	    }
    ...
    case 0:
	do {
	    *pep = (const void *) (((gs_font_dir *) vptr)->scaled_fonts);
	    return (&ptr_struct_procs);
	} while (0);
    case 1:
	do {
	    *pep = (const void *) (((gs_font_dir *) vptr)->fmcache.mdata);
	    return (&ptr_struct_procs);
	} while (0);
    case 2:
	do {
	    *pep = (const void *) (((gs_font_dir *) vptr)->ccache.table);
	    return (&ptr_struct_procs);
	} while (0);
    case 3:
	do {
	    *pep = (const void *) (((gs_font_dir *) vptr)->ccache.mark_glyph_data);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void font_dir_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{; {
	int chi;
	for (chi = ((gs_font_dir *) vptr)->ccache.table_mask; chi >= 0; --chi) {
	    cached_char *cc = ((gs_font_dir *) vptr)->ccache.table[chi];
	    if (cc != 0)
		((cc)->pair = ((cached_fm_pair *) gs_reloc_struct_ptr(((cc)->pair) - cc->pair_index, gcst) + cc->pair_index));
	}
}
((gs_font_dir *) vptr)->orig_fonts = gs_reloc_struct_ptr((const void *) ((const gs_font_dir *) vptr)->orig_fonts, gcst);
((gs_font_dir *) vptr)->scaled_fonts = gs_reloc_struct_ptr((const void *) ((const gs_font_dir *) vptr)->scaled_fonts, gcst);
((gs_font_dir *) vptr)->fmcache.mdata = gs_reloc_struct_ptr((const void *) ((const gs_font_dir *) vptr)->fmcache.mdata, gcst);
((gs_font_dir *) vptr)->ccache.table = gs_reloc_struct_ptr((const void *) ((const gs_font_dir *) vptr)->ccache.table, gcst);
((gs_font_dir *) vptr)->ccache.mark_glyph_data = gs_reloc_struct_ptr((const void *) ((const gs_font_dir *) vptr)->ccache.mark_glyph_data, gcst);
} void gs_font_finalize(void *vptr)
{
    gs_font **ppfirst;
    gs_font *next = ((gs_font *) vptr)->next;
    gs_font *prev = ((gs_font *) vptr)->prev;
    do {
    } while (0);
    if (((gs_font *) vptr)->dir == 0)
	ppfirst = 0;
    else if (((gs_font *) vptr)->base == ((gs_font *) vptr))
	ppfirst = &((gs_font *) vptr)->dir->orig_fonts;
    else {
	if (next || prev || ((gs_font *) vptr)->dir->scaled_fonts == ((gs_font *) vptr))
	    ((gs_font *) vptr)->dir->ssize--;
	ppfirst = &((gs_font *) vptr)->dir->scaled_fonts;
    }
    if (next != 0 && next->prev == ((gs_font *) vptr))
	next->prev = prev;
    if (prev != 0) {
	if (prev->next == ((gs_font *) vptr))
	    prev->next = next;
    } else if (ppfirst != 0 && *ppfirst == ((gs_font *) vptr))
	*ppfirst = next;
}

static gs_ptr_type_t font_enum_ptrs(void *vptr, uint size, uint index, const void **pep)
{
    switch (index) {
    default:
	return 0;
    case 0:
	do {
	    *pep = (const void *) ((((gs_font *) vptr)->base == ((gs_font *) vptr) ? 0 : ((gs_font *) vptr)->next));
	    return (&ptr_struct_procs);
	} while (0);
    case 1:
	do {
	    *pep = (const void *) ((((gs_font *) vptr)->base == ((gs_font *) vptr) ? 0 : ((gs_font *) vptr)->prev));
	    return (&ptr_struct_procs);
	} while (0);
    case 2:
	do {
	    *pep = (const void *) (((gs_font *) vptr)->dir);
	    return (&ptr_struct_procs);
	} while (0);
    case 3:
	do {
	    *pep = (const void *) (((gs_font *) vptr)->base);
	    return (&ptr_struct_procs);
	} while (0);
    case 4:
	do {
	    *pep = (const void *) (((gs_font *) vptr)->client_data);
	    return (&ptr_struct_procs);
	} while (0);
    }
}

static void font_reloc_ptrs(void *vptr, uint size, gc_state_t * gcst)
{;
    ((gs_font *) vptr)->next = gs_reloc_struct_ptr((const void *) ((const gs_font *) vptr)->next, gcst);
    ((gs_font *) vptr)->prev = gs_reloc_struct_ptr((const void *) ((const gs_font *) vptr)->prev, gcst);
    ((gs_font *) vptr)->dir = gs_reloc_struct_ptr((const void *) ((const gs_font *) vptr)->dir, gcst);
    ((gs_font *) vptr)->base = gs_reloc_struct_ptr((const void *) ((const gs_font *) vptr)->base, gcst);
    ((gs_font *) vptr)->client_data = gs_reloc_struct_ptr((const void *) ((const gs_font *) vptr)->client_data, gcst);
} static bool cc_no_mark_glyph(gs_glyph glyph, void *ignore_data)
{
    return ((bool) 0);
}

gs_font_dir *gs_font_dir_alloc(gs_memory_t * mem)
{
    gs_font_dir *pdir = 0; {
	pdir = gs_font_dir_alloc_limits(mem, 50, 500000, 200, 5000, 2500);
    }
    if (pdir == 0)
	pdir = gs_font_dir_alloc_limits(mem, 20, 25000, 40, 500, 100);
    if (pdir == 0)
	return 0;
...
gs_font_dir *gs_font_dir_alloc_limits(gs_memory_t * mem, uint smax, uint bmax, uint mmax, uint cmax, uint upper)
{
    register gs_font_dir *pdir = (gs_font_dir *) (*(mem)->procs.alloc_struct) (mem, &st_font_dir, "font_dir_alloc(dir)");
    int code;
    if (pdir == 0)
	return 0;
    code = gx_char_cache_alloc(mem, pdir, bmax, mmax, cmax, upper);
    if (code < 0) {
	(*(mem)->procs.free_object) (mem, pdir, "font_dir_alloc(dir)");
	return 0;
    }
    pdir->orig_fonts = 0;
    pdir->scaled_fonts = 0;
...
int gs_definefont(gs_font_dir * pdir, gs_font * pfont)
{
    int code;
    pfont->dir = pdir;
    pfont->base = pfont;
    ...
    if ((pfont->next = pdir->orig_fonts) != ((void *) 0))
	pdir->orig_fonts->prev = pfont;
    pfont->prev = 0;
    pdir->orig_fonts = pfont;
    do {
    } while (0);
    return 0;
}

int gs_no_define_font(gs_font_dir * pdir, gs_font * pfont)
{
    return 0;
}

int gs_scalefont(gs_font_dir * pdir, const gs_font * pfont, floatp scale, gs_font ** ppfont)
{
    gs_matrix mat;
...
int gs_makefont(gs_font_dir * pdir, const gs_font * pfont, const gs_matrix * pmat, gs_font ** ppfont)
{
    int code;
    gs_font *prev = 0;
	...
	return code;
    if (pfont->FontType != ft_composite && ((&((const gs_font_base *) pfont)->UID)->id != 0x1000000)) {
	for (; pf_out != 0; prev = pf_out, pf_out = pf_out->next)
	    if (pf_out->FontType == pfont->FontType && pf_out->base == pfont->base && uid_equal(&((gs_font_base *) pf_out)->UID, &((const gs_font_base *) pfont)->UID) && pf_out->FontMatrix.xx == newmat.xx && pf_out->FontMatrix.xy == newmat.xy && pf_out->FontMatrix.yx == newmat.yx && pf_out->FontMatrix.yy == newmat.yy && pf_out->FontMatrix.tx == newmat.tx && pf_out->FontMatrix.ty == newmat.ty) {
		*ppfont = pf_out;
		do {
		} while (0);
		return 0;
    ...
    *ppfont = pf_out;
	...
	if (pdir->ssize >= pdir->smax && prev != 0) {
		...
		(*(prev->memory)->procs.free_object) (prev->memory, (&((gs_font_base *) prev)->UID)->xvalues, "gs_makefont(discarding)");
		((&((gs_font_base *) prev)->UID)->id = 0x1000000, (&((gs_font_base *) prev)->UID)->xvalues = 0);
	    }
	}
	pdir->ssize++;
	if ((pf_out->next = pdir->scaled_fonts) != ((void *) 0))
	    pdir->scaled_fonts->prev = pf_out;
	pf_out->prev = 0;
	pdir->scaled_fonts = pf_out;
    } else {
...
int gs_no_make_font(gs_font_dir * pdir, const gs_font * pfont, const gs_matrix * pmat, gs_font ** ppfont)
{
    return 0;
}

int gs_base_make_font(gs_font_dir * pdir, const gs_font * pfont, const gs_matrix * pmat, gs_font ** ppfont)
{
    if (((&((gs_font_base *) * ppfont)->UID)->id < 0)) {
	uint xsize = ((uint) (-(&((gs_font_base *) * ppfont)->UID)->id));
	long *xvalues = (long *) (*(((gs_font_base *) * ppfont)->memory)->procs.alloc_byte_array) (((gs_font_base *) * ppfont)->memory, xsize, sizeof(long), "gs_base_make_font(XUID)");
	if (xvalues == 0)
	    return ((-25));
	memcpy(xvalues, ((&((gs_font_base *) * ppfont)->UID)->xvalues), xsize * sizeof(long));
	((gs_font_base *) * ppfont)->UID.xvalues = xvalues;
    }
    return 0;
}
...
gs_font *gs_currentfont(const gs_state * pgs)
{
    return pgs->font;
}

gs_font *gs_rootfont(const gs_state * pgs)
{
    return pgs->root_font;
}

void gs_cachestatus(register const gs_font_dir * pdir, register uint pstat[7])
{
...
} int gs_setcachesize(gs_font_dir * pdir, uint size)
{
    pdir->ccache.bmax = size;
    return 0;
}

int gs_setcachelower(gs_font_dir * pdir, uint size)
{
    pdir->ccache.lower = size;
    return 0;
}

int gs_setcacheupper(gs_font_dir * pdir, uint size)
{
    pdir->ccache.upper = size;
    return 0;
}

uint gs_currentcachesize(const gs_font_dir * pdir)
{
    return pdir->ccache.bmax;
}

uint gs_currentcachelower(const gs_font_dir * pdir)
{
    return pdir->ccache.lower;
}

uint gs_currentcacheupper(const gs_font_dir * pdir)
{
    return pdir->ccache.upper;
}

void gs_purge_font(gs_font * pfont)
{
	...
	(lprintf_file_and_line(gs_stderr, "gsfont.c", 502), fprintf(gs_stderr, "purged font 0x%lx not found\n", (ulong) pfont));
    }
