...
enum aes_const { Nrow = 4, Mcol = 8, Ncol = 16 / 4, Shr0 = 0, Shr1 = 1, Shr2 = 16 == 32 ? 3 : 2, Shr3 = 16 == 32 ? 4 : 3 };
...
aes_ret set_blk(const word n_bytes, aes * cx);
void fillrand(char *buf, int len)
{
    ...
    for (i = 0; i < len; ++i) {
	if (count == 4) {
	    *(unsigned long *) r = (((a[0] = 36969 * (a[0] & 65535) + (a[0] >> 16)) << 16) + (a[1] = 18000 * (a[1] & 65535) + (a[1] >> 16)));
	    count = 0;
	}
	buf[i] = r[count++];
}} int encfile(FILE * fin, FILE * fout, aes * ctx, char *fn)
{
    ...
    while (!feof(fin)) {
	i = fread(inbuf + 16 - l, 1, l, fin);
	if (i < l)
	    break;
	for (i = 0; i < 16; ++i)
	    inbuf[i] ^= outbuf[i];
	encrypt((const byte *) inbuf, (byte *) outbuf, ctx);
...
int decfile(FILE * fin, FILE * fout, aes * ctx, char *ifn, char *ofn)
{
    ...
    int i, l, flen;
    if (fread(inbuf1, 1, 16, fin) != 16) {
	printf("Error reading from input file: %s\n", ifn);
	return 9;
    }
    i = fread(inbuf2, 1, 16, fin);
    if (i && i != 16) {
	printf("\nThe input file is corrupt");
	return -10;
    }
    decrypt((const byte *) inbuf2, (byte *) outbuf, ctx);
    for (i = 0; i < 16; ++i)
	outbuf[i] ^= inbuf1[i];
    flen = outbuf[0] & 15;
    l = 15;
	...
	decrypt((const byte *) bp1, (byte *) outbuf, ctx);
	for (i = 0; i < 16; ++i)
	    outbuf[i] ^= bp2[i];
	l = i;
