...
int ulaw_output(gsm_signal * buf)
{
    int i;
    for (i = 0; i < 160; i++, buf++)
	if (fputc((char) (s2u[((unsigned short) ((unsigned short) *buf)) >> 3]), out) == (-1))
	    return -1;
    return 0;
}
