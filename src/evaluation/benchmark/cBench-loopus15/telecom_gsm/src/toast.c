...
extern int audio_init_input(void), audio_init_output(void);
extern int ulaw_input(gsm_signal *), ulaw_output(gsm_signal *);
extern int alaw_input(gsm_signal *), alaw_output(gsm_signal *);
extern int linear_input(gsm_signal *), linear_output(gsm_signal *);
char *progname;
int f_decode = 0;
int f_cat = 0;
int f_force = 0;
...
FILE *in, *out;
char *inname, *outname;
int (*output) (gsm_signal *), (*input) (gsm_signal *);
int (*init_input) (void), (*init_output) (void);
...
struct fmtdesc {
    ...
    int (*init_input) (void), (*init_output) (void);
    int (*input) (gsm_signal *), (*output) (gsm_signal *);
} f_audio = {
"audio", "8 kHz, 8 bit u-law encoding with Sun audio header", ".au", audio_init_input, audio_init_output, ulaw_input, ulaw_output}, f_ulaw = {
"u-law", "plain 8 kHz, 8 bit u-law encoding", ".u", generic_init, generic_init, ulaw_input, ulaw_output}, f_alaw = {
"A-law", "8 kHz, 8 bit A-law encoding", ".A", generic_init, generic_init, alaw_input, alaw_output}, f_linear = {
"linear", "16 bit (13 significant) signed 8 kHz signal", ".l", generic_init, generic_init, linear_input, linear_output};
struct fmtdesc *alldescs[] = { &f_audio, &f_alaw, &f_ulaw, &f_linear, (struct fmtdesc *) ((void *) 0) };
...
static void parse_argv0(char *av0)
{
    ...
    if (!strncmp(av0, "un", 2))
	f_decode = 1;
    if ((l = strlen(av0)) >= 3 && !strcmp(av0 + l - 3, "cat"))
	f_cat = f_decode = 1;
}
...
static int ok_to_replace(char *name)
{
	...
	return 0;
    fprintf(stderr, "%s already exists; do you wish to overwrite %s (y or n)? ", name, name);
    fflush(stderr);
...
} static void update_times(void)
{
    ...
    if (outname) {
	time_t ut[2];
	ut[0] = instat.st_atim.tv_sec;
	ut[1] = instat.st_mtim.tv_sec;
...
} static int okay_as_input(char *name, FILE * f, struct stat *st)
{
	...
	fprintf(stderr, "%s: \"%s\" has %s other link%s -- unchanged.\n", progname, name, st->st_nlink - 1, "s" + (st->st_nlink <= 2));
	return 0;
    }
    return 1;
}
...
static int open_output(char *name)
{
	...
	if (!length_okay(o))
	    return 0;
	if ((outfd = open(o, 01 | 0100 | 0200, 0666)) >= 0)
	    out = fdopen(outfd, "wb");
...
static int process_encode(void)
{
    ...
    (void) gsm_option(r, 2, &f_fast);
    (void) gsm_option(r, 1, &f_verbose);
    while ((cc = (*input) (s)) > 0) {
...
static int process_decode(void)
{
	...
	if (cc != sizeof(s)) {
	    if (cc >= 0)
		fprintf(stderr, "%s: incomplete frame (%d byte%s missing) from %s\n", progname, sizeof(s) - cc, "s" + (sizeof(s) - cc == 1), inname ? inname : "stdin");
	    gsm_destroy(r);
	    (*__errno_location()) = 0;
	    return -1;
	}
...
static int process(char *name)
{
	...
	fprintf(stderr, "%s: error %s %s\n", progname, f_decode ? "writing header to" : "reading header from", f_decode ? (outname ? outname : "stdout") : (inname ? inname : "stdin"));
	goto err;
    }
    ...
    return 0;
  err:if (out && out != stdout) {
	(void) fclose(out), out = (FILE *) 0;
	if (unlink(outname) < 0 && (*__errno_location()) != 2 && (*__errno_location()) != 4) {
	    perror(outname);
...
{
    printf("%s 1.0, version %s\n", progname, "$Id: toast.c,v 1.1.1.1 2000/11/06 19:54:26 mguthaus Exp $");
...
int main1(int ac, char **av)
{
    ...
    extern int optind;
    parse_argv0(*av);
    while ((opt = getopt(ac, av, "fcdpvhuaslVF")) != (-1))
	switch (opt) {
	case 'd':
	    f_decode = 1;
	    break;
