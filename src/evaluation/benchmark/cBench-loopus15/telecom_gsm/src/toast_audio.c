...
extern int (*output) (gsm_signal *), (*input) (gsm_signal *);
extern int alaw_input(gsm_signal *), ulaw_input(gsm_signal *), linear_input(gsm_signal *);
extern int ulaw_output(gsm_signal *);
static int put_u32(FILE * f, unsigned long u)
{
    if (_IO_putc((char) ((u >> 24) & 0x0FF), f) == (-1) || _IO_putc((char) ((u >> 16) & 0x0FF), f) == (-1) || _IO_putc((char) ((u >> 8) & 0x0FF), f) == (-1) || _IO_putc((char) (u & 0x0FF), f) == (-1))
	return -1;
    return 0;
}
...
static int get_u32(FILE * f, unsigned long *up)
{
    int i;
    unsigned long u;
    if ((i = _IO_getc(f)) == (-1) || ((u = (unsigned char) i), (i = _IO_getc(f)) == (-1)) || ((u = (u << 8) | (unsigned char) i), (i = _IO_getc(f)) == (-1)) || ((u = (u << 8) | (unsigned char) i), (i = _IO_getc(f)) == (-1)))
	return -1;
...
int audio_init_input(void)
{
    ...
    if (fgetc(in) != '.' || fgetc(in) != 's' || fgetc(in) != 'n' || fgetc(in) != 'd' || get_u32(in, &len) || get_u32(in, &enc) || get_u32(in, &enc) || fseek(in, (long) (len - 4 * 4), 1) < 0) {
	fprintf(stderr, "%s: bad (missing?) header in Sun audio file \"%s\";\n	Try one of -u, -a, -l instead (%s -h for help).\n", progname, inname ? inname : "stdin", progname);
	return -1;
    }
    switch (enc) {
    case 1:
...
{
    if (fputs(".snd", out) == (-1) || put_u32(out, 32) || put_u32(out, ~(unsigned long) 0) || put_u32(out, 1) || put_u32(out, 8000) || put_u32(out, 1) || put_u32(out, 0) || put_u32(out, 0))
	return -1;
    return 0;
}
