...
int chr;
{
    return (((*__ctype_b_loc())[(int) ((chr))] & (unsigned short int) _ISupper) && !isvowel(chr));
} static int leftmatch(char *pattern, char *context);

static int leftmatch(pattern, context)
char *pattern;
char *context;
{
    char *pat;
	...
	if (((*__ctype_b_loc())[(int) ((*pat))] & (unsigned short int) _ISalpha) || *pat == '\'' || *pat == ' ')
	    if (*pat != *text)
		return (0);
	    else {
		text--;
...
static int rightmatch(pattern, context)
char *pattern;
char *context;
{
    char *pat;
...
static int find_rule(void *arg, out_p out, char *word, int index, Rule(*rules));
static int find_rule(arg, out, word, index, rules)
void *arg;
out_p out;
char *word;
int index;
...
static void guess_word(void *arg, out_p out, char *word);
static void guess_word(arg, out, word)
void *arg;
out_p out;
char *word;
{
    ...
    index = 1;
    do {
	if (((*__ctype_b_loc())[(int) ((word[index]))] & (unsigned short int) _ISupper))
	    type = word[index] - 'A' + 1;
	else
	    type = 0;
...
static void phone_cat(void *arg, char *s);
static void phone_cat(arg, s)
void *arg;
char *s;
{
