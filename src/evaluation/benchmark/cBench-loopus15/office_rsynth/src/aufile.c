...
void au_header(fd, enc, rate, size, comment)
int fd;
unsigned enc;
unsigned rate;
unsigned size;
char *comment;
{
    if (!comment)
	comment = "";
    wblong(fd, 0x2e736e64);
    wblong(fd, 24 + strlen(comment));
    wblong(fd, size);
    wblong(fd, enc);
    wblong(fd, rate);
    wblong(fd, 1);
...
static void aufile_write(n, data)
int n;
short *data;
{
    if (n > 0) {
...
static void aufile_term()
{
    if (au_fd >= 0) {
	off_t here = lseek(au_fd, 0L, 1);
	if (here >= 0) {
	    ftruncate(au_fd, here);
	    if (lseek(au_fd, 8L, 0) == 8) {
    ...
    if (linear_fd >= 0) {
	ftruncate(linear_fd, lseek(linear_fd, 0L, 1));
	if (linear_fd != 1)
	    close(linear_fd);
	linear_fd = -1;
    }
...
int file_init(argc, argv)
int argc;
char *argv[];
{
    argc = getargs("File output", argc, argv, "l", "", &linear_file, "Raw 16-bit linear pathname", "o", "", &au_file, "Sun/Next audio file name", ((void *) 0));
    if (help_only)
	return argc;
    if (au_file) {
	if (strcmp(au_file, "-") == 0) {
	    ...
	    au_fd = open(au_file, 01 | 0100 | 01000, 0666);
	    if (au_fd < 0)
		perror(au_file);
	}
	if (au_fd >= 0) {
		...
		au_encoding = 1;
	    au_header(au_fd, au_encoding, samp_rate, ((unsigned) (~0)), "");
	    au_size = 0;
	}
    }
    if (linear_file) {
	if (strcmp(linear_file, "-") == 0) {
	    ...
	    linear_fd = open(linear_file, 01 | 0100 | 01000, 0666);
	    if (linear_fd < 0)
		perror(linear_file);
	}
    }
