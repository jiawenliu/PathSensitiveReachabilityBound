...
klatt_frame_t def_pars = { 1330, 60, 500, 60, 1500, 90, 2800, 150, 3250, 200, 3700, 200, 4990, 500, 270, 100, 270, 100, 0, 30, 0, 10, 0, 0, 0, 80, 0, 200, 0, 350, 0, 500, 0, 600, 0, 800, 0, 0, 0, 62 };

int init_synth(argc, argv)
int argc;
char *argv[];
{
    ...
    klatt_global.quiet_flag = 1;
    klatt_global.glsource = 2;
    klatt_global.f0_flutter = 0;
    argc = getargs("Synth paramters", argc, argv, "q", ((void *) 0), &klatt_global.quiet_flag, "Quiet - minimal messages", "I", ((void *) 0), &impulse, "Impulse glottal source", "c", "%d", &casc, "Number cascade formants", "F", "%d", &klatt_global.f0_flutter, "F0 flutter", "f", "%lg", &mSec_per_frame, "mSec per frame", "t", "%d", &def_pars.TLTdb, "Tilt dB", "x", "%d", &def_pars.F0hz10, "Base F0 in 0.1Hz", ((void *) 0));
    if (casc > 0) {
	klatt_global.synthesis_model = 1;
	klatt_global.nfcascade = casc;
    } else
	klatt_global.synthesis_model = 2;
    if (impulse)
	klatt_global.glsource = 1;
