...
static char *linear_file = ((void *) 0);
char *prog = "hplay";
static int audio_open(void)
{
    dev_fd = open(dev_file, 01 | 04000);
    if (dev_fd < 0) {
	perror(dev_file);
	return 0;
    }
...
int audio_init(int argc, char *argv[])
{
    ...
    int use_audio = 1;
    prog = argv[0];
    argc = getargs("linux Audio", argc, argv, "r", "%d", &rate_set, "Sample rate", "a", ((void *) 0), &use_audio, "Audio enable", ((void *) 0));
    if (help_only)
	return argc;
    if (use_audio)
	audio_open();
	...
	samp_rate = rate_set;
    if (dev_fd > 0) {
	ioctl(dev_fd, (((2U | 1U) << (((0 + 8) + 8) + 14)) | ((('P')) << (0 + 8)) | (((2)) << 0) | ((((sizeof(int)))) << ((0 + 8) + 8))), &samp_rate);
...
void audio_term()
{
    int dummy;
    if (dev_fd >= 0) {
	ioctl(dev_fd, (((0U) << (((0 + 8) + 8) + 14)) | ((('P')) << (0 + 8)) | (((1)) << 0) | ((0) << ((0 + 8) + 8))), &dummy);
	close(dev_fd);
	dev_fd = -1;
    }
    if (linear_fd >= 0) {
...
void audio_play(int n, short *data)
{
    if (n > 0) {
