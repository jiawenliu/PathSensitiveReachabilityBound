...
static void set_trans(t, a, b, ext, e)
slope_t *t;
Elm_ptr a;
Elm_ptr b;
int ext;
...
static float interpolate(w, p, s, e, mid, t, d)
char *w;
char *p;
slope_t *s;
slope_t *e;
...
}} unsigned holmes(nelm, elm, nsamp, samp_base)
	...
	if (dur > 0) {
		...
		if (tstress == ntstress) {
			...
			if (s || e->feat & 0x10000000) {
				...
				du = elm[j++];
			    } while ((e->feat & 0x10000000) && elm[j++] == s);
			    ntstress += d / 2;
			    break;
			}
			ntstress += du;
		...
		pars.F0hz10 = base + (top - base) * interpolate("", "f0", &stress_s, &stress_e, (float) 0, tstress, ntstress);
...
int init_holmes(argc, argv)
int argc;
char *argv[];
{
    char *par_name = ((void *) 0);
    char *jsru_name = ((void *) 0);
    argc = getargs("Holmes", argc, argv, "p", "", &par_name, "Parameter file for plot", "j", "", &jsru_name, "Data for alternate synth (JSRU)", "S", "%d", &speed, "Speed (1.0 is 'normal')", "K", "%lg", &frac, "Parameter filter 'fraction'", ((void *) 0));
    if (help_only)
	return argc;
    if (par_name) {
...
static int jsru_freq(float f, float base, float inc);
static int jsru_freq(f, base, inc)
float f;
float base;
float inc;
{
...
static int jsru_amp(float a);
static int jsru_amp(a)
float a;
{
    int i = a;
    if (i <= 0)
