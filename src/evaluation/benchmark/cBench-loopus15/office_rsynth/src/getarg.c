...
int getargs(char *module, int argc, char *argv[], ...)
{
	...
	if (*s && (flag == '-' || flag == '+')) {
		...
		char *a;
		int count = 0;
		__builtin_va_start(ap, argv);
		while ((a = __builtin_va_arg(ap, char *))) {
		    int l = strlen(a);
		    char *fmt = __builtin_va_arg(ap, char *);
		    void *var = __builtin_va_arg(ap, void *);
		    char *desc = __builtin_va_arg(ap, char *);
		    if (!strcmp(s, "-help")) {
			help_only = 1;
			if (!done_module++)
			    fprintf(stderr, "%s:\n", module);
			if (fmt) {
				...
				fprintf(stderr, " -%s <string> [%s]\t%s\n", a, *((char **) var) ? *((char **) var) : "", desc);
			} else {
			    fprintf(stderr, " [+|-]%s [%s]\t%s\n", a, *((int *) var) ? "yes" : "no", desc);
		    }} else if (l > 1) {
			if (!count && !strcmp(s, a)) {
			    if (fmt) {
				if (i + off < argc) {
