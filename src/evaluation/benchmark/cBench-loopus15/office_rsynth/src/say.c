...
extern void *dict;
extern unsigned char *dict_find(char *s, unsigned n);
extern int dict_init(int argc, char *argv[]);
...
extern int init_holmes(int argc, char *argv[]);
extern void term_holmes(void);
extern int speed;
extern int phone_append(darray_ptr p, int ch);
unsigned phone_to_elm(char *s, int n, darray_ptr elm);
extern int isvowel(int chr);
extern int isconsonant(int chr);
...
enum phone_e { SIL, B, CH, D, DH, F, G, HH, JH, K, L, M, N, NG, P, R, S, SH, T, TH, TS, V, W, Y, Z, ZH, AX, AA, AA1, AA2, AE, AE1, AE2, AH, AH1, AH2, AO, AO1, AO2, AW, AW1, AW2, AY, AY1, AY2, EA, EA1, EA2, EH, EH1, EH2, ER, ER1, ER2, EY, EY1, EY2, IA, IA1, IA2, IH, IH1, IH2, IY, IY1, IY2, OH, OH1, OH2, OW, OW1, OW2, OY, OY1, OY2, UA, UA1, UA2, UH, UH1, UH2, UW, UW1, UW2, END };
extern char *ph_name[];
extern char *ph_br[];
extern char *ph_am[];
...
extern void say_string(char *s);
extern char *concat_args(int argc, char *argv[]);
extern int suspect_word(char *s, int n);
extern void say_phones(char *phone, int len, int verbose);
extern unsigned spell_out(char *word, int n, darray_ptr phone);
extern unsigned xlate_ordinal(long int value, darray_ptr phone);
extern unsigned xlate_cardinal(long int value, darray_ptr phone);
char *program = "say";
static int verbose = 0;
unsigned spell_out(word, n, phone)
char *word;
int n;
darray_ptr phone;
{
...
int n;
{
    ...
    for (i = 0; i < n; i++) {
	char ch = *s++;
	if (i && last != '-' && ((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISupper))
	    seen_upper = 1;
...
static unsigned xlate_word(char *word, int n, darray_ptr phone);
static unsigned xlate_word(word, n, phone)
char *word;
int n;
darray_ptr phone;
{
...
static once = 0;
void say_phones(phone, len, verbose)
char *phone;
int len;
int verbose;
{
...
unsigned xlate_string(string, phone)
char *string;
darray_ptr phone;
{
    unsigned nph = 0;
    char *s = string;
    char ch;
    while (((*__ctype_b_loc())[(int) ((ch = *s))] & (unsigned short int) _ISspace))
	s++;
    while ((ch = *s)) {
	char *word = s;
	if (((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISalpha)) {
	    while (((*__ctype_b_loc())[(int) ((ch = *s))] & (unsigned short int) _ISalpha) || ((ch == '\'' || ch == '-' || ch == '.') && ((*__ctype_b_loc())[(int) ((s[1]))] & (unsigned short int) _ISalpha)))
		s++;
	    if (!ch || ((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISspace) || ((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISpunct) || (((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISdigit) && !suspect_word(word, s - word)))
		nph += xlate_word(word, s - word, phone);
	    else {
		while ((ch = *s) && !((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISspace) && !((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISpunct))
		    s++;
		nph += spell_out(word, s - word, phone);
	}} else if (((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISdigit) || (ch == '-' && ((*__ctype_b_loc())[(int) ((s[1]))] & (unsigned short int) _ISdigit))) {
	    int sign = (ch == '-') ? -1 : 1;
	    long value = 0;
	    if (sign < 0)
		ch = *++s;
	    while (((*__ctype_b_loc())[(int) ((ch = *s))] & (unsigned short int) _ISdigit)) {
		value = value * 10 + ch - '0';
		s++;
	    } if (ch == '.' && ((*__ctype_b_loc())[(int) ((s[1]))] & (unsigned short int) _ISdigit)) {
		word = ++s;
	    ...
	    while (*s && *s++ != ']');
	    nph += xlate_word(word, s - word, phone);
	} else if (((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISpunct)) {
	    switch (ch) {
	    case '!':
	    case '?':
	    case '.':
		if ((!s[1] || ((*__ctype_b_loc())[(int) ((s[1]))] & (unsigned short int) _ISspace)) && phone->items) {
	    ...
	    while ((ch = *s) && !((*__ctype_b_loc())[(int) ((ch))] & (unsigned short int) _ISspace))
		s++;
	    nph += spell_out(word, s - word, phone);
	} while (((*__ctype_b_loc())[(int) ((ch = *s))] & (unsigned short int) _ISspace))
	    s++;
...
FILE *f;
{
    ...
    darray_init(&phone, sizeof(char), 128);
    while ((darray_fget(f, &line)) && (nlines < 500)) {
	nlines++;
...
char *env[];
{
    ...
    argc = getargs("Misc", argc, argv, "v", ((void *) 0), &verbose, "Verbose, show phonetic form", ((void *) 0));
    if (help_only) {
