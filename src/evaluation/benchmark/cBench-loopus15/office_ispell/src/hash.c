static char Rcs_Id[] = "$Id: hash.c,v 1.20 1994/01/25 07:11:34 geoff Exp $";
...
extern char *tgoto();
extern char *tputs();
int hash(ichar_t * word, int hashtblsize);
...
register int hashtblsize;
{
    ...
    register int i;
    for (i = 4; i-- && *s != 0;)
	h = (h << 8) | (hashheader.upperconv[(*s++)]);
    while (*s != 0) {
	h = (h << 5) | ((h >> (32 - 5)) & ((1 << 5) - 1));
	h ^= (hashheader.upperconv[(*s++)]);
    }
