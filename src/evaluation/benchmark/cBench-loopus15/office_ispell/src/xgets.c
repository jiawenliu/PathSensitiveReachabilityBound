...
FILE *stream;
{
    static char *Include_File = "&Include_File&";
    ...
    if (Include_Len == 0) {
	char *env_variable;
	if ((env_variable = getenv("INCLUDE_STRING")) != ((void *) 0))
	...
	*current_F = stream;
    ...
    for (;;) {
	c = '\0';
	if ((s - str) + 1 < size && (c = _IO_getc(*current_F)) != (-1)) {
	    *s++ = (char) c;
	    if (c != '\n')
		continue;
	}
	*s = '\0';
	...
	if (incfileflag != 0 && strncmp(str, Include_File, (unsigned int) Include_Len) == 0) {
	    ...
	    if (current_F - F < 5 && strlen(file_name) != 0) {
		FILE *f;
