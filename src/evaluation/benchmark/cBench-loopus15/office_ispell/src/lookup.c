...
int linit()
{
	...
	if (hashsize < 0)
	    if (cbench_print)
		fprintf(stderr, "Trouble reading hash table %s\r\n", hashname);
	    else if (hashsize == 0)
		if (cbench_print)
		    fprintf(stderr, "Null hash table %s\r\n", hashname);
		else if (cbench_print)
		    fprintf(stderr, "Truncated hash table %s:  got %d bytes, expected %d\r\n", hashname, hashsize, (int) sizeof hashheader);
	return (-1);
    } else if (hashheader.magic != 0x9602) {
	if (cbench_print)
	    fprintf(stderr, "Illegal format hash table %s - expected magic 0x%x, got 0x%x\r\n", hashname, (unsigned int) 0x9602, (unsigned int) hashheader.magic);
	return (-1);
    } else if (hashheader.magic2 != 0x9602) {
	if (cbench_print)
	    fprintf(stderr, "Illegal format hash table %s - expected magic2 0x%x, got 0x%x\r\n", hashname, (unsigned int) 0x9602, (unsigned int) hashheader.magic2);
	return (-1);
    } else if (hashheader.compileoptions != (0x01 | 0x02 | 0x00) || hashheader.maxstringchars != 100 || hashheader.maxstringcharlen != 10) {
	if (cbench_print)
	    fprintf(stderr, "Hash table options don't agree with buildhash - 0x%x/%d/%d vs. 0x%x/%d/%d\r\n", (unsigned int) hashheader.compileoptions, hashheader.maxstringchars, hashheader.maxstringcharlen, (unsigned int) (0x01 | 0x02 | 0x00), 100, 10);
	return (-1);
    }
    if (nodictflag) {
	hashsize = 1;
	...
	hashtbl[0].word = ((void *) 0);
	hashtbl[0].next = ((void *) 0);
	hashtbl[0].mask[(32 / 32) - 1] &= ~(((long) 1 << (((32) - 6) + 0)) | ((long) 1 << (((32) - 6) + 1)));
	hashstrings = (char *) malloc((unsigned) hashheader.lstringsize);
    } else {
	hashtbl = (struct dent *) malloc((unsigned) hashheader.tblsize * sizeof(struct dent));
    ...
    sflaglist = (struct flagent *) malloc((numsflags + numpflags) * sizeof(struct flagent));
    if (hashtbl == ((void *) 0) || hashstrings == ((void *) 0) || sflaglist == ((void *) 0)) {
	if (cbench_print)
	    fprintf(stderr, "Couldn't allocate space for hash table\r\n");
	return (-1);
    }
	...
	(void) lseek(hashfd, (long) hashheader.stringsize - (long) hashheader.lstringsize + (long) hashheader.tblsize * (long) sizeof(struct dent), 1);
    } else {
	if (read(hashfd, hashstrings, (unsigned) hashheader.stringsize) != hashheader.stringsize || read(hashfd, (char *) hashtbl, (unsigned) hashheader.tblsize * sizeof(struct dent)) != hashheader.tblsize * sizeof(struct dent)) {
	    if (cbench_print)
		fprintf(stderr, "Illegal format hash table\r\n");
	    return (-1);
	}
    }
    if (read(hashfd, (char *) sflaglist, (unsigned) (numsflags + numpflags) * sizeof(struct flagent)) != (numsflags + numpflags) * sizeof(struct flagent)) {
	if (cbench_print)
	    fprintf(stderr, "Illegal format hash table\r\n");
	return (-1);
    }
	...
	if (ind->numents == 0)
	    ind->pu.ent = entry;
	ind->numents++;
	if (!viazero && ind->numents >= 4 && strcmp((char *) (entry->affix), (char *) (ind->pu.ent->affix)) != 0) {
	    entry = ind->pu.ent - 1;
	    i = numsflags - (entry - sflaglist);
	    ind->pu.fp = (struct flagptr *) calloc((unsigned) (128 + hashheader.nstrchars), sizeof(struct flagptr));
	    if (ind->pu.fp == ((void *) 0)) {
		if (cbench_print)
		    fprintf(stderr, "Couldn't allocate space for language tables\r\n");
		return (-1);
	    }
	    ind->numents = 0;
	}
	...
	if (!viazero && ind->numents >= 4 && strcmp((char *) (entry->affix), (char *) (ind->pu.ent->affix)) != 0) {
	    ...
	    ind->pu.fp = (struct flagptr *) calloc(128 + hashheader.nstrchars, sizeof(struct flagptr));
	    if (ind->pu.fp == ((void *) 0)) {
		if (cbench_print)
		    fprintf(stderr, "Couldn't allocate space for language tables\r\n");
		return (-1);
	...
	chartypes = (struct strchartype *) malloc(hashheader.nstrchartype * sizeof(struct strchartype));
	if (chartypes == ((void *) 0)) {
	    if (cbench_print)
		fprintf(stderr, "Couldn't allocate space for language tables\r\n");
	    return (-1);
...
int dotree;
{
    ...
    for (; dp != ((void *) 0); dp = dp->next) {
	s1 = dp->word;
	if (s1 && s1[0] == schar[0] && strcmp(s1 + 1, schar + 1) == 0)
	    return dp;
	while (dp->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 4)))
	    dp = dp->next;
    } if (dotree) {
	dp = treelookup(s);
	return dp;
