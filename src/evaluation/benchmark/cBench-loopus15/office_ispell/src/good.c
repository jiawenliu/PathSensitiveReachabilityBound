static char Rcs_Id[] = "$Id: good.c,v 1.43 1994/11/02 06:56:05 geoff Exp $";
...
extern char *tgoto();
extern char *tputs();
int good(ichar_t * word, int ignoreflagbits, int allhits, int pfxopts, int sfxopts);
int cap_ok(ichar_t * word, struct success *hit, int len);
static int entryhasaffixes(struct dent *dent, struct success *hit);
void flagpr(ichar_t * word, int preflag, int prestrip, int preadd, int sufflag, int sufadd);
...
int sfxopts;
{
    ...
    *q = 0;
	...
	orig_word = w;
    } else if ((dp = lookup(nword, 1)) != ((void *) 0)) {
	hits[0].dictent = dp;
	hits[0].prefix = ((void *) 0);
	hits[0].suffix = ((void *) 0);
	if (allhits || cap_ok(w, &hits[0], n))
	    numhits = 1;
	if (compoundflag == 2 && ((pfxopts | sfxopts) & (1 << 1)) != 0 && hashheader.compoundbit >= 0 && ((dp->mask)[0] & ((long) 1 << (hashheader.compoundbit))) == 0)
	    numhits = 0;
    }
    if (numhits && !allhits)
	return 1;
...
int len;
{
    ...
    if (thiscap == ((long) 1 << (((32) - 6) + 2)))
	return 1;
    else if (thiscap == ((long) 3 << (((32) - 6) + 2))) {
	if (hit->prefix) {
    ...
    dent = hit->dictent;
    for (;;) {
	dentcap = ((dent->mask[(32 / 32) - 1]) & ((long) 3 << (((32) - 6) + 2)));
	if (dentcap != thiscap) {
	    if (dentcap == ((long) 0 << (((32) - 6) + 2)) && thiscap == ((long) 2 << (((32) - 6) + 2)) && entryhasaffixes(dent, hit))
		return 1;
	} else {
	    if (thiscap != ((long) 3 << (((32) - 6) + 2))) {
		if (entryhasaffixes(dent, hit))
		    return 1;
	    } else {
		(void) strtoichar(dentword, dent->word, 100, 1);
		dword = dentword;
		limit = word + preadd;
		if ((hashheader.upperchars[(dword[prestrip])])) {
		    for (w = word; w < limit; w++) {
			if ((hashheader.lowerchars[(*w)]))
			    goto doublecontinue;
		    }
		} else {
		    for (w = word; w < limit; w++) {
			if ((hashheader.upperchars[(*w)]))
			    goto doublecontinue;
		    }
		}
		dword += prestrip;
		...
		dword = limit - 1;
		if ((hashheader.upperchars[(*dword)])) {
		    for (; *w; w++) {
			if ((hashheader.lowerchars[(*w)]))
			    goto doublecontinue;
		    }
...
register struct success *hit;
{
    if (hit->prefix && !((dent->mask)[0] & ((long) 1 << (hit->prefix->flagbit))))
	return 0;
    if (hit->suffix && !((dent->mask)[0] & ((long) 1 << (hit->suffix->flagbit))))
	return 0;
    return 1;
}
...
int sufadd;
{
    register ichar_t *origp;
    int orig_len;
    orig_len = strlen((char *) (orig_word));
    if (preflag > 0) {
	origp = orig_word + preadd;
	if ((hashheader.upperchars[(*origp)])) {
	    for (origp = orig_word; origp < orig_word + preadd; origp++) {
		if ((hashheader.lowerchars[(*origp)]))
		    return;
	    }
	} else {
	    for (origp = orig_word; origp < orig_word + preadd; origp++) {
	...
	if ((hashheader.upperchars[(origp[-1])])) {
	    for (; *origp != 0; origp++) {
		if ((hashheader.lowerchars[(*origp)]))
		    return;
	    }
    ...
    if ((hashheader.upperchars[(*origp)])) {
	while (--prestrip >= 0)
	    (void) fputs(printichar((int) *word++), outfile1);
    } else {
	while (--prestrip >= 0)
	    (void) fputs(printichar((int) (hashheader.lowerconv[(*word++)])), outfile1);
    } for (prestrip = orig_len - preadd - sufadd; --prestrip >= 0; word++)
	(void) fputs(printichar((int) *origp++), outfile1);
    if (origp > orig_word)
	origp--;
    if ((hashheader.upperchars[(*origp)]))
	(void) fputs(ichartosstr(word, 0), outfile1);
    else {
	while (*word) {
	    (void) fputs(printichar((int) (hashheader.lowerconv[(*word++)])), outfile1);
