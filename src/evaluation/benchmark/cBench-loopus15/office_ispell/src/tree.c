...
struct dent *treelookup(ichar_t * word);
static int pdictcmp(struct dent **enta, struct dent **entb);
void treeoutput(void);
void *mymalloc(unsigned int size);
void myfree(void *ptr);
static int cantexpand = 0;
static struct dent *pershtab;
static int pershsize = 0;
static int hcount = 0;
static int goodsizes[] = { 53, 223, 907, 3631 };

static char personaldict[4096];
static FILE *dictf;
static newwords = 0;
...
char *LibDict;
{
    ...
    if (p == ((void *) 0)) {
	dictf = trydict(personaldict, (char *) ((void *) 0), ".ispell_", LibDict);
	secondf = trydict(seconddict, h, ".ispell_", LibDict);
	if (dictf == ((void *) 0) && secondf == ((void *) 0)) {
	    dictf = trydict(personaldict, (char *) ((void *) 0), ".ispell_", "words");
	    secondf = trydict(seconddict, h, ".ispell_", "words");
	}
	if (dictf == ((void *) 0) && secondf == ((void *) 0)) {
	    dictf = trydict(personaldict, (char *) ((void *) 0), ".ispell_", LibDict);
	    secondf = trydict(seconddict, h, ".ispell_", LibDict);
	...
	if (personaldict[0] == '\0') {
		...
		(void) sprintf(personaldict, "%s/%s%s", h, ".ispell_", LibDict);
	}
	if (dictf != ((void *) 0)) {
	    treeload(dictf);
	    (void) fclose(dictf);
	...
	if (abspath) {
	    ...
	    if ((dictf = fopen(personaldict, "r")) != ((void *) 0)) {
		treeload(dictf);
		(void) fclose(dictf);
	    }
	} else {
    ...
    if (!lflag && !aflag && access(personaldict, 2) < 0 && (*__errno_location()) != 2) {
	if (cbench_print)
	    fprintf(stderr, "Warning: Cannot update personal dictionary (%s)\r\n", personaldict);
...
{
    char buf[8192];
    while (fgets(buf, sizeof buf, loadfile) != ((void *) 0))
...
int keep;
{
    ...
    struct dent *oldhtab;
    int oldhsize;
    ichar_t nword[100 + 20];
    int isvariant;
    if (!cantexpand && (hcount * 100) / 70 >= pershsize) {
	oldhsize = pershsize;
	oldhtab = pershtab;
	...
	if (pershtab == ((void *) 0)) {
		...
		dp = &oldhtab[i];
		if (dp->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 0))) {
		    newdp = tinsert(dp);
		    isvariant = (dp->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 4)));
		    dp = dp->next;
		    while (dp != ((void *) 0)) {
			if (isvariant) {
			    isvariant = dp->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 4));
	...
	return;
    if (keep)
	wordent.mask[(32 / 32) - 1] |= ((long) 1 << (((32) - 6) + 1));
    (void) strtoichar(nword, word, sizeof nword, 1);
    upcase(nword);
    if ((dp = lookup(nword, 1)) != ((void *) 0)) {
	...
	dp = tinsert(&wordent);
	if (((dp->mask[(32 / 32) - 1]) & ((long) 3 << (((32) - 6) + 2))) == ((long) 3 << (((32) - 6) + 2)))
	    (void) addvheader(dp);
    } newwords |= keep;
...
register ichar_t *word;
{
    ...
    hp = &pershtab[hcode];
    while (hp != ((void *) 0) && (hp->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 0)))) {
	if (strcmp(chword, hp->word) == 0)
	    break;
	while (hp->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 4)))
	    hp = hp->next;
	hp = hp->next;
    } if (hp != ((void *) 0) && (hp->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 0))))
	return hp;
    else
	return ((void *) 0);
...
void treeoutput()
{
    ...
    register struct dent *lent;
    int pdictsize;
    struct dent **sortlist;
    register struct dent **sortptr;
    register struct dent *ehtab;
    if (newwords == 0)
	return;
    if ((dictf = fopen(personaldict, "w")) == ((void *) 0)) {
	if (cbench_print)
	...
	for (cent = pershtab, ehtab = pershtab + pershsize; cent < ehtab; cent++) {
	    for (lent = cent; lent != ((void *) 0); lent = lent->next) {
		if ((lent->mask[(32 / 32) - 1] & (((long) 1 << (((32) - 6) + 0)) | ((long) 1 << (((32) - 6) + 1)))) == (((long) 1 << (((32) - 6) + 0)) | ((long) 1 << (((32) - 6) + 1))))
		    pdictsize++;
		while (lent->mask[(32 / 32) - 1] & ((long) 1 << (((32) - 6) + 4)))
		    lent = lent->next;
	}} for (cent = hashtbl, ehtab = hashtbl + hashsize; cent < ehtab; cent++) {
	    if ((cent->mask[(32 / 32) - 1] & (((long) 1 << (((32) - 6) + 0)) | ((long) 1 << (((32) - 6) + 1)))) == (((long) 1 << (((32) - 6) + 0)) | ((long) 1 << (((32) - 6) + 1)))) {
		if (((cent->mask[(32 / 32) - 1]) & ((long) 3 << (((32) - 6) + 2))) != ((long) 3 << (((32) - 6) + 2)) && cent->word != ((void *) 0))
		    pdictsize++;
	    }
    ...
    } qsort((char *) sortlist, (unsigned) pdictsize, sizeof(sortlist[0]), (int (*)(const void *, const void *)) pdictcmp);
...
void *ptr;
{
    if (hashstrings != ((void *) 0) && (char *) ptr >= hashstrings && (char *) ptr <= hashstrings + hashheader.stringsize)
	return;
    free(ptr);
}
