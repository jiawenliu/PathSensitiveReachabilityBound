...
extern void jpeg_make_d_derived_tbl(j_decompress_ptr cinfo, JHUFF_TBL * htbl, d_derived_tbl ** pdtbl);
...
extern boolean jpeg_fill_bit_buffer(bitread_working_state * state, register bit_buf_type get_buffer, register int bits_left, int nbits);
extern int jpeg_huff_decode(bitread_working_state * state, register bit_buf_type get_buffer, register int bits_left, d_derived_tbl * htbl, int min_bits);
typedef struct {
    unsigned int EOBRUN;
    int last_dc_val[4];
} savable_state;
...
} static const int extend_test[16] = { 0, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080, 0x0100, 0x0200, 0x0400, 0x0800, 0x1000, 0x2000, 0x4000 };
static const int extend_offset[16] = { 0, ((-1) << 1) + 1, ((-1) << 2) + 1, ((-1) << 3) + 1, ((-1) << 4) + 1, ((-1) << 5) + 1, ((-1) << 6) + 1, ((-1) << 7) + 1, ((-1) << 8) + 1, ((-1) << 9) + 1, ((-1) << 10) + 1, ((-1) << 11) + 1, ((-1) << 12) + 1, ((-1) << 13) + 1, ((-1) << 14) + 1, ((-1) << 15) + 1 };
...
static boolean decode_mcu_AC_refine(j_decompress_ptr cinfo, JBLOCKROW * MCU_data)
{
	...
	for (; k <= Se; k++) { {
		...
		if ((nb = tbl->look_nbits[look]) != 0) {
		    ...
		    nb = 8 + 1;
		  label3:if ((s = jpeg_huff_decode(&br_state, get_buffer, bits_left, tbl, nb)) < 0) {
			goto undoit;
		    }
		    get_buffer = br_state.get_buffer;
		    bits_left = br_state.bits_left;
