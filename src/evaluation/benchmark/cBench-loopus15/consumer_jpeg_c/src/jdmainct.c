...
{
    my_main_ptr the_main = (my_main_ptr) cinfo->main;
    int ci, rgroup;
    int M = cinfo->min_DCT_scaled_size;
    jpeg_component_info *compptr;
    JSAMPARRAY xbuf;
    the_main->xbuffer[0] = (JSAMPIMAGE) (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1, cinfo->num_components * 2 * ((size_t) sizeof(JSAMPARRAY)));
    the_main->xbuffer[1] = the_main->xbuffer[0] + cinfo->num_components;
	...
	xbuf += rgroup;
	the_main->xbuffer[0][ci] = xbuf;
	xbuf += rgroup * (M + 4);
	the_main->xbuffer[1][ci] = xbuf;
    }
}
...
static void make_funny_pointers(j_decompress_ptr cinfo)
{
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
	rgroup = (compptr->v_samp_factor * compptr->DCT_scaled_size) / cinfo->min_DCT_scaled_size;
	xbuf0 = the_main->xbuffer[0][ci];
	xbuf1 = the_main->xbuffer[1][ci];
	buf = the_main->buffer[ci];
...
static void set_bottom_pointers(j_decompress_ptr cinfo)
{
	...
	if (ci == 0) {
	    the_main->rowgroups_avail = (JDIMENSION) ((rows_left - 1) / rgroup + 1);
	}
	xbuf = the_main->xbuffer[the_main->whichptr][ci];
	for (i = 0; i < rgroup * 2; i++) {
...
static void start_pass_main(j_decompress_ptr cinfo, J_BUF_MODE pass_mode)
{
	...
	if (cinfo->upsample->need_context_rows) {
	    the_main->pub.process_data = process_data_context_main;
	    make_funny_pointers(cinfo);
	    the_main->whichptr = 0;
	    the_main->context_state = 0;
	    the_main->iMCU_row_ctr = 0;
	} else {
	    the_main->pub.process_data = process_data_simple_main;
	}
	the_main->buffer_full = 0;
	the_main->rowgroup_ctr = 0;
	break;
    case JBUF_CRANK_DEST:
	the_main->pub.process_data = process_data_crank_post;
	break;
    default:
...
static void process_data_simple_main(j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION * out_row_ctr, JDIMENSION out_rows_avail)
{
    my_main_ptr the_main = (my_main_ptr) cinfo->main;
    JDIMENSION rowgroups_avail;
    if (!the_main->buffer_full) {
	if (!(*cinfo->coef->decompress_data) (cinfo, the_main->buffer))
	    return;
	the_main->buffer_full = 1;
    }
    rowgroups_avail = (JDIMENSION) cinfo->min_DCT_scaled_size;
    (*cinfo->post->post_process_data) (cinfo, the_main->buffer, &the_main->rowgroup_ctr, rowgroups_avail, output_buf, out_row_ctr, out_rows_avail);
    if (the_main->rowgroup_ctr >= rowgroups_avail) {
	the_main->buffer_full = 0;
	the_main->rowgroup_ctr = 0;
    }
}
...
static void process_data_context_main(j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION * out_row_ctr, JDIMENSION out_rows_avail)
{
    my_main_ptr the_main = (my_main_ptr) cinfo->main;
    if (!the_main->buffer_full) {
	if (!(*cinfo->coef->decompress_data) (cinfo, the_main->xbuffer[the_main->whichptr]))
	    return;
	the_main->buffer_full = 1;
	the_main->iMCU_row_ctr++;
    }
    switch (the_main->context_state) {
    case 2:
	(*cinfo->post->post_process_data) (cinfo, the_main->xbuffer[the_main->whichptr], &the_main->rowgroup_ctr, the_main->rowgroups_avail, output_buf, out_row_ctr, out_rows_avail);
	if (the_main->rowgroup_ctr < the_main->rowgroups_avail)
	    return;
	the_main->context_state = 0;
    ...
    case 0:
	the_main->rowgroup_ctr = 0;
	the_main->rowgroups_avail = (JDIMENSION) (cinfo->min_DCT_scaled_size - 1);
	if (the_main->iMCU_row_ctr == cinfo->total_iMCU_rows)
	    set_bottom_pointers(cinfo);
	the_main->context_state = 1;
    case 1:
	(*cinfo->post->post_process_data) (cinfo, the_main->xbuffer[the_main->whichptr], &the_main->rowgroup_ctr, the_main->rowgroups_avail, output_buf, out_row_ctr, out_rows_avail);
	if (the_main->rowgroup_ctr < the_main->rowgroups_avail)
	    return;
	if (the_main->iMCU_row_ctr == 1)
	    set_wraparound_pointers(cinfo);
	the_main->whichptr ^= 1;
	the_main->buffer_full = 0;
	the_main->rowgroup_ctr = (JDIMENSION) (cinfo->min_DCT_scaled_size + 1);
	the_main->rowgroups_avail = (JDIMENSION) (cinfo->min_DCT_scaled_size + 2);
	the_main->context_state = 2;
    }
}
...
{
    my_main_ptr the_main;
    int ci, rgroup, ngroups;
    jpeg_component_info *compptr;
    the_main = (my_main_ptr) (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1, ((size_t) sizeof(my_main_controller)));
    cinfo->main = (struct jpeg_d_main_controller *) the_main;
    the_main->pub.start_pass = start_pass_main;
    if (need_full_buffer)
	((cinfo)->err->msg_code = (JERR_BAD_BUFFER_MODE), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
    if (cinfo->upsample->need_context_rows) {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
	rgroup = (compptr->v_samp_factor * compptr->DCT_scaled_size) / cinfo->min_DCT_scaled_size;
	the_main->buffer[ci] = (*cinfo->mem->alloc_sarray) ((j_common_ptr) cinfo, 1, compptr->width_in_blocks * compptr->DCT_scaled_size, (JDIMENSION) (rgroup * ngroups));
    }
}
