...
static void start_pass_huff(j_compress_ptr cinfo, boolean gather_statistics)
{
	...
	if (gather_statistics) {
		...
		entropy->dc_count_ptrs[dctbl] = (long *) (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1, 257 * ((size_t) sizeof(long)));
	    memset((void *) (entropy->dc_count_ptrs[dctbl]), 0, (size_t) (257 * ((size_t) sizeof(long))));
	    if (entropy->ac_count_ptrs[actbl] == ((void *) 0))
		entropy->ac_count_ptrs[actbl] = (long *) (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1, 257 * ((size_t) sizeof(long)));
...
static void htest_one_block(JCOEFPTR block, int last_dc_val, long dc_counts[], long ac_counts[])
{
    register int temp;
    register int nbits;
    register int k, r;
