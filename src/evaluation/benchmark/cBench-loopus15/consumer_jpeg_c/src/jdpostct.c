...
void jinit_d_post_controller(j_decompress_ptr cinfo, boolean need_full_buffer)
{
	...
	if (need_full_buffer) {
	    post->whole_image = (*cinfo->mem->request_virt_sarray) ((j_common_ptr) cinfo, 1, 0, cinfo->output_width * cinfo->out_color_components, (JDIMENSION) jround_up((long) cinfo->output_height, (long) post->strip_height), post->strip_height);
	} else {
