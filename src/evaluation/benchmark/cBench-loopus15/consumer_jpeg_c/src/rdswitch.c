...
boolean read_scan_script(j_compress_ptr cinfo, char *filename)
{
	...
	if (termchar != ';' && termchar != (-1)) {
	  bogus:fprintf(stderr, "Invalid scan entry format in file %s\n", filename);
	    fclose(fp);
	    return 0;
	}
	scanptr++, scanno++;
    ...
    if (scanno > 0) {
	scanptr = (jpeg_scan_info *) (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1, scanno * ((size_t) sizeof(jpeg_scan_info)));
