...
tstrip_t TIFFComputeStrip(TIFF * tif, uint32 row, tsample_t sample)
{
	...
	if (sample >= td->td_samplesperpixel) {
	    TIFFError(tif->tif_name, "%u: Sample out of range, max %u", sample, td->td_samplesperpixel);
	    return ((tstrip_t) 0);
	}
...
tstrip_t TIFFNumberOfStrips(TIFF * tif)
{
    TIFFDirectory *td = &tif->tif_dir;
    tstrip_t nstrips;
    nstrips = (td->td_rowsperstrip == (uint32) - 1 ? (td->td_imagelength != 0 ? 1 : 0) : ((((uint32) (td->td_imagelength)) + (((uint32) (td->td_rowsperstrip)) - 1)) / ((uint32) (td->td_rowsperstrip))));
    if (td->td_planarconfig == 2)
...
tsize_t TIFFVStripSize(TIFF * tif, uint32 nrows)
{
	...
	tsize_t w = (((((uint32) (td->td_imagewidth)) + (((uint32) (td->td_ycbcrsubsampling[0])) - 1)) / ((uint32) (td->td_ycbcrsubsampling[0]))) * ((uint32) (td->td_ycbcrsubsampling[0])));
	tsize_t scanline = ((((uint32) (w * td->td_bitspersample)) + (((uint32) (8)) - 1)) / ((uint32) (8)));
	...
	return ((tsize_t) (nrows * scanline + 2 * (nrows * scanline / samplingarea)));
    } else
...
tsize_t TIFFScanlineSize(TIFF * tif)
{
    ...
    return ((tsize_t) ((((uint32) (scanline)) + (((uint32) (8)) - 1)) / ((uint32) (8))));
}
...
tsize_t TIFFRasterScanlineSize(TIFF * tif)
{
	...
	return ((tsize_t) ((((uint32) (scanline)) + (((uint32) (8)) - 1)) / ((uint32) (8))) * td->td_samplesperpixel);
}
