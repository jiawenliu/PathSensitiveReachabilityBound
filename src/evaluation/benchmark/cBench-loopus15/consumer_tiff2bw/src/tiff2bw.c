...
int main1(int argc, char *argv[])
{
	...
	case 'r':
	    ...
	    break;
	case 'R':
	    RED = (((atoi(optarg)) * 255) / 100);
	    break;
	case 'G':
	    GREEN = (((atoi(optarg)) * 255) / 100);
	    break;
	case 'B':
	    BLUE = (((atoi(optarg)) * 255) / 100);
	    break;
	case '?':
	    usage();
	}
...
static int processCompressOptions(char *opt)
{
	...
	char *cp = strchr(opt, ':');
	if (cp && ((*__ctype_b_loc())[(int) ((cp[1]))] & (unsigned short int) _ISdigit))
	    quality = atoi(cp + 1);
	if (cp && strchr(cp, 'r'))
...
static void cpTag(TIFF * in, TIFF * out, uint16 tag, uint16 count, TIFFDataType type)
{
	...
	if (count == 1) {
	    ...
	    if (TIFFGetField(in, tag, &shortv, &shortv2))
		TIFFSetField(out, tag, shortv, shortv2);
	} else if (count == (uint16) - 1) {
	    if (TIFFGetField(in, tag, &shortv, &shortav))
		TIFFSetField(out, tag, shortv, shortav);
	}
	break;
    case TIFF_LONG:
	if (TIFFGetField(in, tag, &longv))
	    TIFFSetField(out, tag, longv);
	break;
    case TIFF_RATIONAL:
	if (count == 1) {
	...
	break;
    case TIFF_ASCII:
	if (TIFFGetField(in, tag, &stringv))
	    TIFFSetField(out, tag, stringv);
	break;
    default:
