...
static int ThunderDecode(TIFF * tif, tidata_t op, tsize_t maxpixels)
{
	...
	case 0x40:
	    if ((delta = ((n >> 4) & 3)) != 2) {
		lastpixel = (lastpixel + twobitdeltas[delta]) & 0xf;
		if (npixels++ & 1)
		    *op++ |= lastpixel;
		else
		    op[0] = lastpixel << 4;
	...
	case 0x80:
	    if ((delta = ((n >> 3) & 7)) != 4) {
		lastpixel = (lastpixel + threebitdeltas[delta]) & 0xf;
		if (npixels++ & 1)
		    *op++ |= lastpixel;
		else
		    op[0] = lastpixel << 4;
	...
	TIFFError(tif->tif_name, "ThunderDecode: %s data at scanline %ld (%lu != %lu)", npixels < maxpixels ? "Not enough" : "Too much", (long) tif->tif_row, (long) npixels, (long) maxpixels);
	return (0);
    }
    return (1);
}
