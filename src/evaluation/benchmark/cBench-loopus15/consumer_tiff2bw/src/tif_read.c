...
static int TIFFSeek(TIFF * tif, uint32 row, tsample_t sample)
{
	...
	TIFFError(tif->tif_name, "%lu: Row out of range, max %lu", (u_long) row, (u_long) td->td_imagelength);
	return (0);
    }
    if (td->td_planarconfig == 2) {
	if (sample >= td->td_samplesperpixel) {
	    TIFFError(tif->tif_name, "%lu: Sample out of range, max %lu", (u_long) sample, (u_long) td->td_samplesperpixel);
	    return (0);
	}
...
int TIFFReadScanline(TIFF * tif, tdata_t buf, uint32 row, tsample_t sample)
{
	...
	e = (*tif->tif_decoderow) (tif, (tidata_t) buf, tif->tif_scanlinesize, sample);
	tif->tif_row++;
	if (e)
...
tsize_t TIFFReadEncodedStrip(TIFF * tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
    ...
    if (strip != td->td_nstrips - 1 || (nrows = td->td_imagelength % td->td_rowsperstrip) == 0)
	...
	size = stripsize;
    if (TIFFFillStrip(tif, strip) && (*tif->tif_decodestrip) (tif, (tidata_t) buf, size, (tsample_t) (strip / td->td_stripsperimage))) {
...
static tsize_t TIFFReadRawStrip1(TIFF * tif, tstrip_t strip, tdata_t buf, tsize_t size, const char *module)
{
    TIFFDirectory *td = &tif->tif_dir;
    if (!(((tif)->tif_flags & 0x0800) != 0)) {
	tsize_t cc;
	if (!(((*(tif)->tif_seekproc) ((tif)->tif_clientdata, (toff_t) ((toff_t) td->td_stripoffset[strip]), 0)) == (toff_t) td->td_stripoffset[strip])) {
	    TIFFError(module, "%s: Seek error at scanline %lu, strip %lu", tif->tif_name, (u_long) tif->tif_row, (u_long) strip);
	    return (-1);
	}
	cc = ((*(tif)->tif_readproc) ((tif)->tif_clientdata, buf, size));
	if (cc != size) {
	    TIFFError(module, "%s: Read error at scanline %lu; got %lu bytes, expected %lu", tif->tif_name, (u_long) tif->tif_row, (u_long) cc, (u_long) size);
	    return (-1);
	}
    } else {
	if (((tsize_t) (td->td_stripoffset[strip] + size)) > tif->tif_size) {
	    TIFFError(module, "%s: Read error at scanline %lu, strip %lu; got %lu bytes, expected %lu", tif->tif_name, (u_long) tif->tif_row, (u_long) strip, (u_long) tif->tif_size - td->td_stripoffset[strip], (u_long) size);
	    return (-1);
	}
...
tsize_t TIFFReadRawStrip(TIFF * tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
	...
	return ((tsize_t) - 1);
    if (strip >= td->td_nstrips) {
	TIFFError(tif->tif_name, "%lu: Strip out of range, max %lu", (u_long) strip, (u_long) td->td_nstrips);
	return ((tsize_t) - 1);
    }
    bytecount = td->td_stripbytecount[strip];
    if (bytecount <= 0) {
	TIFFError(tif->tif_name, "%lu: Invalid strip byte count, strip %lu", (u_long) bytecount, (u_long) strip);
	return ((tsize_t) - 1);
    }
...
static int TIFFFillStrip(TIFF * tif, tstrip_t strip)
{
    ...
    if ((((tif)->tif_flags & 0x0800) != 0) && ((((tif)->tif_flags & (td->td_fillorder)) != 0) || (tif->tif_flags & 0x0100))) {
	if ((tif->tif_flags & 0x0200) && tif->tif_rawdata)
	    _TIFFfree(tif->tif_rawdata);
	tif->tif_flags &= ~0x0200;
	if (((tsize_t) td->td_stripoffset[strip] + bytecount) > tif->tif_size) {
	    TIFFError(module, "%s: Read error on strip %lu; got %lu bytes, expected %lu", tif->tif_name, (u_long) strip, (u_long) tif->tif_size - td->td_stripoffset[strip], (u_long) bytecount);
	...
	if (bytecount > tif->tif_rawdatasize) {
	    tif->tif_curstrip = ((tstrip_t) - 1);
	    if ((tif->tif_flags & 0x0200) == 0) {
		TIFFError(module, "%s: Data buffer too small to hold strip %lu", tif->tif_name, (u_long) strip);
		return (0);
	    }
	    if (!TIFFReadBufferSetup(tif, 0, (((((uint32) (bytecount)) + (((uint32) (1024)) - 1)) / ((uint32) (1024))) * ((uint32) (1024)))))
		return (0);
	}
	if (TIFFReadRawStrip1(tif, strip, (u_char *) tif->tif_rawdata, bytecount, module) != bytecount)
	    return (0);
	if (!(((tif)->tif_flags & (td->td_fillorder)) != 0) && (tif->tif_flags & 0x0100) == 0)
...
tsize_t TIFFReadTile(TIFF * tif, tdata_t buf, uint32 x, uint32 y, uint32 z, tsample_t s)
{
	...
	return (-1);
    return (TIFFReadEncodedTile(tif, TIFFComputeTile(tif, x, y, z, s), buf, (tsize_t) - 1));
}
...
tsize_t TIFFReadEncodedTile(TIFF * tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	size = tilesize;
    if (TIFFFillTile(tif, tile) && (*tif->tif_decodetile) (tif, (tidata_t) buf, size, (tsample_t) (tile / td->td_stripsperimage))) {
	(*tif->tif_postdecode) (tif, (tidata_t) buf, size);
	return (size);
    } else
	return (-1);
...
static tsize_t TIFFReadRawTile1(TIFF * tif, ttile_t tile, tdata_t buf, tsize_t size, const char *module)
{
    TIFFDirectory *td = &tif->tif_dir;
    if (!(((tif)->tif_flags & 0x0800) != 0)) {
	tsize_t cc;
	if (!(((*(tif)->tif_seekproc) ((tif)->tif_clientdata, (toff_t) ((toff_t) td->td_stripoffset[tile]), 0)) == (toff_t) td->td_stripoffset[tile])) {
	    TIFFError(module, "%s: Seek error at row %ld, col %ld, tile %ld", tif->tif_name, (long) tif->tif_row, (long) tif->tif_col, (long) tile);
	    return ((tsize_t) - 1);
	}
	cc = ((*(tif)->tif_readproc) ((tif)->tif_clientdata, buf, size));
	if (cc != size) {
	    TIFFError(module, "%s: Read error at row %ld, col %ld; got %lu bytes, expected %lu", tif->tif_name, (long) tif->tif_row, (long) tif->tif_col, (u_long) cc, (u_long) size);
	    return ((tsize_t) - 1);
	}
    } else {
	if (((tsize_t) (td->td_stripoffset[tile] + size)) > tif->tif_size) {
	    TIFFError(module, "%s: Read error at row %ld, col %ld, tile %ld; got %lu bytes, expected %lu", tif->tif_name, (long) tif->tif_row, (long) tif->tif_col, (long) tile, (u_long) tif->tif_size - td->td_stripoffset[tile], (u_long) size);
	    return ((tsize_t) - 1);
	}
...
tsize_t TIFFReadRawTile(TIFF * tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	return ((tsize_t) - 1);
    if (tile >= td->td_nstrips) {
	TIFFError(tif->tif_name, "%lu: Tile out of range, max %lu", (u_long) tile, (u_long) td->td_nstrips);
	return ((tsize_t) - 1);
    }
...
static int TIFFFillTile(TIFF * tif, ttile_t tile)
{
    ...
    bytecount = td->td_stripbytecount[tile];
    if (bytecount <= 0) {
	TIFFError(tif->tif_name, "%lu: Invalid tile byte count, tile %lu", (u_long) bytecount, (u_long) tile);
	return (0);
    }
    if ((((tif)->tif_flags & 0x0800) != 0) && ((((tif)->tif_flags & (td->td_fillorder)) != 0) || (tif->tif_flags & 0x0100))) {
	if ((tif->tif_flags & 0x0200) && tif->tif_rawdata)
	...
	if (bytecount > tif->tif_rawdatasize) {
	    tif->tif_curtile = ((ttile_t) - 1);
	    if ((tif->tif_flags & 0x0200) == 0) {
		TIFFError(module, "%s: Data buffer too small to hold tile %ld", tif->tif_name, (long) tile);
		return (0);
	    }
	    if (!TIFFReadBufferSetup(tif, 0, (((((uint32) (bytecount)) + (((uint32) (1024)) - 1)) / ((uint32) (1024))) * ((uint32) (1024)))))
		return (0);
...
int TIFFReadBufferSetup(TIFF * tif, tdata_t bp, tsize_t size)
{
	...
	tif->tif_rawdatasize = (((((uint32) (size)) + (((uint32) (1024)) - 1)) / ((uint32) (1024))) * ((uint32) (1024)));
	tif->tif_rawdata = (tidata_t) _TIFFmalloc(tif->tif_rawdatasize);
	tif->tif_flags |= 0x0200;
    }
    if (tif->tif_rawdata == ((void *) 0)) {
	TIFFError(module, "%s: No space for data buffer at scanline %ld", tif->tif_name, (long) tif->tif_row);
...
static int TIFFStartStrip(TIFF * tif, tstrip_t strip)
{
    ...
    return ((*tif->tif_predecode) (tif, (tsample_t) (strip / td->td_stripsperimage)));
}
...
static int TIFFStartTile(TIFF * tif, ttile_t tile)
{
    ...
    tif->tif_row = (tile % ((((uint32) (td->td_imagewidth)) + (((uint32) (td->td_tilewidth)) - 1)) / ((uint32) (td->td_tilewidth)))) * td->td_tilelength;
    tif->tif_col = (tile % ((((uint32) (td->td_imagelength)) + (((uint32) (td->td_tilelength)) - 1)) / ((uint32) (td->td_tilelength)))) * td->td_tilewidth;
    tif->tif_rawcp = tif->tif_rawdata;
    tif->tif_rawcc = td->td_stripbytecount[tile];
    return ((*tif->tif_predecode) (tif, (tsample_t) (tile / td->td_stripsperimage)));
}
...
static int TIFFCheckRead(TIFF * tif, int tiles)
{
    ...
    if (tiles ^ (((tif)->tif_flags & 0x0400) != 0)) {
...
{
    (void) tif;
    (((cc & 1) == 0) ? (void) (0) : __assert_fail("(cc & 1) == 0", "tif_read.c", 608, __PRETTY_FUNCTION__));
    TIFFSwabArrayOfShort((uint16 *) buf, cc / 2);
} void _TIFFSwab32BitData(TIFF * tif, tidata_t buf, tsize_t cc)
{
    (void) tif;
    (((cc & 3) == 0) ? (void) (0) : __assert_fail("(cc & 3) == 0", "tif_read.c", 616, __PRETTY_FUNCTION__));
    TIFFSwabArrayOfLong((uint32 *) buf, cc / 4);
} void _TIFFSwab64BitData(TIFF * tif, tidata_t buf, tsize_t cc)
{
    (void) tif;
    (((cc & 7) == 0) ? (void) (0) : __assert_fail("(cc & 7) == 0", "tif_read.c", 624, __PRETTY_FUNCTION__));
