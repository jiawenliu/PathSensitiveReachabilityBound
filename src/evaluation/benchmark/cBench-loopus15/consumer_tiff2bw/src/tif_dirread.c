...
int TIFFReadDirectory(TIFF * tif)
{
	...
	if (fip->field_readcount != -1) {
	    uint32 expected = (fip->field_readcount == -2) ? (uint32) td->td_samplesperpixel : (uint32) fip->field_readcount;
	...
	case 324:
	case 325:
	    (((tif)->tif_dir.td_fieldsset[(fip->field_bit) / 32]) |= (((u_long) 1L) << ((fip->field_bit) & 0x1f)));
	    break;
	case 256:
	case 257:
	case 32997:
    ...
    if (!(((tif)->tif_dir.td_fieldsset[(25) / 32]) & (((u_long) 1L) << ((25) & 0x1f)))) {
	MissingRequired(tif, (((tif)->tif_flags & 0x0400) != 0) ? "TileOffsets" : "StripOffsets");
	goto bad;
    }
    for (dp = dir, n = dircount; n > 0; n--, dp++) {
	if (dp->tdir_tag == 0)
	...
	case 301:
	    ...
	    if (dp->tdir_tag == 320 || dp->tdir_count != (uint32) v) {
		if (!CheckDirCount(tif, dp, (uint32) (3 * v)))
		    break;
	    }
	    v *= sizeof(uint16);
	    cp = CheckMalloc(tif, dp->tdir_count * sizeof(uint16), "to read \"TransferFunction\" tag");
	    if (cp != ((void *) 0)) {
	...
	case 255:
	    ...
	    if (v)
		(void) TIFFSetField(tif, 254, (int) v);
	    break;
	default:
    ...
    if (td->td_photometric == 3 && !(((tif)->tif_dir.td_fieldsset[(26) / 32]) & (((u_long) 1L) << ((26) & 0x1f)))) {
	MissingRequired(tif, "Colormap");
	goto bad;
    }
    if (!(((tif)->tif_dir.td_fieldsset[(24) / 32]) & (((u_long) 1L) << ((24) & 0x1f)))) {
	if ((td->td_planarconfig == 1 && td->td_nstrips > 1) || (td->td_planarconfig == 2 && td->td_nstrips != td->td_samplesperpixel)) {
	    MissingRequired(tif, "StripByteCounts");
	    goto bad;
	}
	TIFFWarning(tif->tif_name, "TIFF directory is missing required \"%s\" field, calculating from imagelength", _TIFFFieldWithTag(tif, 279)->field_name);
	EstimateStripByteCounts(tif, dir, dircount);
    } else if (td->td_nstrips == 1 && (td->td_stripbytecount[0] == 0 || (td->td_compression == 1 && td->td_stripbytecount[0] > ((*(tif)->tif_sizeproc) ((tif)->tif_clientdata)) - td->td_stripoffset[0]))) {
	TIFFWarning(tif->tif_name, "Bogus \"%s\" field, ignoring and calculating from imagelength", _TIFFFieldWithTag(tif, 279)->field_name);
	EstimateStripByteCounts(tif, dir, dircount);
    }
    if (dir)
	_TIFFfree((char *) dir);
    ...
    if (td->td_nstrips == 1 && td->td_compression == 1 && (tif->tif_flags & (0x8000 | 0x0400)) == 0x8000)
...
static void EstimateStripByteCounts(TIFF * tif, TIFFDirEntry * dir, uint16 dircount)
{
    register TIFFDirEntry *dp;
    ...
    td->td_stripbytecount = (uint32 *) CheckMalloc(tif, td->td_nstrips * sizeof(uint32), "for \"StripByteCounts\" array");
    if (td->td_compression != 1) {
	uint32 space = (uint32) (sizeof(TIFFHeader) + sizeof(uint16) + (dircount * sizeof(TIFFDirEntry)) + sizeof(uint32));
    ...
    (((tif)->tif_dir.td_fieldsset[(24) / 32]) |= (((u_long) 1L) << ((24) & 0x1f)));
...
static tsize_t TIFFFetchData(TIFF * tif, TIFFDirEntry * dir, char *cp)
{
    ...
    return (cc);
  bad:TIFFError(tif->tif_name, "Error fetching data for field \"%s\"", _TIFFFieldWithTag(tif, dir->tdir_tag)->field_name);
    return ((tsize_t) 0);
}
...
{
    if (denom == 0) {
	TIFFError(tif->tif_name, "%s: Rational with zero denominator (num = %lu)", _TIFFFieldWithTag(tif, dir->tdir_tag)->field_name, num);
	return (0);
    } else {
...
{
    long l = ((uint32) ((tif)->tif_header.tiff_magic == 0x4d4d ? ((dir->tdir_offset) >> (tif)->tif_typeshift[dir->tdir_type]) & (tif)->tif_typemask[dir->tdir_type] : (dir->tdir_offset) & (tif)->tif_typemask[dir->tdir_type]));
    float v = *(float *) &l;;
    return (v);
}
...
} static int TIFFFetchRationalArray(TIFF * tif, TIFFDirEntry * dir, float *v)
{
    int ok = 0;
    uint32 *l;
    l = (uint32 *) CheckMalloc(tif, dir->tdir_count * tiffDataWidth[dir->tdir_type], "to fetch array of rationals");
    if (l) {
...
static int TIFFFetchAnyArray(TIFF * tif, TIFFDirEntry * dir, double *v)
{
    ...
    default:
		TIFFError(tif->tif_name, "Cannot read TIFF_ANY type %d for field \"%s\"", _TIFFFieldWithTag(tif, dir->tdir_tag)->field_name);
		return (0);
    }
    return (1);
}
...
static int TIFFFetchNormalTag(TIFF * tif, TIFFDirEntry * dp)
{
	...
	case TIFF_SSHORT:{
		...
		if (type != TIFF_LONG && type != TIFF_SLONG) {
		    uint16 v = (uint16) ((uint32) ((tif)->tif_header.tiff_magic == 0x4d4d ? ((dp->tdir_offset) >> (tif)->tif_typeshift[dp->tdir_type]) & (tif)->tif_typemask[dp->tdir_type] : (dp->tdir_offset) & (tif)->tif_typemask[dp->tdir_type]));
		    ok = (fip->field_passcount ? TIFFSetField(tif, dp->tdir_tag, 1, &v) : TIFFSetField(tif, dp->tdir_tag, v));
		    break;
		}
	    }
	case TIFF_LONG:
	case TIFF_SLONG:{
		uint32 v32 = ((uint32) ((tif)->tif_header.tiff_magic == 0x4d4d ? ((dp->tdir_offset) >> (tif)->tif_typeshift[dp->tdir_type]) & (tif)->tif_typemask[dp->tdir_type] : (dp->tdir_offset) & (tif)->tif_typemask[dp->tdir_type]));
		ok = (fip->field_passcount ? TIFFSetField(tif, dp->tdir_tag, 1, &v32) : TIFFSetField(tif, dp->tdir_tag, v32));
	    }
	    break;
	case TIFF_RATIONAL:
	case TIFF_SRATIONAL:
	case TIFF_FLOAT:{
		float v = (dp->tdir_type == TIFF_FLOAT ? TIFFFetchFloat(tif, dp) : TIFFFetchRational(tif, dp));
		ok = (fip->field_passcount ? TIFFSetField(tif, dp->tdir_tag, 1, &v) : TIFFSetField(tif, dp->tdir_tag, v));
	    } break;
	case TIFF_DOUBLE:{
		double v;
...
static int TIFFFetchStripThing(TIFF * tif, TIFFDirEntry * dir, long nstrips, uint32 ** lpp)
{
	...
	return (0);
    if (*lpp == ((void *) 0) && (*lpp = (uint32 *) CheckMalloc(tif, nstrips * sizeof(uint32), "for strip array")) == ((void *) 0))
	return (0);
    lp = *lpp;
    if (dir->tdir_type == (int) TIFF_SHORT) {
	uint16 *dp = (uint16 *) CheckMalloc(tif, dir->tdir_count * sizeof(uint16), "to fetch strip tag");
	if (dp == ((void *) 0))
	    return (0);
...
static void ChopUpSingleUncompressedStrip(TIFF * tif)
{
    ...
    newcounts = (uint32 *) CheckMalloc(tif, nstrips * sizeof(uint32), "for chopped \"StripByteCounts\" array");
    newoffsets = (uint32 *) CheckMalloc(tif, nstrips * sizeof(uint32), "for chopped \"StripOffsets\" array");
