...
int TIFFWriteScanline(TIFF * tif, tdata_t buf, uint32 row, tsample_t sample)
{
    ...
    tstrip_t strip;
    if (!(((tif)->tif_flags & 0x0040) || TIFFWriteCheck((tif), 0, module)))
	return (-1);
    if (!(((tif)->tif_flags & 0x0010) || TIFFWriteBufferSetup((tif), ((void *) 0), (tsize_t) - 1)))
	return (-1);
    td = &tif->tif_dir;
	...
	if (strip >= td->td_stripsperimage && imagegrew)
	    td->td_stripsperimage = ((((uint32) (td->td_imagelength)) + (((uint32) (td->td_rowsperstrip)) - 1)) / ((uint32) (td->td_rowsperstrip)));
...
tsize_t TIFFWriteEncodedTile(TIFF * tif, ttile_t tile, tdata_t data, tsize_t cc)
{
    ...
    TIFFDirectory *td;
    tsample_t sample;
    if (!(((tif)->tif_flags & 0x0040) || TIFFWriteCheck((tif), 1, module)))
	return ((tsize_t) - 1);
    td = &tif->tif_dir;
    ...
    tif->tif_row = (tile % ((((uint32) (td->td_imagelength)) + (((uint32) (td->td_tilelength)) - 1)) / ((uint32) (td->td_tilelength)))) * td->td_tilelength;
    tif->tif_col = (tile % ((((uint32) (td->td_imagewidth)) + (((uint32) (td->td_tilewidth)) - 1)) / ((uint32) (td->td_tilewidth)))) * td->td_tilewidth;
    if ((tif->tif_flags & 0x0020) == 0) {
	if (!(*tif->tif_setupencode) (tif))
	    return ((tsize_t) - 1);
	tif->tif_flags |= 0x0020;
...
tsize_t TIFFWriteRawTile(TIFF * tif, ttile_t tile, tdata_t data, tsize_t cc)
{
	...
	TIFFError(module, "%s: Tile %lu out of range, max %lu", tif->tif_name, (u_long) tile, (u_long) tif->tif_dir.td_nstrips);
	return ((tsize_t) - 1);
    }
...
static int TIFFSetupStrips(TIFF * tif)
{
	...
	td->td_stripsperimage = ((((tif)->tif_dir.td_fieldsset[(2) / 32]) & (((u_long) 1L) << ((2) & 0x1f))) && (tif)->tif_dir.td_imagelength == 0) ? td->td_samplesperpixel : TIFFNumberOfTiles(tif);
    else
	td->td_stripsperimage = ((((tif)->tif_dir.td_fieldsset[(17) / 32]) & (((u_long) 1L) << ((17) & 0x1f))) && (tif)->tif_dir.td_imagelength == 0) ? td->td_samplesperpixel : TIFFNumberOfStrips(tif);
    ...
    (((tif)->tif_dir.td_fieldsset[(25) / 32]) |= (((u_long) 1L) << ((25) & 0x1f)));
    (((tif)->tif_dir.td_fieldsset[(24) / 32]) |= (((u_long) 1L) << ((24) & 0x1f)));
    return (1);
}

static int TIFFWriteCheck(TIFF * tif, int tiles, const char *module)
{
    if (tif->tif_mode == 00) {
	...
	TIFFError(module, "%s: No space for %s arrays", tif->tif_name, (((tif)->tif_flags & 0x0400) != 0) ? "tile" : "strip");
	return (0);
    }
...
int TIFFWriteBufferSetup(TIFF * tif, tdata_t bp, tsize_t size)
{
    ...
    if (size == (tsize_t) - 1) {
	size = ((((tif)->tif_flags & 0x0400) != 0) ? tif->tif_tilesize : tif->tif_scanlinesize);
	if (size < 8 * 1024)
	    size = 8 * 1024;
	bp = ((void *) 0);
    }
...
{
    TIFFDirectory *td = &tif->tif_dir;
    ((td->td_planarconfig == 1) ? (void) (0) : __assert_fail("td->td_planarconfig == 1", "tif_write.c", 570, __PRETTY_FUNCTION__));
    td->td_stripoffset = (uint32 *) _TIFFrealloc(td->td_stripoffset, (td->td_nstrips + delta) * sizeof(uint32));
    td->td_stripbytecount = (uint32 *) _TIFFrealloc(td->td_stripbytecount, (td->td_nstrips + delta) * sizeof(uint32));
    if (td->td_stripoffset == ((void *) 0) || td->td_stripbytecount == ((void *) 0)) {
	td->td_nstrips = 0;
...
static int TIFFAppendToStrip(TIFF * tif, tstrip_t strip, tidata_t data, tsize_t cc)
{
    TIFFDirectory *td = &tif->tif_dir;
	...
	if (td->td_stripoffset[strip] != 0) {
		...
		TIFFError(module, "%s: Seek error at scanline %lu", tif->tif_name, (u_long) tif->tif_row);
		return (0);
	    }
	} else
	    td->td_stripoffset[strip] = ((*(tif)->tif_seekproc) ((tif)->tif_clientdata, (toff_t) ((toff_t) 0), 2));
	tif->tif_curoff = td->td_stripoffset[strip];
    }
    if (!(((*(tif)->tif_writeproc) ((tif)->tif_clientdata, (tdata_t) data, (tsize_t) cc)) == (tsize_t) cc)) {
	TIFFError(module, "%s: Write error at scanline %lu", tif->tif_name, (u_long) tif->tif_row);
	return (0);
    }
    tif->tif_curoff += cc;
...
{
    if (tif->tif_rawcc > 0) {
	if (!(((tif)->tif_flags & (tif->tif_dir.td_fillorder)) != 0) && (tif->tif_flags & 0x0100) == 0)
	    TIFFReverseBits((u_char *) tif->tif_rawdata, tif->tif_rawcc);
	if (!TIFFAppendToStrip(tif, (((tif)->tif_flags & 0x0400) != 0) ? tif->tif_curtile : tif->tif_curstrip, tif->tif_rawdata, tif->tif_rawcc))
	    return (0);
	tif->tif_rawcc = 0;
	tif->tif_rawcp = tif->tif_rawdata;
    }
