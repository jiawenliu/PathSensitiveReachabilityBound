...
static int TIFFNoEncode(TIFF * tif, char *method)
{
	...
	if (!strncmp(c->name, "LZW", 3)) {
	    TIFFError(tif->tif_name, "%s %s encoding is no longer implemented due to Unisys patent enforcement", c->name, method);
	} else {
	    TIFFError(tif->tif_name, "%s %s encoding is not implemented", c->name, method);
	}
    } else {
	TIFFError(tif->tif_name, "Compression scheme %u %s encoding is not implemented", tif->tif_dir.td_compression, method);
    }
    return (-1);
}
...
static int TIFFNoDecode(TIFF * tif, char *method)
{
    const TIFFCodec *c = TIFFFindCODEC(tif->tif_dir.td_compression);
    if (c)
	TIFFError(tif->tif_name, "%s %s decoding is not implemented", c->name, method);
    else
	TIFFError(tif->tif_name, "Compression scheme %u %s decoding is not implemented", tif->tif_dir.td_compression, method);
    return (-1);
}
...
static codec_t *registeredCODECS = ((void *) 0);
...
} TIFFCodec *TIFFRegisterCODEC(uint16 scheme, const char *name, TIFFInitMethod init)
{
	...
	TIFFError("TIFFRegisterCODEC", "No space to register compression scheme %s", name);
    return (cd->info);
}
...
void TIFFUnRegisterCODEC(TIFFCodec * c)
{
    ...
    TIFFError("TIFFUnRegisterCODEC", "Cannot remove compression scheme %s; not registered", c->name);
}
