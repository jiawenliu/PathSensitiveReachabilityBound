...
uint16 compression = 32773;
uint32 rowsperstrip = (uint32) - 1;
int process_by_block = 0;
...
int main1(int argc, char *argv[])
{
	...
	case 'c':
		...
		compression = 1;
	    else if ((strcmp(optarg, "packbits") == 0))
		compression = 32773;
...
static int cvt_by_tile(TIFF * in, TIFF * out)
{
    ...
    TIFFGetField(in, 257, &height);
    if (!TIFFGetField(in, 322, &tile_width) || !TIFFGetField(in, 323, &tile_height)) {
	TIFFError(TIFFFileName(in), "Source image not tiled");
	return (0);
    }
    TIFFSetField(out, 322, tile_width);
    TIFFSetField(out, 323, tile_height);
	...
	for (col = 0; ok && col < width; col += tile_width) {
	    ...
	    if (TIFFWriteEncodedTile(out, TIFFComputeTile(out, col, row, 0, 0), raster, 4 * tile_width * tile_height) == -1) {
		ok = 0;
		break;
	    }
	}
...
static int cvt_by_strip(TIFF * in, TIFF * out)
{
	...
	if (TIFFWriteEncodedStrip(out, row / rowsperstrip, raster, 4 * rows_to_write * width) == -1) {
	    ok = 0;
	    break;
	}
    }
...
static int cvt_whole_image(TIFF * in, TIFF * out)
{
	...
	if (row + rowsperstrip > height)
	    ...
	    rows_to_write = rowsperstrip;
	if (TIFFWriteEncodedStrip(out, row / rowsperstrip, raster_strip, 4 * rows_to_write * width) == -1) {
	    _TIFFfree(raster);
	    return 0;
	}
    }
...
static int tiffcvt(TIFF * in, TIFF * out)
{
    ...
    TIFFSetField(out, 257, height);
    TIFFSetField(out, 258, 8);
    TIFFSetField(out, 259, compression);
    TIFFSetField(out, 262, 2);
    if (TIFFGetField(in, 266, &shortv))
	TIFFSetField(out, 266, shortv);
    TIFFSetField(out, 274, 1);
    TIFFSetField(out, 277, 4);
    v[0] = 1;
    TIFFSetField(out, 338, 1, v);
    ...
    TIFFSetField(out, 305, TIFFGetVersion());
    if (TIFFGetField(in, 269, &stringv))
	TIFFSetField(out, 269, stringv);
...
static char *usageMsg[] = { "usage: tiff2rgba [-c comp] [-r rows] [-b] input... output\n", "where comp is one of the following compression algorithms:\n", " jpeg\t\tJPEG encoding\n", " lzw\t\tLempel-Ziv & Welch encoding\n", " packbits\tPackBits encoding\n", " none\t\tno compression\n", "and the other options are:\n", " -r\trows/strip\n", " -b (progress by block rather than as a whole image)\n", ((void *) 0) };
