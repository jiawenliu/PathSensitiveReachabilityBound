...
void lame_init_infile(lame_global_flags * gfp)
{
    ...
    if (GetSndSampleRate())
	gfp->in_samplerate = GetSndSampleRate();
    if (GetSndChannels())
	gfp->num_channels = GetSndChannels();
...
} int lame_readframe(lame_global_flags * gfp, short int Buffer[2][1152])
{
    ...
    if (iread == 0)
	gfp->totalframes = ((gfp->totalframes) < (gfp->frameNum + 2) ? (gfp->totalframes) : (gfp->frameNum + 2));
    if (gfp->frameNum > (gfp->totalframes - 1))
...
int get_audio(lame_global_flags * gfp, short buffer[2][1152], int stereo)
{
	...
	remaining = num_samples - ((num_samples) < (num_samples_read) ? (num_samples) : (num_samples_read));
...
FILE *OpenSndFile(lame_global_flags * gfp, const char *inPath, int default_samp, int default_channels)
{
    struct stat sb;
	...
	if ((musicin = fopen(inPath, "rb")) == ((void *) 0)) {
    ...
    if (num_samples == 0xFFFFFFFF && musicin != stdin) {
	if (0 == stat(inPath, &sb)) {
	    if (gfp->input_format == sf_mp3) {
...
static int parse_wave_header(FILE * sf)
{
	...
	if (type == 0x666d7420) {
	    ...
	    subSize -= 2;
	    wave_info.samples_per_sec = Read32Bits(sf);
	    subSize -= 4;
	    wave_info.avg_bytes_per_sec = Read32Bits(sf);
	    subSize -= 4;
	    ...
	    subSize -= 2;
	    if (subSize > 0) {
		if (fskip(sf, (long) subSize, 1) != 0)
		    return 0;
	    };
	} else if (type == 0x64617461) {
	    subSize = Read32Bits(sf);
	    data_length = subSize;
	    is_wav = 1;
	    break;
...
static void aiff_check2(const char *file_name, IFF_AIFF * pcm_aiff_data)
{
    ...
    if (pcm_aiff_data->sampleSize != sizeof(short) * 8) {
	fprintf(stderr, "Sound data is not %d bits in \"%s\".\n", (unsigned int) sizeof(short) * 8, file_name);
	exit(1);
    }
...
static int parse_aiff_header(FILE * sf)
{
    ...
    if (Read32BitsHighLow(sf) != 0x41494646)
	return 0;
    while (chunkSize > 0) {
	u_int type = 0;
	chunkSize -= 4;
	...
	if (type == 0x434f4d4d) {
	    ...
	    subSize -= 4;
	    if (fskip(sf, aiff_info.blkAlgn.offset, 1) != 0)
		return 0;
