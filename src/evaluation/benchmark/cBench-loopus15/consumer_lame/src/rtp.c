...
{
    char *buf = __builtin_alloca(len + sizeof(struct rtpheader));
    int *cast = (int *) foo;
    int *outcast = (int *) buf;
...
int makesocket(char *szAddr, unsigned short port, int TTL, struct sockaddr_in *sSockAddr)
{
    ...
    int iSocket = socket(2, SOCK_DGRAM, 0);
    if (iSocket < 0) {
	fprintf(stderr, "socket() failed.\n");
	exit(1);
    }
    tempaddr = inet_addr(szAddr);
    sSockAddr->sin_family = sin.sin_family = 2;
    sSockAddr->sin_port = sin.sin_port = htons(port);
    sSockAddr->sin_addr.s_addr = tempaddr;
    iRet = setsockopt(iSocket, 1, 2, &iLoop, sizeof(int));
    if (iRet < 0) {
	fprintf(stderr, "setsockopt SO_REUSEADDR failed\n");
	exit(1);
    }
    if ((ntohl(tempaddr) >> 28) == 0xe) {
	iRet = setsockopt(iSocket, IPPROTO_IP, 33, &cTtl, sizeof(char));
	if (iRet < 0) {
	    fprintf(stderr, "setsockopt IP_MULTICAST_TTL failed.  multicast in kernel?\n");
	    exit(1);
	}
	cLoop = 1;
	iRet = setsockopt(iSocket, IPPROTO_IP, 34, &cLoop, sizeof(char));
	if (iRet < 0) {
