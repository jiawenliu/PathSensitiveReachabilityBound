...
defdouble ConvertFromIeeeSingle(char *bytes)
{
    ...
    long mantissa, expon;
    long bits;
    bits = ((unsigned long) (bytes[0] & 0xFF) << 24) | ((unsigned long) (bytes[1] & 0xFF) << 16) | ((unsigned long) (bytes[2] & 0xFF) << 8) | (unsigned long) (bytes[3] & 0xFF);
    if ((bits & 0x7FFFFFFF) == 0) {
	f = 0;
    } else {
	expon = (bits & 0x7F800000) >> (32 - 8 - 1);
	if (expon == 255) {
	    f = (__builtin_huge_val());
	} else {
	    if (expon == 0) {
		mantissa = (bits & 0x7fffff);
		f = ldexp((defdouble) mantissa, (int) (expon - 127 - (32 - 8 - 1) + 1));
	    } else {
		mantissa = (bits & 0x7fffff) + 0x800000;
		f = ldexp((defdouble) mantissa, (int) (expon - 127 - (32 - 8 - 1)));
    }}} if (bits & (-2147483647L - 1L))
	return -f;
    else
	return f;
}
...
void ConvertToIeeeSingle(defdouble num, char *bytes)
{
	...
	if ((expon > (255 - 127 + 1)) || !(fMant < 1)) {
	    bits = sign | 0x7F800000;
	} else {
	    long mantissa;
	    if (expon < -(127 - 2)) {
		int shift = ((32 - 8 - 1) + 1) + (127 - 2) + expon;
		if (shift < 0) {
		    bits = sign;
		} else {
		...
		mantissa = (long) floor(fMant * (1L << ((32 - 8 - 1) + 1)));
		mantissa -= (1L << (32 - 8 - 1));
		bits = sign | ((long) ((expon + 127 - 1)) << (32 - 8 - 1)) | mantissa;
...
} defdouble ConvertFromIeeeDouble(char *bytes)
{
    ...
    first = ((unsigned long) (bytes[0] & 0xFF) << 24) | ((unsigned long) (bytes[1] & 0xFF) << 16) | ((unsigned long) (bytes[2] & 0xFF) << 8) | (unsigned long) (bytes[3] & 0xFF);
    second = ((unsigned long) (bytes[4] & 0xFF) << 24) | ((unsigned long) (bytes[5] & 0xFF) << 16) | ((unsigned long) (bytes[6] & 0xFF) << 8) | (unsigned long) (bytes[7] & 0xFF);
    if (first == 0 && second == 0) {
	f = 0;
    } else {
	expon = (first & 0x7FF00000) >> (32 - 11 - 1);
	if (expon == 2047) {
	    f = (__builtin_huge_val());
	} else {
	    if (expon == 0) {
		mantissa = (first & 0x000FFFFF);
		f = ldexp((defdouble) mantissa, (int) (expon - 1023 - (32 - 11 - 1) + 1));
		f += ldexp((((defdouble) ((long) ((second) - 2147483647L - 1))) + 2147483648.0), (int) (expon - 1023 - (32 - 11 - 1) + 1 - 32));
	    } else {
		mantissa = (first & 0x000FFFFF) + 0x00100000;
		f = ldexp((defdouble) mantissa, (int) (expon - 1023 - (32 - 11 - 1)));
		f += ldexp((((defdouble) ((long) ((second) - 2147483647L - 1))) + 2147483648.0), (int) (expon - 1023 - (32 - 11 - 1) - 32));
    }}} if (first & 0x80000000)
	return -f;
    else
	return f;
...
void ConvertToIeeeDouble(defdouble num, char *bytes)
{
	...
	int expon;
	fMant = frexp(num, &expon);
	if ((expon > (2047 - 1023 + 1)) || !(fMant < 1)) {
	    first = sign | 0x7FF00000;
	    second = 0;
	} else {
	    long mantissa;
	    if (expon < -(1023 - 2)) {
		int shift = ((32 - 11 - 1) + 1) + (1023 - 2) + expon;
		if (shift < 0) {
		    first = sign;
		    shift += 32;
		    if (shift < 0) {
			...
			second = ((unsigned long) (((long) ((floor(ldexp(fMant, shift))) - 2147483648.0)) + 2147483647L + 1));
		}} else {
		    fsMant = ldexp(fMant, shift);
		    mantissa = (long) floor(fsMant);
		    first = sign | mantissa;
		    second = ((unsigned long) (((long) ((floor(ldexp(fsMant - mantissa, 32))) - 2147483648.0)) + 2147483647L + 1));
	    }} else {
		fsMant = ldexp(fMant, (32 - 11 - 1) + 1);
		mantissa = (long) floor(fsMant);
		mantissa -= (1L << (32 - 11 - 1));
		fsMant -= (1L << (32 - 11 - 1));
		first = sign | ((long) ((expon + 1023 - 1)) << (32 - 11 - 1)) | mantissa;
		second = ((unsigned long) (((long) ((floor(ldexp(fsMant - mantissa, 32))) - 2147483648.0)) + 2147483647L + 1));
...
} defdouble ConvertFromIeeeExtended(char *bytes)
{
	...
	if (expon == 0x7FFF) {
	    ...
	    expon -= 16383;
	    f = ldexp((((defdouble) ((long) ((hiMant) - 2147483647L - 1))) + 2147483648.0), (int) (expon -= 31));
	    f += ldexp((((defdouble) ((long) ((loMant) - 2147483647L - 1))) + 2147483648.0), (int) (expon -= 32));
    }} if (bytes[0] & 0x80)
	return -f;
    else
	return f;
