...
char inPath[300];
char outPath[300];
void lame_usage(lame_global_flags * gfp, char *name)
{
    ...
    exit(1);
} void lame_help(lame_global_flags * gfp, char *name)
{
    ...
    exit(0);
} void lame_presets_info(lame_global_flags * gfp, char *name)
{
    lame_print_version(stdout);
    fprintf(stdout, "\n");
...
} void lame_parse_args(lame_global_flags * gfp, int argc, char **argv)
{
	...
	if (*token++ == '-') {
	    ...
	    if (!*token) {
		if (inPath[0] == '\0')
		    strncpy(inPath, argv[i], 300);
		else if (outPath[0] == '\0')
		    strncpy(outPath, argv[i], 300);
	    }
	    if (*token == '-') {
		token++;
