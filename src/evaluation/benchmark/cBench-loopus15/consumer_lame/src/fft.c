...
static FLOAT costab[(5 - 1) * 2];
static FLOAT window[1024 / 2], window_s[256 / 2];
static __inline void fht(FLOAT * fz, short n)
{
    short k4;
    FLOAT *fi, *fn, *gi;
    FLOAT *tri;
...
static const short rv_tbl[] = {  };

void fft_short(FLOAT x_real[3][256], int chn, short *buffer[2])
{
    short i, j, b;
    for (b = 0; b < 3; b++) {
	FLOAT *x = &x_real[b][256 / 2];
	short k = (576 / 3) * (b + 1);
	j = 256 / 8 - 1;
	if (chn < 2) {
	    do {
		FLOAT f0, f1, f2, f3, w;
		i = rv_tbl[j << 2];
		f0 = (window_s[i] * (buffer[chn][i + k]));
		w = (window_s[0x7f - i] * (buffer[chn][i + k + 0x80]));
		f1 = f0 - w;
		f0 = f0 + w;
		f2 = (window_s[i + 0x40] * (buffer[chn][i + k + 0x40]));
		w = (window_s[0x3f - i] * (buffer[chn][i + k + 0xc0]));
		f3 = f2 - w;
		f2 = f2 + w;
		x -= 4;
		x[0] = f0 + f2;
		...
		x[1] = f1 + f3;
		x[3] = f1 - f3;
		f0 = (window_s[i + 0x01] * (buffer[chn][i + k + 0x01]));
		w = (window_s[0x7e - i] * (buffer[chn][i + k + 0x81]));
		f1 = f0 - w;
		f0 = f0 + w;
		f2 = (window_s[i + 0x41] * (buffer[chn][i + k + 0x41]));
		w = (window_s[0x3e - i] * (buffer[chn][i + k + 0xc1]));
		f3 = f2 - w;
		f2 = f2 + w;
		x[256 / 2 + 0] = f0 + f2;
		x[256 / 2 + 2] = f0 - f2;
		...
		FLOAT f0, f1, f2, f3, w;
		i = rv_tbl[j << 2];
		f0 = (window_s[i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k] + buffer[1][i + k])));
		w = (window_s[0x7f - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k + 0x80] + buffer[1][i + k + 0x80])));
		f1 = f0 - w;
		f0 = f0 + w;
		f2 = (window_s[i + 0x40] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k + 0x40] + buffer[1][i + k + 0x40])));
		w = (window_s[0x3f - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k + 0xc0] + buffer[1][i + k + 0xc0])));
		f3 = f2 - w;
		f2 = f2 + w;
		x -= 4;
		x[0] = f0 + f2;
		...
		x[1] = f1 + f3;
		x[3] = f1 - f3;
		f0 = (window_s[i + 0x01] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k + 0x01] + buffer[1][i + k + 0x01])));
		w = (window_s[0x7e - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k + 0x81] + buffer[1][i + k + 0x81])));
		f1 = f0 - w;
		f0 = f0 + w;
		f2 = (window_s[i + 0x41] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k + 0x41] + buffer[1][i + k + 0x41])));
		w = (window_s[0x3e - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + k + 0xc1] + buffer[1][i + k + 0xc1])));
		f3 = f2 - w;
		f2 = f2 + w;
		x[256 / 2 + 0] = f0 + f2;
		x[256 / 2 + 2] = f0 - f2;
...
void fft_long(FLOAT x[1024], int chn, short *buffer[2])
{
    short i, jj = 1024 / 8 - 1;
    x += 1024 / 2;
    if (chn < 2) {
	do {
	    FLOAT f0, f1, f2, f3, w;
	    i = rv_tbl[jj];
	    f0 = (window[i] * (buffer[chn][i]));
	    w = (window[0x1ff - i] * (buffer[chn][i + 0x200]));
	    f1 = f0 - w;
	    f0 = f0 + w;
	    f2 = (window[i + 0x100] * (buffer[chn][i + 0x100]));
	    w = (window[0x0ff - i] * (buffer[chn][i + 0x300]));
	    f3 = f2 - w;
	    f2 = f2 + w;
	    x -= 4;
	    x[0] = f0 + f2;
	    ...
	    x[1] = f1 + f3;
	    x[3] = f1 - f3;
	    f0 = (window[i + 0x001] * (buffer[chn][i + 0x001]));
	    w = (window[0x1fe - i] * (buffer[chn][i + 0x201]));
	    f1 = f0 - w;
	    f0 = f0 + w;
	    f2 = (window[i + 0x101] * (buffer[chn][i + 0x101]));
	    w = (window[0x0fe - i] * (buffer[chn][i + 0x301]));
	    f3 = f2 - w;
	    f2 = f2 + w;
	    x[1024 / 2 + 0] = f0 + f2;
	    x[1024 / 2 + 2] = f0 - f2;
	...
	do {
	    FLOAT f0, f1, f2, f3, w;
	    i = rv_tbl[jj];
	    f0 = (window[i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i] + buffer[1][i])));
	    w = (window[0x1ff - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + 0x200] + buffer[1][i + 0x200])));
	    f1 = f0 - w;
	    f0 = f0 + w;
	    f2 = (window[i + 0x100] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + 0x100] + buffer[1][i + 0x100])));
	    w = (window[0x0ff - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + 0x300] + buffer[1][i + 0x300])));
	    f3 = f2 - w;
	    f2 = f2 + w;
	    x -= 4;
	    x[0] = f0 + f2;
	    ...
	    x[1] = f1 + f3;
	    x[3] = f1 - f3;
	    f0 = (window[i + 0x001] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + 0x001] + buffer[1][i + 0x001])));
	    w = (window[0x1fe - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + 0x201] + buffer[1][i + 0x201])));
	    f1 = f0 - w;
	    f0 = f0 + w;
	    f2 = (window[i + 0x101] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + 0x101] + buffer[1][i + 0x101])));
	    w = (window[0x0fe - i] * (((FLOAT) (0.5 * 1.41421356237309504880)) * (buffer[0][i + 0x301] + buffer[1][i + 0x301])));
	    f3 = f2 - w;
	    f2 = f2 + w;
	    x[1024 / 2 + 0] = f0 + f2;
	    x[1024 / 2 + 2] = f0 - f2;
...
{
    int i;
    FLOAT r = 3.14159265358979323846 * 0.125;
    for (i = 0; i < (5 - 1); i++) {
	costab[i * 2] = cos(r);
	costab[i * 2 + 1] = sin(r);
	r *= 0.25;
    ...
    for (i = 0; i < 1024 / 2; i++)
	window[i] = 0.5 * (1.0 - cos(2.0 * 3.14159265358979323846 * (i + 0.5) / 1024));
    for (i = 0; i < 256 / 2; i++)
	window_s[i] = 0.5 * (1.0 - cos(2.0 * 3.14159265358979323846 * (i + 0.5) / 256));
}
