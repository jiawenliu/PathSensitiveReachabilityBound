...
void mdct_sub48(lame_global_flags * gfp, short *w0, short *w1, FLOAT8 mdct_freq[2][2][576], III_side_info_t * l3_side)
{
	...
	for (gr = 0; gr < gfp->mode_gr; gr++) {
	    ...
	    for (k = 0; k < 18 / 2; k++) {
		window_subband(wk, samp, (&win[2][4]));
		window_subband(wk + 32, samp + 32, (&win[2][4]));
		...
		if (band >= gfp->lowpass_band || band <= gfp->highpass_band) {
			...
			for (k = 2; k >= 0; --k) {
			    FLOAT8 w1 = win[2][k];
			    (&win[2][4])[k] = sb_sample[ch][gr][k + 6][band] * w1 - sb_sample[ch][gr][11 - k][band];
			    (&win[2][4])[k + 3] = sb_sample[ch][gr][k + 12][band] + sb_sample[ch][gr][17 - k][band] * w1;
			    (&win[2][4])[k + 6] = sb_sample[ch][gr][k + 12][band] * w1 - sb_sample[ch][gr][17 - k][band];
			    (&win[2][4])[k + 9] = sb_sample[ch][1 - gr][k][band] + sb_sample[ch][1 - gr][5 - k][band] * w1;
			    (&win[2][4])[k + 12] = sb_sample[ch][1 - gr][k][band] * w1 - sb_sample[ch][1 - gr][5 - k][band];
			    (&win[2][4])[k + 15] = sb_sample[ch][1 - gr][k + 6][band] + sb_sample[ch][1 - gr][11 - k][band] * w1;
			}
			mdct_short(mdct_enc, (&win[2][4]));
		    } else {
			for (k = 8; k >= 0; --k) {
			    (&win[2][4])[k] = win[type][k] * sb_sample[ch][gr][k][band] - win[type][k + 9] * sb_sample[ch][gr][17 - k][band];
			    (&win[2][4])[9 + k] = win[type][k + 18] * sb_sample[ch][1 - gr][k][band] + win[type][k + 27] * sb_sample[ch][1 - gr][17 - k][band];
			}
			mdct_long(mdct_enc, (&win[2][4]));
		    }
		}
		if (type != 2) {
		    if (band == 0)
...
void mdct_init48(void)
{
    ...
    FLOAT8 max;
    for (k = 0; k < 8; k++) {
	static const FLOAT8 c[8] = { -0.6, -0.535, -0.33, -0.185, -0.095, -0.041, -0.0142, -0.0037 };
	sq = 1.0 + c[k] * c[k];
	sq = sqrt(sq);
	ca[k] = c[k] / sq;
	cs[k] = 1.0 / sq;
    } for (i = 0; i < 36; i++)
	win[0][i] = sin(3.14159265358979323846 / 36 * (i + 0.5));
    for (i = 0; i < 18; i++)
	win[1][i] = win[0][i];
    for (; i < 24; i++)
	win[1][i] = 1.0;
    for (; i < 30; i++)
	win[1][i] = cos(3.14159265358979323846 / 12 * (i + 0.5));
    for (; i < 36; i++)
	win[1][i] = 0.0;
    for (i = 0; i < 36; i++)
	win[3][i] = win[1][35 - i];
	...
	do {
	    m = all[j];
	    for (k = 0; k < 36 / 4; k++) {
		*cos_l0++ = sq * cos((3.14159265358979323846 / (4 * 36)) * (2 * m + 1) * (4 * k + 2 + 36));
	    }
	    for (k = 0; k < 36 / 4; k++) {
		*cos_l0++ = sq * cos((3.14159265358979323846 / (4 * 36)) * (2 * m + 1) * (4 * k + 2 + 36 * 3));
	    }
	} while (--j >= 0);
	...
	do {
	    m = d9[j];
	    *cos_l0++ = sq * cos((3.14159265358979323846 / (4 * 36)) * (2 * m + 1) * (2 + 36));
	    *cos_l0++ = sq * cos((3.14159265358979323846 / (4 * 36)) * (2 * m + 1) * (4 * 2 + 2 + 36));
	} while (--j >= 0);
    }
    max = enwindow[256 - 8]; {
	FLOAT8 *wp = enwindow;
	...
	for (i = 15; i >= 0; --i) {
	    for (k = 1; k < 32; k++) {
		*wp++ = cos((2 * i + 1) * k * 3.14159265358979323846 / 64) * mmax[k - 1];
	    }
	}
    }
    for (k = 0; k < 4; k++) {
    ...
    sq = 4.0 / 12;
    for (i = 0; i < 12 / 4; i++) {
	FLOAT8 w2 = cos(3.14159265358979323846 / 12 * (i + 0.5)) * max / 32768 * sq;
	win[2][i] = tan(3.14159265358979323846 / 12 * (i + 0.5));
	for (m = 0; m < 12 / 2; m++) {
	    cos_s[m][i] = w2 * cos((3.14159265358979323846 / (4 * 12)) * (2 * m + 1) * (4 * i + 2 + 12));
	    cos_s[m][i + 12 / 4] = w2 * cos((3.14159265358979323846 / (4 * 12)) * (2 * m + 1) * (4 * i + 2 + 12 * 3));
	}
    }
}
