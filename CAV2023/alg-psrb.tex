...
Now for a program $c$, we use $\absG(c)$, its refined program $\rprog$ and all the ranking functions to compute the \emph{reachability-bound} for each of its control location.
...
%
In Example~\ref{ex:relatedNestedWhileOdd-overview}, since there isn't nested loop for $\tpath_1$ and $\tpath_2$, their local bound $\frac{m}{4}$ is indeed the global \emph{path reachability-bound}.
