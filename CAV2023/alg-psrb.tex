...
% If there isn't outer loop for a path such as $\tpath_1$,  we will use the local bound directly as global \emph{path reachability-bound}
% and skip computing the \emph{loop reachability-bound}.
...
Then we compute for each program point in a program $c$, $l \in \lvar(c)$,
%  in a program $c$,
