...
As pre-procedures, we first introduce the loop bound, $BD(\rprog', c)$ for every sub loop program $\rprog'$ of $c$ in $\rprog$ and the local \emph{path reachability-bound}.
...
% The \emph{reachability-bound} for program points is computed as follows.
For each program point in a program $c$, $l \in \lvar(c)$,
%  in a program $c$,
its \emph{reachability-bound}, he \emph{reachability-bound}, $\psRB(c, l)$ during the execution of $c$ is computed as follows.
%
