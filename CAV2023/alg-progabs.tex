...
% with a vertex set $\absV(c)$ and an edge set $\absE(c)$.
%
% \\
% Every 
...
% $ = DC(\vardom  \cup \scvardom) \cup \booldom\cup \{\top\}$  
is either a
...
% is generated from the command with label $l$.
% , which describes the abstract execution of this command.
\end{defn}
