...
The algorithm initializes each candidate with a \emph{simple transition path} and generates a new candidate program by composing and deleting existing ones in each iteration and updating the candidate set until it has only one candidate as the output.
%
...
% In the first pattern, the second loop path $\tpath_4$ will execute after the first path containing the nested loop is done.
%  $\rprog_1^1 = 1: \rprepeat(\tpath_1; 4:\rprepeat(\tpath_3); \tpath_2; \tpath_4)$
% $ 
% \tpath_0; \rpchoose{\rprepeat(\tpath_1; 4:\rprepeat(\tpath_3); \tpath_2),\rprepeat(\tpath_4) }; \tpath_5
% $.
