...
% \end{itemize}
% \end{defn}
Then we collect all the \emph{simple transition paths}s, $\tpath \in \paths(\absG(c))$ from an abstract transition graph.
...
For example, the $1 \to 2 \to 8 \to 1$ is a \emph{simple transition path} on $\absG(\kw{relatedNestedWhileOdd}(n, m))$ in Figure~\ref{fig:relatedNestedWhileOdd_abscfg}(b).
