...
Then for every program point $l$, we sum up all the $\inoutB(\rprog, \tpath)$ over $\tpath$ that contains $l$ and get $\psRB(c, l)$.
Since point $5$ only shows up on $\tpath_3$, we compute \highlight{$\psRB(5) = n - m$}.
