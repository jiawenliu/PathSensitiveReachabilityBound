...
% \\
\highlight{
%     The tight \emph{reachability-bound}s for the locations inside the loop $L_4$ is 
...
\cite{GulwaniJK09} considers a similar quantity, the \emph{Progress Invariant}, but it bounds the iteration times of \highlight{inner loop $L_4$} w.r.t. one iteration of $L_1$, which is $m$. They still over-approximate the bound for locations inside $L_4$ as $m \times (n - m)$.
% \end{itemize}
