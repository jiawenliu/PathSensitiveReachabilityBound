...
% \\
\highlight{The \emph{reachability-bounds} for the locations inside the loop $L_4$ is 
...
Paper~\cite{GulwaniJK09} considers a similar quantity, the \emph{Progress Invariant}. But it bounds the iteration times of inner loop $L_4$ w.r.t. one iteration of $L_1$, which is $m$. Then they still over-approximate the bound for locations inside $L_4$ as $m \times (n - m)$ by multiplication.
...
\input{alg-overview}
We also prove the soundness of each estimating algorithm in the Appendices.
