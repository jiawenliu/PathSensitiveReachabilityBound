...
Again, existing approach either computes loose bound for the loop or ignores the path sensitivity.
...
By multiplying the \emph{path reachability-bound} of $\tpath_3$ in $L_4$ with its \emph{loop reachability-bound} w.r.t the $L_1$, we can compute a tight
\emph{reachability-bounds} for location $5$.
