...
% \paragraph{Walk through Example.}
% \todo{The walk through example}
By this approach, we compute a more precise loop bound $BD(\rprog)$ for each subprogram of our walk through example in Figure~\ref{fig:relatedNestedWhileOdd-overview}(b), in comparison to the purely ranking function-based approach in Equation~\ref{eq:absBD}.
...
% \todo{necessary?}
\highlight{
Notice here we can already compute the \emph{reachability-bound} by simply using the $BD$ and $\outinB(\rprog, \tpath)$. By removing the restriction, $l: \rprog_l = \kw{enclosed}(\rprog, \tpath)$ in Definition~\ref{def:pathlocalrb} and use $\rprog$
