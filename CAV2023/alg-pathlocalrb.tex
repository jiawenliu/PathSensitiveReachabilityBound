...
\begin{defn}[Path Local Reachability-bound Computation]
    ...
    \begin{array}{ll}
      ...
      \outinB(\rprog_1, \tpath) + \outinB(\rprog_2, \tpath) & \rprog = \rprog_1;\rprog_2 \\
      ...
      % & 
      BD(\rprepeat(\rprog'), c) \times \outinB(\rprog', \tpath) & \rprog = \rprepeat(\rprog')
      %  \\
...
% \todo{necessary?}
% \highlight{
Notice here, we can compute a loose $\psRB$ just using $\outinB(\rprog, \tpath)$. By replacing the restriction, $l: \rprog_l = \kw{enclosed}(\rprog, \tpath)$ in Definition~\ref{def:pathlocalrb} with $\rprog$, $\outinB(\rprog, \tpath)$ computes global \emph{path reachability-bound} loosely.
...
\end{equation}
It computes
% the path reachability-bound
% $\outinB(\rprog, \tpath_i)$
% for $\tpath_0$ and $\tpath_5$ as $1$,
% for paths $\tpath_1$, $\tpath_2$ and $\tpath_4$ as $\frac{m}{4}$ and 
$\outinB(\rprog, \tpath_3) = \frac{m}{4} \times (n - m)$ in Example~\ref{ex:relatedNestedWhileOdd-overview}.
% $\outinB(\rprog, \tpath_0) = 1$,
...
It is loose because the Eq.~\ref{eq:psrb-sub} assumes the loop $L_4$ is reached in every iteration of the outer loop $L_1$. In this sense, $\psRB(c, 4) = \frac{m}{4} \times (n - m)$ is loose as well.
