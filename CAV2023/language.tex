...
the label, $l$ associated to this command and an evaluated value.
 ...
 and we use $\eventset^{\asn}$ and $\eventset^{\test}$ to denote the set of all assignment events and testing events, respectively where $\eventset = \eventset^{\asn} \cup \eventset^{\test}$.
...
Our symbolic bound language is a subset of the language expression, and we use the same evaluation notation for evaluating the symbolic bound expression.
