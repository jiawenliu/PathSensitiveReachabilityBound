...
and
% $\inpvardom$ the universe of all input variables and 
$\inpvar(c) \subseteq \vardom$ denotes the set of all input variables in a program $c$.
...
%   All the free variables
%   showing up in $c$ such that they are never defined before using, are actually the input variables of this program.
%   We denote by $\mathcal{V}_{\lin}$ the universe of all input variables and $\inpvar(c)$ the set of input variables in a given program $c$.

% \paragraph{{Trace-based Operational Semantics}}
% \label{sec:operational_semantics}
\paragraph{Event and Trace.}
...
%  An \emph{assignment event} tracks the execution of an assignment and consists of the variable, the label of the command that generates it, the value assigned to the variable.
%  A \emph{testing event} tracks the evaluation of $b$ in
% the guard for a $\eif(\clabel{b}^l, c_1, c_2)$ command or $\ewhile \clabel{b}^l \edo (c)$.
...
collecting the events generated in each program evaluation step during the excution. 
% \[
% \begin{array}{llll}
% \mbox{Trace} & \trace
% & ::= & [] ~|~ \trace :: \event 
...
%  of  for assignment commands and guards in $\eif$ and $\ewhile$ command.
...
%  when executing under some initial trace,
% it produces the infinite trace from $\inftdom$,
% , which records a non-terminating computation,
% So we denote by $\tdom$ the set of all traces, 
...
We use $\ftdom_0(c)$ to denote the set of all initial traces for program $c$, and each input variable has an initial in an initial trace.
% $\trace \in \ftdom_0(c)$.
...
$\trace_0 \in \ftdom_0(c)$,
then the program's execution terminates and produces a finite execution trace $\trace \in \ftdom$. We also have non-terminating execution. The formal definition for the non-terminating and infinite trace can be found in Appendices~\ref{apdx:language}.
...
%   % \footnote{This definition follows the intuition of the maximal trace semantics in Equation (12) from Section 2.5 of paper~\cite{Cousot19a}, and the program model in~\cite{SinnZV17}.}
% {\small 
% \[
%   \begin{array}{l}
%   %   \lim(\uparrow) 
...
% \end{defn}
% % \footnotetext{
%   This definition follows the intuition of the maximal trace semantics in Equation (12) from Section 2.5 of paper~\cite{Cousot19a} and the program model in~\cite{SinnZV17}.
% % }
%
