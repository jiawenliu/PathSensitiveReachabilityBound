...
following by 
the label, $l$ associated to this command and the evaluated value.
%   \begin{center}
%     $ \event ::= 
...
We use $\ftdom_0(c)$ to denote the set of all initial traces for program $c$, and each input variable $x \in \inpvar(c)$ has an initial value in an initial trace $\trace_0 \in \ftdom_0(c)$.
% $\trace \in \ftdom_0(c)$.
