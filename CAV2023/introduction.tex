...
%  which are used in both~\cite{ZulegerGSV11,SinnZV14} as well, 
and limit the iterations of the refinement algorithm to a constant in our bound analysis algorithm.
% To leverage the efficiency and non-terminating,
...
4. A prototype implementation with evaluation over four different benchmarks.
  ...
  %  For program with multi-path loop, we compute different bounds for the points on different paths.
\end{itemize}