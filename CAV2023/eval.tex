...
Including 75 {\tt .c} programs from different benchmarks\footnote{(including ABC : 15, C4B : 15, KoAT~\cite{BrockschmidtEFFG14,FalkeKS12,FalkeKS11} : 3, Loopus 11 + 14 + JAR: 10, Rank10 : 3, SPEED : 27, WTC : 37)} and 37 challenging {\tt .c} programs 
...
Table Tab.~\ref{tb:accuracy-eval} shows the evaluation results including the theoretical bounds, and empirical bounds on different path locations.
We are the only one that computes different bounds on different path points,
we present only the bounded numbers from other tools. As shown in column $4$, in almost all the multipath loops, the bounds on different path location are significantly different, which never be computed be any of the existing tools.
...
Our tool suffers the efficiency problem because of the implementation and the possible non-terminating in REFINE algorithm and the ranking function estimating algorithm.
We have the performance evaluation result in Appendices~\ref{apdx:eval-performance} Tab.~\ref{tb:performance-eval}.
