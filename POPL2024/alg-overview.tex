...
\textbf{Step1: Program Abstraction.}
...
\textbf{Step 2: Path Interleaving Refinement.} 
The two possible execution paths in Figure~\ref{fig:relatedNestedWhileOdd-overview}(b)
can interleave each other during the program execution.
In this step, the Algorithm~\ref{alg:prog-refine} in Section~\ref{sec:refine} compute their interleaving order
of the two paths by exhaustively checking the transition relation of each path.
There are two possible interleaving orders,
either $ rprog_1$ executes after one iteration of $rprog_2$ or vice versa.
So we generate two interleaving order 
\[
