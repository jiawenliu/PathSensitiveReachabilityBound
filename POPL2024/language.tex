...
The commands are the typical ones from while languages with each annotated with a label $l \in (\mathbb{N} \cup \{\lex\})$ to uniquely identify each program point. The program is a labeled command $c$ having the following syntax. 
\\
\[
{c} ::= 
...
\]
\\
Expressions include linear arithmetic expression over integers, $\aexpr \in \mathcal{A}$ (with the value $n \in \mathbb{Z}$ and the variable $x$ from a finite universe of all variables, $\vardom$) and boolean expression, $\bexpr \in \booldom$, where we denote by $\infty$ a value s.t. $n < \infty $ for all $n \in \mathbb{N}$.
