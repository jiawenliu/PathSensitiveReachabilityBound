...
\subsection{Abstract Transition Graph Refinement}
\label{sec:refine}
Three sub-steps:
\\
...
\highlight{
  A \emph{Repeat Pattern},
  $\rpattern \in \mathcal{P}(\paths({\absG(c)}))$ for a program $c$ is
% is either a simple path or sequence of repeat patterns of this program $c$. 
% is a sequence of simple transition paths with annotations $\rprepeat$
% it is a simple path or
\\
 ==> 
a sequence of simple transition paths $\tpath$ with annotation $\rprepeat$ on certain sub-sequences
% or a sub \emph{repeat pattern} $\rpattern'$ with annotation $\rprepeat$,
...
then \highlight{it must consecutively execute without being interleaved until its invariant is false};
% can consecutively execute at least twice w.r.t. some initial trace.
\\
2. In a repeat pattern $\rpattern = \rpattern_0; \cdots; \rpattern_n$,
%  $\rpattern$ is a sequence of sub-repeat patterns,
% i.e.,  $\rpattern = \rpattern_0; \cdots; \rpattern_n$,
% then 
every $\rpattern_i$ in this sequence can execute
after the previous $\rpattern_{i - 1}$'s iteration is finished.
