...
The \emph{Abstract Transition Graph} for a program $c$ is $\absG(c) =(\absV(c), \absE(c))$.
...
\\
Each edge in $\absE(c)$ is an abstract transition
...
Each edge is annotated by a constraint generated for the command with label $l$.
